...
const protectedDirty = computed(() => dirty.value && controller.selected.value?.status === 'DRAFT');
...
async function synchronize(): Promise<void> {
  ...
  const broadcastId = typeof route.params.broadcastId === 'string' ? route.params.broadcastId : '';
  const authorityKey = `${projectId}:${broadcastId}:${Object.values(permissions.value).join(':')}`;
  ...
  const sameSelected = !authorityChanged && controller.selected.value?.id === broadcastId;
  if (sameSelected) {
    if (!dirty.value) await controller.refresh();
  } else {
    await controller.open(broadcastId);
  }
  if (controller.selected.value && controller.selected.value.status !== 'DRAFT')
    await controller.loadDeliveries();
}

async function refresh(): Promise<void> {
  if (!(await controller.refresh())) return;
  if (controller.selected.value?.status !== 'DRAFT') await controller.loadDeliveries();
}
...
async function mutate(operation: () => Promise<boolean>, loadOutcomes = true): Promise<void> {
...
<template>
  <TelegramBroadcastWorkspace
    ...
    :loading="controller.detailLoading.value || controller.deliveriesLoading.value"
  ...
  <section v-else class="page broadcast-detail-state">
    <p v-if="controller.detailLoading.value" role="status" aria-live="polite">
