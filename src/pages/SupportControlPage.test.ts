...
async function render(value: SupportLeadSummary, options: RenderOptions = {}) {
  ...
  auth.$patch({
    ...
    project: {
      ...
      effectivePermissionCodes: [
        ...
        ...(allowAvailability
          ? ['project.support.availability.read', 'project.support.availability.self_manage']
          : []),
        ...(allowAssignment
          ? [
              'project.cases.read',
  ...
  api.readSummary.mockReturnValue(initialRequests?.summary ?? Promise.resolve(value));
  api.readReadiness.mockReturnValue(initialRequests?.readiness ?? Promise.resolve(readiness));
  api.readCapacityRisks.mockReturnValue(initialRequests?.capacity ?? Promise.resolve(capacity));
  ...
  api.readCaseRisks.mockImplementation(
    (_, riskType) =>
      initialRequests?.risks ??
      Promise.resolve({
  ...
  api.readCases.mockReturnValue(
    initialRequests?.cases ??
      Promise.resolve({
        items: [
          {
            id: 'case-1',
...
describe('SupportControlPage', () => {
  ...
  it('keeps one full-page skeleton until every initial dashboard projection settles', async () => {
    ...
    const pageSkeleton = () => wrapper.find('[data-testid="support-control-page-skeleton"]');
    ...
    expect(wrapper.get('#attention-heading').text()).toBe('Что требует внимания');
    expect(wrapper.get('#capacity-heading').text()).toBe('Ёмкость по очередям');
    expect(wrapper.get('#risk-heading').text()).toBe('Обращения в риске');
    expect(wrapper.get('#alerts-heading').text()).toBe('Операционные сигналы');
  });
  ...
  it('keeps the settled capacity grid visible during a manual refresh', async () => {
    ...
    const refreshButton = wrapper.findAll('button').find((button) => button.text() === 'Обновить');
    expect(refreshButton).toBeDefined();
    await refreshButton!.trigger('click');
    await flushPromises();

    expect(wrapper.text()).toContain('Платежи');
    expect(wrapper.find('[data-testid="support-control-page-skeleton"]').exists()).toBe(false);
    expect(wrapper.get('[aria-labelledby="capacity-heading"]').find('.p-skeleton').exists()).toBe(
      false,
    );
  ...
  it('exposes the shared Lead assignment action from a Control risk drill-down', async () => {
    ...
    expect(wrapper.get("button[aria-label='Управлять назначением лида']").text()).toContain(
      'Назначить',
    );
  ...
  it('mounts active alerts only for the exact alert-read permission', async () => {
    ...
    expect(wrapper.text()).toContain('Управление сигналами доступно по отдельному разрешению');
  });
