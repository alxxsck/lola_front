...
const page = {
  id: 'ui-1', projectId: 'project-1', code: 'bonuses', name: 'Бонусы', kind: 'PAGE' as const,
  route: '/bonuses', config: {}, enabled: true, aiEnabled: false, aiDescription: null, aiAliases: [],
}
...
describe('InterfacePage AI target exposure', () => {
  beforeEach(() => {
    ...
    mocks.updateElement.mockImplementation(async (_projectId: string, _id: string, patch: object) => ({ ...page, ...patch }))
  })
  ...
  it('requires description and audit reason before an OWNER exposes a bound target to AI', async () => {
    ...
    const aiToggle = wrapper.findAllComponents(ToggleSwitch).find((item) => item.attributes('aria-label') === 'Доступно AI')!
    aiToggle.vm.$emit('update:modelValue', true)
    await wrapper.vm.$nextTick()
    await wrapper.get('#element-form').trigger('submit')

    expect(wrapper.text()).toContain('Описание для AI должно содержать от 20 до 1000 символов')
    expect(mocks.updateElement).not.toHaveBeenCalled()

    wrapper.findAllComponents(Textarea).find((item) => item.attributes('id') === 'ai-target-description')!.vm.$emit('update:modelValue', 'Страница, где пользователь просматривает доступные бонусы.')
    wrapper.findAllComponents(InputText).find((item) => item.attributes('id') === 'ai-target-aliases')!.vm.$emit('update:modelValue', 'награды, rewards')
    wrapper.findAllComponents(InputText).find((item) => item.attributes('id') === 'ai-target-audit-reason')!.vm.$emit('update:modelValue', 'Expose bonuses target for OPEN_PAGE')
    await wrapper.vm.$nextTick()
    await wrapper.get('#element-form').trigger('submit')
    await flushPromises()
    ...
    expect(mocks.updateElement).toHaveBeenCalledWith('project-1', 'ui-1', {
      aiEnabled: true,
      aiDescription: 'Страница, где пользователь просматривает доступные бонусы.',
  ...
  it('keeps AI exposure controls disabled for an EDITOR', async () => {
    ...
    expect(wrapper.text()).toContain('Только OWNER может менять доступ AI')
  })
  ...
  it('requires an exposed target to be disabled through the audited editor flow', async () => {
    ...
    const activeToggle = wrapper.findAllComponents(ToggleSwitch)
