...
describe('страница профилей с единым рабочим пространством', () => {
  ...
  it('обновляет deep-link при выборе другого диалога в workspace', async () => {
    ...
    wrapper.getComponent(UserWorkspaceDialog).vm.$emit('conversation-selected', 'conversation-2');
    await flushPromises();
  ...
  it('открывает пользователя глубокой ссылки по защищённой проекции', async () => {
    ...
    expect(wrapper.getComponent(UserWorkspaceDialog).props('endUserId')).toBe(offPage.endUserId);
  });
  ...
  it('не пытается открывать deep link и не показывает ошибку для пустого route-параметра', async () => {
    ...
    expect(mocks.profile).not.toHaveBeenCalled();
    expect(wrapper.text()).not.toContain('Не удалось открыть пользователя по ссылке');
  });
  ...
  it('не применяет запоздалую страницу после более нового запроса', async () => {
    ...
    expect((wrapper.vm as unknown as { items: (typeof profile)[] }).items[0]?.endUserId).toBe(
      'new-result',
    );
  });
  ...
  it('сохраняет понятную продуктовую навигацию к настройке полей', async () => {
    ...
    expect(wrapper.find('button-stub[to="/profile-fields"]').exists()).toBe(true);
  });
