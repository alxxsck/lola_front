...
describe('ProjectUserAttributesPage', () => {
  ...
  it('explains profile fields in Russian without exposing internal platform terms', async () => {
    ...
    expect(wrapper.find('button-stub[label="Как передавать данные"]').exists()).toBe(true);
  ...
  it('publishes the reviewed command against the current publication head', async () => {
    const validatedWorkspace = structuredClone(workspace) as AttributeContractWorkspaceResponseDto;
  ...
  it('shows a backend validation message, code, and concrete field action', async () => {
    const invalidWorkspace = structuredClone(workspace) as AttributeContractWorkspaceResponseDto;
    ...
    expect(
      wrapper.find('button-stub[label="3. Опубликовать"]').attributes('disabled'),
    ).toBeDefined();
    vm.fixIssue(vm.errors[0]);
    expect(mocks.routerPush).toHaveBeenCalledWith('/profile-fields/definition-loyalty');
  });

  it('does not repeat the same warning from local and server validation', async () => {
    const warningWorkspace = structuredClone(workspace) as AttributeContractWorkspaceResponseDto;
  ...
  it('does not label a new or changed draft field as active', async () => {
    ...
    const publishedWorkspace = structuredClone(workspace) as AttributeContractWorkspaceResponseDto;
    ...
    expect(vm.fieldPublicationState({ ...published, label: 'Новый город' })).toBe('changed');
    expect(
    ...
    vm.workspace.draft.document.fields = [{ ...published, label: 'Новый город' }];
    await nextTick();
  ...
  it.each([
    ...
    expect(wrapper.text()).not.toContain('Черновик проверен — можно публиковать');
  });

  it('allows removing a saved field that only exists in the draft', async () => {
    const draftOnlyWorkspace = structuredClone(workspace) as AttributeContractWorkspaceResponseDto;
    ...
    const removeButton = wrapper.find('button-stub[aria-label="Удалить Черновой город"]');
  ...
  it('does not allow removing a field from the current revision', async () => {
    ...
    expect(wrapper.find('button-stub[aria-label="Удалить Опубликованный город"]').exists()).toBe(
      false,
    );
  });
