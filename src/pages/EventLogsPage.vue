...
const filters = reactive({
  eventCode: queryValue('eventCode'),
...
const eventOptions = computed(() => [{ label: 'Все события', value: '' }, ...eventDefinitions.value.map((item) => ({ label: `${item.name} · ${item.code}`, value: item.code }))])
const sourceOptions = [
...
const activeFilterCount = computed(() => [filters.eventCode, filters.externalUserId.trim(), filters.source, filters.status, filters.receivedFrom, filters.receivedTo, filters.occurredFrom, filters.occurredTo].filter(Boolean).length)
  ...
  <section class="page event-logs-page">
    ...
    <template v-else>
      <section class="filter-panel card">
        <div class="filter-main"><div class="field"><label for="event-filter">Событие</label><Select id="event-filter" v-model="filters.eventCode" :options="eventOptions" option-label="label" option-value="value" filter /></div><div class="field user-filter"><label for="user-filter">External user ID</label><span class="input-icon"><i class="pi pi-user" /><InputText id="user-filter" v-model="filters.externalUserId" class="mono" placeholder="user_123" @keydown.enter="applyFilters" /></span></div><div class="field"><label for="status-filter">Статус</label><Select id="status-filter" v-model="filters.status" :options="statusOptions" option-label="label" option-value="value" /></div><div class="field"><label for="source-filter">Источник</label><Select id="source-filter" v-model="filters.source" :options="sourceOptions" option-label="label" option-value="value" /></div></div>
      ...
      <div class="stream-summary"><span><i class="pi pi-lock" /> Snapshot страницы зафиксирован cursor‑ом</span><span v-if="appliedUserId"><i class="pi pi-user" /> Путь <strong class="mono">{{ appliedUserId }}</strong></span><span><i class="pi pi-desktop" /> {{ frontendCount }} frontend</span><span :class="{ danger: failedCount }"><i class="pi pi-exclamation-circle" /> {{ failedCount }} ошибок</span></div>
