...
function isCurrentProject(projectId: string, generation: number) {
  return generation === projectGeneration && auth.project?.id === projectId;
}
...
async function loadPage(
  cursor: string | undefined,
  index: number,
  requestFilters: EventLogFilters = appliedFilters.value,
  afterSuccess?: () => void,
) {
  const projectId = auth.project?.id;
...
function refreshLogs() {
  return loadPage(undefined, 0, appliedFilters.value, () => {
    pageCursors.value = [undefined];
  });
}
...
<template>
  <section class="page event-logs-page">
    <header class="page-header">
      ...
      <div class="header-actions">
        <div class="view-switch" role="group" aria-label="Вид журнала">
          ...
          >
            <i class="pi pi-table" /> Таблица</button
          ><button
            type="button"
    ...
    <template v-else>
      <section class="filter-panel card">
        <div class="filter-main">
          ...
          <div class="field user-filter">
            <label for="user-filter">External user ID</label
      ...
      <div v-if="loading" class="loading-list card">
        <Skeleton v-for="item in 8" :key="item" height="68px" />
      </div>
      <div v-else-if="!logs.length" class="empty card">
        <i class="pi pi-search" /><strong>События не найдены</strong>
        <p>Измените фильтры или временной диапазон.</p>
        <Button
          v-if="activeFilterCount"
        ...
        >
          ...
          <Column header="Главные параметры"
            ><template #body="{ data }"
              ...
              <span v-else class="muted">Пустой payload</span></template
            ></Column
          >
          ...
          <Column header="Получено"
            ><template #body="{ data }"
              ><div class="time-cell">
                <strong :title="formatDate(data.receivedAt)">{{
          ...
          <Column
            ><template #body="{ data }"
              ><Button
                ...
                rounded
                severity="secondary"
                :aria-label="`Открыть лог ${data.eventName}`"
                @click.stop="openDetail(data)" /></template
          ></Column>
        </DataTable>
      </div>
      ...
      <footer v-if="logs.length" class="pagination">
        <span>Страница {{ pageIndex + 1 }} · {{ logs.length }} записей</span>
        <div>
          <Button
            label="Назад"
  ...
  >
    <template #header
      ><div v-if="selectedLog" class="drawer-title">
        <div class="eyebrow">Event log · {{ selectedLog.eventCode }}</div>
        <h2>{{ selectedLog.eventName }}</h2>
      </div></template
    >
      ...
      <template v-else>
        <div class="detail-hero">
          <div>
            <span>Статус</span
            ><Tag
              :value="statusLabel(selectedLog.status)"
              :severity="severity(selectedLog.status)"
            />
          </div>
          <div>
            <span>Источник</span
        ...
        <section class="detail-section">
          <header>
            <div>
              ...
              <div>
                <h3>Что произошло</h3>
                <p>Событие и связь с каталогом</p>
              </div>
            </div>
            <Button
              label="Открыть event"
              ...
              severity="secondary"
              outlined
              @click="openEventDefinition(selectedLog)"
            />
          </header>
        ...
        <section class="detail-section">
          ...
          <pre>{{ json(selectedLog.payload) }}</pre>
        </section>
        <section class="detail-section">
          <header>
            <div>
              ...
              <div>
                <h3>Context</h3>
                <p>Контекст получения, не используемый как доверенный input</p>
              </div>
            </div>
          </header>
          <pre>{{ json(selectedLog.context) }}</pre>
        </section>
        <section v-if="selectedLog.processingResult" class="detail-section">
          <header>
            <div>
              <span class="section-index">04</span>
              <div>
                <h3>Результат обработки</h3>
                <p>Совпавшие сценарии и созданные runs</p>
              </div>
            </div>
          </header>
          <pre>{{ json(selectedLog.processingResult) }}</pre>
        </section>
        <div class="timestamps">
          <span>Произошло: {{ formatDate(selectedLog.occurredAt) }}</span
          ><span>Получено: {{ formatDate(selectedLog.receivedAt) }}</span>
        </div>
      </template>
    </div>
  </Drawer>
...
.filter-advanced {
  display: grid;
  grid-template-columns: 1.2fr 1.1fr 0.75fr 0.75fr;
  gap: 10px;
}
.filter-advanced {
  padding-top: 13px;
...
.stream-summary span:first-child {
  margin-right: auto;
}
.stream-summary i {
  margin-right: 5px;
}
.stream-summary .danger {
...
.event-dot.failed {
  ...
  box-shadow: 0 0 0 4px var(--status-danger-soft);
}
.event-cell strong,
.event-cell small,
.user-cell strong,
...
.timeline-item {
  position: relative;
  display: grid;
  grid-template-columns: 24px 78px minmax(0, 1fr) auto 18px;
  align-items: center;
  gap: 10px;
  width: 100%;
  border: 0;
...
.timeline-rail i.processed {
  background: var(--status-success);
  box-shadow: 0 0 0 4px var(--status-success-soft);
}
.timeline-rail i.failed {
  background: var(--status-danger);
  box-shadow: 0 0 0 4px var(--status-danger-soft);
}
...
@media (max-width: 1050px) {
  ...
  .log-table :deep(th:nth-child(3)),
  .log-table :deep(td:nth-child(3)) {
    display: none;
  }
}
@media (max-width: 700px) {
  ...
  .filter-panel footer > .p-button:last-child {
    width: 100%;
    order: 6;
  }
  .stream-summary span:first-child {
  ...
  .log-table :deep(th:nth-child(2)),
  .log-table :deep(td:nth-child(2)) {
    display: none;
  }
  .timeline {
    padding: 14px;
...
@media (max-width: 700px) {
  .stream-summary span:not(:first-child) {
    display: none;
  }
}
</style>
