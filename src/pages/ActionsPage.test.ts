...
const action = {
  ...
  nameOverride: null, descriptionOverride: null, scenarioEnabled: false, aiEnabled: true,
  aiUsageDescription: 'Use when the user explicitly asks to open bonuses.', configuration: {}, lifecycle: 'ACTIVE', createdAt: 'now', updatedAt: 'now',
  ...
  actionTypeRevision: {
    ...
    executorAdapter: 'FRONTEND_COMMAND', inputSchema: {}, resultSchema: {}, projectConfigSchema: { type: 'object', properties: {}, required: [], additionalProperties: false }, uiSchema: { fields: [] },
    supportedSurfaces: ['SCENARIO', 'AI'], risk: 'UI_EFFECT', confirmationPolicy: 'NEVER', multipleInstances: false,
  },
} satisfies ProjectAction

const store = {
  catalogByProject: { 'project-1': [{ id: 'type-1', key: 'OPEN_PAGE', origin: 'SYSTEM', ownerProjectId: null, activeRevisionId: 'revision-1', activeRevision: action.actionTypeRevision }] },
...
vi.mock('@/features/auth/auth.store', () => ({ useAuthStore: () => ({ project: { id: 'project-1' }, user: { role: 'OWNER' } }) }))
vi.mock('@/features/project-actions/model/project-actions.store', () => ({ useProjectActionsStore: () => store }))
...
describe('ActionsPage', () => {
  ...
  it('loads Project Actions and reconciles an OWNER edit through the domain store', async () => {
    ...
    await flushPromises()
    expect(mocks.loadPreview).toHaveBeenCalledWith('project-1', 'action-1', false)

    const editor = wrapper.getComponent(ProjectActionEditor)
    editor.vm.$emit('save', { scenarioEnabled: false, aiEnabled: true })
    await flushPromises()

    expect(mocks.configure).toHaveBeenCalledWith('project-1', 'action-1', { scenarioEnabled: false, aiEnabled: true })
    expect(mocks.loadPreview).toHaveBeenLastCalledWith('project-1', 'action-1', true)
  })
  ...
  it('loads authoritative previews for the AI-only overview', async () => {
    ...
    await flushPromises()
    const aiTab = wrapper.findAll('.view-tabs button').find((button) => button.text().includes('AI capabilities'))!
    ...
    await flushPromises()
    expect(mocks.loadPreview).toHaveBeenCalledWith('project-1', 'action-1', true)
  })
})
