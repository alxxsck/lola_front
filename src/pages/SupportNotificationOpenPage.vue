...
async function resolve(): Promise<void> {
  ...
  try {
    ...
    if (!project || target.target !== "SUPPORT_OPERATOR_WORKSPACE" || target.selection.kind !== "CASE") {
      clearSupportNotificationCapability();
      failed.value = true;
      return;
    }
...
<template>
  <section class="notification-open-page">
    ...
    <div v-else class="open-card" role="alert">
      ...
      <Button label="Открыть входящие" icon="pi pi-inbox" @click="router.replace('/support/inbox')" />
    </div>
  </section>
</template>

<style scoped>
.notification-open-page{display:grid;min-height:calc(100dvh - 90px);place-items:center;padding:24px}.open-card{display:grid;justify-items:center;width:min(440px,100%);padding:34px;border:1px solid var(--line);border-radius:22px;background:var(--surface-card);box-shadow:var(--shadow-dialog);text-align:center}.open-icon{display:grid;width:52px;height:52px;place-items:center;margin-bottom:12px;border-radius:16px;background:var(--brand-soft);color:var(--text-brand);font-size:1.1rem}.open-icon.muted{background:var(--surface-subtle);color:var(--text-muted)}.open-card h1{margin:0;font-size:1.4rem}.open-card p{margin:10px 0 20px;color:var(--text-muted);font-size:.76rem;line-height:1.55}
</style>
