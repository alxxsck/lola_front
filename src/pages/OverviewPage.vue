...
import type { ActivityItem, DashboardStats } from '@/shared/types/domain'
...
const activityMeta: Record<ActivityItem['type'], { icon: string; label: string; severity: 'info' | 'success' | 'warn' | 'danger' | 'secondary' }> = {
  EVENT: { icon: 'pi pi-bolt', label: 'Событие', severity: 'info' },
  MESSAGE: { icon: 'pi pi-comment', label: 'Сообщение', severity: 'success' },
  SCENARIO: { icon: 'pi pi-sitemap', label: 'Сценарий', severity: 'warn' },
  COMMAND: { icon: 'pi pi-send', label: 'Команда', severity: 'secondary' },
  ERROR: { icon: 'pi pi-exclamation-triangle', label: 'Ошибка', severity: 'danger' },
}
...
async function loadDashboard() {
  ...
  try {
    ...
    activity.value = nextActivity.slice(0, 6)
  } catch (cause) {
    ...
    <section class="dashboard-grid">
      <div class="card activity-card">
        <div class="section-heading">
          ...
          <RouterLink :to="repository.mode === 'api' ? '/operations' : '/users'" class="section-link">{{ repository.mode === 'api' ? 'Открыть операции' : 'Все пользователи' }} <i class="pi pi-arrow-right" /></RouterLink>
        </div>
        ...
        <div v-else-if="activity.length" class="activity-list">
          <article v-for="item in activity" :key="item.id" class="activity-item">
            <span class="activity-icon"><i :class="activityMeta[item.type].icon" /></span>
            <div class="activity-copy">
              <div class="activity-title"><strong>{{ item.title }}</strong><Tag :value="activityMeta[item.type].label" :severity="activityMeta[item.type].severity" rounded /></div>
              <p>{{ item.description }}</p>
            </div>
            <time :datetime="item.timestamp">{{ relativeTime(item.timestamp) }}</time>
          </article>
        </div>
        <div v-else class="empty"><i class="pi pi-inbox" />{{ repository.mode === 'api' ? 'Детальный поток доступен в операционном центре.' : 'Активность появится после первых событий.' }}</div>
      </div>
