...
import { VueFlow, type Node, type NodeDragEvent, type ViewportTransform } from '@vue-flow/core';
...
const localizationPolicy = ref<ScenarioLocalizationPolicyDto>(defaultLocalizationPolicy());
...
const ruleBuilder = ref<{
  focusIssue: (target: { nodeId?: string; fieldPath?: string; message?: string }) => void;
} | null>(null);
...
const admissionSettings = ref<ScenarioAdmissionSettingsResponseDto | null>(null);
...
const translationStates = reactive<Record<string, Record<string, TranslationUiState>>>({});
...
function requestTranslation(payload: { fieldPath: string; targets: string[] }) {
  ...
  saveError.value = '';
  void translationController.start({ ...payload, sourceLocale }).catch((cause: unknown) => {
...
function retryTranslation(payload: { fieldPath: string; locale: string }) {
  void translationController.retry(payload.fieldPath, payload.locale).catch((cause: unknown) => {
...
const canReadProjectActions = computed(() =>
  hasProjectPermission(auth.project?.effectivePermissionCodes ?? [], 'project.actions.read'),
);
const projectActions = computed(() =>
  canReadProjectActions.value ? projectActionsStore.actionsForProject(auth.project?.id ?? '') : [],
);
const actionCatalogProjection = computed(() => projectScenarioActionCatalog(projectActions.value));
const actionCatalog = computed(() => actionCatalogProjection.value.catalog);
const scenarioPickerActions = computed(() => scenarioAvailableActions(actionCatalog.value));
const actionsError = computed(
  () => actionsLoadError.value || actionCatalogProjection.value.error?.message || '',
);
const selectedAction = computed(
  () => form.actions.find((action) => action.nodeKey === selectedNodeKey.value) ?? null,
);
...
const canChooseFirstAction = computed(() => canEdit.value && form.actions.length > 1);
...
const actionOutlineItems = computed(() =>
  form.actions.map((action) => {
    const definition = findScenarioActionCatalogItem(actionCatalog.value, action.type);
    const issues = actionIssues.value.filter((issue) => issue.nodeKey === action.nodeKey);
...
const filteredActionOutlineItems = computed(() => {
  ...
  return actionOutlineItems.value.filter((item) => {
    ...
    return [item.label, item.action.type, item.action.nodeKey, item.summary].some((value) =>
      String(value ?? '')
...
const graphIsLarge = computed(() => scenarioGraphShowsMinimap(form.actions.length));
...
const canManage = computed(() =>
  hasProjectPermission(auth.project?.effectivePermissionCodes ?? [], 'project.scenarios.write'),
);
const canPublishScenario = computed(() =>
  hasProjectPermission(auth.project?.effectivePermissionCodes ?? [], 'project.scenarios.publish'),
);
...
const usesGlobalFrequency = computed(() => admissionSettings.value?.mode === 'PROJECT_GLOBAL_V1');
...
const sourceSnapshotUnavailable = computed(
  () =>
    !authoringEditable.value && authoringUnavailableReason.value === 'SOURCE_SNAPSHOT_UNAVAILABLE',
);
...
const selectedAuthoringEvent = computed(() =>
  authoringContract.value?.events.find((event) => event.definitionId === form.eventDefinitionId),
);
...
const ruleSerialization = computed(() =>
  ruleContext.value ? serializeRuleDraft(ruleDraft.value, ruleContext.value) : null,
);
...
const audienceSummary = computed(() =>
  audienceContext.value ? summarizeAudience(audienceDraft.value, audienceContext.value) : null,
);
...
>(() => [
  ...
  },
  {
    ...
    status: actionIssues.value.length ? 'invalid' : form.actions.length ? 'valid' : 'empty',
  },
  {
    key: 'delivery',
    label: 'Доставка',
  ...
  const profilePaths =
    audience?.version === 2
      ? (
          ...
          audience.attributes
            .filter((attribute) => attribute.authoringAvailability === 'AVAILABLE')
...
const templateVariables = computed(() => {
  const typedByPath = new Map(
    templateAttributes.value.map((attribute) => [`user.attributes.${attribute.key}`, attribute]),
  );
...
const baseGraphViewModel = computed(() =>
  buildScenarioGraphViewModel({
    ...
    presentAction: (action) => {
      const item = actionOutlineItems.value.find(({ action: candidate }) => candidate === action);
      const definition = findScenarioActionCatalogItem(actionCatalog.value, action.type);
...
const graphPresentationLayout = ref<ScenarioGraphLayout>(createAutoScenarioGraphLayout());
const graphPresentationScope = computed<ScenarioGraphLayoutScope | null>(() => {
  ...
  const routeScenarioId =
    typeof route.params.scenarioId === 'string' ? route.params.scenarioId : 'new';
  const scenarioId = form.id ?? (routeScenarioId !== 'new' ? routeScenarioId : '');
  return operatorId && projectId && scenarioId ? { operatorId, projectId, scenarioId } : null;
});
...
const graphLayoutStatus = ref<'idle' | 'loading' | 'laid-out' | 'fallback'>('idle');
...
interface ScenarioGraphFlowApi {
  ...
  setViewport?: (viewport: ViewportTransform, options?: { duration?: number }) => Promise<boolean>;
}
...
async function performGraphAutoLayout(
  ...
  graphViewModel.value = mergeScenarioGraphPresentation(result.viewModel, baseGraphViewModel.value);
...
function requestExplicitGraphAutoLayout() {
  setGraphPresentationLayout(createAutoScenarioGraphLayout());
...
function initializeGraphFlow(instance: ScenarioGraphFlowApi) {
  ...
  if (savedViewport && instance.setViewport) {
...
watch(
  ...
  (model) => {
    if (graphPresentationLayout.value.mode === 'manual') {
      ...
      const reconciled = reconcileScenarioGraphLayout(graphPresentationLayout.value, model);
    ...
    if (fingerprint === graphLayoutFingerprint) {
      graphViewModel.value = mergeScenarioGraphPresentation(graphViewModel.value, model);
      return;
    }
    graphLayoutFingerprint = fingerprint;
    graphViewModel.value = model;
...
function selectGraphLayoutMode(mode: ScenarioGraphLayoutMode) {
  ...
  const manual = createManualScenarioGraphLayout(graphViewModel.value, viewport);
  setGraphPresentationLayout(manual);
  graphViewModel.value = applyScenarioGraphLayout(graphViewModel.value, manual);
  graphLayoutStatus.value = 'idle';
}
...
function rememberGraphViewport(viewport: ViewportTransform) {
  const next = updateScenarioGraphViewport(graphPresentationLayout.value, viewport);
  setGraphPresentationLayout(next);
}
...
async function load() {
  ...
  try {
    const [scenarios] = await Promise.all([
      ...
      canReadProjectActions.value
        ...
        : Promise.resolve().then(() => {
            actionsLoadError.value = 'У вас нет права читать Project Actions этого проекта.';
          }),
      ...
      repository.mode === 'api'
        ? attributeContractRepository
            .workspace(projectId)
            .then((workspace) => {
              const publishedFields = workspace.currentPublication?.document.fields ?? [];
              templateAttributeKeys.value = publishedFields
                .filter((field) => field.policies.templateRead && field.lifecycle !== 'ARCHIVED')
                .map((field) => field.key);
              ...
              templatePolicyWarnings.value = publishedFields
                .filter((field) => field.policies.templateRead && field.lifecycle === 'DEPRECATED')
                .map(
                  (field) =>
    ...
    const scenario =
      scenarioId && scenarioId !== 'new' ? scenarios.find((item) => item.id === scenarioId) : null;
    ...
    if (scenario) {
      ...
      form.respectsQuietHours = admissionSettings.value?.quietHours.enabled === true;
      const catalogDefinitionIds = new Set(
        authoringContract.value?.events.map((event) => event.definitionId) ?? [],
      );
      form.eventDefinitionId =
        events.value.find((event) => event.enabled && catalogDefinitionIds.has(event.id))?.id ??
    ...
    if (authoringContract.value?.localization?.enabled) {
      form.actions.splice(
        0,
        form.actions.length,
        ...normalizeLocalizedActionContent(form.actions, authoringContract.value.localization),
      );
    }
...
function focusDraftIssue(issue: { code: string; path: string; message: string; locale?: string }) {
  ...
  if (
    ...
    if (action) {
      ...
      const pathLocale =
        issue.locale ?? [...segments].reverse().find((segment) => projectLocales.has(segment));
...
function published(
  ...
  saveNotice.value = publishedStateUnchanged
...
function addNode(type: string) {
  ...
  void nextTick(() => {
    const candidates = Array.from(
      document.querySelectorAll<HTMLElement>(`[data-action-node-key="${node.nodeKey}"]`),
    );
...
function visibleActionFocusFallback() {
  ...
  return (
    candidates.find((candidate) => candidate.getClientRects().length > 0) ?? candidates[0] ?? null
  );
}
...
function restoreActionViewFocus() {
  ...
  void nextTick(() => {
    const targetVisible = target?.isConnected && target.getClientRects().length > 0;
...
function toggleGraphExpanded() {
  if (graphExpanded.value) {
    ...
    if (compactActionLayout.value && selectedAction.value && !openedFromOutlineCenter) {
      focusActionInspector();
    } else {
...
function createTarget(
  ...
  else if (kind === 'timeout') source.config = { ...source.config, onTimeout: target };
  else if (kind === 'goal') source.config = { ...source.config, onGoal: target };
  else if (kind === 'condition' && index !== undefined)
    source.config.branches = conditionBranches(source).map((branch, branchIndex) =>
...
function activateGraphNodeFromKeyboard(event: KeyboardEvent) {
  ...
  const node = flowNodes.value.find((candidate) => candidate.id === nodeElement.dataset.id);
  if (!node) return;
...
async function centerGraphNode(nodeKey: string) {
  ...
  if (graphLayoutTimer !== undefined || graphLayoutEngine || graphLayoutStatus.value === 'loading')
    return;
...
function createTypeReplacementPreview(
  ...
  const definition = findScenarioActionCatalogItem(actionCatalog.value, type);
  const currentDefinition = findScenarioActionCatalogItem(actionCatalog.value, action.type);
  ...
  return {
    ...
    targetType: type,
    plan: planScenarioActionTypeReplacement(action, definition, defaultConfig, localizedKeys),
    sourceFingerprint: actionChangeFingerprint(),
    ...(refreshed ? { refreshed: true } : {}),
  };
}

function scheduleActionChangePreview(preview: ScenarioActionChangePreview) {
...
async function save() {
  if (!canEdit.value) {
    saveError.value =
      authoringUnavailableReason.value ?? 'Исходные настройки доступны только для просмотра.';
    return;
  }
  ...
  const ruleResult =
    currentRuleSummary.status === 'empty' ? null : serializeRuleDraft(ruleDraft.value, context);
  ...
  if (audienceResult && !audienceResult.ok) {
    studioStage.value = 'audience';
    saveError.value =
      audienceResult.issues[0]?.message ?? 'Черновик аудитории заполнен не полностью';
    return;
  }
  ...
  if (!deliveryResult.ok) {
    studioStage.value = 'delivery';
    saveError.value =
      deliveryResult.issues[0]?.message ?? 'Политика доставки заполнена не полностью';
    return;
  }
  saving.value = true;
  try {
    ...
    const payload = {
      ...
      respectsQuietHours: form.importanceClass === 'SECURITY' ? false : form.respectsQuietHours,
      cooldownSeconds: usesGlobalFrequency.value ? undefined : form.cooldownSeconds || undefined,
      maxRunsPerUser: usesGlobalFrequency.value ? undefined : form.maxRunsPerUser || undefined,
  ...
  <div class="scenario-studio">
    <header class="studio-header">
      <div class="header-left">
        ...
        />
        <div>
          <span>Редактор сценария</span><strong>{{ form.name || 'Новый сценарий' }}</strong>
        </div>
      </div>
      <div class="header-center">
        <span>{{ stages.find((stage) => stage.key === studioStage)?.label }}</span
        ><span v-if="currentDraftVersion">Черновик v{{ currentDraftVersion }}</span
        ><span
          :class="{
            invalid:
              actionIssues.length ||
    ...
    <template v-else>
      <Message v-if="saveError" severity="error" class="save-error" closable @close="saveError = ''"
        ><span>{{ saveError }}</span
        ><Button
          v-if="draftConflict"
      ...
      >
        ...
        <div>
          ...
          <p v-if="!changeDraftStarted">
            Создайте черновик на её основе. Новые запуски получат изменения только после публикации,
            а активные продолжат исходную версию.
          </p>
          <p v-else>
            Новые запуски перейдут на неё только после публикации. Активные запуски останутся на
            исходной версии; их миграция выполняется отдельно.
          </p>
        </div>
        <Button
      ...
      <div
        ...
        :class="[
          `stage-${studioStage}`,
          scenarioGraphWorkspaceEnabled ? 'graph-workspace-v2' : 'graph-workspace-fallback',
          {
      ...
      >
        <aside class="studio-sidebar">
          ...
          <section v-if="studioStage === 'actions'" class="action-workflow-nav">
            ...
            <p v-else-if="form.actions.length" class="action-outline-empty" role="status">
              {{
                actionOutlineIssuesOnly
                  ? 'Действий с ошибками не найдено.'
                  : 'Поиск не нашёл действий.'
              }}
            </p>
        ...
        >
          <header v-if="form.actions.length" class="graph-toolbar">
            ...
            <div class="graph-toolbar-actions">
              <div v-if="graphLocaleOptions.length > 1" class="graph-locale-control">
              ...
              <button
                type="button"
                :aria-label="
                  graphExpanded ? 'Вернуться к настройке действия' : 'Развернуть схему сценария'
                "
          ...
          <details v-if="authoringContract?.localization" class="localization-policy-card">
            <summary>
              Языки контента ·
              {{
          ...
          <Message
            ...
            >Поля профиля для сообщений недоступны: {{ templatePolicyError }}</Message
          >
          <Message
          ...
          >
          <Message v-if="actionsError" severity="warn" :closable="false" class="actions-warning"
          ...
          <section class="mobile-action-outline" aria-label="Линейный список действий и ожиданий">
            <header>
              <div>
                ...
                ><strong>{{
                  russianCount(form.actions.length, 'действие', 'действия', 'действий')
                }}</strong>
              </div>
              <small>Выберите действие для настройки или откройте схему целиком.</small>
            </header>
            <button
            ...
            <div
              v-if="form.actions.length"
              class="action-outline-tools mobile-action-outline-tools"
            >
              <label class="action-outline-search">
                <i class="pi pi-search" aria-hidden="true" />
                <input
          ...
          >
            ...
            <div>
              ...
              <p>
                Например, отправьте сообщение или дождитесь события. После добавления откроется
                схема сценария.
              </p>
            </div>
            <ActionPicker
        ...
        <main v-else-if="studioStage === 'eligibility'" class="rule-canvas">
          <header class="stage-section-header">
            <div>
              ...
              <p>Добавьте проверки текущего события или недавних действий пользователя.</p>
            </div>
          </header>
          ...
          >
            ...
            >
              <i
                :class="ruleValidationStatus.ready ? 'pi pi-check-circle' : 'pi pi-info-circle'"
                aria-hidden="true"
              />
              <span>
          ...
          <div v-else-if="ruleContext" class="stage-empty">
            ...
            <template v-if="sourceSnapshotUnavailable">
              ...
              <p>
                Сценарий продолжает использовать условия опубликованной версии, но показать их без
                сохранённого исходника редактора нельзя.
              </p>
            </template>
          ...
          <div v-else class="stage-empty">
            ...
            <p>
              Конструктор использует точную ревизию Event из каталога. Мы не связываем ревизии
              только по одинаковому коду.
            </p>
        ...
        <main v-else-if="studioStage === 'audience' && audienceContext" class="audience-canvas">
          <Message v-if="!canManage" severity="info" :closable="false"
            >Аудитория и история сегментов доступны для просмотра. Изменять и публиковать их могут
            только владельцы и администраторы.</Message
          >
          <Message v-if="audienceSegmentsError" severity="warn" :closable="false"
            >{{ audienceSegmentsError }}. Условия по языку, стране и полям профиля остаются
            доступны.</Message
          >
          <nav class="segment-library-link" aria-label="Управление сегментами">
            <div>
              ...
              ><small
                >Сценарий только закрепляет точную опубликованную версию и никогда не обновляет её
                автоматически.</small
              >
            </div>
            <RouterLink :to="{ name: 'segments' }">Управление сегментами</RouterLink
            ><RouterLink :to="{ name: 'segments-guide' }">Как работают сегменты</RouterLink
            ><RouterLink
              v-if="canManage"
              :to="{
                name: 'segment-create',
          ...
          <div v-else class="readonly-stage-card">
            ...
            <div>
              ...
              <template v-if="sourceSnapshotUnavailable">
                ...
                <p>
                  Опубликованная версия продолжает работать, но её аудиторию нельзя достоверно
                  восстановить по данным выполнения.
                </p>
              </template>
              <p v-else>
        ...
        <main v-else class="stage-overview">
          <div v-if="studioStage === 'trigger'" class="overview-card">
            ...
            <p>
              Это событие становится поводом проверить сценарий. Поля текущего события затем
              доступны в конструкторе условий.
            </p>
            <div v-if="selectedAuthoringEvent" class="selected-trigger">
              <i class="pi pi-bolt" />
              <div>
                <strong>{{
                  eventDisplayName(selectedAuthoringEvent.code, selectedAuthoringEvent.name)
                }}</strong
            ...
            <Message v-else severity="warn" :closable="false"
              >Выбранная версия события отсутствует в каталоге сценариев. Выберите доступную
              версию.</Message
            >
          </div>
          <div v-else-if="studioStage === 'audience'" class="overview-card unavailable">
            <span class="eyebrow">Этап 2</span>
            <h1>Аудитория временно недоступна</h1>
            <p>
              Каталог аудитории не пришёл в текущем контракте. Сценарий без аудитории остаётся
              совместимым и может быть опубликован как раньше.
            </p>
            <Message severity="warn" :closable="false"
              >Обновите серверный контракт и повторите загрузку. Старые условия user.* не
              переносятся автоматически: это защищает их семантику.</Message
            >
          </div>
          <div v-else-if="studioStage === 'delivery'" class="delivery-stage">
            ...
            <div v-else class="readonly-stage-card">
              ...
              <div>
                ...
                <template v-if="sourceSnapshotUnavailable">
                  ...
                  <p>
                    Сценарий продолжает доставлять сообщения по опубликованной версии. Пустые
                    значения редактора здесь не показываются, чтобы не исказить фактическое
                    поведение.
                  </p>
                </template>
            ...
            <Message severity="info" :closable="false"
              >Ожидание пользователя в сети не продлевает срок цели. При публикации условия и
              аудитория фиксируются по версии; перед доставкой обе проверки выполняются
              независимо.</Message
            ...
            ><Message v-else-if="!canManage" severity="info" :closable="false"
              >У вас есть право публикации, но нет права изменять сценарий. Попросите редактора
              подготовить и сохранить новую версию.</Message
            ><Message v-else-if="!authoringEditable" severity="info" :closable="false"
              >Чтобы изменить настройки и выпустить новую версию, восстановите исходник редактора
              или создайте новый сценарий.</Message
            ><Message v-else-if="!canEdit" severity="info" :closable="false"
              >Сначала создайте черновик изменений на основе опубликованной версии. После сохранения
              его можно проверить и опубликовать.</Message
            ><Message v-else severity="warn" :closable="false"
        ...
        <aside
          v-if="studioStage === 'eligibility' && ruleContext && canEdit && validationOpen"
          ...
          >
            <div class="settings-head readonly-action-head">
              <div>
                ...
                <h2>
                  {{
                    findScenarioActionCatalogItem(actionCatalog, selectedAction.type)?.name ??
                    selectedAction.type
                  }}
                </h2>
              </div>
        ...
        <aside v-else-if="studioStage === 'trigger'" class="settings-panel">
          ...
          <section>
            <div class="field">
              ...
              ><InputText id="scenario-name" v-model="form.name" placeholder="Предложение бонуса" />
            </div>
            <div class="field">
          ...
          <section>
            ...
            <div class="field">
              ...
              <div v-if="firstAction" class="first-action-card" data-testid="scenario-first-action">
                <span><i class="pi pi-play" /></span>
                <div>
                  <strong>{{ firstActionDefinition?.name ?? firstAction.type }}</strong>
              ...
              <small v-if="firstAction && form.actions.length > 1 && canEdit">
                Сначала покажем влияние на связи и недостижимые шаги. Изменение применится только
                после подтверждения.
              </small>
              <small v-else-if="!firstAction"
                >Добавьте действие на этапе «Действия» — оно станет первым после события
                запуска.</small
              >
            </div>
            <div class="field">
              ...
              >
                ...
                <div>
                  ...
                  ><small>Управление активной версией выполняется через публикацию V2.</small>
                </div>
              </div>
              <Select
                v-else
              ...
              /><small v-if="form.status !== 'ACTIVE'"
                >Активация V2 выполняется только атомарной публикацией версии.</small
              >
            </div>
            <div class="field">
              ...
              ><Select
                ...
                option-label="label"
                option-value="value"
              /><small>Настройка применяется ко всем сообщениям в рамках запуска сценария.</small>
            </div>
            ...
            <Message v-if="form.importanceClass === 'SECURITY'" severity="warn" :closable="false">
              <strong>Вне общих лимитов.</strong> Сценарий безопасности игнорирует частоту и тихие
              часы. Для сохранения и публикации нужны специальные права и причина.
            </Message>
            <label class="quiet-hours-control">
              <span>
                ...
                <small v-else-if="admissionSettings && !admissionSettings.quietHours.enabled">
                  В проекте тихие часы выключены; настройка сохранится на будущее.
                </small>
                <small v-else>
            ...
            <p class="priority-helper">
              Если одно событие подходит нескольким сценариям, запускается один: сначала класс
              «Безопасность», затем большее значение priority. При равенстве порядок детерминирован
              системой.
            </p>
            <Message v-if="usesGlobalFrequency" severity="info" :closable="false">
              Частота задаётся общими настройками проекта. Индивидуальные лимиты этого сценария
              доступны только в технической истории.
            </Message>
            <template v-else>
              ...
              <small class="legacy-frequency-note"
                >Устаревающий режим. Перейдите на общую частоту в настройках проекта.</small
              >
            </template>
          </section>
        </aside>
...
.action-outline-search input::placeholder {
  color: var(--text-small-muted);
}
...
.action-outline-error-filter:disabled {
  cursor: default;
  opacity: 0.45;
}
.action-outline-list {
...
.action-outline-row.active .action-outline-center {
  opacity: 1;
}
...
@container scenario-studio (max-width:860px) {
  ...
  .studio-grid:is(
    ...
    ).stage-actions.has-action-inspector {
    grid-template-columns: minmax(0, 1fr);
