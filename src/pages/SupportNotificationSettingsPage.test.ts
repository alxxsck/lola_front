...
describe('SupportNotificationSettingsPage', () => {
  ...
  it('renders the three-layer readiness and the three independent personal topics', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.text()).toContain('Подписка этого браузера'));
  ...
  it('shows the project policy equation only with the exact manage permission', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.find('.policy-equation').exists()).toBe(true));
    await flushPromises();
    await vi.waitFor(() => expect(wrapper.text()).toContain('Личная подписка'));
    expect(wrapper.text()).toContain('Личная подписка');
    expect(wrapper.get("a[href='/support/settings/notifications/new-cases']").text()).toContain(
      'Настроить политику',
    );
  });
  ...
  it('explains why browser permission cannot be requested while device registration is disabled', async () => {
    ...
    await vi.waitFor(() =>
      expect(wrapper.text()).toContain('Регистрация новых браузеров временно недоступна'),
    );
    expect(wrapper.get('button[disabled]').text()).toContain('Подключить этот браузер');
  });
});
