...
async function copyEventContract(item: EventDefinition) {
  try {
    await navigator.clipboard.writeText(formatEventContractMarkdown({
      name: item.name,
      code: item.code,
    ...
    <div v-else-if="filteredEvents.length" class="events-list">
      <article v-for="item in filteredEvents" :key="item.id" class="event-card card" :class="{ inactive: !item.enabled, system: isSystemEvent(item) }">
        ...
        <div class="event-actions">
          ...
          <Button label="Скопировать" icon="pi pi-copy" severity="secondary" text size="small" :aria-label="`Скопировать контракт события ${item.name}`" @click="copyEventContract(item)" />
