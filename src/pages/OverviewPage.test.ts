...
describe("OverviewPage activity", () => {
  ...
  it("fills the project feed from the event log endpoint", async () => {
    ...
    expect(wrapper.get(".section-link").attributes("data-to")).toBe(
      "/event-logs",
    );
  });
  ...
  it("shows the loading and empty activity states", async () => {
    ...
    expect(wrapper.get(".empty").text()).toContain(
      "Активность появится после первых событий",
    );
  });
  ...
  it("does not render dashboard links or load activity outside exact Permissions", async () => {
    ...
    expect(mocks.getStats).toHaveBeenCalledWith("project-1", [
  ...
  it("shows the Project AI command surface only with both execution Permissions", async () => {
    mocks.auth.project.effectivePermissionCodes = [
    ...
    expect(wrapper.findComponent(AICommandComposer).exists()).toBe(true);

    mocks.auth.project.effectivePermissionCodes = ["project.cms_agent.use"];
    const missingRun = mountOverview();
    await flushPromises();

    expect(missingRun.findComponent(AICommandComposer).exists()).toBe(false);

    mocks.auth.project.effectivePermissionCodes = ["project.ai_analyses.run"];
    const missingUse = mountOverview();
    await flushPromises();

    expect(missingUse.findComponent(AICommandComposer).exists()).toBe(false);
  });
});
