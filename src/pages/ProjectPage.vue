  ...
  <div class="page project-page">
    ...
    <div v-else-if="project" class="settings-layout">
      <div class="settings-main stack">
        <form id="project-settings-form" class="settings-main-form stack" @submit.prevent="saveProject">
        ...
        <section class="card card-pad settings-section" :class="{ collapsed: !voiceSettingsExpanded }">
          <div class="section-title">
            <span class="section-icon blue"><i class="pi pi-microphone" /></span>
            <div><h2>Голосовой чат</h2><p>Настройте голосовые диалоги, голос Lola и манеру её речи.</p></div>
            <Button
              type="button"
              :icon="voiceSettingsExpanded ? 'pi pi-chevron-up' : 'pi pi-chevron-down'"
              severity="secondary"
              text
              rounded
              :aria-label="voiceSettingsExpanded ? 'Свернуть настройки голосового чата' : 'Развернуть настройки голосового чата'"
      ...
      <aside class="settings-aside stack">
        ...
        <section class="save-card">
          ...
          <Button type="submit" form="project-settings-form" label="Сохранить настройки" icon="pi pi-check" :loading="saving" :disabled="!isDirty" fluid />
        </section>
      </aside>
    </div>
...
<style scoped>
.project-status{display:flex;align-items:center;gap:11px;padding:11px 14px;background:var(--surface-card);border:1px solid var(--line);border-radius:14px}.project-status>span:last-child{display:flex;flex-direction:column}.project-status strong{font-size:.78rem}.project-status small{font-size:.65rem;color:var(--muted);margin-top:2px}.page-message{margin-bottom:18px}.message-row{display:flex;align-items:center;justify-content:space-between;gap:16px;width:100%}.settings-layout{display:grid;grid-template-columns:minmax(0,1fr) 310px;gap:18px;align-items:start}.settings-main-tts{margin-top:0}.settings-section{padding:26px}.section-title{display:flex;align-items:flex-start;gap:13px;padding-bottom:21px;margin-bottom:21px;border-bottom:1px solid var(--border-subtle)}.settings-section.collapsed .section-title{padding-bottom:0;margin-bottom:0;border-bottom:0}.section-title>div{min-width:0;flex:1}.section-title>button{flex:0 0 auto}.section-title h2{font-size:1.08rem}.section-title p{color:var(--muted);font-size:.76rem;margin:4px 0 0}.section-icon{display:grid;place-items:center;width:39px;height:39px;border-radius:12px;flex:0 0 auto}.section-icon.lime{background:var(--project-tone-lime-soft);color:var(--project-tone-lime-foreground)}.section-icon.violet{background:var(--project-tone-violet-soft);color:var(--project-tone-violet-foreground)}.section-icon.coral{background:var(--project-tone-coral-soft);color:var(--project-tone-coral-foreground)}.form-grid{display:grid;gap:18px}.form-grid.columns{grid-template-columns:minmax(180px,.7fr) minmax(260px,1.3fr)}.field small{font-size:.68rem;color:var(--text-small-muted);text-align:right}.assistant-fields{display:grid;grid-template-columns:140px minmax(0,1fr);gap:24px}.assistant-preview{display:flex;flex-direction:column;align-items:center;padding:22px 12px;background:var(--surface-subtle);border:1px solid var(--border-subtle);border-radius:17px}.assistant-orbit{display:grid;place-items:center;width:76px;height:76px;border-radius:50%;background:linear-gradient(145deg,var(--status-violet),var(--action-primary));box-shadow:0 0 0 8px var(--status-violet-soft),0 13px 26px color-mix(in srgb,var(--status-violet) 24%,transparent);margin:8px 0 20px}.assistant-orbit span{font:700 1.7rem Manrope;color:var(--project-assistant-on-orbit)}.assistant-preview small{font-size:.62rem;text-transform:uppercase;letter-spacing:.09em;color:var(--text-small-muted)}.assistant-preview strong{font-size:.84rem;margin-top:4px;text-align:center}.settings-aside{position:sticky;top:24px}.meta-card .eyebrow{margin-bottom:14px}.meta-row{padding:12px 0;border-top:1px solid var(--border-subtle)}.meta-row span,.meta-row code{display:block}.meta-row span{font-size:.68rem;color:var(--muted);margin-bottom:5px}.meta-row code{font-size:.72rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--text-primary)}.meta-card p{display:flex;gap:8px;margin:14px 0 0;padding:11px;background:var(--surface-subtle);border-radius:11px;color:var(--text-small-muted);font-size:.68rem;line-height:1.45}.meta-card p i{margin-top:2px}.save-card{display:flex;flex-direction:column;gap:14px;padding:20px;background:var(--surface-emphasis);color:var(--text-on-emphasis);border-radius:18px}.save-card strong,.save-card span{display:block}.save-card strong{font-size:.83rem}.save-card span{font-size:.69rem;color:var(--text-on-emphasis-muted);line-height:1.4;margin-top:4px}.save-card :deep(.p-message-text){font-size:.72rem}.skeleton-card{display:flex;flex-direction:column;gap:20px}.skeleton-card:nth-child(2){min-height:280px}
.system-prompt-control{position:relative}.system-prompt-textarea{display:block;resize:none;overflow:auto;padding-bottom:32px}.system-prompt-resizer{position:absolute;right:3px;bottom:3px;display:grid;place-items:center;width:34px;height:34px;padding:0;border:0;border-radius:9px;background:transparent;color:var(--text-secondary);cursor:ns-resize;touch-action:none;user-select:none}.system-prompt-resizer:hover{background:var(--surface-hover);color:var(--text-primary)}.system-prompt-resizer:focus-visible{outline:2px solid var(--focus-ring);outline-offset:-2px}.system-prompt-resizer i{font-size:.82rem;pointer-events:none}
...
.section-icon.green{background:var(--project-tone-green-soft);color:var(--project-tone-green-foreground)}.section-icon.blue{background:var(--project-tone-blue-soft);color:var(--project-tone-blue-foreground)}.integration-unknown{margin-left:auto;display:flex;align-items:center;gap:7px;color:var(--text-small-muted);font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em}.voice-settings{display:flex;flex-direction:column;gap:18px}.setting-toggle{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:15px}.setting-toggle strong,.setting-toggle span{display:block}.setting-toggle strong{font-size:.82rem}.setting-toggle span{max-width:620px;margin-top:4px;color:var(--muted);font-size:.7rem;line-height:1.45}.setting-toggle.compact{min-height:67px}.setting-toggle.disabled{opacity:.6}
.contract-link{display:flex;align-items:center;gap:12px;margin-top:18px;padding:13px 14px;transition:.16s ease}.contract-link:hover{border-color:color-mix(in srgb,var(--text-brand) 35%,var(--border-default));background:var(--brand-soft)}.contract-link-icon{display:grid;place-items:center;width:37px;height:37px;border-radius:11px;background:var(--brand-soft);color:var(--text-brand)}.contract-link>span:nth-child(2){flex:1}.contract-link strong,.contract-link small{display:block}.contract-link strong{font-size:.78rem}.contract-link small{font-size:.67rem;color:var(--text-small-muted);margin-top:3px}.contract-link>i{font-size:.75rem;color:var(--text-secondary)}
</style>
