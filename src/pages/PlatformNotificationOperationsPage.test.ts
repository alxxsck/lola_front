...
vi.mock('@/features/notification-operations/api/notification-operations.api', () => ({
...
describe('Platform notification operations page', () => {
  ...
  it('shows fresh login without replaying a denied command', async () => {
    ...
    await wrapper.get(`button[aria-label="Повторить доставку ${deliveryId}"]`).trigger('click');
    ...
    await wrapper.get('[data-action="notification-operations-fresh-login"]').trigger('click');
    await flushPromises();
