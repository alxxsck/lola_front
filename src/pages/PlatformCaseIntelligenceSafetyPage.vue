...
const canManage = computed(() =>
  (auth.user?.platformPermissionCodes ?? []).includes('platform.case_intelligence.safety.manage'),
);
const models = computed(() => catalog.value?.items ?? []);
const selectedModel = computed(() => models.value.find((item) => item.id === draft.value.modelId));
const issues = computed(() =>
  validatePlatformSafetyDraft(draft.value, models.value, catalog.value?.stale ?? false),
);
...
const publicationBlocked = computed(
  () => publicationOutcomeUnknown.value || issues.value.some((issue) => issue.path !== 'reason'),
);
const formStatus = computed(() => {
  if (validationVisible.value && issues.value.length) return `${issues.value.length} ошибок`;
  return draft.value.reason.trim() ? 'Готово к публикации' : 'Добавьте причину публикации';
});
...
const riskRows = Object.entries(platformSafetyClassLabels).map(([code, label]) => ({
  code: code as keyof typeof platformSafetyClassLabels,
  label,
  action: platformSafetyClassActions[code as keyof typeof platformSafetyClassActions],
}));
...
async function reconcileUnknownPublication(): Promise<void> {
  ...
  error.value = '';
  try {
    const next = await lookupPlatformCaseIntelligenceSafetyCommand(pendingCommandKey.value);
    state.value = next;
    pendingCommandKey.value = '';
    pendingPublication.value = null;
    publicationOutcomeUnknown.value = false;
  ...
  <section class="page platform-safety-page">
    <header class="page-header">
      <div>
        ...
        <p class="subtitle">
          Одна проверка для всех проектов. Проекты настраивают реакцию на риск, но не могут
          отключить саму защиту.
        </p>
      </div>
      <Button
        label="Обновить"
    ...
    <Message v-if="error" severity="error" :closable="false">
      <div class="message-action">
        ...
        />
        <Button v-else label="Повторить" size="small" severity="secondary" @click="load(false)" />
      </div>
    </Message>
    ...
    <section v-else class="safety-workbench">
      <article class="safety-status card" :data-active="Boolean(state)">
        ...
        <div>
          ...
          <p v-else>
            До активации Case Intelligence работает fail-closed: публикация проектных конфигураций
            заблокирована.
          </p>
        </div>
        <Tag
      ...
      <article class="coverage-rail card" aria-labelledby="coverage-title">
        ...
        <div class="coverage-rail__flow">
          <div>
            <i class="pi pi-building" /><span>Все проекты</span><small>текущие и новые</small>
          </div>
          <i class="pi pi-arrow-right" />
          <div>
            <i class="pi pi-language" /><span>Любой язык</span><small>включая неизвестный</small>
          </div>
          <i class="pi pi-arrow-right" />
          <div>
            <i class="pi pi-comments" /><span>Все каналы</span><small>текст, голос, Telegram</small>
          </div>
          <i class="pi pi-arrow-right" />
          <div>
            <i class="pi pi-shield" /><span>4 критических риска</span><small>единое ядро</small>
          </div>
        </div>
      </article>
      ...
      <form v-if="catalog" class="safety-form card" @submit.prevent="preparePublication">
        ...
        <section class="configuration-grid">
          <div class="configuration-copy">
            ...
            <div>
              ...
              <p>
                Выберите модель из каталога, который доступен текущему ключу xAI. Grok 4.5 —
          ...
          <div class="field-stack">
            <label>
              ...
              />
              <small v-if="validationVisible && issueMap.modelId" class="field-error">{{
                issueMap.modelId
              }}</small>
              <small v-else-if="selectedModel">
                Вход ${{ selectedModel.inputPricePerMillion }} · выход ${{
            ...
            <label>
              ...
              <Select
                ...
                fluid
              />
              <small v-if="validationVisible && issueMap.reasoningEffort" class="field-error">{{
                issueMap.reasoningEffort
              }}</small>
              <small v-else>
                {{
        ...
        <section class="risk-ledger" aria-labelledby="risk-title">
          ...
          <div class="risk-list">
            <article v-for="risk in riskRows" :key="risk.code">
              ...
              <Tag
                :value="risk.code === 'RESPONSIBLE_GAMING_CRISIS' ? 'Высокий' : 'Срочный'"
                :severity="risk.code === 'RESPONSIBLE_GAMING_CRISIS' ? 'warn' : 'danger'"
              />
            </article>
          </div>
        </section>
        ...
        <label class="reason-field">
          ...
          <small v-if="validationVisible && issueMap.reason" class="field-error">{{
            issueMap.reason
          }}</small>
        </label>
    ...
    >
      ...
      <template #footer>
        ...
        />
        <Button
          data-testid="publish-safety"
          label="Опубликовать для всех проектов"
          icon="pi pi-shield"
          :loading="publishing"
          @click="publish"
        />
...
.safety-status__mark {
  ...
  background: color-mix(in srgb, var(--danger-color) 10%, var(--surface));
  font-size: 1.2rem;
}
.safety-status[data-active='true'] .safety-status__mark {
...
.coverage-rail__flow span {
  font-size: 0.82rem;
  font-weight: 780;
}
.coverage-rail__flow small {
  color: var(--muted);
}
.coverage-rail__flow > i {
  color: var(--muted);
  font-size: 0.75rem;
}
.safety-form {
...
.confirmation-copy dl > div {
  display: grid;
  gap: 3px;
  padding: 10px;
  border-right: 1px solid var(--line);
}
.confirmation-copy dl > div:last-child {
  border-right: 0;
}
.confirmation-copy dt {
  color: var(--muted);
  font-size: 0.7rem;
}
.confirmation-copy dd {
  margin: 0;
  font-weight: 800;
}
