...
const canReadProductApi = computed(() =>
  (auth.project?.effectivePermissionCodes ?? []).includes('project.integration_api_requests.read'),
);
...
const selectedProductApiOutcome = computed(() =>
  selectedProductApiRequest.value
    ? PRODUCT_API_OUTCOME_PRESENTATION[selectedProductApiRequest.value.outcome]
    : null,
);
const productApiCopyMessage = computed(() => {
  ...
  return feedback.outcome === 'success' ? `${label} скопирован` : `Не удалось скопировать ${label}`;
});
...
const filteredRuns = computed(() =>
  scenarioRuns.value.filter(
    (item) =>
      ...
      (!query.value ||
        [item.scenarioCode, item.scenarioName, item.userExternalId, item.id].some((value) =>
...
const severity = (value: string): 'success' | 'danger' | 'warn' | 'info' | 'secondary' => {
  ...
  if (['RUNNING', 'WAITING_ACK', 'WAITING_INPUT', 'WAITING_TIME'].includes(value)) return 'warn';
  return 'secondary';
};
...
function auditRequest(cursor?: string) {
  return {
    ...
    ...(status.value !== 'ALL' ? { outcome: status.value as AuditEvent['outcome'] } : {}),
    ...(cursor ? { cursor } : {}),
  };
}
...
function productApiRequest(cursor?: string) {
  return {
    ...
    ...(status.value !== 'ALL' ? { outcome: status.value as ProductApiRequestLog['outcome'] } : {}),
    ...(cursor ? { cursor } : {}),
  };
}
...
async function copyProductApiValue(field: ProductApiCopyField, value: string) {
  try {
...
async function load() {
  ...
  if (results[0].status === 'fulfilled' && currentRunsRequestId === runsRequestId) {
    scenarioRuns.value = results[0].value.items;
    runsNextCursor.value = results[0].value.nextCursor;
  } else if (results[0].status === 'rejected' && currentRunsRequestId === runsRequestId)
    errors.value.runs = message(results[0].reason);
  if (results[1].status === 'fulfilled' && currentAuditRequestId === auditRequestId) {
  ...
  } else if (results[1].status === 'rejected' && currentAuditRequestId === auditRequestId)
...
async function refresh() {
  await load();
  if (section.value === 'productApi' && canReadProductApi.value) await loadProductApiPage();
}
...
async function loadProductApiPage() {
  ...
  try {
    const page = await repository.getProductApiRequestLogsPage(projectId, productApiRequest());
    ...
    if (requestId === productApiRequestId && auth.project?.id === projectId)
      errors.value.productApi =
        cause instanceof Error ? cause.message : 'Не удалось загрузить запросы Product API';
  } finally {
    if (requestId === productApiRequestId && auth.project?.id === projectId)
...
async function openProductApiRequest(item: ProductApiRequestLog) {
  ...
  try {
    ...
    if (requestId === productApiDetailRequestId)
      productApiDetailError.value =
        cause instanceof Error ? cause.message : 'Не удалось загрузить JSON запроса';
  } finally {
    if (requestId === productApiDetailRequestId) productApiDetailLoading.value = false;
  }
}
...
async function loadAuditPage() {
  ...
  try {
    ...
    if (requestId === auditRequestId && auth.project?.id === projectId)
      errors.value.audit =
        cause instanceof Error ? cause.message : 'Не удалось загрузить события аудита';
  } finally {
    if (requestId === auditRequestId && auth.project?.id === projectId) auditLoading.value = false;
  }
}
...
async function loadMoreAuditEvents() {
  ...
  try {
    ...
    if (requestId === auditRequestId && auth.project?.id === projectId)
      errors.value.audit =
        cause instanceof Error ? cause.message : 'Не удалось загрузить следующую страницу аудита';
  } finally {
    if (requestId === auditRequestId && auth.project?.id === projectId)
      loadingMoreAudit.value = false;
  }
...
async function loadMoreRuns() {
  ...
  try {
    ...
    if (requestId === runsRequestId && auth.project?.id === projectId)
      errors.value.runs =
        cause instanceof Error ? cause.message : 'Не удалось загрузить следующую страницу запусков';
  } finally {
    if (requestId === runsRequestId && auth.project?.id === projectId)
...
watch([section, search, status], ([currentSection], [previousSection]) => {
  ...
  if (currentSection === 'audit') {
    if (previousSection === 'audit' || auditLoadedFilterKey.value !== auditFilterKey())
      searchTimer = setTimeout(() => void loadAuditPage(), previousSection === 'audit' ? 300 : 0);
  ...
  <section class="page operations-page">
    <header class="page-header">
      <div>
        ...
        <p class="subtitle">Выполнение сценариев и действия администраторов в одном потоке.</p>
      </div>
      <Button
        label="Обновить"
        icon="pi pi-refresh"
    ...
    <Message v-if="sectionError" severity="error" class="mb"
      ...
      ><Button label="Повторить" icon="pi pi-refresh" size="small" text @click="refresh"
    /></Message>
    <Message
    ...
    <template v-else>
      <div class="filters card">
        ...
        <span class="data-source"
          ...
          {{
            repository.mode === 'api' ? 'Retenive Backend · live data' : 'Демонстрационные данные'
          }}</span
        >
      </div>
      ...
      <div class="card table-card">
        ...
        >
          ...
          <Column header="Статус"
            ><template #body="{ data }"
              ><Tag :value="data.status" :severity="severity(data.status)" rounded /></template
          ></Column>
          <Column header="Старт"
            ><template #body="{ data }"
        ...
        >
          ...
          <Column header="Действие"
            ><template #body="{ data }"
              ><div class="primary-cell">
                <strong>{{ data.operation || data.resourceType || data.eventType }}</strong
          ...
          <Column header="Статус"
            ><template #body="{ data }"
              ><Tag :value="data.outcome" :severity="severity(data.outcome)" rounded /></template
          ></Column>
          <Column header="Время"
            ><template #body="{ data }"
        ...
        >
          <template #empty
            ><div class="empty">
              <i class="pi pi-code" />Запросов Product API по выбранным фильтрам нет.
            </div></template
          >
          ...
          <Column header="Method"
            ><template #body="{ data }"
              ><Tag :value="data.method" severity="secondary" rounded /></template
          ></Column>
          <Column header="Path"
            ><template #body="{ data }"
          ...
          <Column header="Размер" class="mobile-hide"
            ><template #body="{ data }">{{ formatBytes(data.payloadBytes) }}</template></Column
          >
          <Column header="Duration"
            ><template #body="{ data }">{{ formatDuration(data.durationMs) }}</template></Column
          >
          <Column
            ><template #body="{ data }"
              ><button
        ...
        <div v-if="section === 'productApi' && productApiNextCursor" class="load-more">
          <Button
        ...
        <div class="steps">
          <article v-for="step in selectedRun.steps" :key="step.id" class="step-card">
            ...
            <div class="step-copy">
              ...
              <small class="mono muted">{{ step.nodeKey }} · {{ step.executor }}</small
              ><small v-if="step.errorCode" class="error-code">{{ step.errorCode }}</small>
              <div v-if="step.command" class="command">
                <span><i class="pi pi-send" /> Command #{{ step.command.sequence }}</span
                ><Tag :value="step.command.status" severity="secondary" /><small class="mono">{{
                  step.command.id
                }}</small>
              </div>
            </div>
  ...
  >
    <template #header
      ><div>
        ...
        <h2>
          {{ selectedAudit?.operation || selectedAudit?.resourceType || selectedAudit?.eventType }}
        </h2>
      </div></template
    >
    <div v-if="selectedAudit" class="detail-stack">
      <div class="detail-hero">
        <div>
          <span>Результат</span
          ><Tag :value="selectedAudit.outcome" :severity="severity(selectedAudit.outcome)" />
        </div>
        <div>
          <span>Актор</span><strong>{{ selectedAudit.actor.name || selectedAudit.actor.id }}</strong
          ><small>{{ selectedAudit.actor.email || selectedAudit.actor.type }}</small>
        </div>
        <div>
          <span>Время</span><strong>{{ formatDate(selectedAudit.occurredAt) }}</strong>
        </div>
      </div>
      <div class="audit-facts">
        <div>
          <span>Событие</span><strong class="mono">{{ selectedAudit.eventType }}</strong>
        </div>
        <div>
          <span>Цель</span
          ><strong class="mono"
            >{{ selectedAudit.target.kind }} · {{ selectedAudit.target.id }}</strong
          >
        </div>
        <div v-if="selectedAudit.requiredPermissionCode">
          <span>Разрешение</span
          ><strong class="mono">{{ selectedAudit.requiredPermissionCode }}</strong>
        </div>
        <div v-if="selectedAudit.auditReason">
          <span>Причина изменения</span><strong>{{ selectedAudit.auditReason }}</strong>
        </div>
        <div v-if="selectedAudit.reasonCode">
          <span>Код результата</span><strong class="mono">{{ selectedAudit.reasonCode }}</strong>
        </div>
        <div v-if="selectedAudit.requestId">
          <span>Request ID</span><strong class="mono">{{ selectedAudit.requestId }}</strong>
        </div>
        <div v-if="selectedAudit.correlationId">
          <span>Correlation ID</span><strong class="mono">{{ selectedAudit.correlationId }}</strong>
        </div>
        <div v-if="selectedAudit.ipAddress">
          <span>IP</span><strong class="mono">{{ selectedAudit.ipAddress }}</strong>
        </div>
    ...
    <template #header
      ><div class="request-drawer-header">
        ...
        <div class="request-title">
          <Tag :value="selectedProductApiRequest?.method" severity="secondary" rounded />
          <h2 id="product-api-request-title" ref="productApiTitleElement" tabindex="-1">
            {{ selectedProductApiRequest?.path }}
          </h2>
        </div>
      </div></template
    >
    <div v-if="selectedProductApiRequest" class="detail-stack request-detail">
      <section class="request-summary" aria-label="Итог запроса">
        <div class="request-status" :class="selectedProductApiOutcome?.tone">
        ...
        <div class="request-vitals">
          <span
            ><i class="pi pi-clock" />{{ formatDate(selectedProductApiRequest.receivedAt) }}</span
          >
          <span
            ><i class="pi pi-bolt" />{{
          ...
          >
          <span
            ><i class="pi pi-file" />{{ formatBytes(selectedProductApiRequest.payloadBytes) }}</span
          >
        </div>
      </section>
      ...
      <section class="request-section">
        ...
        <dl class="request-metadata">
          <div>
            <dt>Request ID</dt>
            <dd>
              <code>{{ selectedProductApiRequest.requestId || '—' }}</code>
              <button
                v-if="selectedProductApiRequest.requestId"
                type="button"
                aria-label="Скопировать Request ID"
                title="Скопировать Request ID"
                @click="copyProductApiValue('requestId', selectedProductApiRequest.requestId)"
              >
          ...
          <div>
            ...
            <dd>
              ...
              <button
                ...
                @click="copyProductApiValue('credentialId', selectedProductApiRequest.credentialId)"
              >
          ...
          <div>
            ...
            <dd>
              ...
              <button
                ...
                @click="copyProductApiValue('logId', selectedProductApiRequest.id)"
              >
          ...
          <div>
            <dt>Хранится до</dt>
            <dd>
              <span>{{ formatDate(selectedProductApiRequest.retainUntil) }}</span>
            </dd>
          </div>
        </dl>
        <span class="sr-only" role="status" aria-live="polite">{{ productApiCopyMessage }}</span>
      </section>
