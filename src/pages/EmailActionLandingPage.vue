...
const successCopy = computed(() =>
  props.action === 'verification' ? 'Email подтверждён' : 'Email изменён',
);
...
async function consume() {
  ...
  try {
    ...
    if (props.action === 'initial-access') {
      if (!('kind' in result) || result.kind !== 'PASSWORD_SETUP_REQUIRED')
...
.email-action-card :deep(.p-message) {
  text-align: left;
}
.email-action-card :deep(.p-button) {
  justify-content: center;
  margin-top: 4px;
}
.state-icon {
