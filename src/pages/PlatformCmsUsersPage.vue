...
const actions = computed(() =>
  detail.value ? availableCmsUserActions(detail.value, permissions.value, auth.user?.id) : [],
);
...
async function submitProvisioning(): Promise<void> {
  ...
  try {
    ...
    if ('initialAccessSecret' in response) {
      ...
      createNotice.value =
        'Запрос уже был обработан. Секрет повторно не раскрывается; при необходимости сбросьте учётные данные пользователя.';
    } else {
    ...
    if (provisioningOutcomeUnknown.value) {
      createError.value =
        'Результат запроса неизвестен. Не создавайте пользователя повторно: обновите список и, если пользователь появился, используйте «Сбросить доступ».';
      ...
      createError.value =
        'Не удалось создать пользователя. Исправьте данные или повторите попытку.';
    }
  } finally {
...
onMounted(async () => {
  ...
  if (typeof cmsUserId === 'string') {
    ...
    if (detail.value?.id === cmsUserId) {
      await Promise.all([loadPlatformAuthority(cmsUserId), loadUserSessions(cmsUserId)]);
    }
  }
});
</script>
...
<template>
  <section class="page platform-users-page">
    <header class="page-header">
      <div>
        ...
        <h1>CMS Users</h1>
        <p class="subtitle">
          Глобальные identities и их доступ к административным поверхностям Retenive.
        </p>
      </div>
      <div class="header-actions">
        <Button
          v-if="canCreate"
          label="Создать CMS User"
        ...
        <Button
          ...
          outlined
          :loading="loading"
          @click="directory.load()"
        />
      </div>
    </header>

    <Message v-if="createNotice" severity="success" :closable="true" @close="createNotice = ''">{{
      createNotice
    }}</Message>
    ...
    <div class="filters card">
      ...
      <span class="filter-note"
        >Список фильтруется сервером; курсор действует только внутри выбранного состояния.</span
      >
    </div>

    <div class="card table-card">
      <div v-if="loading" class="loading-list">
        <Skeleton v-for="row in 7" :key="row" height="58px" />
      </div>
      <DataTable
        v-else
        :value="items"
      ...
      >
        <template #empty
          ><div class="empty">
            <i class="pi pi-users" /><strong>CMS Users не найдены</strong>
          </div></template
        >
        <Column header="CMS User">
          <template #body="{ data }"
            ><div class="identity">
              <span>{{ data.displayName.slice(0, 1) }}</span>
              <div>
        ...
        <Column header="Email"
          ><template #body="{ data }"
            ><Tag
              :value="data.emailVerified ? 'Подтверждён' : 'Не подтверждён'"
              :severity="data.emailVerified ? 'success' : 'warn'" /></template
        ></Column>
        <Column field="projectCount" header="Projects" class="mobile-hide" />
        <Column header="Последний вход" class="mobile-hide"
          ><template #body="{ data }">{{ formatDate(data.lastLoginAt) }}</template></Column
        >
        <Column header="Открыть"
          ><template #body="{ data }"
            ><Button
              ...
              rounded
              severity="secondary"
              :aria-label="`Открыть ${data.displayName}`"
              @click.stop="openUser(data)" /></template
        ></Column>
      </DataTable>
  ...
  <Drawer
    aria-labelledby="cms-user-detail-title"
  ...
  >
    <template #header
      ><div>
        <small>CMS User</small>
        <h2 id="cms-user-detail-title">{{ detail?.displayName ?? 'Загрузка…' }}</h2>
      </div></template
    >
    <div v-if="detailLoading" class="loading-list">
      <Skeleton v-for="row in 5" :key="row" height="52px" />
    </div>
    ...
    <div v-else-if="detail" class="detail-stack">
      <Message v-if="operation.kind === 'VERSION_CONFLICT'" severity="warn" :closable="false"
        >Данные уже изменились другим оператором. Загружена актуальная версия; проверьте её перед
        повторным действием.</Message
      >
      <Message v-else-if="operation.kind === 'STEP_UP_REQUIRED'" severity="warn" :closable="false"
        ><div class="message-action">
          <span><strong>Требуется свежий вход.</strong> Действие не повторялось.</span
          ><Button label="Войти заново" size="small" @click="requireFreshLogin" /></div
      ></Message>
      <Message v-else-if="operation.kind === 'MFA_REQUIRED'" severity="warn" :closable="false"
        >Для действия требуется MFA. Действие не повторялось.</Message
      >
      <Message v-else-if="operation.kind === 'OUTCOME_UNKNOWN'" severity="warn" :closable="false"
        >Результат запроса неизвестен. Не повторяйте его вслепую: проверьте состояние и при
        необходимости осознанно перевыпустите доступ.</Message
      >
      ...
      <div class="detail-hero">
        ...
        <div>
          <span>Версия</span><strong>{{ detail.version }}</strong>
        </div>
      </div>
      ...
      <section class="detail-section">
        <div class="section-heading">
          <h3>Профиль</h3>
          <Button
            v-if="actions.includes('EDIT') && !editing"
            label="Изменить"
            text
            size="small"
            @click="beginEdit"
        ...
        <form v-if="editing" class="profile-form" @submit.prevent="saveProfile">
          ...
          <div>
            <Button
              ...
              text
              @click="editing = false"
            /><Button type="submit" label="Сохранить" :loading="operation.kind === 'SUBMITTING'" />
          </div>
        </form>
        <dl v-else>
          <div>
            <dt>Имя</dt>
            <dd>{{ detail.givenName }}</dd>
          </div>
          <div>
            <dt>Фамилия</dt>
            <dd>{{ detail.familyName }}</dd>
          </div>
          <div>
      ...
      <section class="detail-section">
        ...
        <template v-else>
          <Message v-if="platformRoleState === 'VERSION_CONFLICT'" severity="warn" :closable="false"
            >Роли уже изменились другим оператором. Загружена актуальная версия; действие не
            повторялось.</Message
          >
          <Message
            v-else-if="platformRoleState === 'STEP_UP_REQUIRED'"
            severity="warn"
            :closable="false"
            ><div class="message-action">
              <span><strong>Требуется свежий вход.</strong> Изменение ролей не повторялось.</span
              ><Button label="Войти заново" size="small" @click="requireFreshLogin" /></div
          ></Message>
          <Message
            v-else-if="platformRoleState === 'LAST_PLATFORM_OPERATOR'"
            severity="warn"
            :closable="false"
            >Нельзя снять роль у последнего активного Platform Operator. Сначала назначьте её
            другому активному пользователю.</Message
          >
          <Message v-else-if="platformRoleState === 'ERROR'" severity="error" :closable="false"
            >Не удалось безопасно загрузить или изменить платформенные роли.</Message
          >
          <form
            v-if="platformRoleEditing"
            class="platform-role-form"
            @submit.prevent="submitPlatformRoles"
          >
            <label>
            ...
            <label>
              ...
              <Textarea
                v-model="platformRoleReason"
                data-testid="platform-role-reason"
                rows="3"
                maxlength="500"
                :aria-invalid="Boolean(platformRoleReasonError)"
              />
            </label>
            <Message v-if="platformRoleReasonError" severity="error" :closable="false">{{
              platformRoleReasonError
            }}</Message>
            <div class="form-actions">
              <Button
                ...
                severity="secondary"
                text
                :disabled="platformRoleSubmitting"
                @click="platformRoleEditing = false"
              />
          ...
          <template v-else>
            <div class="tag-list">
              <Tag
                v-for="role in renderedPlatformRoleKeys"
                :key="role"
                :value="role"
                severity="info"
              /><span v-if="!renderedPlatformRoleKeys.length">Роли не назначены</span>
            </div>
            <ul class="permission-list">
              <li v-for="permission in renderedPlatformPermissionCodes" :key="permission">
      ...
      <section v-if="detail.deactivationReason" class="detail-section">
        <h3>Деактивация</h3>
        <p>{{ detail.deactivationReason }}</p>
        <small>{{ formatDate(detail.deactivatedAt) }}</small>
      </section>

      <section class="detail-section">
        <h3>Активные сессии</h3>
        <Skeleton v-if="userSessionsLoading" height="64px" />
        <Message v-else-if="userSessionsError" severity="error" :closable="false">{{
          userSessionsError
        }}</Message>
      ...
      <section v-if="actions.some((action) => action !== 'EDIT')" class="detail-section">
        <h3>Управление доступом</h3>
        <div class="action-grid">
          <Button
            v-for="action in actions.filter((item) => item !== 'EDIT')"
            :key="action"
            :data-action="action"
            :label="actionLabel(action)"
            :severity="
              action === 'DEACTIVATE' || action === 'RESET_CREDENTIALS' ? 'danger' : 'secondary'
            "
            outlined
            @click="beginLifecycle(action as Exclude<CmsUserAction, 'EDIT'>)"
          />
        </div>
      </section>
      <Message v-else-if="detail.id === auth.user?.id" severity="info" :closable="false"
        >Собственный доступ нельзя приостановить, деактивировать или сбросить через operator
        flow.</Message
      >
    </div>
  </Drawer>
  ...
  <Dialog
    ...
    @update:visible="!$event && (pendingAction = null)"
  >
    <div class="reason-dialog">
      <p>Причина попадёт в security audit. Не указывайте пароли, токены и другие секреты.</p>
      <label
        ><span>Причина</span
        ><Textarea
          v-model="reason"
          ...
          maxlength="500"
          autofocus
          :aria-invalid="Boolean(reasonError)" /></label
      ><small>{{ reason.length }}/500</small
      ><Message v-if="reasonError" severity="error" :closable="false">{{ reasonError }}</Message>
    </div>
    <template #footer
      ><Button
        label="Отмена"
        ...
        @click="pendingAction = null" /><Button
        data-testid="submit-lifecycle"
        :label="pendingAction ? actionLabel(pendingAction) : 'Продолжить'"
        :severity="
          pendingAction === 'DEACTIVATE' || pendingAction === 'RESET_CREDENTIALS'
            ? 'danger'
            : undefined
        "
  ...
  >
    <div class="reason-dialog">
      ...
      <label
        ><span>Причина</span
        ><Textarea
          v-model="sessionRevokeReason"
          data-testid="session-revoke-reason"
          rows="4"
          maxlength="500"
      /></label>
      <Message
        v-if="sessionRevokeError"
        :severity="sessionRevokeStepUp ? 'warn' : 'error'"
        :closable="false"
        >{{ sessionRevokeError }}</Message
      >
      <Button
        v-if="sessionRevokeStepUp"
        label="Войти заново"
        size="small"
    ...
    <template #footer>
      <Button
        ...
        severity="secondary"
        text
        :disabled="sessionRevokeSubmitting"
        @click="pendingSessionRevoke = null"
      />
      <Button
        data-testid="submit-session-revoke"
        label="Завершить сессию"
        severity="danger"
        :loading="sessionRevokeSubmitting"
        @click="submitSessionRevoke"
      />
    </template>
  </Dialog>
  ...
  >
    <form id="cms-user-create-form" class="create-form" @submit.prevent="submitProvisioning">
      <div class="identity-fields">
        <label
          ><span>Имя</span
          ><InputText v-model="createForm.givenName" maxlength="100" autocomplete="off"
        /></label>
        <label
          ><span>Фамилия</span
          ><InputText v-model="createForm.familyName" maxlength="100" autocomplete="off"
        /></label>
        <label
          ><span>Email</span
          ><InputText v-model="createForm.email" type="email" maxlength="320" autocomplete="off"
        /></label>
        <label
          ><span>Первоначальный доступ</span
          ><Select
            v-model="createForm.deliveryMode"
      ...
      <section class="assignment-section">
        <div class="section-heading">
          <div>
            ...
            <p>
              Необязательно. Без назначений будет создан platform-only пользователь; платформенную
              роль назначьте после создания.
            </p>
          </div>
          <Button
            type="button"
        ...
        <div v-if="projectsLoading" class="loading-list">
          <Skeleton v-for="row in 2" :key="row" height="58px" />
        </div>
        <div v-else class="assignment-list">
          <div
            v-for="(assignment, index) in createForm.assignments"
            :key="assignment.id"
          ...
          >
            <label
              ...
              ><Select
                ...
                @update:model-value="selectAssignmentProject(index, $event)"
            /></label>
            <label
              ><span>Роли</span
              ><MultiSelect
                v-model="assignment.roleIds"
                :options="rolesByProject[assignment.projectId] ?? []"
                option-label="name"
                option-value="id"
                display="chip"
          ...
          <Message v-if="createForm.assignments.length === 0" severity="info" :closable="false"
            >Пользователь будет создан без доступа к проектам.</Message
          >
        </div>
      </section>
      <Message
        v-if="createError"
        :severity="provisioningOutcomeUnknown ? 'warn' : 'error'"
        :closable="false"
        >{{ createError }}</Message
      >
    </form>
    <template #footer>
      ...
      />
      <Button
        form="cms-user-create-form"
        type="submit"
        label="Создать пользователя"
        icon="pi pi-user-plus"
        :loading="createSubmitting"
        :disabled="projectsLoading || provisioningOutcomeUnknown"
      />
    </template>
  </Dialog>

  <InitialAccessSecretDialog
    v-if="secret"
    :secret="secret.value"
    :expires-at="secret.expiresAt"
    :status="secret.status"
    @acknowledged="directory.acknowledgeSecret"
  />
  <InitialAccessSecretDialog
    v-if="provisionedSecret"
    :secret="provisionedSecret.value"
    :expires-at="provisionedSecret.expiresAt"
    :status="provisionedSecret.status"
    @acknowledged="acknowledgeProvisionedSecret"
  />
</template>
...
.assignment-section .section-heading p {
  margin: 4px 0 0;
  color: var(--muted);
  font-size: 0.72rem;
}
...
.profile-form label,
.platform-role-form label,
.reason-dialog label {
  display: grid;
  gap: 6px;
}
.profile-form label span,
.platform-role-form label span,
.reason-dialog label span {
  color: var(--muted);
  font-size: 0.68rem;
  font-weight: 700;
}
...
.platform-role-form .form-actions {
  display: flex;
  justify-content: flex-end;
  gap: 8px;
}
