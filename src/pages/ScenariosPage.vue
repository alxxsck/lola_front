...
<template>
  <div class="page scenarios-page">
    ...
    <section class="metric-strip card">
      <div>
        <span class="metric-icon active"><i class="pi pi-play" /></span>
        <div>
      ...
      <div>
        <span class="metric-icon draft"><i class="pi pi-pencil" /></span>
        <div>
      ...
      <div class="table-toolbar">
        <div class="search-box">
          <i class="pi pi-search" /><InputText
            v-model="search"
            placeholder="Название, код или событие"
          />
        </div>
        <Select
      ...
      >
        <template #empty>
          <div class="empty">
            ...
            <Button
              v-if="!scenarios.length && canWrite"
              label="Создать сценарий"
              icon="pi pi-plus"
              size="small"
              @click="openCreate"
          ...
          <template #body="{ data }">
            <div class="scenario-name">
              ...
              <div>
                ...
                ><small class="mono">{{ data.code }}</small>
              </div>
            </div>
          </template>
        </Column>
        <Column header="Триггер" style="min-width: 220px">
          <template #body="{ data }">
            <div class="trigger-cell">
              ...
              <div>
                <strong>{{ eventName(data) }}</strong
                ><small class="mono">{{ eventCode(data) }}</small>
              </div>
            </div>
          </template>
        </Column>
        <Column header="Приоритет" style="width: 100px">
          <template #body="{ data }"
            ><span class="priority">{{ data.priority }}</span></template
          >
        </Column>
        ...
        <Column header="Тихие часы" style="min-width: 145px">
          <template #body="{ data }">
          ...
          <template #body="{ data }">
            <div class="row-actions" @click.stop>
              <Button
                ...
                :loading="pendingIds.has(data.id)"
                :aria-label="
                  data.status === 'ACTIVE'
                    ? 'Поставить на паузу'
                    : data.status === 'PAUSED'
              ...
              />
              <Button
                :icon="canWrite ? 'pi pi-pencil' : 'pi pi-eye'"
                text
                rounded
                size="small"
                :aria-label="canWrite ? 'Редактировать' : 'Просмотреть'"
                @click="openEdit(data)"
              />
              <Button
                v-if="canWrite"
...
:deep(.scenario-table .p-datatable-tbody > tr) {
  cursor: pointer;
}
:deep(.scenario-table .p-datatable-tbody > tr > td) {
  padding-top: 14px;
  padding-bottom: 14px;
}
:deep(.p-paginator) {
