...
vi.mock('@/features/auth/auth.store', () => ({
  useAuthStore: () => ({ project: { id: 'project-1' }, get user() { return { role: mocks.role } } }),
}))
...
vi.mock('@/shared/api/repository/scenario-authoring', async (importOriginal) => {
  ...
  return { ...original, scenarioAuthoringRepository: {
...
const contract: ScenarioAuthoringContract = {
  ...
  audience: {
    ...
    country: { source: 'profile.country', valueType: 'countryCode', semantics: 'ISO_3166_1_ALPHA_2_UPPERCASE', operators: ['eq'], control: 'COUNTRY_CODE', authoringAvailability: 'AVAILABLE' }, attributes: [],
    segmentSource: { operators: ['is_member'], searchEndpoint: '/segments', control: 'SEARCH', authoringAvailability: 'AVAILABLE' },
...
describe('ScenarioEditorPage V2 rule journey', () => {
  beforeEach(() => {
    ...
    mocks.getScenarioDocument.mockResolvedValue({
      scenarioId: scenario.id, projectId: 'project-1', code: scenario.code, name: scenario.name,
      status: scenario.status, triggerEventDefinitionRevisionId: event.id, currentRevisionId: null,
      editable: true, source: undefined, draft: undefined, createdAt: 'now', updatedAt: 'now',
    })
    mocks.saveScenarioDraft.mockResolvedValue({
      id: 'draft-1', version: 1, baseRevisionId: null, catalogRevision: contract.revision,
      deliveryPolicy: { kind: 'IMMEDIATE' }, graph: { actions: [] }, createdAt: 'now', updatedAt: 'now',
    })
  ...
  it('keeps Trigger, Audience, Eligibility, Actions and Delivery as explicit stages', async () => {
    ...
    expect(wrapper.findComponent(SegmentManager).exists()).toBe(true)
  ...
  it('restores the durable source document and observed concurrency versions after reload', async () => {
    mocks.getScenarioDocument.mockResolvedValueOnce({
      scenarioId: scenario.id, projectId: 'project-1', code: scenario.code, name: scenario.name,
      status: 'ACTIVE', triggerEventDefinitionRevisionId: event.id, currentRevisionId: 'revision-4', editable: true,
      source: undefined,
      draft: {
        id: 'draft-1', version: 7, baseRevisionId: 'revision-4', catalogRevision: contract.revision,
        rule: { version: 1, root: { kind: 'activityDayStreak', compare: { operator: 'gte', value: 3 } } },
        audience: { version: 1, root: { kind: 'locale', operator: 'eq', value: 'ru-RU' } },
        deliveryPolicy: { kind: 'WAIT_UNTIL_ONLINE', expiryMs: 90_000, recheckEligibility: true },
        graph: { actions: [{ position: 0, nodeKey: 'say', nextNodeKey: null, type: 'SAY', config: { text: 'Hello' } }] },
    ...
    expect(wrapper.getComponent(ScenarioRuleBuilder).props('modelValue')).toMatchObject({ root: { kind: 'activityDayStreak', compare: { value: 3 } } })
    await stageButton(wrapper, 'Аудитория').trigger('click')
    expect(wrapper.getComponent(AudienceRuleBuilder).props('modelValue')).toMatchObject({ root: { kind: 'locale', value: 'ru-RU' } })
    await stageButton(wrapper, 'Доставка').trigger('click')
    expect(wrapper.getComponent(DeliveryPolicyEditor).props('modelValue')).toEqual({ kind: 'WAIT_UNTIL_ONLINE', expiryMs: 90_000, recheckEligibility: true })
    expect(wrapper.getComponent(ScenarioPublishPanel).props()).toMatchObject({ expectedCurrentRevisionId: 'revision-4', expectedDraftVersion: 7 })
  ...
  it('keeps Audience draft dirty and sends it to validation and atomic publish as a separate contract', async () => {
    ...
    await stageButton(wrapper, 'Условия').trigger('click')
    expect(wrapper.getComponent(RuleValidationPreview).props()).toMatchObject({ audienceDraft, audienceDraftRevision: 1 })
    await stageButton(wrapper, 'Доставка').trigger('click')
    expect(wrapper.getComponent(ScenarioPublishPanel).props()).toMatchObject({ audienceDraft })
  })
  ...
  it('does not leave Studio after legacy save when Audience is the only V2 change', async () => {
    ...
    await flushPromises()
    await stageButton(wrapper, 'Аудитория').trigger('click')
    wrapper.getComponent(AudienceRuleBuilder).vm.$emit('update:modelValue', { version: 1, root: { nodeId: 'audience-root', kind: 'locale', operator: 'eq', value: 'ru-RU' } } satisfies AudienceDraft)
    await wrapper.vm.$nextTick()
    ...
    expect(mocks.saveScenarioDraft).toHaveBeenCalledWith('project-1', 'scenario-1', expect.objectContaining({
      audience: { version: 1, root: { kind: 'locale', operator: 'eq', value: 'ru-RU' } },
  ...
  it('saves the durable document before legacy metadata and never sends actions through the legacy PATCH', async () => {
    ...
    expect(mocks.updateScenarioMetadata).toHaveBeenCalledWith('project-1', 'scenario-1', expect.not.objectContaining({ actions: expect.anything() }))
    expect(mocks.saveScenarioDraft.mock.invocationCallOrder[0]).toBeLessThan(mocks.updateScenarioMetadata.mock.invocationCallOrder[0]!)
    expect(mocks.push).not.toHaveBeenCalledWith('/scenarios')
    expect(mocks.saveScenarioDraft).toHaveBeenCalledWith('project-1', 'scenario-1', expect.objectContaining({
      rule: { version: 1, root: { kind: 'all', children: [{ kind: 'activityDayStreak', compare: { operator: 'gte', value: 2 } }] } },
    }))
    expect(wrapper.text()).toContain('Черновик v1 сохранён на сервере')
  })
  ...
  it('blocks publication again when any durable document section changes after save', async () => {
    ...
    wrapper.getComponent(DeliveryPolicyEditor).vm.$emit('update:modelValue', { kind: 'SKIP_IF_OFFLINE' })
    await wrapper.vm.$nextTick()

    expect(wrapper.getComponent(ScenarioPublishPanel).props('blockedReason')).toContain('сохраните все изменения')
  })
  ...
  it('keeps local edits and offers a reload when durable draft concurrency fails', async () => {
    ...
    mocks.saveScenarioDraft.mockRejectedValueOnce(new ApiError(409, 'Draft changed', {}, undefined, 'SCENARIO_DRAFT_CONFLICT'))
    const wrapper = mountPage()
    await flushPromises()
    await stageButton(wrapper, 'Условия').trigger('click')
    wrapper.getComponent(ScenarioRuleBuilder).vm.$emit('update:modelValue', {
      version: 1,
      root: { nodeId: 'streak', kind: 'activityDayStreak', compare: { operator: 'gte', value: 3 } },
    } satisfies RuleDraft)
    await wrapper.vm.$nextTick()
    await wrapper.find('button-stub[label="Сохранить"]').trigger('click')
    await flushPromises()
  ...
  it('keeps Delivery Policy separate from Goal Deadline and inside the page dirty guard', async () => {
    ...
    editor.vm.$emit('update:modelValue', { kind: 'WAIT_UNTIL_ONLINE', expiryMs: 86_400_000, recheckEligibility: true })
    await wrapper.vm.$nextTick()
  ...
  it('does not offer legacy ACTIVE activation before atomic V2 publish', async () => {
    ...
    const page = wrapper.vm as unknown as { statusOptions: Array<{ value: string }> }
  ...
  it('blocks legacy graph save when a persisted Goal config fails domain validation', async () => {
    mocks.getScenarios.mockResolvedValue([{
      ...scenario,
      actions: [
        { position: 0, nodeKey: 'wait', type: 'WAIT_FOR_GOAL', config: { eventCode: '', onGoal: 'done', onTimeout: 'timeout' } },
  ...
  it('blocks atomic publish when the document graph or Goal is invalid', async () => {
    mocks.getScenarios.mockResolvedValue([{
      ...scenario,
      actions: [{ position: 0, nodeKey: 'wait', type: 'WAIT_FOR_GOAL', config: { eventCode: '', onGoal: '', onTimeout: '' } }],
    }])
    const wrapper = mountPage()
    await flushPromises()
    await stageButton(wrapper, 'Доставка').trigger('click')

    expect(wrapper.getComponent(ScenarioPublishPanel).props('blockedReason')).toContain('Исправьте ошибки')
  })
  ...
  it('renders a mobile-safe action outline that can open a node without canvas gestures', async () => {
    mocks.getScenarios.mockResolvedValue([{ ...scenario, actions: [{ position: 0, nodeKey: 'welcome_message', type: 'SAY', config: { text: 'Добро пожаловать' } }] }])
    const wrapper = mountPage()
    await flushPromises()
    await stageButton(wrapper, 'Действия').trigger('click')

    expect(wrapper.get('.mobile-action-outline').text()).toContain('welcome_message')
    expect(wrapper.get('.mobile-library summary').text()).toContain('Добавить узел')
  ...
  it('keeps Audience and publish mutations read-only outside OWNER and ADMIN roles', async () => {
    ...
    expect(wrapper.getComponent(SegmentManager).props('readonly')).toBe(true)
    ...
    expect(wrapper.text()).toContain('Публикация доступна только владельцам и администраторам')
  })
})
