...
vi.mock('@/features/scenarios/model/scenario-graph-auto-layout', async (importOriginal) => ({
...
vi.mock('vue-router', () => ({
  ...
  onBeforeRouteLeave: (guard: () => boolean) => mocks.routeLeaveGuards.push(guard),
}));
...
function projectAction(code: string, overrides: Partial<ProjectAction> = {}): ProjectAction {
  return {
    id: `action-${code}`,
    projectId: 'project-1',
...
function projectActionFromCatalogItem(
  ...
  return projectAction(item.type, {
    ...
    actionTypeRevision: {
      ...
      executorAdapter: item.executor === 'FRONTEND' ? 'FRONTEND_COMMAND' : 'SERVER_HANDLER',
    ...
    global: {
      stubs: {
        VueFlow: {
          ...
          emits: ['init', 'node-click', 'node-drag-stop', 'viewport-change-end'],
        ...
        ScenarioActionInspectorDock: {
          ...
          emits: ['resize'],
          template: '<section class="scenario-action-inspector-dock-stub"><slot /></section>',
        },
      },
    },
  });
...
async function openValidation(wrapper: ReturnType<typeof mountPage>) {
  await wrapper.find('button-stub[label="Проверить условия"]').trigger('click');
}
...
describe('ScenarioEditorPage V2 rule journey', () => {
  ...
  it('keeps Trigger, Audience, Eligibility, Actions and Delivery as explicit stages', async () => {
    ...
    expect(stageButton(wrapper, 'Аудитория').text()).not.toContain('Пока недоступна');
  });
  ...
  it('keeps legacy frequency controls in one balanced row', async () => {
    ...
    expect(wrapper.find('.legacy-frequency-note').text()).toContain('Перейдите на общую частоту');
  });
  ...
  it('restores the durable source document and observed concurrency versions after reload', async () => {
    ...
    expect(wrapper.getComponent(ScenarioRuleBuilder).props('modelValue')).toMatchObject({
      root: { kind: 'activityDayStreak', compare: { value: 3 } },
    });
    await stageButton(wrapper, 'Аудитория').trigger('click');
    expect(wrapper.getComponent(AudienceRuleBuilder).props('modelValue')).toMatchObject({
      root: { kind: 'locale', value: 'ru-RU' },
    });
    await stageButton(wrapper, 'Доставка').trigger('click');
    expect(wrapper.getComponent(DeliveryPolicyEditor).props('modelValue')).toEqual({
      kind: 'WAIT_UNTIL_ONLINE',
      expiryMs: 90_000,
      recheckEligibility: true,
    });
  ...
  it('keeps Audience draft dirty and sends it to validation and atomic publish as a separate contract', async () => {
    ...
    wrapper.getComponent(AudienceRuleBuilder).vm.$emit('update:modelValue', audienceDraft);
    await wrapper.vm.$nextTick();
  ...
  it('does not leave Studio after saving an Audience-only draft change', async () => {
    ...
    await flushPromises();
    await stageButton(wrapper, 'Аудитория').trigger('click');
    wrapper.getComponent(AudienceRuleBuilder).vm.$emit('update:modelValue', {
      version: 1,
      root: {
        nodeId: 'audience-root',
        kind: 'locale',
  ...
  it('opens the Trigger stage from the graph and edits its first action', async () => {
    ...
    expect(wrapper.get('[data-testid="scenario-first-action"]').text()).toContain('Сказать текст');
    expect(wrapper.text()).not.toContain('Смена корня отключена');
    await wrapper.get('button-stub[label="Заменить первое действие"]').trigger('click');

    expect(stageButton(wrapper, 'Действия').classes()).toContain('active');
    expect(wrapper.getComponent(ScenarioNodeInspector).props('action')).toMatchObject({
      nodeKey: 'welcome_message',
      type: 'SAY',
    });
  });
  ...
  it('keeps drag presentation-only, restores it locally and never enables edge reconnect', async () => {
    ...
    expect(
      vueFlow.props('nodes').find(({ id }: { id: string }) => id === 'welcome_message').position,
    ).toEqual({ x: 740, y: 310 });
    expect(
      JSON.parse(
    ...
    expect(
      reopenedFlow.props('nodes').find(({ id }: { id: string }) => id === 'welcome_message')
        .position,
    ).toEqual({ x: 740, y: 310 });
    expect(setViewport).toHaveBeenCalledWith({ x: -120, y: 45, zoom: 0.9 }, { duration: 0 });
  });
  ...
  it('restores the saved viewport when the same automatic-layout draft is reopened', async () => {
    ...
    await flushPromises();
    await stageButton(wrapper, 'Действия').trigger('click');
    const setViewport = vi.fn().mockResolvedValueOnce(false).mockResolvedValue(true);
    ...
    expect(setViewport).toHaveBeenLastCalledWith({ x: -360, y: 128, zoom: 0.72 }, { duration: 0 });
    expect(wrapper.getComponent({ name: 'VueFlow' }).props('nodesDraggable')).toBe(false);
  });
  ...
  it('keeps the canvas pannable but disables node movement in read-only mode', async () => {
    ...
    expect(wrapper.getComponent(ScenarioGraphLayoutToolbar).props('canArrange')).toBe(false);
    ...
    expect(wrapper.getComponent(ScenarioFlowControls).props('selectedNodeId')).toBeNull();
    expect(wrapper.get('[aria-label="Найти действие"]').attributes('disabled')).toBeUndefined();
  });
  ...
  it('keeps authoring available when browser layout storage is blocked', async () => {
    ...
    try {
      ...
      expect(wrapper.getComponent(ScenarioGraphLayoutToolbar).props('mode')).toBe('auto');
      wrapper.unmount();
    } finally {
  ...
  it('keeps manual positions through inspector edits and places new nodes without a global relayout', async () => {
    ...
    expect(
      flow.props('nodes').find(({ id }: { id: string }) => id === 'welcome_message').position,
    ).toEqual({ x: 680, y: 300 });

    wrapper
      .getComponent(ScenarioNodeInspector)
      .vm.$emit('rename', 'welcome_message', 'renamed_message');
    await flushPromises();
    expect(
      flow.props('nodes').find(({ id }: { id: string }) => id === 'renamed_message').position,
    ).toEqual({ x: 680, y: 300 });
    expect(
      JSON.parse(
        window.localStorage.getItem(
    ...
    expect(actionNodes.find(({ id }: { id: string }) => id === 'renamed_message').position).toEqual(
      { x: 680, y: 300 },
    );
  ...
  it('migrates an ephemeral new-scenario layout only after the scenario receives an id', async () => {
    ...
    expect(
      window.localStorage.getItem('retenive:scenario-graph-layout:v1:operator-1:project-1:new'),
    ).toBeNull();
    await wrapper.find('button-stub[label="Сохранить"]').trigger('click');
    await flushPromises();
  ...
  it('retries fitting when nodes are not measured on the first attempt', async () => {
    ...
    await stageButton(wrapper, 'Действия').trigger('click');
    await flushPromises();
    const delayedFit = vi.fn().mockResolvedValueOnce(false).mockResolvedValue(true);
    wrapper.getComponent({ name: 'VueFlow' }).vm.$emit('init', {
      fitView: delayedFit,
    });
    await new Promise((resolve) => setTimeout(resolve, 40));
  ...
  it('cancels the worker and ignores a late layout result after unmount', async () => {
    ...
    mocks.layoutGraph.mockImplementation(
      (viewModel) =>
        new Promise((resolve) => {
  ...
  it('previews a linear entry-point change and never connects the old prefix after the selected branch', async () => {
    ...
    expect(firstActionPicker.props('description')).toContain('какие связи и действия изменятся');
    ...
    expect(wrapper.getComponent(ScenarioActionChangeDialog).props('preview')).toMatchObject({
      refreshed: true,
    });
    ...
    expect(wrapper.get('[data-testid="scenario-first-action"]').text()).toContain('OPEN_CHAT');
  });

  it('explains why a branch target cannot become the entry point and keeps the graph unchanged', async () => {
    mocks.projectActions = [projectAction('ASK_CHOICE'), projectAction('SAY')];
    setAuthoringActions([
      {
        position: 0,
        nodeKey: 'question',
    ...
    expect(wrapper.get('button-stub[label="Заменить первое действие"]').attributes('label')).toBe(
      'Заменить первое действие',
    );
  });
  ...
  it('requires an explicit change draft for a published head and saves it against the immutable revision', async () => {
    ...
    expect(
      wrapper.get('[data-testid="scenario-first-action"] button-stub').attributes('disabled'),
    ).toBe('false');
  ...
  it('opens the Rule Builder only for the exact catalog Event revision', async () => {
    ...
    expect(wrapper.find('.stage-section-header button-stub').exists()).toBe(false);
    const validationActions = wrapper.get('[data-testid="rule-validation-actions"]');
    expect(validationActions.text()).toContain('Добавьте хотя бы одно условие');
    expect(
      validationActions.get('button-stub[label="Проверить условия"]').attributes('label'),
    ...
    await validationActions.get('button-stub[label="Проверить условия"]').trigger('click');
    ...
    await wrapper.find('button-stub[aria-label="Закрыть проверку условий"]').trigger('click');
    expect(wrapper.findComponent(RuleValidationPreview).exists()).toBe(false);
  });
  ...
  it('blocks publication again when any durable document section changes after save', async () => {
    ...
    expect(wrapper.getComponent(ScenarioPublishPanel).props('blockedReason')).toBe('');
    wrapper
      .getComponent(DeliveryPolicyEditor)
      .vm.$emit('update:modelValue', { kind: 'SKIP_IF_OFFLINE' });
    await wrapper.vm.$nextTick();

    expect(wrapper.getComponent(ScenarioPublishPanel).props('blockedReason')).toContain(
  ...
  it('keeps local edits and offers a reload when durable draft concurrency fails', async () => {
    ...
    mocks.saveScenarioDraft.mockRejectedValueOnce(
      new ApiError(409, 'Draft changed', {}, undefined, 'SCENARIO_DRAFT_CONFLICT'),
    );
    const wrapper = mountPage();
    await flushPromises();
    await stageButton(wrapper, 'Условия').trigger('click');
    ...
    expect(wrapper.find('button-stub[label="Загрузить актуальный черновик"]').exists()).toBe(true);
    expect(mocks.guardDirty?.value).toBe(true);
  });
  ...
  it('keeps newer session edits dirty when an older publish request completes', async () => {
    ...
    wrapper.getComponent(ScenarioPublishPanel).vm.$emit('published', 'revision-2', {
  ...
  it('does not offer ACTIVE activation before atomic publish', async () => {
    ...
    expect(page.statusOptions.map((option) => option.value)).not.toContain('ACTIVE');
  });
  ...
  it('shows an already active scenario as an explicit read-only status', async () => {
    ...
    expect(wrapper.get('[aria-label="Текущий статус сценария"]').text()).toContain('Активен');
    expect(wrapper.find('select-stub[aria-label="Статус сценария"]').exists()).toBe(false);
  });
  ...
  it('does not expose or retain scenario conditions from the removed editor', async () => {
    ...
    expect((wrapper.vm as unknown as { form: Record<string, unknown> }).form).not.toHaveProperty(
      'conditions',
    );
  });
  ...
  it('uses active scenario-enabled Project Actions as the action picker authority', async () => {
    ...
    expect(addPicker.props('catalog').map((action) => action.name)).toEqual(['Открыть страницу']);
  });
  ...
  it('uses the pinned Project Action revision as the editor definition catalog', async () => {
    ...
    expect(wrapper.text()).not.toContain('Действие SHOW_ASSISTANT отсутствует в каталоге проекта');
    const addPicker = wrapper
      .findAllComponents(ActionPicker)
      .find((picker) => picker.classes().includes('action-library-picker'))!;
    expect(addPicker.props('catalog').map((action) => action.type)).toContain('SHOW_ASSISTANT');
    ...
    expect(wrapper.getComponent(ScenarioNodeInspector).props('issues')).not.toContain(
  ...
  it('renders a mobile-safe action outline that can open a node without canvas gestures', async () => {
    ...
    expect(wrapper.get('.mobile-action-outline').text()).toContain('welcome_message');
    ...
    await wrapper.get('button[aria-label="Открыть узел welcome_message"]').trigger('click');

    expect(wrapper.findComponent({ name: 'ScenarioNodeInspector' }).exists()).toBe(true);
  });
  ...
  it('creates and wires both WAIT_FOR_GOAL outcome actions', async () => {
    ...
    await flushPromises();
    await stageButton(wrapper, 'Действия').trigger('click');
    await wrapper.get('button[aria-label="Открыть узел wait_for_deposit"]').trigger('click');

    wrapper.getComponent(ScenarioNodeInspector).vm.$emit('createTarget', 'SAY', 'goal');
    await wrapper.vm.$nextTick();
    await wrapper.get('button[aria-label="Открыть узел wait_for_deposit"]').trigger('click');
    wrapper.getComponent(ScenarioNodeInspector).vm.$emit('createTarget', 'CLOSE_CHAT', 'timeout');
    await wrapper.vm.$nextTick();
    ...
    const source = page.form.actions.find((action) => action.nodeKey === 'wait_for_deposit');
    const goalAction = page.form.actions.find((action) => action.nodeKey === source?.config.onGoal);
  ...
  it('opens the graph as a dedicated mobile overview only when requested', async () => {
    ...
    expect(wrapper.get('button[aria-label="Вернуться к настройке действия"]').text()).toContain(
      'К настройке',
    );
    wrapper.unmount();
    vi.unstubAllGlobals();
  ...
  it('uses the created-action list as the primary navigation and keeps full names visible', async () => {
    ...
    expect(actionButton.text()).toContain('Отправить пользователю приветственное сообщение');
    const addPicker = wrapper
      .findAllComponents(ActionPicker)
      .find((picker) => picker.classes().includes('action-library-picker'))!;
    ...
    expect(wrapper.get('.studio-grid').classes()).not.toContain('graph-is-expanded');
  ...
  it('uses a Russian semantic name when an action is absent from the catalog', async () => {
    ...
    expect(wrapper.get('[data-action-node-key="wait"] .action-outline-main strong').text()).toBe(
      'Ожидание',
    );
    expect(
      wrapper
        ...
        .props('nodes')
        .find((node: { id: string }) => node.id === 'wait')?.data.label,
    ).toBe('Ожидание');
  });
  ...
  it('keeps the desktop canvas mounted while outline search, error filter and centering drive one selection', async () => {
    ...
    await wrapper
      .get('[data-action-node-key="welcome_message"] .action-outline-main')
      .trigger('click');
    await wrapper.vm.$nextTick();
  ...
  it('keeps Audience and publish mutations read-only outside OWNER and ADMIN roles', async () => {
    ...
    expect(wrapper.text()).toContain('У вас нет права публиковать сценарии');
  });
  ...
  it('explains split publish-only permissions without blaming the trigger', async () => {
    ...
    await stageButton(wrapper, 'Доставка').trigger('click');
    expect(wrapper.findComponent(ScenarioPublishPanel).exists()).toBe(false);
    expect(wrapper.text()).toContain('есть право публикации, но нет права изменять сценарий');
    expect(wrapper.text()).not.toContain('Сначала выберите событие запуска');
  });
  ...
  it('explains an unavailable source snapshot without exposing the backend code', async () => {
    ...
    expect(wrapper.find('button-stub[label="Сохранить"]').exists()).toBe(false);
    await wrapper.get('button-stub[label="Создать новый сценарий"]').trigger('click');
  ...
  it('shows system event names in Russian while preserving their codes', async () => {
    ...
    mocks.getScenarios.mockResolvedValue([{ ...scenario, eventDefinitionId: systemEvent.id }]);
