...
describe('OperationsPage', () => {
  ...
  it('opens on scenario runs and does not expose an events section', async () => {
    ...
    expect(wrapper.findAll('.section-tabs button').map((tab) => tab.text())).toEqual([
      'Запуски сценариев0',
      'Аудит0',
      'Логи API0',
      'Решения о запуске0',
    ]);
    expect(wrapper.findAll('.section-tabs button')[0]!.attributes('aria-selected')).toBe('true');
  ...
  it('filters, opens JSON detail and appends Product API requests by cursor', async () => {
    ...
    returnFocusTarget.id = 'product-api-request-trigger-request-log-1';
    ...
    await flushPromises();
    expect(mocks.getProductApiRequestLogsPage).toHaveBeenLastCalledWith('project-1', {
      limit: 50,
      path: 'sessions',
    });
    ...
    await flushPromises();
    expect(mocks.getProductApiRequestLog).toHaveBeenCalledWith('project-1', 'request-log-1');
    ...
    expect(detail.findAll('.request-metadata dt').map((term) => term.text())).toEqual([
      'Request ID',
      'Credential prefix',
      'Log ID',
      'Хранится до',
    ...
    await detail.get('[aria-label="Скопировать Request ID"]').trigger('click');
    ...
    await wrapper.find('button-stub[label="Загрузить ещё запросов"]').trigger('click');
    await flushPromises();
    expect(mocks.getProductApiRequestLogsPage).toHaveBeenLastCalledWith('project-1', {
      limit: 50,
      path: 'sessions',
  ...
  it('opens a real server-filtered Runs surface for an active-wait deep link', async () => {
    ...
    expect(wrapper.find('[data-test="active-wait-filter"]').exists()).toBe(true);
  });
  ...
  it('reloads on Project switch and rejects a late response from the old tenant', async () => {
    ...
    mocks.getScenarioRunsPage.mockImplementation((projectId: string) =>
      projectId === 'project-1' ? old.promise : Promise.resolve({ items: [], nextCursor: null }),
    );
    const wrapper = shallowMount(OperationsPage);
    await flushPromises();
  ...
  it('filters, paginates and opens canonical audit-event details', async () => {
    ...
    const wrapper = shallowMount(OperationsPage, {
      global: {
        stubs: {
          Drawer: {
            props: ['visible'],
            template: '<aside v-if="visible"><slot name="header" /><slot /></aside>',
          },
        },
      },
    });
    ...
    await wrapper.find('button-stub[label="Загрузить ещё событий аудита"]').trigger('click');
    await flushPromises();
    expect(mocks.getAuditEventsPage).toHaveBeenLastCalledWith('project-1', {
      limit: 50,
      search: 'draft',
    ...
    expect(table.vm.$attrs.value.map((item) => item.id)).toEqual(['audit-1', 'audit-2']);
  ...
  it('keeps the newest Runs and Audit snapshot when a slower reload finishes last', async () => {
    ...
    const oldAudit = deferred<Awaited<ReturnType<typeof mocks.getAuditEventsPage>>>();
    const run = (id: string) => ({
      id,
      scenarioId: 'scenario-1',
      scenarioCode: 'welcome',
    ...
    mocks.getAuditEventsPage.mockReturnValueOnce(oldAudit.promise).mockResolvedValueOnce({
      items: [
        {
          id: 'audit-new',
          eventType: 'iam.project_resource.changed',
