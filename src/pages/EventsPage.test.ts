...
function selectControl(wrapper: ReturnType<typeof mountPage>, ariaLabel: string) {
...
describe('EventsPage event editor journey', () => {
  ...
  it('filters the catalog by system ownership', async () => {
    ...
    selectControl(wrapper, 'Тип события').vm.$emit('update:modelValue', 'SYSTEM');
    await wrapper.vm.$nextTick();
  ...
  it('combines active reception and frontend-source filters and can reset them', async () => {
    ...
    selectControl(wrapper, 'Приём события').vm.$emit('update:modelValue', 'ENABLED');
    selectControl(wrapper, 'Приём с фронтенда').vm.$emit('update:modelValue', 'ACCEPTING');
    await wrapper.vm.$nextTick();
  ...
  it('sorts the visible catalog by event code', async () => {
    ...
    selectControl(wrapper, 'Сортировка событий').vm.$emit('update:modelValue', 'CODE');
    await wrapper.vm.$nextTick();

    expect(wrapper.findAll('.event-card h3').map((heading) => heading.text())).toEqual([
      'Второе по названию',
      'Первое по названию',
    ]);
  });
  ...
  it('sorts the catalog by reception status and schema version', async () => {
    ...
    await wrapper.vm.$nextTick();
    expect(wrapper.findAll('.event-card h3')[0]?.text()).toBe('Принимающее событие');

    sort.vm.$emit('update:modelValue', 'VERSION');
    await wrapper.vm.$nextTick();
    expect(wrapper.findAll('.event-card h3')[0]?.text()).toBe('Выключенное событие');
  });
  ...
  it('keeps a toggled card in the current result and reports the outcome beside the switch', async () => {
    ...
    mocks.listDefinitions.mockResolvedValueOnce([existingEvent]).mockResolvedValue([disabledEvent]);
    ...
    selectControl(wrapper, 'Приём события').vm.$emit('update:modelValue', 'ENABLED');
    await wrapper.vm.$nextTick();
    wrapper.getComponent({ name: 'ToggleSwitch' }).vm.$emit('update:modelValue', false);
    await flushPromises();

    expect(wrapper.findAll('.event-card')).toHaveLength(1);
    expect(wrapper.get('.event-card').text()).toContain('Успешный депозит');
    expect(wrapper.get('.event-policy-feedback').text()).toBe('Приём выключен');
    expect(wrapper.get('.event-policy-feedback').classes()).toContain('success');
  });
  ...
  it('keeps a policy update error beside the affected switch', async () => {
    ...
    expect(wrapper.get('.event-policy-feedback').text()).toContain('Сервис недоступен');
  ...
  it('puts system events first and explains their lock without a managed badge', async () => {
    ...
    expect(wrapper.findAll('.event-group-header h2').map((heading) => heading.text())).toEqual([
      'Системные события Retenive',
      'События проекта',
    ]);
  ...
  it('labels inactive custom events without making the card or its actions look disabled', async () => {
    ...
    expect(card.find('button-stub[aria-label="Редактировать Успешный депозит"]').exists()).toBe(
      true,
    );
    ...
    mocks.listDefinitions.mockResolvedValue([
      existingEvent,
      {
        ...
        metadata: {
          ...
          description: 'Пользователь открыл страницу продукта из браузера после авторизации.',
        },
        policy: {
          ...existingEvent.policy,
          clientIngestible: true,
  ...
  it('keeps common actions visible and moves secondary actions into overflow', async () => {
    ...
    expect(card.get('.event-more-actions summary').attributes('aria-label')).toContain(
      'Другие действия',
    );
    expect(
      card
        .get('button-stub[aria-label="Скопировать контракт события Успешный депозит"]')
        .attributes('label'),
  ...
  it('lets the operator reveal and collapse a long event description', async () => {
    ...
    expect(wrapper.get('.event-description').classes()).not.toContain('clamped');
    expect(wrapper.find('button-stub[label="Свернуть описание"]').exists()).toBe(true);
  });
  ...
  it('loads the Archive projection and keeps lifecycle mutations in the workspace', async () => {
    ...
    expect(wrapper.find('button-stub[label="Новое событие"]').exists()).toBe(false);
    expect(wrapper.getComponent({ name: 'ToggleSwitch' }).attributes('disabled')).toBe('true');
  });
  ...
  it('reloads for a new Project and ignores the previous Project response', async () => {
    ...
    expect(mocks.listDefinitions).toHaveBeenLastCalledWith('project-2', 'ACTIVE');
  ...
  it('copies the event contract directly from its card', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      expect.stringContaining('| `eventCode` | `string` | обязательно | `deposit.succeeded` |'),
    );
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      expect.stringContaining('| `payload.amount` | `integer` | обязательно | Сумма в центах |'),
    );
