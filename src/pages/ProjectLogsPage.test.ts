...
const mocks = vi.hoisted(() => ({
  permissions: ['project.event_logs.read', 'project.integration_activity.read'] as string[],
...
describe('ProjectLogsPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.permissions = ['project.event_logs.read', 'project.integration_activity.read'];
    mocks.query = {};
  });
  ...
  it('keeps legacy links on Product Events and exposes both permitted tabs', async () => {
    ...
    expect(wrapper.find('integration-activity-log-view-stub').exists()).toBe(false);
  ...
  it('normalizes to Integrations when it is the only permitted tab', async () => {
    ...
    expect(wrapper.findAll('[role="tab"]').map((tab) => tab.text())).toEqual(['Интеграции']);
    expect(wrapper.find('integration-activity-log-view-stub').exists()).toBe(true);
