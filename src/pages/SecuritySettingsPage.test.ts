    ...
    global: {
      ...
      stubs: {
        ...
        Button: {
          ...
          emits: ['click'],
          template:
            '<button type="button" :disabled="disabled || loading" :aria-label="ariaLabel" @click="$emit(\'click\')">{{ label }}</button>',
        },
...
describe('SecuritySettingsPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(notificationPreferencesApi.setEmailCaseEscalations).mockImplementation(
      async (subscribed) => ({
        topic: 'CASE_ESCALATION',
        channel: 'EMAIL',
        subscribed,
  ...
  it('keeps every settings action in a predictable responsive action area', async () => {
    ...
    expect(wrapper.find('.security-header [data-testid="revoke-other-sessions"]').exists()).toBe(
      false,
    );
    expect(
      wrapper.get('[aria-labelledby="sessions-heading"] [data-testid="revoke-other-sessions"]'),
    ).toBeDefined();
    expect(
      wrapper.get('.identity-summary .identity-actions [data-testid="email-verification-action"]'),
    ).toBeDefined();
    ...
    const currentSessionAction = wrapper.get(
      `[data-session-id="${sessions[0]!.id}"] .item-actions button`,
    );
    ...
    expect(currentSessionAction.attributes('aria-label')).toBe(
      'Завершить текущую сессию Chrome, номер 1',
    );
  });
  ...
  it('associates rejected credentials with the password field that needs correction', async () => {
    ...
    expect(wrapper.get('#security-email-password-error').text()).toBe(
    ...
    expect(passwordInputs[0]!.attributes('aria-describedby')).toBe(
      'security-current-password-error',
    );
    expect(wrapper.get('#security-current-password-error').text()).toBe(
      'Текущий пароль указан неверно.',
    );
  });
  ...
  it('requests verification and applies the server-provided cooldown', async () => {
    ...
    expect(wrapper.get('[data-testid="email-verification-action"]').text()).toContain(
      'Повторить через 45 с',
    );
  ...
  it('keeps Case escalation email opt-in disabled until the current address is verified', async () => {
    ...
    expect(
      wrapper.get('[data-testid="case-escalation-email-toggle"]').attributes('disabled'),
    ).toBeDefined();
  });
  ...
  it('lists passkeys and shows rotated recovery codes only in memory until acknowledged', async () => {
    ...
    expect(
      wrapper.get('[data-testid="rotated-recovery-codes"] button').attributes('disabled'),
    ).toBeDefined();
