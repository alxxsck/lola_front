...
const previewVersion = computed(
  () => preview.value?.compilerVersion.match(/v(\d+)$/u)?.[1] ?? "—",
);
const draftOverview = computed(() => {
  try {
    ...
    return {
      groups,
      priorityRuleCount: Array.isArray(value.priorityRules)
        ? value.priorityRules.length
        : 0,
    };
  } catch {
...
async function load(): Promise<void> {
  ...
  try {
    const [policyValue, costValue] = await Promise.all([
      ...
      canReadCost.value
        ? endUserCasesRepository.cost(projectId.value)
    ...
    error.value =
      cause instanceof Error ? cause.message : "Не удалось загрузить настройки";
  } finally {
    loading.value = false;
  }
}
  ...
  try {
    ...
    error.value =
      cause instanceof Error
        ? cause.message
        : "Не удалось прочитать формат настроек";
    return null;
  }
}
...
async function previewDraft(): Promise<void> {
  ...
  message.value = "";
  try {
    preview.value = await endUserCasesRepository.previewPolicy(
      projectId.value,
      draft,
    );
    message.value =
      "Правила корректны. Проверка не изменила опубликованные настройки.";
  } catch (cause) {
    error.value =
      cause instanceof Error ? cause.message : "Политика не прошла проверку";
  } finally {
    loading.value = false;
  }
}
  ...
  const groups = Array.isArray(compiled.groups)
    ? compiled.groups.filter(
        (group) =>
          group &&
          typeof group === "object" &&
          (group as { code?: unknown }).code !== "UNMAPPED",
      )
    : [];
  return {
    groups,
    priorityRules: Array.isArray(compiled.priorityFloors)
      ? compiled.priorityFloors
      : [],
    scheduling:
      compiled.scheduling && typeof compiled.scheduling === "object"
        ? compiled.scheduling
        : {},
  };
}
  ...
  <section class="page case-settings-page">
    ...
    <div v-if="cost" class="cost-grid">
      ...
      <div>
        ...
        <strong>{{
          integerFormatter.format(Number(cost.budget.projectDailyTokenHardCap))
        }}</strong>
      </div>
    </div>
    <Message
      v-if="cost?.budget.emergencyPaused"
      severity="warn"
      :closable="false"
    >
      Анализ обращений приостановлен платформой. Сигналы сохраняются и будут
      обработаны после снятия паузы.
    </Message>
    <Message
      v-else-if="cost && cost.budget.backlogCount > 0"
      severity="warn"
      :closable="false"
    >
      ...
      Новые сообщения сохраняются и будут обработаны автоматически после
      восстановления доступности или лимита.
    </Message>
      ...
      <section class="card card-pad">
        <div class="section-heading">
          <div>
            ...
            <p>
              Категории, минимальные приоритеты и интервалы проверки. Резервная
              категория добавляется автоматически.
            </p>
          </div>
          <span v-if="policy">
            Версия {{ (policy.draft ?? policy.published).version }}
          </span>
        </div>
        <div class="draft-summary">
          ...
          >
          <span
            >Правил минимального приоритета:
