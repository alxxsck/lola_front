...
describe('SupportCaseNotificationPolicyPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(supportCaseNotificationPolicySource.read).mockResolvedValue(snapshot as never);
    vi.mocked(supportCaseNotificationPolicySource.readMetrics).mockResolvedValue({
  ...
  async function render(permissions = ['project.support.notification_policy.manage']) {
    ...
    await vi.waitFor(() => expect(wrapper.text()).toContain('Какие обращения учитывать'));
    return wrapper;
  }
