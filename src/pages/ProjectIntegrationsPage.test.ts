...
const mocks = vi.hoisted(() => ({
  ...
  updateSlack: vi.fn(),
  testSlack: vi.fn(),
  permissions: ['project.notifications.read', 'project.notifications.manage'] as string[],
...
vi.mock('@/features/notification-destinations/notification-destinations.api', () => ({
...
vi.mock('@/features/notification-destinations/OperationalTelegramCard.vue', () => ({
  default: { template: '<section data-testid="telegram-card-stub" />' },
}));

vi.mock('@/features/telegram-product-installations/ProductTelegramCard.vue', () => ({
  default: {
...
vi.mock('@/features/integration-connections/IntegrationConnectionsCard.vue', () => ({
  default: {
...
vi.mock('@/features/integration-event-routes/IntegrationEventRoutesCard.vue', () => ({
  default: {
    props: ['projectId', 'canRead', 'canManage', 'canReadActivity', 'provider', 'focusRouteId'],
    template: `
        <section
...
vi.mock('@/features/integration-inbound-connections/IntegrationInboundConnectionsCard.vue', () => ({
  default: {
    props: ['projectId', 'canRead', 'canManage', 'provider'],
    template: '<section :data-testid="provider + \'-inbound-connections-stub\'" />',
  },
}));

vi.mock('@/features/integration-inbound-routes/IntegrationInboundRoutesCard.vue', () => ({
  default: {
    props: ['projectId', 'canRead', 'canManage', 'provider'],
    template: '<section :data-testid="provider + \'-inbound-routes-stub\'" />',
  },
}));

vi.mock('@/features/integration-inbound-activity/IntegrationInboundActivityCard.vue', () => ({
  default: {
...
vi.mock('@/features/integration-recovery/IntegrationRecoveryOperationsCard.vue', () => ({
  default: {
    props: ['projectId', 'canReadActivity', 'canReadIntegrations', 'canManage'],
    template: `
        <section
...
describe('ProjectIntegrationsPage', () => {
  beforeEach(() => {
    ...
    mocks.permissions = ['project.notifications.read', 'project.notifications.manage'];
    mocks.auth = reactive({
      project: {
        id: 'project-1',
        name: 'Project One',
  ...
  it('renders Slack setup with the shared integration card and field contract', async () => {
    ...
    expect(wrapper.get('[data-integration="slack"]').classes()).toContain('integration-card');
    expect(wrapper.text()).toContain('Slack для команды');
    expect(wrapper.findAll('form[data-form="create-slack"] .integration-field')).toHaveLength(2);
    expect(wrapper.get('input[name="displayName"]').element.closest('label')).not.toBeNull();
    expect(wrapper.get('input[name="webhookUrl"]').element.closest('label')).not.toBeNull();
  });

  it('separates communication channels and providers into persistent tabs', async () => {
    mocks.permissions = ['project.integrations.read', 'project.integrations.manage'];
    mocks.auth.project.effectivePermissionCodes = [...mocks.permissions];
    ...
    expect(wrapper.find('[data-testid="telegram-card-stub"]').exists()).toBe(false);
    expect(wrapper.text()).not.toContain('Slack');
    expect(wrapper.findAll('[role="tab"]')).toHaveLength(4);
    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Для пользователей');
    expect(wrapper.get('[data-integration-section="users"]').isVisible()).toBe(true);
    expect(wrapper.get('[data-testid="product-telegram-card-stub"]').attributes()).toMatchObject({
      'data-project-id': 'project-1',
      'data-can-read': 'true',
      'data-can-manage': 'true',
    });

    await wrapper.get('[role="tab"][data-section="CUSTOMER_IO"]').trigger('click');

    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Customer.io');
    expect(wrapper.get('[data-integration-section="CUSTOMER_IO"]').classes()).toContain(
      'is-active',
    );
    expect(wrapper.get('[data-integration-section="users"]').classes()).not.toContain('is-active');

    await wrapper
      .get('[role="tab"][data-section="CUSTOMER_IO"]')
      .trigger('keydown', { key: 'ArrowRight' });

    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Amplitude');
    expect(
      wrapper.get('[data-testid="AMPLITUDE-connections-card-stub"]').attributes(),
    ).toMatchObject({
      'data-project-id': 'project-1',
      'data-can-read': 'true',
      'data-can-manage': 'true',
    });
    expect(wrapper.get('[data-testid="CUSTOMER_IO-connections-card-stub"]')).toBeTruthy();
    expect(wrapper.get('[data-testid="CUSTOMER_IO-routes-card-stub"]')).toBeTruthy();
    expect(wrapper.get('[data-testid="AMPLITUDE-inbound-connections-stub"]')).toBeTruthy();
    expect(wrapper.get('[data-testid="AMPLITUDE-inbound-routes-stub"]')).toBeTruthy();
    expect(wrapper.get('[data-testid="CUSTOMER_IO-inbound-connections-stub"]')).toBeTruthy();
    expect(wrapper.get('[data-testid="CUSTOMER_IO-inbound-routes-stub"]')).toBeTruthy();
    expect(
      wrapper
    ...
    expect(
      wrapper.get('[data-testid="canonical-identity-policy-stub"]').attributes(),
    ).toMatchObject({
      'data-project-id': 'project-1',
      'data-can-read': 'true',
      'data-can-manage': 'true',
  ...
  it('opens a linked Customer.io route from an Event Schema blocker', async () => {
    ...
    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Customer.io');
    expect(
      wrapper.get('[data-testid="CUSTOMER_IO-routes-card-stub"]').attributes('data-focus-route-id'),
    ).toBe('route-1');
  });
  ...
  it('mounts recovery operations only for integration activity readers', async () => {
    ...
    expect(wrapper.get('[data-testid="integration-recovery-stub"]').attributes()).toMatchObject({
      'data-project-id': 'project-1',
  ...
  it('keeps recovery available to activity readers without integrations access', async () => {
    ...
    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Общие правила');
    expect(wrapper.get('[data-testid="integration-recovery-stub"]').attributes()).toMatchObject({
      'data-can-read-activity': 'true',
      'data-can-read-integrations': 'false',
      'data-can-manage': 'false',
    });
    expect(wrapper.find('[data-testid="canonical-identity-policy-stub"]').exists()).toBe(false);
  });
  ...
  it('creates and tests a destination, then clears the write-only webhook input', async () => {
    ...
    await wrapper.get('input[name="displayName"]').setValue('Команда поддержки');
    await wrapper
    ...
    expect(
      (wrapper.find('input[name="webhookUrl"]').element as HTMLInputElement | undefined)?.value ??
        '',
    ).toBe('');
  ...
  it('requires successful test before activation and uses the latest server version', async () => {
    ...
    mocks.updateSlack.mockResolvedValue(destination({ status: 'ACTIVE', version: 5 }));
    const wrapper = mount(ProjectIntegrationsPage);
    await flushPromises();
    ...
    expect(mocks.updateSlack).toHaveBeenCalledWith('project-1', 'destination-1', {
      expectedVersion: 4,
  ...
  it('reloads stale state after an optimistic concurrency conflict without exposing backend details', async () => {
    ...
    expect(wrapper.text()).toContain('Настройки уже изменились в другой вкладке');
  ...
  it('polls the same durable test command until the worker stores a terminal result', async () => {
    ...
    expect(mocks.testSlack.mock.calls[0]?.[3]).toEqual(mocks.testSlack.mock.calls[1]?.[3]);
    expect(wrapper.text()).toContain('Проверка Slack прошла успешно');
    vi.useRealTimers();
  });
  ...
  it('reuses the create idempotency key after an ambiguous client-side failure', async () => {
    ...
    expect(mocks.createSlack.mock.calls[0]?.[2]).toEqual(mocks.createSlack.mock.calls[1]?.[2]);
  });
  ...
  it('does not test a rotated credential after the operator switches projects', async () => {
    ...
    await wrapper
      .get('input[name="webhookUrl"]')
      .setValue('https://hooks.slack.com/services/T/B/rotated-project-a-secret');
    await wrapper.get('form[data-form="rotate-slack"]').trigger('submit');
    mocks.auth.project = {
      id: 'project-2',
      name: 'Project Two',
    ...
    await flushPromises();
    resolveRotation(destination({ status: 'PENDING_TEST', version: 6, secretRevision: 2 }));
    await flushPromises();
  ...
  it('clears loaded state and fences late operations when read permission is lost', async () => {
    ...
    await wrapper.get('button[data-action="disable"]').trigger('click');
    mocks.auth.project.effectivePermissionCodes = ['project.notifications.read'];
    await flushPromises();
