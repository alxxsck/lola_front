...
function isRecoverableValidation(cause: unknown): boolean {
  return cause instanceof ApiError && cause.code === 'PASSWORD_POLICY_REJECTED';
}

function isRetryableFailure(cause: unknown): boolean {
  return (
    cause instanceof ApiError &&
    (cause.code === 'RATE_LIMITED' || cause.status === 0 || cause.status >= 500)
  );
}
  ...
  <main class="reset-page">
    <section class="reset-card" aria-live="polite">
      ...
      <template v-else>
        ...
        <p>
          Пароль изменится только после отправки формы. Затем войдите в аккаунт обычным способом.
        </p>
        <form :aria-busy="state === 'PENDING'" @submit.prevent="submit">
          <div class="field">
            ...
            <InputText
              ...
              :aria-describedby="
                error ? 'reset-password-rules password-reset-error' : 'reset-password-rules'
              "
            />
