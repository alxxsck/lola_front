...
describe('ScenariosPage V2 activation boundary', () => {
  beforeEach(() => {
    ...
    mocks.updateScenarioMetadata.mockImplementation(async (_projectId, _scenarioId, payload) => ({
      ...scenario,
      ...payload,
    }));
  });
  ...
  it('routes inactive scenarios to Studio for atomic publication', async () => {
    ...
    await (
      wrapper.vm as unknown as { toggleScenario: (value: Scenario) => Promise<void> }
  ...
  it('resumes a paused published scenario without creating a new revision', async () => {
    ...
    expect(mocks.updateScenarioMetadata).toHaveBeenCalledWith('project-1', 'scenario-1', {
      status: 'ACTIVE',
