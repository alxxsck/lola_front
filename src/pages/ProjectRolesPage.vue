...
async function submit(): Promise<void> {
  ...
  if (action === 'CREATE') {
    ...
    await library.reassign(projectId, target.value, replacementRoleIds.value, normalizedReason);
...
<template>
  <section class="page project-roles-page">
    ...
    <Message v-if="library.listError.value" severity="error" :closable="false">{{
      library.listError.value
    }}</Message>
    <Message v-if="library.catalogError.value" severity="error" :closable="false">{{
      library.catalogError.value
    }}</Message>
    <Message
      v-if="library.operation.value.kind === 'VERSION_CONFLICT'"
      severity="warn"
      :closable="false"
    >
    ...
    <Message
      v-else-if="library.operation.value.kind === 'ROLE_IMPACT_CHANGED'"
      severity="warn"
      :closable="false"
    >
    ...
    <Message
      v-else-if="library.operation.value.kind === 'ROLE_IN_USE'"
      severity="warn"
      :closable="false"
    >
    ...
    <Message
      v-else-if="library.operation.value.kind === 'MANAGED_ROLE_PROTECTED'"
      severity="error"
      :closable="false"
    >
    ...
    <Message
      v-else-if="library.operation.value.kind === 'PERMISSION_DENIED'"
      severity="error"
      :closable="false"
    >
    ...
    <Message
      v-else-if="library.operation.value.kind === 'STEP_UP_REQUIRED'"
      severity="warn"
      :closable="false"
    >
    ...
    <div class="role-grid" data-testid="role-library">
      <article v-for="role in library.items.value" :key="role.id" class="role-card">
        <div class="role-card__header">
          ...
          <Tag
            :value="role.managed ? 'Системная' : 'Пользовательская'"
            :severity="role.managed ? 'secondary' : 'info'"
          />
        </div>
        ...
        <div class="role-meta">
          ...
          <span
            >{{ role.assignedMembershipCount }} назначений{{
              role.assignedMembershipCountCapped ? '+' : ''
            }}</span
          >
    ...
    >
      <div class="role-form">
        ...
        <template v-if="dialogAction === 'CREATE' || dialogAction === 'UPDATE'">
          <label v-if="dialogAction === 'CREATE'"
            >Ключ<InputText v-model="key" data-testid="role-key"
          /></label>
          <label>Название<InputText v-model="name" data-testid="role-name" /></label>
          <label
            >Описание<Textarea v-model="description" rows="3" data-testid="role-description"
          /></label>
          <fieldset>
            <legend>Права из серверного каталога</legend>
            <section
              v-for="group in delegableGroups"
              :key="`${group.category}:${group.risk}`"
        ...
        <div v-if="requiresImpactConfirmation && target" class="impact-confirmation">
          <strong
            >Влияние: {{ target.assignedMembershipCount }} активных назначений{{
              target.assignedMembershipCountCapped ? '+' : ''
            }}</strong
          >
          <label
            ><input v-model="impactConfirmed" type="checkbox" data-testid="impact-confirmation" />
            Подтверждаю это влияние и версию v{{ target.version }}</label
          >
        </div>
      ...
      <template #footer>
        <Button
          label="Отмена"
          text
          :disabled="library.operation.value.kind === 'SUBMITTING'"
          @click="closeDialog"
        />
        <Button
