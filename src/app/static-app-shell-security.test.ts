...
const automaticallyLoadedExternalAsset = /<(?:iframe|img|link|script|source|video)\b[^>]*\b(?:href|src)=["']https?:\/\//i
const remoteStylesheetDependency = /@import\s+(?:url\()?\s*["']?https?:\/\/|url\(\s*["']?https?:\/\//i
...
describe('static app shell security', () => {
  ...
  it('does not load third-party fonts or automatic external assets', () => {
    ...
    const textAssets = emittedFiles.filter((file) => ['.css', '.html', '.js'].includes(extname(file)))
    const emittedSource = textAssets.map((file) => readFileSync(file, 'utf8')).join('\n')
    const html = readFileSync(join(outputDirectory, 'index.html'), 'utf8')
