...
const generatedApiRoot = resolve(sourceRoot, 'shared/api/generated');
...
const namedColorDeclaration =
  /(?:^|[;{])\s*(?:color|background(?:-color)?|border(?:-(?:top|right|bottom|left))?(?:-color)?|fill|stroke)\s*:\s*(?:black|white|red|blue|green|gray|grey|orange|purple|yellow|pink|brown|cyan|magenta|lime|navy|teal|maroon|olive|silver|aqua|fuchsia)(?=\s*[;}])/i;

describe('theme color boundary', () => {
  it.each(['color: rgba(0, 0, 0, 0.5)', 'background: hsla(120, 50%, 50%, 0.5)'])(
    'rejects legacy functional raw color %s',
    (source) => {
  ...
  it('keeps raw UI colors inside the theme token file', () => {
    const violations = readdirSync(sourceRoot, { recursive: true })
      .map(String)
      .filter((file) => !resolve(sourceRoot, file).startsWith(generatedApiRoot))
