...
vi.mock('@/shared/api/repository/scenario-authoring', () => ({
  scenarioAuthoringRepository: { previewGoal: mocks.preview },
}));
...
describe('ScenarioGoalPreview', () => {
  beforeEach(() => {
    ...
    mocks.preview.mockResolvedValue({
      ...
      dependency: {
        ...
        eventDefinitionRevisionId: 'event-revision-4',
        eventCode: 'deposit.succeeded',
        schemaVersion: 4,
      },
      window: {
        ...
        deadlineAt: '2026-07-19T10:00:00Z',
  ...
  it('previews a typed goal against an Event Log and never renders a redacted actual value', async () => {
    const wrapper = shallowMount(ScenarioGoalPreview, {
      props: { projectId: 'project-1', config, contract },
    });
    await flushPromises();
