...
const measureOptions = [
  { label: 'Количество событий', value: 'count' },
  { label: 'Сумма поля', value: 'sum' },
];
...
const selectedEvent = computed(() =>
  props.contract.events.find((event) => event.code === draft.eventCode),
);
const eventOptions = computed<EventPickerOption[]>(() =>
  props.contract.events.map((event) => ({
    value: event.code,
    name: event.name,
    code: event.code,
...
const selectedEventOption = computed(() =>
  eventOptions.value.find((event) => event.value === draft.eventCode),
);
const numericFields = computed(
  () =>
    selectedEvent.value?.fields.filter((field) =>
      field.capabilities.aggregateMeasure.measures.includes('sum'),
    ) ?? [],
);
...
function periodFromMs(milliseconds: number): [number, DurationUnit] {
  for (const unit of [...unitOptions].reverse()) {
    if (milliseconds >= unit.multiplier && milliseconds % unit.multiplier === 0)
      return [milliseconds / unit.multiplier, unit.value];
  }
...
function rawConfig(): Record<string, unknown> {
  return {
    goal: {
      ...
      ...(draft.measure === 'sum' && draft.numericFieldKey
        ? { numericFieldKey: draft.numericFieldKey }
        : {}),
...
function changeDuration() {
  const multiplier =
    unitOptions.find((unit) => unit.value === durationUnit.value)?.multiplier ?? 1_000;
  draft.timeoutMs = Number(duration.value) * multiplier;
  commit();
}

function createBranchTarget(target: ScenarioActionTargetOption, branch: 'goal' | 'timeout') {
...
function filterOperators(filter: GoalFilterDraft) {
  return (fieldFor(filter)?.capabilities.aggregateFilter.operators ?? []).filter(
    (operator): operator is GoalFilterOperator =>
      supportedFilterOperators.has(operator as GoalFilterOperator),
  );
}
  ...
  <div class="goal-editor">
    ...
    <div class="field">
      ...
      <select
        ...
        aria-label="Что считать для цели"
    ...
    <div v-if="draft.measure === 'sum'" class="field">
      ...
      <select
        id="goal-number-field"
        v-model="draft.numericFieldKey"
        aria-label="Поле суммы цели"
        @change="commit"
      >
        <option value="">Выберите числовое поле</option>
        <option v-for="field in numericFields" :key="field.fieldKey" :value="field.fieldKey">
    ...
    <div class="filters-head">
      <div>
        <strong>Какие события учитывать</strong
        ><small>До 20 типизированных фильтров payload.</small>
      </div>
      <button
        type="button"
        aria-label="Добавить фильтр цели"
        :disabled="draft.filters.length >= 20 || !selectedEvent"
        @click="addFilter"
      >
        + Фильтр
      </button>
    ...
    <div v-for="(filter, index) in draft.filters" :key="index" class="filter-card">
      <select
        ...
        :aria-label="`Поле фильтра цели ${index + 1}`"
        @change="changeFilterField(filter)"
      >
        <option value="">Поле</option>
        <option
          v-for="field in selectedEvent?.fields"
          :key="field.fieldKey"
      ...
      <select
        ...
        :aria-label="`Оператор фильтра цели ${index + 1}`"
        @change="changeFilterOperator(filter)"
      >
        <option v-for="operator in filterOperators(filter)" :key="operator" :value="operator">
          {{ goalFilterOperatorLabel(operator) }}
        </option>
      </select>
      <select
        v-if="fieldFor(filter)?.allowedValues && filter.operator === 'in'"
        v-model="filter.value"
        multiple
        :aria-label="`Значения фильтра цели ${index + 1}`"
        @change="commit"
      >
        <option
          v-for="value in fieldFor(filter)?.allowedValues"
          :key="String(value)"
          :value="value"
        >
          {{ value }}
      ...
      <select
        v-else-if="fieldFor(filter)?.allowedValues && filter.operator !== 'exists'"
        v-model="filter.value"
        :aria-label="`Значение фильтра цели ${index + 1}`"
        @change="commit"
      >
      ...
      <select
        v-else-if="fieldFor(filter)?.valueType === 'boolean' && filter.operator !== 'exists'"
        v-model="filter.value"
        :aria-label="`Значение фильтра цели ${index + 1}`"
        @change="commit"
      >
        <option :value="undefined">Значение</option>
      ...
      <input
        ...
        :aria-label="`Значения фильтра цели ${index + 1}`"
        @input="changeListFilterValue(filter, $event)"
      />
      <input
        v-else-if="filter.operator !== 'exists'"
        :value="filter.value"
        :type="
          ['number', 'integer'].includes(fieldFor(filter)?.valueType ?? '') ? 'number' : 'text'
        "
        :aria-label="`Значение фильтра цели ${index + 1}`"
        @input="changeScalarFilterValue(filter, $event)"
      />
      <button
        type="button"
        :aria-label="`Удалить фильтр цели ${index + 1}`"
    ...
    <div class="compare-grid">
      <div class="field">
        <label for="goal-compare">Успех, когда результат</label
        ><select
          id="goal-compare"
          v-model="draft.compare.operator"
          aria-label="Сравнение цели"
          @change="commit"
        >
      ...
      <div class="field">
        <label for="goal-threshold">Порог</label
        ><input
          id="goal-threshold"
          v-model="draft.compare.value"
          inputmode="decimal"
          aria-label="Порог цели"
          @input="commit"
        />
      </div>
    </div>
    ...
    <div class="deadline-card">
      ...
      <p>
        Ожидание всегда конечное. Этот срок выбирает Timeout-ветку и не является сроком ожидания
        online.
      </p>
      <div class="duration-grid">
        ...
        /><select v-model="durationUnit" aria-label="Единица срока цели" @change="changeDuration">
    ...
    <div class="goal-summary">
      <strong>Как это работает</strong><span>{{ summary }}</span>
    </div>
    <ul v-if="issues.length" class="goal-issues">
      <li v-for="item in issues" :key="`${item.code}:${item.field}`">{{ item.message }}</li>
    </ul>
  </div>
</template>
...
<style scoped>
.goal-editor {
  --event-picker-trigger-height: 38px;
  --catalog-picker-trigger-height: 38px;
  display: grid;
  gap: 13px;
}
.field {
