  ...
  events: [
    {
      ...
      fields: [
        ...
        {
          fieldKey: 'deposit.approved',
          label: 'Подтверждён',
          path: 'event.payload.approved',
          required: true,
          valueType: 'boolean',
...
describe('ScenarioGoalEditor', () => {
  ...
  it('keeps numeric filter values typed as numbers', async () => {
    ...
    const config = wrapper.emitted('update:modelValue')?.at(-1)?.[0] as {
      goal: { filters: Array<{ value: unknown }> };
    };
  ...
  it('authors an in filter as a typed multi-value selection', async () => {
    ...
    expect(
      wrapper
        .get('select[aria-label="Оператор фильтра цели 1"]')
        .findAll('option')
        .map((option) => option.text()),
    ).toEqual(['равно', 'одно из']);
  ...
  it('parses boolean filters and numeric in lists using the catalog field type', async () => {
    const typedContract: ScenarioAuthoringContract = {
      ...
      events: [
        {
          ...contract.events[0]!,
          fields: contract.events[0]!.fields.map((field) =>
            field.fieldKey === 'deposit.amount'
              ? {
                  ...field,
                  capabilities: {
                    ...field.capabilities,
    ...
    const wrapper = mount(ScenarioGoalEditor, {
      props: {
        ...
        targets: [target('done', 'Готово'), target('timeout', 'Срок')],
      },
    });
    wrapper.getComponent(EventPicker).vm.$emit('update:modelValue', 'deposit.succeeded');
    await wrapper.vm.$nextTick();
