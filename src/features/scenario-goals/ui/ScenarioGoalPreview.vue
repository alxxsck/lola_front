...
import {
  ...
  type PreviewScenarioGoalResponseDto,
...
const eventLogOptions = computed(() =>
  ...
  try {
    ...
    error.value =
      cause instanceof Error
        ? cause.message
        : 'Не удалось загрузить журнал событий для предпросмотра';
  } finally {
...
async function preview() {
  ...
  try {
    ...
    if (requestId === previewSequence && requestedEventLogId === eventLogId.value)
      result.value = response;
  } catch (cause) {
    if (requestId === previewSequence)
      error.value = cause instanceof Error ? cause.message : 'Не удалось проверить цель';
  } finally {
...
<template>
  <div class="goal-preview">
    <header>
      <strong>Предпросмотр цели</strong
      ><small>Проверка привязана к записи события и конкретной версии его определения.</small>
    </header>
    <Select
      v-model="eventLogId"
      ...
      option-label="label"
      option-value="value"
      placeholder="Выберите запись события пользователя"
      aria-label="Запись события для предпросмотра цели"
      :loading="loadingLogs"
      fluid
    />
    <Button
      label="Проверить цель"
      icon="pi pi-play"
      severity="secondary"
      outlined
      :disabled="!eventLogId"
    ...
    <section v-if="result" class="goal-preview-result">
      <div>
        <Tag
          :value="result.matched ? 'Цель достигнута' : 'Цель не достигнута'"
          :severity="result.matched ? 'success' : 'secondary'"
        /><span
          >Совпадений: {{ result.matchedCount ?? result.explanation?.matchedCount ?? '—' }}</span
        >
      </div>
      <p><strong>Фактическое значение:</strong> {{ actualLabel() }}</p>
      <p v-if="result.window">
        <strong>Окно:</strong> {{ result.window.from }} — {{ result.window.to }} · срок
        {{ result.window.deadlineAt }}
      </p>
      <p v-if="result.dependency">
        <strong>Зафиксированная версия события:</strong>
        <code>{{ result.dependency.eventDefinitionRevisionId }}</code> · схема v{{
          result.dependency.schemaVersion
        }}
        · постоянный ключ <code>{{ result.dependency.definitionKeyId }}</code>
      </p>
...
.goal-preview header strong {
  font-size: 0.72rem;
}
.goal-preview header small,
.goal-preview-result {
  color: var(--muted);
  font-size: 0.66rem;
  line-height: 1.45;
...
.goal-preview-result {
  ...
  gap: 7px;
  padding-top: 8px;
  border-top: 1px solid var(--status-accent);
}
.goal-preview-result > div {
  display: flex;
  align-items: center;
  gap: 8px;
}
.goal-preview-result p,
.goal-preview-result ul {
  margin: 0;
}
.goal-preview-result code {
