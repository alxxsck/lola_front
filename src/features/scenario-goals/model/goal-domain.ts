...
export type GoalSerializationResult =
  | {
      ok: true;
      value: { goal: ScenarioGoalDto; timeoutMs: number; onGoal: string; onTimeout: string };
    }
...
export function goalDraftFromConfig(value: Record<string, unknown>): GoalDraft {
  ...
  if (Array.isArray(goal.filters)) {
    draft.filters = goal.filters.map(record).map((filter) => ({
      ...
      operator: ['eq', 'neq', 'in', 'exists'].includes(String(filter.operator))
        ? (filter.operator as GoalFilterOperator)
        : 'eq',
      ...(filter.value === undefined ? {} : { value: cloneFilterValue(filter.value) }),
    }));
  }
  const compare = record(goal.compare);
  if (['eq', 'neq', 'gt', 'gte', 'lt', 'lte'].includes(String(compare.operator)))
    draft.compare.operator = compare.operator as GoalCompareOperator;
  if (typeof compare.value === 'string' || typeof compare.value === 'number')
    draft.compare.value = String(compare.value);
...
function scalarValueMatches(
  value: GoalFilterDraft['value'],
  valueType: string,
...
function valueMatches(
  ...
  if (operator === 'in')
    return (
      Array.isArray(value) &&
      value.length > 0 &&
      value.every((item) => scalarValueMatches(item, valueType, allowedValues))
    );
...
export function validateGoalDraft(
  ...
  if (!event)
    return [issue('event-required', 'Выберите опубликованное событие цели.', 'eventCode')];
  ...
  if (
    draft.measure === 'sum' &&
    (!numericField || !numericField.capabilities.aggregateMeasure.measures.includes('sum'))
  ) {
    issues.push(
      issue(
        'numeric-field-required',
        'Для суммы выберите доступное числовое поле.',
        'numericFieldKey',
      ),
    );
  }
  if (draft.filters.length > 20)
    issues.push(issue('filters-limit', 'Для цели допустимо не более 20 фильтров.', 'filters'));
  draft.filters.forEach((filter, index) => {
    const field = event.fields.find((candidate) => candidate.fieldKey === filter.fieldKey);
    if (!field) {
      issues.push(
        issue('filter-field-required', 'Выберите поле фильтра.', `filters.${index}.fieldKey`),
      );
      return;
    }
    if (!field.capabilities.aggregateFilter.operators.includes(filter.operator)) {
      issues.push(
        issue(
          ...
          'Выбранное сравнение недоступно для поля.',
      ...
      if (filter.value !== undefined)
        issues.push(
          issue(
            ...
            'Для проверки наличия значение не требуется.',
            `filters.${index}.value`,
          ),
        );
  ...
  if (draft.measure === 'sum' && numericField?.semanticType?.startsWith('money')) {
    ...
    if (currencyFilters.length !== 1) {
      issues.push(
        issue(
          'currency-required',
          'Для денежной суммы добавьте ровно один фильтр валюты «равно».',
          'filters',
        ),
      );
    }
  }
  if (!decimalPattern.test(draft.compare.value))
    issues.push(issue('threshold-invalid', 'Укажите точное конечное число.', 'compare.value'));
  if (
    !Number.isInteger(draft.timeoutMs) ||
    draft.timeoutMs < MIN_TIMEOUT_MS ||
    draft.timeoutMs > MAX_TIMEOUT_MS
  ) {
    issues.push(
      issue('deadline-invalid', 'Срок цели должен быть от 1 секунды до 90 дней.', 'timeoutMs'),
    );
  }
  if (!draft.onGoal)
    issues.push(issue('goal-branch-required', 'Выберите действие при достижении цели.', 'onGoal'));
  if (!draft.onTimeout)
    issues.push(
      issue('timeout-branch-required', 'Выберите действие по истечении срока.', 'onTimeout'),
    );
  return issues;
}

export function serializeGoalDraft(
  draft: GoalDraft,
  contract: ScenarioAuthoringContract,
...
export function summarizeGoalDraft(draft: GoalDraft, contract: ScenarioAuthoringContract): string {
  ...
  const measure =
    draft.measure === 'sum'
      ? `сумма поля «${numericField?.label ?? draft.numericFieldKey ?? 'не выбрано'}»`
      : 'количество событий';
  const compare = {
    eq: 'равно',
    neq: 'не равно',
