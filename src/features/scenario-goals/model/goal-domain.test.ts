  ...
  events: [
    {
      ...
      definitionId: 'deposit-revision-1',
...
describe('WAIT_FOR_GOAL domain', () => {
  it('serializes the registration to two-day deposit Goal/Timeout worked example', () => {
    ...
    Object.assign(draft, {
      ...
      measure: 'sum',
      numericFieldKey: 'deposit.amount',
    ...
    expect(serializeGoalDraft(draft, contract)).toEqual({
      ok: true,
      value: {
        goal: {
          version: 1,
    ...
    if (!result.ok)
      expect(result.issues.map((issue) => issue.code)).toEqual(
        expect.arrayContaining([
          'currency-required',
          'deadline-invalid',
          'goal-branch-required',
          'timeout-branch-required',
        ]),
      );
  });
  ...
  it('restores scalar arrays from reactive Scenario config', () => {
    const config = reactive({
      goal: {
        eventCode: 'deposit.succeeded',
        measure: 'count',
        filters: [{ fieldKey: 'deposit.currency', operator: 'in', value: ['EUR', 'USD'] }],
        compare: { operator: 'gte', value: '2' },
      },
      timeoutMs: 10_000,
      onGoal: 'done',
  ...
  it('rejects scalar values for in and arrays for scalar operators', () => {
    ...
    const scalarIn = validateGoalDraft(
      { ...base, filters: [{ fieldKey: 'deposit.currency', operator: 'in', value: 'EUR' }] },
      contract,
    );
    const arrayEq = validateGoalDraft(
      { ...base, filters: [{ fieldKey: 'deposit.currency', operator: 'eq', value: ['EUR'] }] },
      contract,
    );
