...
export const platformSafetyClassActions = {
  ...
  CREDIBLE_THREAT_OR_VIOLENCE: 'Безопасный ответ, обращение, эскалация и тревога',
  HARM_INVOLVING_MINORS: 'Безопасный ответ, обращение, эскалация и тревога',
  RESPONSIBLE_GAMING_CRISIS: 'Безопасный ответ, обращение и эскалация',
} as const;
...
export function createPlatformSafetyDraft(
  ...
  const selected =
    ...
    models.find(
      (item) => item.id === 'grok-4.5' && item.selectable && item.providerAvailable !== false,
    ) ??
    models.find((item) => item.selectable && item.providerAvailable !== false);
  const activeEffort = state?.profile.reasoningEffort;
  const requestedEffort: PlatformSafetyReasoningEffort =
    activeEffort === 'medium' || activeEffort === 'high' ? activeEffort : 'medium';
...
export function validatePlatformSafetyDraft(
  ...
  if (!model) issues.push({ path: 'modelId', message: 'Выберите модель безопасности.' });
  ...
  if (!reason) issues.push({ path: 'reason', message: 'Укажите причину публикации.' });
