...
describe('SupportCaseBrief', () => {
  it('keeps the handoff brief compact and reveals long context on demand', async () => {
    const wrapper = mount(SupportCaseBrief, {
      props: {
        ...
        blockers: ['Ожидается ответ платёжного провайдера', 'Нет подтверждённого срока зачисления'],
        limitations: ['Нельзя обещать время поступления средств'],
      },
      global: { stubs },
    });

    expect(wrapper.get('.case-brief__summary').text()).toContain('Платёж найден');
    expect(wrapper.get('.case-brief__blockers').text()).toContain('Блокеры · 2');
    ...
    expect(dialog.get('.case-brief-sheet__summary').text()).toContain('Что произошло');
    expect(dialog.get('.case-brief-sheet__goal').text()).toContain('Что нужно получить');
    expect(dialog.get('.case-brief-sheet__signals').attributes('aria-label')).toBe(
      'Риски и ограничения',
    );
  ...
  it('distinguishes confirmed absence of blockers from unavailable data', () => {
    ...
    expect(confirmedClear.get('.case-brief__blockers').text()).toContain('Блокеры · 0');
    expect(confirmedClear.get('.case-brief__blockers').text()).toContain('Активных препятствий');
