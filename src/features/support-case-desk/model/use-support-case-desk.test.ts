...
describe('support case desk controller', () => {
  it('loads the exact Case projection and its causal activity', async () => {
    ...
    expect(controller.detail.value?.case.allowedActions).toContain('CHANGE_CLASSIFICATION');
    expect(controller.detail.value?.timeline.events[0]?.reason).toBe('Уточнено после проверки');
  });
  ...
  it('fails closed for transitions not present in allowedActions', async () => {
    ...
    await expect(controller.transition('CANCELLED', 'Проверено оператором')).rejects.toThrow(
      'недоступно',
    );
  ...
  it('surfaces a typed 409, refreshes authority and keeps the operator draft external', async () => {
    ...
    await expect(controller.transition('RESOLVED', 'Пользователь подтвердил')).rejects.toThrow(
      'Case changed',
    );
  ...
  it('never combines an exact Case from the previous project with a new route scope', async () => {
    ...
    await expect(controller.transition('RESOLVED', 'Новый маршрут')).rejects.toThrow(
  ...
  it('turns a typed 409 from same-key replay into conflict reconciliation', async () => {
    const api = source();
    api.workflow.mockRejectedValueOnce(new ApiError(0, 'Connection lost')).mockRejectedValueOnce(
    ...
    api.detail.mockResolvedValueOnce(makeBundle(4)).mockResolvedValueOnce(makeBundle(5));
    const controller = createSupportCaseDeskController(api, {
      projectId: () => 'project-1',
      caseId: () => 'case-1',
      canRead: () => true,
  ...
  it('retires a definitively rejected same-key replay and reloads authority', async () => {
    ...
    await expect(controller.retryReconcile()).rejects.toThrow('Invalid transition');
  ...
  it('rejects an escalation receipt scoped to another Case', async () => {
    ...
    await expect(controller.escalate('PAYMENT_REVIEW', 'Проверить платёж')).rejects.toThrow(
      'некорректное подтверждение',
    );
    expect(controller.reconciling.value).toBe(true);
    expect(controller.reconciliationReason.value).toBe('UNKNOWN');
  });
  ...
  it('applies the typed 409 receipt and remains fail-closed when its reload fails', async () => {
    ...
    expect(controller.reconciling.value).toBe(true);
    await expect(controller.transition('RESOLVED', 'Повтор')).rejects.toThrow(
      'Актуальное состояние',
    );
  ...
  it.each([403, 404])('purges private Case state and refreshes authority on %s', async (status) => {
    ...
    await expect(controller.transition('RESOLVED', 'Завершено')).rejects.toThrow(
      'Case unavailable',
    );
  ...
  it('keeps the mock operator flow contract-complete for live UI verification', async () => {
    resetMockEndUserCases();
    const controller = createSupportCaseDeskController(mockEndUserCasesRepository, {
    ...
    await controller.load();
    expect(controller.exactCase.value?.allowedActions).toContain('CHANGE_CLASSIFICATION');
