...
export type AudienceLeafKind = 'locale' | 'language' | 'country' | 'userAttribute' | 'segmentMembership'
export type AudienceComparisonOperator = 'eq' | 'neq' | 'gt' | 'gte' | 'lt' | 'lte' | 'in' | 'exists' | 'not_exists'

export interface AudienceDomainContext {
  catalog: AudienceCatalogResponseDto
...
export type AudienceLeafDraftNode = AudienceDraftNodeBase & (
  | { kind: 'locale'; operator: AudienceComparisonOperator; value?: string | string[] }
  | { kind: 'language'; operator: AudienceComparisonOperator; value?: string | string[] }
  | { kind: 'country'; operator: AudienceComparisonOperator; value?: string | string[] }
  | { kind: 'userAttribute'; definitionId: string; operator: AudienceComparisonOperator; value?: AudienceLiteral }
  | { kind: 'segmentMembership'; segmentId: string; segmentRevisionId: string; operator: 'is_member' | 'is_not_member' }
)

export type AudienceLeafInput = AudienceLeafDraftNode extends infer Leaf
  ? Leaf extends AudienceDraftNodeBase ? Omit<Leaf, 'nodeId'> : never
  : never
...
export type AudienceDraftNode =
  ...
  | (AudienceDraftNodeBase & { kind: 'opaque'; source: unknown; reportedKind?: string })
