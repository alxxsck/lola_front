...
export function applyAudienceCommand(
  ...
  if (command.type === 'add' || command.type === 'addGroup') {
    ...
    if (!location.parent || location.parent.kind === 'not' || location.index === undefined)
      return {
        ok: false,
        draft,
        error: {
...
function parseNode(source: unknown, issues: AudienceDraftIssue[]): AudienceDraftNode {
  ...
  if (
    (source.kind === 'locale' || source.kind === 'language' || source.kind === 'country') &&
  ...
  if (
    ...
    return {
      ...
      operator: source.operator as AudienceComparisonOperator,
      ...(Object.hasOwn(source, 'value') ? { value: clone(source.value) as AudienceLiteral } : {}),
    };
  if (
  ...
  return {
    ...
    issues: [
      ...
      ...collectAudienceIssues(draft, context).filter(
        (issue) =>
          !issues.some((known) => known.nodeId === issue.nodeId && known.code === issue.code),
      ),
    ],
  };
}
...
function catalogAttribute(context: AudienceDomainContext, definitionId: string) {
  return context.catalog.attributes.find((attribute) => attribute.definitionId === definitionId);
}

function v1Source(context: AudienceDomainContext, kind: 'locale' | 'language' | 'country') {
...
function compareDecimal(left: string, right: string): number | null {
  ...
  const aDigits = `${a.integer}${a.fraction.padEnd(width, '0')}`.replace(/^0+(?=\d)/, '');
  const bDigits = `${b.integer}${b.fraction.padEnd(width, '0')}`.replace(/^0+(?=\d)/, '');
  const absolute =
...
function typedAttributeLiteralIssue(
  ...
  const constraints = 'constraints' in attribute ? attribute.constraints : undefined;
  ...
  if (valueType === 'INTEGER') {
    ...
    if (typeof value !== 'number' || !Number.isFinite(value)) return 'Введите конечное число.';
    if (typeof constraints?.minimum === 'number' && value < constraints.minimum)
      return `Значение должно быть не меньше ${constraints.minimum}.`;
    if (typeof constraints?.maximum === 'number' && value > constraints.maximum)
      return `Значение должно быть не больше ${constraints.maximum}.`;
...
function serializeNode(
    ...
    return {
      ...
      children: node.children
        .map((child, childIndex) =>
          serializeNode(child, `${path}.children.${childIndex}`, index, version),
        )
  ...
  delete value.nodeId;
  if (version === 2 && value.kind === 'userAttribute') value.kind = 'profileAttribute';
...
export function serializeAudienceDraft(
  ...
  const freshnessHint =
    draft.freshness ?? (version === 2 ? { mode: 'USE_LAST_KNOWN' as const } : undefined);
  const freshness =
    version === 2 &&
...
export function summarizeAudience(
  ...
  const pristineEmpty =
    (draft.root.kind === 'all' || draft.root.kind === 'any') && draft.root.children.length === 0;
    ...
    return {
      ...
      ...(entry
        ? {
            nodeId: entry.nodeId,
            fieldPath:
              normalizedPath === nodePath ? undefined : normalizedPath.slice(nodePath.length + 1),
          }
        : {}),
    };
  });
