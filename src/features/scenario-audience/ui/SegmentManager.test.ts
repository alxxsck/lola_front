...
describe('SegmentManager', () => {
  ...
  it('creates a Segment from a typed Audience rule and pins the observed catalog revision', async () => {
    ...
    expect(
      wrapper.get('button[aria-label="Должны выполняться все условия"]').attributes('aria-pressed'),
    ).toBe('true');
  ...
  it('withholds irreversible archive while backend impact preflight is unavailable', async () => {
    ...
    expect(wrapper.get('.archive-note').text()).toContain('пока нельзя архивировать');
  ...
  it('keeps the draft while refreshing a concurrently changed Segment head', async () => {
    ...
    vi.mocked(scenarioAuthoringRepository.publishSegmentRevision).mockRejectedValue(
      new ApiError(
        409,
        'conflict',
    ...
    expect(wrapper.get('input[aria-label="Название сегмента"]').element).toHaveProperty(
      'value',
    ...
    expect(wrapper.get(`.recovery button:nth-of-type(2)`).attributes('disabled')).toBeDefined();
