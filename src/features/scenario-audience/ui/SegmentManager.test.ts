...
const detail: SegmentDetailResponseDto = {
  ...
  revisions: [
    summary.currentRevision,
    {
      ...
      revision: 1,
      catalogRevision: "audience-catalog-1",
      contentHash: "hash-1",
...
describe("SegmentManager", () => {
  beforeEach(() => {
    ...
    vi.mocked(scenarioAuthoringRepository.searchSegments).mockResolvedValue(
      search,
    );
  ...
  it("searches project Segments and shows immutable revision history", async () => {
    const wrapper = mount(SegmentManager, {
      props: { projectId: "project-1", catalog },
    });
    await flushPromises();
    expect(wrapper.text()).toContain("Русскоязычные");
    await wrapper
      .get('button[aria-label="Открыть сегмент Русскоязычные"]')
      .trigger("click");
    await flushPromises();
    ...
    expect(scenarioAuthoringRepository.searchSegments).toHaveBeenCalledWith(
      "project-1",
      expect.objectContaining({ includeArchived: false }),
    );
    expect(scenarioAuthoringRepository.getSegment).toHaveBeenCalledWith(
      "project-1",
      "segment-1",
    );
    wrapper.unmount();
  ...
  it("creates a Segment from a typed Audience rule and pins the observed catalog revision", async () => {
    vi.mocked(scenarioAuthoringRepository.createSegment).mockResolvedValue({
      ...summary,
    ...
    expect(scenarioAuthoringRepository.createSegment).toHaveBeenCalledWith(
      "project-1",
      {
        catalogRevision: "audience-catalog-3",
  ...
  it("withholds irreversible archive while backend impact preflight is unavailable", async () => {
    const wrapper = mount(SegmentManager, {
      props: { projectId: "project-1", catalog },
    });
    await flushPromises();
  ...
  it("keeps the draft while refreshing a concurrently changed Segment head", async () => {
    const nextDetail = {
      ...
      currentRevision: {
        ...
        segmentRevisionId: "segment-revision-3",
        revision: 3,
      },
    };
    vi.mocked(scenarioAuthoringRepository.getSegment)
    ...
    vi.mocked(
      ...
      new ApiError(
        409,
        "conflict",
        { currentRevisionId: "segment-revision-3" },
        undefined,
    ...
    await wrapper
      .get('input[aria-label="Название сегмента"]')
      .setValue("Мой сохранённый черновик");
    await wrapper
      .get('button[aria-label="Опубликовать сегмент"]')
      .trigger("click");
    await flushPromises();
    ...
    expect(
      (
        wrapper.vm as unknown as {
          form: { expectedCurrentRevisionId: string };
        }
      ).form.expectedCurrentRevisionId,
    ).toBe("segment-revision-2");
    expect(
      wrapper.get(`.recovery button:nth-of-type(2)`).attributes("disabled"),
    ).toBeDefined();
    await wrapper.get(`.recovery button:nth-of-type(3)`).trigger("click");
    await flushPromises();
    expect(navigator.clipboard.writeText).toHaveBeenCalled();
    vi.spyOn(window, "confirm").mockReturnValue(true);
    await wrapper.get(`.recovery button:nth-of-type(2)`).trigger("click");
    expect(
      (
        wrapper.vm as unknown as {
          form: { expectedCurrentRevisionId: string };
  ...
  it("ignores a stale search response after a newer query has completed", async () => {
    ...
    const oldRequest = new Promise<SegmentSearchResponseDto>((resolve) => {
      resolveOld = resolve;
    });
    const newRequest = new Promise<SegmentSearchResponseDto>((resolve) => {
      resolveNew = resolve;
    });
    ...
    await wrapper.get("form.search").trigger("submit");
    resolveNew({
      items: [{ ...summary, segmentId: "new", name: "Новый результат" }],
      nextCursor: null,
    });
    await flushPromises();
    resolveOld({
      items: [{ ...summary, segmentId: "old", name: "Устаревший результат" }],
      nextCursor: null,
    });
    await flushPromises();
  ...
  it("keeps the cursor paired with the query that produced it", async () => {
    ...
    await flushPromises();
    await wrapper
      .get('input[aria-label="Поиск сегментов"]')
      .setValue("ещё не применён");
    await wrapper.get("button.more").trigger("click");
    await flushPromises();

    expect(scenarioAuthoringRepository.searchSegments).toHaveBeenLastCalledWith(
      "project-1",
      expect.objectContaining({ query: undefined, cursor: "cursor-2" }),
    );
  });

  it("hides Segment mutations in read-only mode", async () => {
    const wrapper = mount(SegmentManager, {
      props: { projectId: "project-1", catalog, readonly: true },
    });
    await flushPromises();

    expect(wrapper.find('button[aria-label="Создать сегмент"]').exists()).toBe(
      false,
    );
    expect(
      wrapper.find('button[aria-label^="Архивировать сегмент"]').exists(),
    ).toBe(false);
  ...
  it("maps backend Segment rule issues back to the exact Audience control", async () => {
    vi.mocked(scenarioAuthoringRepository.createSegment).mockRejectedValue(
      new ApiError(
        422,
        "invalid",
        {
            ...
            issues: [
              {
                ...
                message: "Страна больше недоступна",
