...
import type { AudienceCatalogResponseDto, SegmentSummaryResponseDto } from '@/shared/api/generated/models'
import { createAudienceDraft, serializeAudienceDraft, type AudienceDomainContext, type AudienceDraft } from '../model'
...
const catalog: AudienceCatalogResponseDto = {
  ...
  locales: [{ code: 'ru-RU', language: 'ru', label: 'Русский (Россия)' }, { code: 'en-US', language: 'en', label: 'English (US)' }],
  ...
  attributes: [{
    definitionId: 'attribute-1', definitionRevisionId: 'attribute-revision-4', revision: 4, key: 'vipLevel', label: 'VIP-уровень', description: 'Уровень программы лояльности', valueType: 'number', required: false, sensitive: true,
    operators: ['eq', 'gte', 'exists', 'not_exists'], control: 'NUMBER', authoringAvailability: 'AVAILABLE',
  }],
  segmentSource: { operators: ['is_member', 'is_not_member'], searchEndpoint: '/segments', control: 'SEARCH', authoringAvailability: 'AVAILABLE' },
...
const segments: SegmentSummaryResponseDto[] = [{
  segmentId: 'segment-1', key: 'vip', name: 'VIP-пользователи', status: 'ACTIVE',
  currentRevision: { segmentRevisionId: 'segment-revision-2', revision: 2, catalogRevision: 'audience-catalog-3', contentHash: 'hash', publishedAt: '2026-07-18T00:00:00.000Z' },
}]

const context: AudienceDomainContext = { catalog, segments }

function mountBuilder(domainContext = context, segmentSearch?: (query: string) => Promise<SegmentSummaryResponseDto[]>) {
  return mount(defineComponent({
    ...
    template: '<AudienceRuleBuilder ref="builder" v-model="draft" :context="context" :segment-search="segmentSearch" />',
...
describe('AudienceRuleBuilder', () => {
  it('creates a locale condition without raw paths or JSON', async () => {
    ...
    expect(serializeAudienceDraft(draft, context)).toMatchObject({ ok: true, value: { version: 1, root: { kind: 'all', children: [{ kind: 'locale', operator: 'eq', value: 'ru-RU' }] } } })
  ...
  it('uses a multi-select value for the in operator', async () => {
    ...
    await wrapper.get('select[aria-label="Значения locale"]').setValue(['ru-RU', 'en-US'])
    await wrapper.get('button[aria-label="Применить условие аудитории"]').trigger('click')

    const draft = (wrapper.vm as unknown as { draft: AudienceDraft }).draft
    expect(serializeAudienceDraft(draft, context)).toMatchObject({ ok: true, value: { root: { children: [{ kind: 'locale', operator: 'in', value: ['ru-RU', 'en-US'] }] } } })
    wrapper.unmount()
  })
  ...
  it('uses a typed attribute control and warns that preview values are redacted', async () => {
    ...
    await wrapper.get('select[aria-label="Атрибут пользователя"]').setValue('attribute-1')
    await wrapper.get('select[aria-label="Оператор атрибута VIP-уровень"]').setValue('gte')
    expect(wrapper.text()).toContain('Чувствительное значение')
    expect(wrapper.text()).toContain('backend скроет фактическое значение')
    await wrapper.get('input[aria-label="Значение атрибута VIP-уровень"]').setValue('3')
    await wrapper.get('button[aria-label="Применить условие аудитории"]').trigger('click')

    const draft = (wrapper.vm as unknown as { draft: AudienceDraft }).draft
    expect(serializeAudienceDraft(draft, context)).toMatchObject({ ok: true, value: { root: { children: [{ kind: 'userAttribute', definitionId: 'attribute-1', operator: 'gte', value: 3 }] } } })
    wrapper.unmount()
  })
  ...
  it('pins the selected Segment revision and explains why the version is fixed', async () => {
    ...
    const draft = (wrapper.vm as unknown as { draft: AudienceDraft }).draft
    expect(serializeAudienceDraft(draft, context)).toMatchObject({ ok: true, value: { root: { children: [{ kind: 'segmentMembership', segmentId: 'segment-1', segmentRevisionId: 'segment-revision-2', operator: 'is_member' }] } } })
  ...
  it('searches the backend catalog before selecting a Segment beyond the preload', async () => {
    const searchedSegment: SegmentSummaryResponseDto = {
      segmentId: 'segment-101', key: 'whales', name: 'Whales', status: 'ACTIVE',
      currentRevision: { segmentRevisionId: 'segment-revision-9', revision: 9, catalogRevision: 'audience-catalog-3', contentHash: 'hash-9', publishedAt: '2026-07-18T00:00:00.000Z' },
    }
    ...
    const draft = (wrapper.vm as unknown as { draft: AudienceDraft }).draft
    expect(serializeAudienceDraft(draft, { ...context, segments: [...segments, searchedSegment] })).toMatchObject({ ok: true, value: { root: { children: [{ segmentId: 'segment-101', segmentRevisionId: 'segment-revision-9' }] } } })
    wrapper.unmount()
  })
  ...
  it('supports nested groups, keyboard buttons and explicit NOT independently from delete', async () => {
    ...
    await wrapper.get('button[aria-label="Применить условие аудитории"]').trigger('click')
    await wrapper.get('button[aria-label^="Инвертировать условие аудитории:"]').trigger('click')
  ...
  it('can invert a nested group, not only a leaf', async () => {
    ...
    await nestedGroup.get('button[aria-label^="Инвертировать группу аудитории:"]').trigger('click')

    expect(wrapper.get('.audience-node .audience-node .not-card').text()).toContain('Результат меняется на противоположный')
    wrapper.unmount()
  })
  ...
  it('focuses the exact editor control requested by a backend issue', async () => {
    ...
    ;(wrapper.getComponent(AudienceRuleBuilder).vm as unknown as { focusIssue: (issue: object) => void }).focusIssue({ nodeId, fieldPath: 'value', message: 'Страна недоступна' })
    await flushPromises()
