...
const groupLabel = computed(
  () => groupOptions.find(({ value }) => value === props.node.kind)?.label ?? '',
);
const leafPresentation = computed(() => {
  ...
  if (props.node.kind === 'opaque')
    return { icon: 'pi pi-exclamation-triangle', label: 'Недоступное условие' };
...
<template>
  <li
    class="audience-node"
    :class="[`kind-${node.kind}`, { root }]"
    :data-audience-node="node.nodeId"
  >
    <section v-if="node.kind === 'all' || node.kind === 'any'" tabindex="-1" class="group-card">
      <header>
        <div class="logic-switch" role="group" :aria-label="`Логика группы ${groupLabel}`">
          <button
            ...
            @click="
              emit('command', { type: 'changeGroup', nodeId: node.nodeId, kind: option.value })
            "
          >
            {{ option.label }}
          </button>
      ...
      <footer>
        <button
          type="button"
          :aria-label="`Добавить условие аудитории в ${groupLabel}`"
          @click="
            emit('addCondition', node.nodeId, groupLabel, $event.currentTarget as HTMLElement)
          "
        >
        ...
        <button
          type="button"
          :aria-label="`Добавить группу аудитории в ${groupLabel}`"
          :disabled="(depth ?? 0) >= 3"
          @click="emit('command', { type: 'addGroup', parentNodeId: node.nodeId })"
        >
    ...
    <article v-else-if="node.kind === 'not'" tabindex="-1" class="not-card">
      <header>
        ...
        <div>
          <strong>Подходящие пользователи будут исключены</strong><small>{{ summary }}</small>
        </div>
        <button
          type="button"
          :aria-label="`Отменить исключение: ${summary}`"
          title="Отменить исключение"
          @click="emit('command', { type: 'unwrapNot', nodeId: node.nodeId })"
        >
          <i class="pi pi-undo" />
        </button>
        <button
          v-if="!root"
          type="button"
          :aria-label="`Удалить условие аудитории: ${summary}`"
    ...
    <article v-else tabindex="-1" class="leaf-card">
      ...
      <div>
        <span>{{ leafPresentation.label }}</span
        ><strong>{{ summary }}</strong>
      </div>
      <div v-if="!lockedByNot" class="leaf-actions">
        <button
          v-if="node.kind !== 'opaque'"
          type="button"
          :aria-label="`Изменить условие аудитории: ${summary}`"
          title="Изменить условие"
          @click="emit('edit', node.nodeId, $event.currentTarget as HTMLElement)"
        >
