...
export interface RunExplainView {
  ...
  audience: {
    ...
    lastRecheck: null | {
      ...
      >['freshness'];
      source?: NonNullable<ScenarioRunExplainResponseDto['audience']['lastRecheck']>['source'];
      truth?: NonNullable<ScenarioRunExplainResponseDto['audience']['lastRecheck']>['truth'];
      rows: ExplainRuleRow[];
    };
  };
  goals: Array<
...
function explainValue(value: unknown): string | undefined {
  ...
  if (
    ...
    const items = source.value
      .slice(0, 20)
      .map((item) =>
        item === true ? 'Да' : item === false ? 'Нет' : item === null ? 'null' : String(item),
      );
...
function flattenRule(node: unknown, depth = 0, id = 'root'): ExplainRuleRow[] {
  ...
  const row: ExplainRuleRow = {
    ...
    ...(typeof source.unknownReason === 'string' ? { unknownReason: source.unknownReason } : {}),
  ...
  if (Array.isArray(source.children))
    return [
      row,
      ...source.children.flatMap((child, index) => flattenRule(child, depth + 1, `${id}.${index}`)),
    ];
