...
<template>
  <section class="run-explain" aria-labelledby="run-explain-title">
    ...
    <div v-else-if="error" role="alert" class="explain-error">
      <span>{{ error }}</span
    ...
    <template v-else-if="explain">
      ...
      <section class="explain-section">
        <div class="section-title">
          <h4>Eligibility</h4>
          <span
            >{{ runtimeExplainLabel(explain.eligibility.decision) }} ·
            {{ runtimeExplainLabel(explain.eligibility.fidelity) }}</span
          >
        </div>
        <ol class="eligibility-tree">
          <li
            v-for="row in explain.eligibility.rows"
            :key="row.id"
        ...
        <div v-if="explain.eligibility.lastRecheck" class="recheck-card">
          <div class="section-title">
          ...
          <p>
            Независимая повторная проверка на
            {{ date(explain.eligibility.lastRecheck.evaluatedAt) }}.
          </p>
        </div>
      </section>
      ...
      <section class="explain-section audience-explain">
        <div class="section-title">
          <h4>Audience при запуске</h4>
          <span
            >{{ runtimeExplainLabel(explain.audience.decision) }} ·
            {{ runtimeExplainLabel(explain.audience.fidelity) }}</span
          >
        </div>
        ...
        <ol class="eligibility-tree">
          <li
            v-for="row in explain.audience.rows"
            :key="`audience-${row.id}`"
            :style="{ '--depth': row.depth }"
            :class="row.state"
          >
            ...
            <dl>
              <template v-if="row.definitionId"
                ><dt>Attribute definition</dt>
                <dd>{{ row.definitionId }}</dd></template
              ><template v-if="row.segmentRevisionId"
                ><dt>Segment revision</dt>
                <dd>{{ row.segmentRevisionId }}</dd></template
              ><template v-if="row.actual"
                ><dt>Фактически</dt>
                <dd>{{ row.actual }}</dd></template
              ><template v-if="row.expected"
        ...
        <div v-if="explain.audience.lastRecheck" class="recheck-card">
          <div class="section-title">
            <h4>Повторная проверка перед доставкой · Audience recheck</h4>
            <span
          ...
          <ol class="eligibility-tree">
            <li
              v-for="row in explain.audience.lastRecheck.rows"
              :key="`recheck-${row.id}`"
              :style="{ '--depth': row.depth }"
              :class="row.state"
            >
      ...
      <section class="explain-section">
        ...
        <div v-else class="goal-list">
          <article v-for="goal in explain.goals" :key="goal.waitId">
            <div>
              <strong>{{ goal.winnerLabel }}</strong
              ><span>{{ runtimeExplainLabel(goal.outcome) }}</span>
            </div>
            <dl>
              <dt>Deadline</dt>
              <dd>{{ date(goal.deadlineAt) }}</dd>
              <dt>Результат</dt>
              <dd>{{ date(goal.resolvedAt) }}</dd>
              <dt>Ветка</dt>
              <dd>
      ...
      <section class="explain-section">
        <div class="section-title">
          <h4>Доставка</h4>
          <span>{{ runtimeExplainLabel(explain.delivery.policy.kind) }}</span>
        </div>
        <p>{{ explain.delivery.summary }}</p>
        <div v-if="explain.delivery.waits.length" class="wait-list">
          <span v-for="wait in explain.delivery.waits" :key="wait.waitId"
            ><strong>{{ runtimeExplainLabel(wait.outcome) }}</strong> · до
            {{ date(wait.deadlineAt) }}</span
          >
        </div>
      </section>
      ...
      <section class="explain-section">
        <div class="section-title">
          <h4>Actions и continuations</h4>
          <span
            >{{ explain.actions.length }} actions ·
            {{ explain.continuations.length }} continuations</span
          >
        </div>
      ...
      <section class="explain-section">
        <h4>Timeline</h4>
        <ol class="timeline">
          <li v-for="entry in explain.timeline" :key="entry.id">
            <i class="pi pi-circle-fill" />
            <div>
              <strong>{{ runtimeExplainLabel(entry.type) }}</strong
...
.eligibility-tree li.matched {
  border-color: var(--status-success);
}
.eligibility-tree li.unavailable {
  border-color: var(--status-warning);
  background: var(--status-warning-soft);
}
.eligibility-tree li > div {
  display: flex;
  align-items: center;
  gap: 7px;
}
.eligibility-tree li > div > span {
  color: var(--text-small-muted);
  font-size: 0.61rem;
}
.eligibility-tree strong {
  font-size: 0.7rem;
}
.eligibility-tree dl,
.goal-list dl {
