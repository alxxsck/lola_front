...
describe('support Case SLA and routing presentation', () => {
  ...
  it('does not disguise degraded or stale routing as an assignment', () => {
    expect(routingReasonLabel('DEGRADED')).toBe('Маршрутизация временно работает с ограничениями');
    expect(routingReasonLabel('STALE_INPUT')).toBe('Данные маршрутизации устарели');
  });
});
