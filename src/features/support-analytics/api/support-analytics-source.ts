...
export interface SupportAnalyticsSource {
  catalog(
    projectId: string,
    signal?: AbortSignal,
  ): Promise<ReportingCatalogResponseDto>;
  validate(
    projectId: string,
...
const apiSource: SupportAnalyticsSource = {
  async catalog(projectId, signal) {
    try {
      return await reportingCatalogRead(
        projectId,
  ...
  async validate(projectId, query, signal) {
    try {
      return await reportingQueryValidate(
        projectId,
        query,
        signal ? { signal } : undefined,
      );
    ...
    try {
      ...
      const run = await reportingQueryRunCreate(
        ...
        },
        {
          ...
          ...(signal ? { signal } : {}),
          headers: {
            "Idempotency-Key": `support-query-${estimate.canonicalQueryHash}`,
          },
        },
      );
...
const metrics: Record<string, Array<[string, string, string]>> = {
  ...
  SUPPORT_ASSIGNMENT: [
    ["assignment_event_count", "События назначения", "COUNT"],
  ],
  SUPPORT_SLA: [
  ...
  SUPPORT_EXTERNAL_WORK: [
    ["external_work_event_count", "Внешние задачи", "COUNT"],
  ],
  SUPPORT_CONTENT_USAGE: [
    ["content_usage_count", "Использование контента", "COUNT"],
  ],
  SUPPORT_AI_USAGE: [
...
const dimensions: Record<string, string[]> = {
  SUPPORT_QUALITY: ["OCCURRED_DAY", "TEAM", "OPERATOR", "QUALITY_ITEM"],
  SUPPORT_CASE: [
    "OCCURRED_DAY",
    "QUEUE",
    "TEAM",
...
const mockCatalog: ReportingCatalogResponseDto = {
  datasets: families.map((family, index): ReportingCatalogDatasetDto => ({
    ...
    readiness: {
      status: family === "SUPPORT_QUALITY" ? "READY" : "UNAVAILABLE",
      coverageFrom:
        family === "SUPPORT_QUALITY"
          ? new Date(Date.now() - 90 * 86_400_000).toISOString()
          : null,
      coverageUntil:
        family === "SUPPORT_QUALITY" ? new Date().toISOString() : null,
      dataAsOf:
        family === "SUPPORT_QUALITY"
          ? new Date(Date.now() - 18_000).toISOString()
          : null,
      projectionLagMs: family === "SUPPORT_QUALITY" ? 18000 : null,
      missingSourceFamilies: family === "SUPPORT_QUALITY" ? [] : [family],
    },
...
function resultRows(
  ...
  return base.map((value, index) => ({
    day: new Date(Date.now() - (base.length - index) * 86_400_000)
      .toISOString()
      .slice(0, 10),
    ...
    metrics: metricCodes.map((code, metricIndex) =>
      metricCell(
        code,
        code.includes("count") ? 34 + index * 3 : value - metricIndex * 4,
      ),
    ),
  }));
}
        ...
        comparison: query.comparison
          ? {
              ...
              range: {
                ...query.range,
                from: new Date(
                  new Date(query.range.from).getTime() - 7 * 86_400_000,
                ).toISOString(),
...
export const supportAnalyticsSource: SupportAnalyticsSource =
  isMockMode || import.meta.env.MODE === "test" ? mockSource : apiSource;
