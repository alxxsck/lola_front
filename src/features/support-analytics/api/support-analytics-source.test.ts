...
import { metricLabel, supportAnalyticsSource } from './support-analytics-source';
...
describe('supportAnalyticsSource', () => {
  it('exposes every production owner family after the B1 publishers are ready', async () => {
    ...
    expect(
      catalog.datasets.every(({ readiness }) => readiness.missingSourceFamilies.length === 0),
    ).toBe(true);
  });

  it('returns bounded Quality rows, comparison and a privacy receipt', async () => {
    const dataset = (await supportAnalyticsSource.catalog('project-1')).datasets.find(
      ({ datasetCode }) => datasetCode === 'SUPPORT_QUALITY',
    )!;
    const metric = dataset.metrics.find(({ code }) => code === 'quality_score_average')!;
