...
} from "./support-analytics-curation";
...
describe("Support analytics curation", () => {
  it("keeps every route focused and below the visible-widget ceiling", () => {
    for (const view of Object.values(CURATED_SUPPORT_VIEWS)) {
      ...
      expect(new Set(view.widgets.map(({ id }) => id))).toHaveLength(
        view.widgets.length,
      );
    }
  });
  ...
  it("resolves only ready, permitted metrics and keeps unavailable cards honest", () => {
    ...
    catalog[1]!.metrics[0]!.requiredPermissionCodes = [
    ...
    expect(widgets.find(({ id }) => id === "verified")?.state).toBe(
      "FORBIDDEN",
    );
    expect(widgets.find(({ id }) => id === "assigned")?.state).toBe(
      "UNAVAILABLE",
    );
  });
});
