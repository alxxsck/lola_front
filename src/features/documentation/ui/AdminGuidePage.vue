...
const sections = computed(() =>
  parseMarkdownDocument(props.source)
...
<template>
  <div class="admin-guide-page">
    <RouterLink :to="{ name: 'documentation' }" class="guide-back"
      ><i class="pi pi-arrow-left" /> Вся документация</RouterLink
    >
    <header class="guide-hero">
      <div class="hero-copy">
        <span class="guide-kicker"
          ><i class="pi pi-sparkles" />
          {{ audienceLabel ?? 'Руководство для администратора' }}</span
        >
        <h1>{{ title }}</h1>
        <p>{{ description }}</p>
    ...
    <div class="guide-layout">
      <aside class="guide-nav card" :aria-label="`Содержание: ${title}`">
        <div>
          <span>На этой странице</span><strong>{{ sections.length }} разделов</strong>
        </div>
        <a class="skip-guide" href="#guide-content"
          >Перейти к руководству <i class="pi pi-arrow-down"
        /></a>
        <nav>
      ...
      <section id="guide-content" class="guide-content card" :aria-label="contentLabel">
        ...
        <footer>
          ...
          <RouterLink :to="{ name: footerRouteName }"
            >{{ footerAction }} <i class="pi pi-arrow-right"
          /></RouterLink>
        </footer>
      </section>
    </div>
...
.visual-line {
  width: 42px;
  height: 2px;
  background: linear-gradient(90deg, var(--text-secondary), var(--brand));
}
.hero-visual small {
  position: absolute;
  top: 82px;
...
.screen-frame small {
  ...
  padding: 6px 8px;
  border-radius: 8px;
  background: color-mix(in srgb, var(--surface-emphasis) 88%, transparent);
  color: var(--text-on-emphasis);
  font-size: 0.62rem;
}
.screen-marker {
  ...
  box-shadow: 0 2px 8px color-mix(in srgb, var(--surface-emphasis) 28%, transparent);
}
.guide-layout {
  display: grid;
...
@media (max-width: 700px) {
  ...
  .skip-guide {
    ...
    background: var(--status-accent-soft);
    color: var(--status-accent-text) !important;
    font-weight: 750;
  }
  .guide-nav nav {
    grid-template-columns: 1fr;
