...
function headingIcon(text: string): string {
  ...
  if (value.includes('сегмент') || value.includes('audience') || value.includes('пользовател'))
    return 'pi pi-users';
  ...
  if (value.includes('goal') || value.includes('deadline') || value.includes('ожидан'))
    return 'pi pi-clock';
  if (value.includes('delivery') || value.includes('online')) return 'pi pi-send';
  if (value.includes('чернов') || value.includes('провер') || value.includes('preview'))
    return 'pi pi-pencil';
  if (value.includes('публик') || value.includes('верс') || value.includes('откат'))
    return 'pi pi-lock';
  ...
  <article class="markdown-article">
    ...
    >
      <component
        ...
        :id="block.id"
        :class="{ 'section-heading': block.level === 2 }"
      >
        <span v-if="block.level === 2" class="heading-icon" aria-hidden="true"
          ><i :class="headingIcon(block.text)"
        /></span>
        <template v-for="(token, tokenIndex) in block.inline" :key="tokenIndex"
      ...
      <aside v-else-if="block.type === 'quote'" class="guide-note">
      ...
      <div
        ...
        tabindex="0"
        role="region"
        aria-label="Таблица из руководства"
      >
        <table>
          <thead>
            <tr>
      ...
      <figure v-else class="code-example">
        <figcaption><i class="pi pi-code" /> Пример</figcaption>
        <pre tabindex="0" aria-label="Пример из руководства"><code>{{ block.value }}</code></pre>
      </figure>
    </template>
  </article>
</template>
...
.markdown-article :deep(h2:first-of-type) {
  margin-top: 8px;
  padding-top: 0;
  border-top: 0;
}
...
.guide-note {
  ...
  margin: 18px 0 24px;
  padding: 15px 16px;
  border: 1px solid color-mix(in srgb, var(--status-accent) 28%, var(--border-default));
  border-radius: 14px;
  background: var(--status-accent-soft);
  color: var(--status-accent-text);
}
