...
const props = withDefaults(defineProps<{ source: string; fallbackHeadingIcon?: string }>(), {
  fallbackHeadingIcon: 'pi pi-bookmark',
})
...
function headingIcon(text: string): string {
  ...
  if (value.includes('сегмент') || value.includes('audience') || value.includes('пользовател')) return 'pi pi-users'
  if (value.includes('профил')) return 'pi pi-id-card'
  ...
  return props.fallbackHeadingIcon
}
</script>
...
<template>
  <article class="markdown-article">
    <template v-for="(block, index) in blocks" :key="block.type === 'heading' ? block.id : `${block.type}-${index}`">
      ...
      <aside v-else-if="block.type === 'quote'" class="guide-note"><i class="pi pi-info-circle" /><p><template v-for="(token, tokenIndex) in block.inline" :key="tokenIndex"><code v-if="token.type === 'code'">{{ token.value }}</code><template v-else>{{ token.value }}</template></template></p></aside>
      <div v-else-if="block.type === 'table'" class="table-frame" tabindex="0" role="region" aria-label="Таблица из руководства">
        <table>
          <thead><tr><th v-for="(cell, cellIndex) in block.headers" :key="cellIndex"><template v-for="(token, tokenIndex) in cell" :key="tokenIndex"><code v-if="token.type === 'code'">{{ token.value }}</code><template v-else>{{ token.value }}</template></template></th></tr></thead>
          <tbody><tr v-for="(row, rowIndex) in block.rows" :key="rowIndex"><td v-for="(cell, cellIndex) in row" :key="cellIndex"><template v-for="(token, tokenIndex) in cell" :key="tokenIndex"><code v-if="token.type === 'code'">{{ token.value }}</code><template v-else>{{ token.value }}</template></template></td></tr></tbody>
        </table>
      </div>
      <figure v-else class="code-example"><figcaption><i class="pi pi-code" /> Пример</figcaption><pre tabindex="0" aria-label="Пример из руководства"><code>{{ block.value }}</code></pre></figure>
    </template>
  </article>
</template>

<style scoped>
.markdown-article{min-width:0;color:var(--surface-emphasis-hover)}.markdown-article :deep(h1){display:none}.markdown-article :deep(h2),.markdown-article :deep(h3){scroll-margin-top:26px}.markdown-article :deep(h2){display:flex;align-items:center;gap:12px;margin:48px 0 16px;padding-top:30px;border-top:1px solid var(--line);font-size:1.48rem}.markdown-article :deep(h2:first-of-type){margin-top:8px;padding-top:0;border-top:0}.markdown-article :deep(h3){margin:28px 0 10px;font-size:1rem;color:var(--text-secondary)}.heading-icon{display:grid;place-items:center;flex:0 0 38px;height:38px;border-radius:12px;background:var(--status-accent-soft);color:var(--status-accent-text);font-size:.88rem}.markdown-article p{max-width:78ch;margin:0 0 14px;color:var(--text-secondary);font-size:.94rem;line-height:1.72}.markdown-article ul,.markdown-article ol{display:grid;gap:8px;max-width:78ch;margin:0 0 20px;padding-left:25px;color:var(--text-secondary)}.markdown-article li{padding-left:4px;line-height:1.58}.markdown-article li::marker{color:var(--status-accent-text);font-weight:800}.markdown-article code{border-radius:6px;background:var(--surface-subtle);padding:2px 5px;color:var(--status-accent-text);font:600 .84em ui-monospace,SFMono-Regular,Menlo,monospace}.code-example{max-width:78ch;margin:18px 0 24px;overflow:hidden;border:1px solid var(--border-default);border-radius:16px;background:var(--surface-emphasis);box-shadow:var(--shadow-raised)}.code-example figcaption{display:flex;align-items:center;gap:7px;padding:10px 14px;border-bottom:1px solid var(--border-on-emphasis);color:var(--text-on-emphasis-muted);font-size:.68rem;text-transform:uppercase;letter-spacing:.1em}.code-example figcaption i{color:var(--brand)}.code-example pre{margin:0;padding:17px 18px;overflow:auto;color:var(--text-on-emphasis);font:500 .78rem/1.65 ui-monospace,SFMono-Regular,Menlo,monospace}.code-example pre code{padding:0;background:transparent;color:inherit;font:inherit}@media(max-width:700px){.markdown-article :deep(h2){align-items:flex-start;font-size:1.25rem}.markdown-article p{font-size:.9rem}.code-example{border-radius:12px}}
.table-frame{max-width:78ch;margin:18px 0 24px;overflow:auto;border:1px solid var(--border-default);border-radius:16px;background:var(--surface-card);box-shadow:var(--shadow-raised)}.table-frame table{width:100%;border-collapse:collapse;font-size:.8rem;line-height:1.5}.table-frame th,.table-frame td{min-width:150px;padding:12px 14px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}.table-frame th{background:var(--surface-subtle);color:var(--text-primary);font-weight:750}.table-frame td{color:var(--text-secondary)}.table-frame tbody tr:last-child td{border-bottom:0}
.guide-note{display:flex;align-items:flex-start;gap:11px;max-width:78ch;margin:18px 0 24px;padding:15px 16px;border:1px solid color-mix(in srgb,var(--status-accent) 28%,var(--border-default));border-radius:14px;background:var(--status-accent-soft);color:var(--status-accent-text)}.guide-note>i{margin-top:3px}.guide-note p{margin:0;color:inherit;font-weight:700}
