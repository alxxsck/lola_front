...
describe('support inbox controller', () => {
  ...
  it('does not commit a page that resolves after the workspace was reset', async () => {
    ...
    const source = {
      readCases: vi.fn(
        () =>
          new Promise<{ items: SupportWorkspaceCaseRow[]; nextCursor: null }>((resolve) => {
            resolvePage = resolve;
          }),
      ),
  ...
  it('appends the next cursor page without duplicate conversations', async () => {
    ...
    expect(controller.items.value.map((item) => item.id)).toEqual(['one', 'two', 'three']);
    expect(controller.nextCursor.value).toBeNull();
  });
  ...
  it('keeps Case and Conversation cursor pages scoped to the active mode', async () => {
    ...
    await controller.load();
    expect(controller.items.value).toEqual([expect.objectContaining({ kind: 'CASE', id: '42' })]);
  ...
  it('ignores a Case page that resolves after mode changed to Conversations', async () => {
    let mode: SupportInboxMode = 'CASES';
    let resolveCases!: (value: { items: SupportWorkspaceCaseRow[]; nextCursor: null }) => void;
    const source = {
      readCases: vi.fn(
        () =>
          new Promise<{ items: SupportWorkspaceCaseRow[]; nextCursor: null }>((resolve) => {
  ...
  it('keeps loaded rows when the next cursor page fails', async () => {
    ...
    expect(controller.items.value).toEqual([expect.objectContaining({ id: 'conversation-1' })]);
    expect(controller.error.value).toBe('Не удалось загрузить следующую страницу диалогов');
