...
describe('SupportInboxPane', () => {
  it('uses the unified view picker as the only inbox source selector', () => {
    ...
    expect(wrapper.find('[role="group"][aria-label="Режим входящих"]').exists()).toBe(false);
  ...
  it('shows the active view in the compact summary and collapses after selection', async () => {
    ...
    expect(wrapper.emitted('selectView')?.[0]).toEqual([{ kind: 'SYSTEM', code: 'MY_ACTIVE' }]);
    expect(trigger.attributes('aria-expanded')).toBe('false');
    expect(wrapper.find('.inbox-tools__panel').exists()).toBe(false);
  });
  ...
  it('renders a compact Case row without duplicating its attention state', () => {
    ...
    expect(row.get('.case-row__metadata').text()).toContain('Нужен оператор');
    expect(row.get('.case-row__priority').classes()).toContain('case-row__priority--emphasis');
  ...
  it('renders mode-shaped skeleton rows without PrimeVue stretch markers', () => {
    ...
    expect(conversations.get('.inbox-skeleton-row').classes()).toContain('is-conversation');
  });
  ...
  it('renders one compact SLA forecast with the full explanation available', () => {
    ...
    expect(signal.attributes('title')).toContain('Прогноз не является договорным сроком');
  ...
  it('keeps pane geometry and exposes retry for initial and pagination errors', async () => {
    ...
    expect(emptyError.get('[role=alert]').text()).toContain('Не удалось загрузить входящие');
  ...
  it('renders a mode-specific empty state', () => {
    ...
    expect(render({ items: [], mode: 'ALL_CONVERSATIONS' }).text()).toContain('Чатов пока нет');
  });
  ...
  it('marks the search result for the open chat as selected', () => {
    ...
    expect(row.attributes('data-selection-key')).toBe('CONVERSATION:conversation-1');
  });
