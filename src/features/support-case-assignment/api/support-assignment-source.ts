...
export type SupportAssignmentIntent =
  | SupportAssignmentClaimIntent
  | SupportAssignmentReleaseIntent
  | SupportAssignmentTransferIntent;
...
export interface SupportAssignmentSource {
  ...
  listOffers(
    projectId: string,
    signal?: AbortSignal,
  ): Promise<SupportAssignmentOffer[]>;
  actOnOffer(
    projectId: string,
...
function mapSnapshot(
  ...
  if (
    current &&
    (!current.id || !current.version || !current.actionEtag)
  )
    throw new SupportAssignmentIntegrityError();
  ...
  return {
    ...
    currentAssignment: current
      ...
      : null,
    workforceRevision: workforce
      ? { id: workforce.id!, number: workforce.number! }
      : null,
  ...
  async execute(projectId, intent, idempotencyKey, signal) {
    try {
          ...
          : intent.kind === "RELEASE"
            ...
            : await (() => {
              ...
              const team = intent.snapshot.teams.find(
                (candidate) =>
                  candidate.id === intent.teamId && candidate.actions.transfer,
              );
              const operator = team?.operators.find(
                (candidate) =>
                  candidate.id === intent.operatorId &&
                  candidate.actions.transfer,
              );
              if (!team || !operator)
                throw new SupportAssignmentIntegrityError();
      ...
      if (
        ...
        value.assignment.caseId !== intent.snapshot.caseId
      )
        throw new SupportAssignmentIntegrityError();
      return {
        intent: value.intent,
  ...
  async listOffers(projectId, signal) {
    try {
      ...
      return value.offers.map((offer) => {
        if (!offer.acceptToken || !offer.actionEtag)
          throw new SupportAssignmentIntegrityError();
        return {
  ...
  async actOnOffer(projectId, intent, idempotencyKey, signal) {
    try {
      ...
      const expectedOutcome =
        intent.kind === "ACCEPT" ? "ACCEPTED" : "DECLINED";
      if (
        value.assignmentId !== intent.offer.assignmentId ||
...
const mockSource: SupportAssignmentSource = {
  ...
  async execute(_projectId, intent) {
    ...
    if (!assignment)
      throw new ApiError(409, "mock stale assignment", undefined, undefined, "ASSIGNMENT_VERSION_CONFLICT");
...
export const supportAssignmentSource: SupportAssignmentSource = isMockMode
  ? mockSource
  : apiSource;
