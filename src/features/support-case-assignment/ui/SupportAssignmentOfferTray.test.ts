...
describe('SupportAssignmentOfferTray', () => {
  it('keeps private offers globally actionable without rendering capabilities', async () => {
    ...
    await wrapper.get("button[aria-label='Принять предложение назначения']").trigger('click');

    expect(assignment.actOnOffer).toHaveBeenCalledWith('offer-assignment-1', 'ACCEPT');
