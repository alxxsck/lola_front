  ...
  <section class="views-rail" aria-label="Представления поддержки">
    ...
    <details v-if="saved.length" class="saved-list" open>
      ...
      <button
        ...
        type="button"
        :class="{
          active: selection?.kind === 'SAVED' && selection.id === item.id,
        }"
    ...
    <form v-if="creating" class="view-form" @submit.prevent="submitCreate">
      ...
      <label v-if="scope === 'TEAM'"
        >Идентификатор команды<input v-model="teamId" required
      /></label>
      <div>
        <button type="button" @click="creating = false">Отмена</button
        ><button type="submit" :disabled="mutating">Создать</button>
      </div>
    </form>
    ...
    <div v-if="activeSaved" class="view-actions">
      ...
      <button
        v-if="activeSaved.permissions.replaceDraft"
        type="button"
        :disabled="mutating"
        @click="
          renaming = !renaming;
          renamed = activeSaved.draft.displayName;
        "
      >
        <i class="pi pi-pencil" /> Переименовать
      </button>
      <button
        v-if="activeSaved.permissions.publish"
        type="button"
        :disabled="mutating"
        @click="emit('publish', activeSaved)"
      >
        <i class="pi pi-send" /> Опубликовать
      </button>
      <button
        v-if="activeSaved.permissions.archive"
        type="button"
        class="danger"
        :disabled="mutating"
        @click="emit('archive', activeSaved)"
      >
        <i class="pi pi-trash" /> В архив
      </button>
    </div>
    <form
    ...
    <button
      v-else-if="selection && !activeSaved"
      type="button"
      class="default-action"
      :disabled="mutating"
      @click="emit('setDefault', selection)"
    >
      <i class="pi pi-home" /> Сделать основным
    </button>
...
.views-rail {
  ...
  border-radius: 10px;
  background: color-mix(in srgb, var(--surface-card) 88%, var(--brand-soft));
}
.views-heading {
  display: flex;
  align-items: center;
...
.views-heading-actions {
  display: flex;
  align-items: center;
  gap: 4px;
}
.views-heading button,
.default-action {
  min-height: 32px;
  padding: 0 4px;
  display: inline-flex;
...
.views-heading button > i,
.default-action > i,
.view-actions button > i,
.view-list button > span > i,
.saved-list button > span > i {
  width: 16px;
  height: 16px;
  flex: 0 0 16px;
  display: inline-grid;
