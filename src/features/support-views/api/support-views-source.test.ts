...
vi.mock('@/shared/api/generated/retenive-backend', () => ({
  savedSupportViewCatalog: vi.fn(),
  savedSupportViewCreate: vi.fn(),
  savedSupportViewDefaultView: vi.fn(),
  savedSupportViewPublish: vi.fn(),
  savedSupportViewArchive: vi.fn(),
  savedSupportViewReplace: vi.fn(),
  savedSupportViewQuery: vi.fn(),
  savedSupportViewReplaceDefaultView: vi.fn(),
  supportViewPresetCatalog: vi.fn(),
  supportViewPresetQuery: vi.fn(),
  supportViewPresetQueryAllCases: vi.fn(),
  supportViewPresetQueryAllConversations: vi.fn(),
  supportViewPresetQueryMyTeamUnassigned: vi.fn(),
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(supportViewPresetCatalog).mockResolvedValue({ freshness, items: [] });
  vi.mocked(savedSupportViewDefaultView).mockResolvedValue(defaultView);
});
...
describe('support views source', () => {
  ...
  it('sends opaque ETag and stable idempotency headers', async () => {
    ...
    expect(savedSupportViewPublish).toHaveBeenCalledWith('project-1', 'view-1', {
      headers: { 'If-Match': '"sv1"', 'Idempotency-Key': 'attempt-1' },
    });
  });

  it('verifies the authoritative preset receipt', async () => {
    vi.mocked(supportViewPresetQuery).mockResolvedValue({
      freshness,
      items: [],
      nextCursor: null,
      preset: { code: 'MY_ACTIVE' },
    });
    await expect(
      supportViewsSource.query('project-1', { kind: 'SYSTEM', code: 'MY_ACTIVE' }, '', undefined),
    ).resolves.toMatchObject({ authorityKey: 'system:MY_ACTIVE' });
  });
  ...
  it('creates with an idempotency key', async () => {
    ...
    const command = {
      ...
      draft: { surface: 'CASES' as const } as never,
    };
    await supportViewsSource.create('project-1', command, 'create-1');
    expect(savedSupportViewCreate).toHaveBeenCalledWith('project-1', command, {
  ...
  it('keeps mock saved views isolated by project', async () => {
    const command = reactive({
      code: 'project-a-only',
      scope: 'PERSONAL' as const,
      draft: { surface: 'CASES' as const, filters: { priorities: ['HIGH'] } } as never,
    });
