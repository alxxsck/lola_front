...
import type { SupportSearchFreshness, SupportSearchResult } from "@/features/support-search/api/support-search-source";
import type { SupportViewSelection, SupportViewsSource } from "@/features/support-views/api/support-views-source";
...
export function defaultSelection(value: SupportDefaultViewResponseDto): SupportViewSelection | null {
...
export function createSupportViewsController(context: SupportViewsContext, source: SupportViewsSource) {
  ...
  const valid = (candidate: SupportViewSelection | null): candidate is SupportViewSelection =>
    candidate?.kind === "SYSTEM"
      ...
      : candidate?.kind === "SAVED"
        ? saved.value.some((item) => item.id === candidate.id && item.permissions.read && item.lifecycle === "ACTIVE")
        : false;

  const sameSelection = (left: SupportViewSelection | null, right: SupportViewSelection | null): boolean =>
    left?.kind === right?.kind && (left?.kind === "SYSTEM"
  ...
  function scrub(candidate: SupportViewSelection): void {
    if (candidate.kind === "SYSTEM") system.value = system.value.filter((item) => item.code !== candidate.code);
  ...
  function current(projectId: string, requestGeneration: number): boolean {
    return generation === requestGeneration && context.projectId() === projectId && context.canSearch();
  }
  ...
  async function query(cursor?: string, recoveryAttempted = false): Promise<void> {
    ...
    try {
      ...
      if (cursor) {
        ...
        items.value = [...items.value, ...page.items.filter((item) => !known.has(`${item.kind}:${item.id}`))];
  ...
  async function load(requested: SupportViewSelection | null, excluded: SupportViewSelection | null = null, custom = false): Promise<void> {
    const projectId = context.projectId();
    if (!projectId || !context.canSearch()) { reset(); return; }
    const requestGeneration = ++generation;
    ...
    try {
      ...
      system.value = catalog.system.filter((item) => !sameSelection({ kind: "SYSTEM", code: item.code }, excluded));
      saved.value = catalog.saved.filter((item) => !sameSelection({ kind: "SAVED", id: item.id }, excluded));
      ...
      const permittedSystem = system.value.find((item) => !sameSelection({ kind: "SYSTEM", code: item.code }, excluded));
      const permittedSaved = saved.value.find((item) => !sameSelection({ kind: "SAVED", id: item.id }, excluded));
      selection.value = valid(requested) && !sameSelection(requested, excluded)
        ? requested
  ...
  async function mutate(action: (projectId: string) => Promise<unknown>, requiresManage = true): Promise<boolean> {
    const projectId = context.projectId();
    ...
    try {
      ...
      if (!current(projectId, requestGeneration) || (requiresManage && !context.canMutate())) return false;
  ...
  return {
    system, saved, defaultView, selection, items, nextCursor, freshness, authorityKey, loading, mutating, error, conflict,
    load, loadCustom, select, clearSelection, query,
    loadMore: () => nextCursor.value && !loading.value ? query(nextCursor.value) : Promise.resolve(),
    create: (draft: CreateSavedSupportViewDto, key: string) => mutate((projectId) => source.create(projectId, draft, key)),
    replace: (view: SavedSupportViewResponseDto, draft: ReplaceSavedSupportViewDraftDto, key: string) => mutate((projectId) => source.replace(projectId, view, draft, key)),
    publish: (view: SavedSupportViewResponseDto, key: string) => mutate((projectId) => source.publish(projectId, view, key)),
    archive: (view: SavedSupportViewResponseDto, key: string) => mutate((projectId) => source.archive(projectId, view, key)),
    setDefault: (choice: ReplaceSupportDefaultViewDtoSelection, key: string) => defaultView.value
      ? mutate((projectId) => source.setDefault(projectId, defaultView.value!, choice, key), false)
      : Promise.resolve(false),
    reset, resetResults, purgeSaved,
  };
}
