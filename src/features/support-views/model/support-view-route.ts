...
export function shouldLoadCustomSupportView(
  query: LocationQuery,
...
export function readSupportViewSelection(query: LocationQuery): SupportViewSelection | null {
  ...
  if (raw.startsWith('system:')) {
    ...
    return SYSTEM_CODES.has(code) ? { kind: 'SYSTEM', code: code as SupportSystemViewCode } : null;
  }
...
export function writeSupportViewSelection(
  selection: SupportViewSelection | null,
): LocationQueryRaw {
  if (!selection) return { view: 'custom' };
  return {
    view: selection.kind === 'SYSTEM' ? `system:${selection.code}` : `saved:${selection.id}`,
  };
}
