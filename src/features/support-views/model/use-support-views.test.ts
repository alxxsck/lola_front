...
const freshness = { state: "READY" as const, lagSeconds: 0, indexedThrough: "2026-08-08T00:00:00Z" };
const system = { code: "MY_ACTIVE" as const, permitted: true, surface: "CASES" as const, scope: "SYSTEM" as const, displayNameKey: "my", count: { state: "EXACT" as const, value: 7, cappedAt: 100 }, freshness };
...
function source(overrides: Partial<SupportViewsSource> = {}): SupportViewsSource {
  return { catalog: vi.fn().mockResolvedValue({ system: [system], saved: [], defaultView }), query: vi.fn().mockResolvedValue({ items: [], nextCursor: null, freshness, authorityKey: "system:MY_ACTIVE" }), create: vi.fn(), replace: vi.fn(), publish: vi.fn(), archive: vi.fn(), setDefault: vi.fn(), ...overrides };
}

describe("support views controller", () => {
  it("uses only an authoritative permitted fallback", async () => {
    const onSelection = vi.fn(); const api = source();
    const controller = createSupportViewsController({ projectId: () => "project-1", canSearch: () => true, canReadSaved: () => false, canMutate: () => false, phrase: () => "", onSelection }, api);
  ...
  it("drops a late query after project switch", async () => {
    let projectId = "project-1"; let resolve!: (value: never) => void;
    const query = vi.fn()
      .mockResolvedValueOnce({ items: [], nextCursor: null, freshness, authorityKey: "system:MY_ACTIVE" })
      .mockImplementationOnce(() => new Promise((done) => { resolve = done; }));
    const api = source({ query: query as never });
    const controller = createSupportViewsController({ projectId: () => projectId, canSearch: () => true, canReadSaved: () => false, canMutate: () => false, phrase: () => "", onSelection: vi.fn() }, api);
    await controller.load(null);
    const pending = controller.query(); projectId = "project-2"; resolve({ items: [{ id: "leak" }], nextCursor: null, freshness, authorityKey: "system:MY_ACTIVE" } as never); await pending;
  ...
  it("allows a search-only operator to replace the personal default", async () => {
    ...
    const controller = createSupportViewsController({ projectId: () => "project-1", canSearch: () => true, canReadSaved: () => false, canMutate: () => false, phrase: () => "", onSelection: vi.fn() }, api);
    await controller.load(null);
    await expect(controller.setDefault({ kind: "SYSTEM", presetCode: "ALL_CASES" }, "default-1")).resolves.toBe(true);
    expect(api.setDefault).toHaveBeenCalledOnce();
  });

  it("purges a concealed view and resolves a permitted fallback", async () => {
    const saved = { id: "11111111-1111-4111-8111-111111111111", lifecycle: "ACTIVE", permissions: { read: true }, draft: { displayName: "Private" } } as never;
    const api = source({
      catalog: vi.fn().mockResolvedValueOnce({ system: [system], saved: [saved], defaultView }).mockResolvedValueOnce({ system: [system], saved: [], defaultView }),
      query: vi.fn().mockRejectedValueOnce(new ApiError(404, "concealed")).mockResolvedValueOnce({ items: [], nextCursor: null, freshness, authorityKey: "system:MY_ACTIVE" }),
    });
    const controller = createSupportViewsController({ projectId: () => "project-1", canSearch: () => true, canReadSaved: () => true, canMutate: () => false, phrase: () => "", onSelection: vi.fn() }, api);
    await controller.load({ kind: "SAVED", id: "11111111-1111-4111-8111-111111111111" });
  ...
  it("bounds forbidden recovery when every refreshed fallback is denied", async () => {
    ...
    const conversations = { ...system, code: "ALL_CONVERSATIONS" as const, surface: "CONVERSATIONS" as const, displayNameKey: "conversations" };
    const privateSaved = { id: "11111111-1111-4111-8111-111111111111", lifecycle: "ACTIVE", permissions: { read: true }, draft: { displayName: "Private" } } as never;
    const api = source({
      catalog: vi.fn().mockResolvedValue({ system: [system, allCases, conversations], saved: [privateSaved], defaultView }),
  ...
  it("clears a custom memory-only phrase before querying a selected view", async () => {
    ...
    expect(api.query).toHaveBeenLastCalledWith("project-1", { kind: "SYSTEM", code: "MY_ACTIVE" }, "", undefined);
  });
});
