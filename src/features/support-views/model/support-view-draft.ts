...
export function buildSavedViewDraft(name: string, state: SupportSearchRouteState) {
  ...
  if (displayName.length < 2 || displayName.length > 120 || state.scope === 'END_USERS')
    return null;
  ...
  if (state.scope === 'CONVERSATIONS') {
    ...
    return {
      ...
      sort: {
        field:
          state.sort.field === 'ACTIVITY_AT' ? ('ACTIVITY_AT' as const) : ('RELEVANCE' as const),
  ...
  return {
    ...
    sort: {
      field: state.sort.field === 'ACTIVITY_AT' ? ('ACTIVITY_AT' as const) : ('RELEVANCE' as const),
      direction: state.sort.direction,
    },
  };
}

export function createSavedViewCommand(
  name: string,
  code: string,
  scope: 'PERSONAL' | 'TEAM' | 'PROJECT',
  teamId: string,
  state: SupportSearchRouteState,
): CreateSavedSupportViewDto | null {
  const draft = buildSavedViewDraft(name, state);
  const normalizedCode = code.trim().toLowerCase();
  if (
    !draft ||
    !/^[a-z][a-z0-9-]{1,63}$/.test(normalizedCode) ||
    (scope === 'TEAM' && !teamId.trim())
  )
    return null;
  return {
    code: normalizedCode,
    scope,
    ...(scope === 'TEAM' ? { teamId: teamId.trim() } : {}),
    draft,
  };
}

export function replaceSavedViewCommand(
  name: string,
  state: SupportSearchRouteState,
): ReplaceSavedSupportViewDraftDto | null {
  const draft = buildSavedViewDraft(name, state);
