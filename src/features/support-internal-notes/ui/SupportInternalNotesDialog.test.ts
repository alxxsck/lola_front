...
function note(overrides: Partial<SupportInternalNote> = {}): SupportInternalNote {
  return {
    id: 'note-1',
    caseId: 'case-1',
...
describe('support internal notes dialog', () => {
  ...
  it('shows a tombstone without retaining a removed note body', () => {
    ...
    expect(wrapper.find('[data-testid="internal-note-body"]').exists()).toBe(false);
  ...
  it('renders an attachment-only active note without a false unavailable warning', () => {
    ...
    expect(wrapper.text()).not.toContain('Текст заметки недоступен');
    expect(wrapper.get('button[title="Скачать файл"]').attributes('disabled')).toBeUndefined();
  });
  ...
  it('purges a correction draft when correction authority is revoked', async () => {
    ...
    expect(
      (wrapper.get('.internal-note-correction textarea').element as HTMLTextAreaElement).value,
  ...
  it('blocks a multibyte correction above the backend byte limit', async () => {
    ...
    await wrapper
      ...
      .find((button) => button.text().includes('Исправить'))!
      .trigger('click');
    await wrapper.get('.internal-note-correction textarea').setValue('я'.repeat(10_241));

    expect(
      wrapper.get('.internal-note-correction__counter').text().replaceAll('\u00a0', ' '),
