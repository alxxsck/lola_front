...
function translationState(enabled: boolean): ConversationTranslationResponseDto {
  return {
...
describe('единое рабочее пространство пользователя', () => {
  beforeEach(() => {
    ...
    mocks.subscribe.mockImplementation((events: string[], handler: (value: unknown) => void) => {
  ...
  function mountWorkspace(preferredConversationId?: string, options: { readOnly?: boolean } = {}) {
  ...
  it('renders the selected Users chat through the canonical Conversation Surface', async () => {
    ...
    expect(wrapper.findAll('[aria-label="Режим отображения сообщений"]')).toHaveLength(1);
  });
  ...
  it('keeps profile and chat history visible but removes every write control in read-only mode', async () => {
    ...
    expect(wrapper.get('[data-testid="end-user-telegram-panel"]').attributes('data-can-send')).toBe(
      'false',
    );
    expect(wrapper.get('[data-testid="end-user-ai-allowance"]').attributes()).toMatchObject({
  ...
  it('keeps AI suspension controls beside the shared Surface and revokes mutation authority at runtime', async () => {
    ...
    expect(wrapper.findComponent(ConversationAISuspensionHeaderActions).exists()).toBe(true);
    expect(mocks.suspensionLoadDetail).toHaveBeenCalledWith('user-1', current.id);
    ...
    mocks.permissions.splice(mocks.permissions.indexOf('project.conversations.ai_suspend'), 1);
    mocks.permissionRevision!.value += 1;
    await flushPromises();

    expect(wrapper.find('[aria-label="Приостановить AI"]').exists()).toBe(false);
    expect(wrapper.findComponent(ConversationSurface).exists()).toBe(true);
  });
  ...
  it('opens as a spacious profile overview and moves chat behind an explicit action', async () => {
    ...
    expect(wrapper.get('[data-testid="end-user-ai-usage"]').attributes()).toMatchObject({
      'data-project-id': 'project-1',
      'data-end-user-id': 'user-1',
    });
    expect(wrapper.get<HTMLElement>('[data-testid="chat-workspace"]').element.style.display).toBe(
      'none',
    );
  ...
  it('shows and opens the independently authorized AI allowance in the user profile', async () => {
    mocks.permissions.push('project.ai_allowance.read', 'project.ai_allowance.grant');
    const wrapper = mountWorkspace();
    await flushPromises();
    ...
    expect(
      wrapper.get('[data-testid="ai-allowance-user-dialog"]').attributes('data-initial-mode'),
    ).toBe('grant');

    await card.get('[data-action="open-allowance-journal"]').trigger('click');
    await flushPromises();
    expect(mocks.updateVisible).not.toHaveBeenCalledWith(false);
    expect(wrapper.get('[data-testid="ai-allowance-journal"]').attributes()).toMatchObject({
      'data-project-id': 'project-1',
      'data-end-user-id': 'user-1',
      'data-cursor': '',
      'data-embedded': '',
  ...
  it('does not expose AI allowance without its read permission', async () => {
    ...
    expect(wrapper.find('[data-testid="end-user-ai-allowance"]').exists()).toBe(false);
    expect(wrapper.find('[data-testid="ai-allowance-user-dialog"]').exists()).toBe(false);
  });

  it.each([
    ['user allowance dialog', 'open-allowance-details', 'allowance-fresh-login'],
    ...
    async (_surface, openAction, freshLoginAction) => {
      mocks.permissions.push('project.ai_allowance.read', 'project.ai_allowance.manage');
      const wrapper = mountWorkspace();
      await flushPromises();
  ...
  it('фильтрует список диалогов по названию без перезагрузки истории', async () => {
    ...
    expect(wrapper.get('.conversation-list').text()).toContain('Текущий разговор');
    expect(wrapper.get('.conversation-list').text()).toContain('Депозит не зачислен');

    await wrapper.get('input[aria-label="Поиск по диалогам"]').setValue('депозит');

    expect(wrapper.get('.conversation-list').text()).not.toContain('Текущий разговор');
    expect(wrapper.get('.conversation-list').text()).toContain('Депозит не зачислен');
  ...
  it('сохраняет геометрию диалога скелетонами и блокирует composer до загрузки', async () => {
    let resolveMessages:
      ((value: Awaited<ReturnType<typeof mocks.getMessages>>) => void) | undefined;
    ...
    expect(wrapper.find('.conversation-composer.is-blocked').exists()).toBe(true);
    expect(wrapper.get('.conversation-composer textarea').attributes('disabled')).toBeDefined();

    resolveMessages?.({ items: [], nextCursor: null });
    await flushPromises();
    expect(wrapper.find('.conversation-surface__skeletons').exists()).toBe(false);
  });
  ...
  it('показывает прогресс массового перевода без перестановки сообщений', async () => {
    ...
    let resolveTranslation: ((value: { items: never[]; queued: boolean }) => void) | undefined;
    vi.spyOn(conversationTranslationApi, 'translateMessages').mockReturnValueOnce(
      new Promise((resolve) => {
    ...
    const wrapper = mountWorkspace(current.id);
    await flushPromises();
    await wrapper.get('[data-action="show-translated-messages"]').trigger('click');
    await flushPromises();

    expect(wrapper.get('.conversation-surface__translation-progress').text()).toContain('0 из 3');
    expect(wrapper.findAll('[data-message-id]')).toHaveLength(3);

    resolveTranslation?.({ items: [], queued: false });
    await flushPromises();
    expect(wrapper.find('.conversation-surface__translation-progress').exists()).toBe(false);
  });
  ...
  it('скрывает историю синхронизации без права чтения профилей', async () => {
    mocks.permissions.splice(mocks.permissions.indexOf('project.profiles.read'), 1);
    const wrapper = mountWorkspace();
    await flushPromises();

    expect(wrapper.find('[data-testid="profile-sync-history"]').exists()).toBe(false);
  });
  ...
  it('includes a Telegram composer draft in the workspace close guard', async () => {
    ...
    const wrapper = mountWorkspace();
    await flushPromises();
    await wrapper.get('button[data-action="telegram-draft-dirty"]').trigger('click');
    await wrapper.get('button[data-action="close-dialog"]').trigger('click');

    expect(confirm).toHaveBeenCalledWith('Закрыть рабочее пространство и потерять черновик?');
  ...
  it('загружает профиль независимо от ошибки realtime activation', async () => {
    mocks.activateProject.mockRejectedValueOnce(new Error('Realtime unavailable'));
    const wrapper = mountWorkspace();
    await flushPromises();
  ...
  it('позволяет вернуться к профилю из чата без диалогов', async () => {
    ...
    await wrapper.get('.chat-empty [data-action="open-profile"]').trigger('click');
    await flushPromises();
  ...
  it('показывает live-состояние компактным тегом без изменяющего высоту сообщения', async () => {
    ...
    await flushPromises();
    expect(wrapper.get('[data-testid="live-connection-status"]').text()).toContain('Подключение');
    expect(wrapper.find('.realtime-message').exists()).toBe(false);

    mocks.stateHandler?.('DEGRADED');
    await flushPromises();
    expect(wrapper.get('[data-testid="live-connection-status"]').text()).toContain('Ошибка связи');
    expect(wrapper.find('.realtime-message').exists()).toBe(false);
  });
  ...
  it('переключает все сообщения между оригиналом и рабочим переводом из шапки', async () => {
    ...
    await wrapper.get('[data-action="show-original-messages"]').trigger('click');
    expect(wrapper.text()).toContain('Guten Tag');
    expect(wrapper.text()).not.toContain('Добрый день');
  });
  ...
  it('загружает настройки перевода только после явного действия оператора', async () => {
    ...
    await wrapper.get('button[aria-label="Другие действия с диалогом"]').trigger('click');
    await flushPromises();
  ...
  it('определяет английский язык по сообщениям, а не по locale профиля', async () => {
    ...
    const getTranslation = vi.spyOn(conversationTranslationApi, 'getConversation');
  ...
  it('для русского диалога включает режим перевода без API и ошибок', async () => {
    ...
    expect(getTranslation).not.toHaveBeenCalled();
    expect(translate).not.toHaveBeenCalled();
    expect(wrapper.get('[data-action="show-translated-messages"]').attributes('aria-pressed')).toBe(
      'true',
    );
  ...
  it('оставляет перевод ответа доступным даже в русскоязычном диалоге', async () => {
    ...
    await wrapper.get('textarea[aria-label="Ответ пользователю"]').setValue('Проверяю ваш вопрос');

    expect(
      wrapper.findAll('button').some((button) => button.text().includes('Перевести ответ')),
    ).toBe(true);
  ...
  it('разворачивает Users workspace через общий presentation shell', async () => {
    ...
    const toggle = wrapper.get('button[aria-label="Развернуть рабочее место на всю вкладку"]');
  ...
  it('показывает ошибку перевода вне chat layout', async () => {
    ...
    const translationBanner = wrapper.getComponent(ConversationTranslationBanner);
  ...
  it('после reload восстанавливает composer из scoped translation draft', async () => {
    ...
    const getReplyDraft = vi.spyOn(conversationTranslationApi, 'getReplyDraft').mockResolvedValue({
      conversationId: current.id,
    ...
    expect(
      (wrapper.get('textarea[aria-label="Ответ пользователю"]').element as HTMLTextAreaElement)
        .value,
    ).toBe(sourceText);
    expect(wrapper.getComponent(ReplyTranslationPreview).props('draft')?.translatedText).toBe(
  ...
  it('сохраняет визуальный anchor при добавлении предыдущей страницы истории', async () => {
    let resolveOlder: ((value: Awaited<ReturnType<typeof mocks.getMessages>>) => void) | undefined;
    mocks.getMessages
      .mockResolvedValueOnce({
        items: [
          {
    ...
    const wrapper = mountWorkspace(current.id);
    await flushPromises();
    const history = wrapper.get<HTMLElement>('.conversation-surface__log').element;
    ...
    expect(wrapper.findAll('[data-message-id]').map((message) => message.text())).toEqual([
  ...
  it('следует за live-сообщением только у нижней границы истории', async () => {
    ...
    expect(wrapper.find('.conversation-surface__new-messages').exists()).toBe(false);
  ...
  it('восстанавливает отдельный public draft при возврате в диалог', async () => {
    ...
    mocks.getMessages.mockImplementation(async (_projectId, _endUserId, conversationId) => ({
      items: [
        {
    ...
    const wrapper = mountWorkspace(current.id);
    await flushPromises();
    const composer = () => wrapper.get('textarea[aria-label="Ответ пользователю"]');
    const conversationButton = (title: string) =>
      wrapper.findAll('.conversation-list button').find((button) => button.text().includes(title))!;
    ...
    await flushPromises();
    expect((composer().element as HTMLTextAreaElement).value).toBe('Черновик текущего разговора');

    await conversationButton('Второй разговор').trigger('click');
    await flushPromises();
    expect((composer().element as HTMLTextAreaElement).value).toBe('Черновик второго разговора');
  });
  ...
  it('purges the scoped Surface draft on project and user switch', async () => {
    ...
    expect(wrapper.get('.conversation-surface').element).not.toBe(previousSurface);
    expect(
      (wrapper.get('textarea[aria-label="Ответ пользователю"]').element as HTMLTextAreaElement)
        .value,
    ).toBe('');
  ...
  it('purges the selected conversation and draft when read permission is revoked', async () => {
    ...
    mocks.permissions.splice(mocks.permissions.indexOf('project.conversations.read'), 1);
    mocks.permissionRevision!.value += 1;
    await flushPromises();
  ...
  it('purges the composer attempt and draft when reply permission is revoked', async () => {
    ...
    mocks.permissions.splice(mocks.permissions.indexOf('project.conversations.reply'), 1);
    mocks.permissionRevision!.value += 1;
    await flushPromises();
    expect(wrapper.find('textarea[aria-label="Ответ пользователю"]').exists()).toBe(false);
  ...
  it('closes and purges the new-conversation draft when reply permission is revoked', async () => {
    ...
    expect(wrapper.find('textarea[aria-label="Первое сообщение нового диалога"]').exists()).toBe(
      false,
    );
  ...
  it('purges cached translations and returns to original mode when translation permission is revoked', async () => {
    ...
    mocks.permissions.splice(mocks.permissions.indexOf('project.translation.create'), 1);
    mocks.permissionRevision!.value += 1;
    await flushPromises();

    expect(wrapper.text()).toContain('Sensitive original');
    expect(wrapper.text()).not.toContain('Секретный перевод');
    expect(wrapper.find('[aria-label="Режим отображения сообщений"]').exists()).toBe(false);
  ...
  it('treats realtime without a server ordinal as a REST reconcile hint', async () => {
    ...
    expect(mocks.getMessages).toHaveBeenCalledWith('project-1', 'user-1', current.id, {
      limit: 20,
    });
  ...
  it('не отсекает substantive Cyrillic future realtime до backend', async () => {
    ...
    vi.spyOn(conversationTranslationApi, 'updateConversation').mockResolvedValue(
      translationState(true),
    );
    const translate = vi
      .spyOn(conversationTranslationApi, 'translateMessages')
    ...
    await flushPromises();
    await wrapper.get('.conversation-more').trigger('click');
    wrapper.getComponent(ConversationTranslationBanner).vm.$emit('updateEnabled', true);
    await flushPromises();
    translate.mockClear();
  ...
  it('возвращает выбранный диалог из мобильного списка при повторном выборе', async () => {
    ...
    expect(wrapper.get('[data-testid="chat-workspace"]').attributes('data-mobile-pane')).toBe(
      'LIST',
    );
  ...
  it('открывает форму тикета без ложной отправки в неподключённый API', async () => {
    ...
    await wrapper.get('.conversation-composer__actions > button').trigger('click');
  ...
  it('открывает галерею шаблонов только из отдельной кнопки', async () => {
    ...
    await wrapper.get('.conversation-composer__actions > button').trigger('click');
    expect(wrapper.get('.conversation-composer__action-menu').text()).not.toContain(
      'Шаблон ответа',
    );
  ...
  it('не запрашивает presence без project.end_users.read', async () => {
    mocks.permissions.splice(0, mocks.permissions.length, 'project.conversations.read');
    mountWorkspace();
    await flushPromises();
  ...
  it('показывает Memory и Review по их permissions без доступа к профилю', async () => {
    ...
    expect(wrapper.find('[data-testid="user-memory-panel"]').exists()).toBe(true);
    const review = wrapper.get('[data-testid="ai-review-entry"]');
    expect(review.text()).toContain('AI-анализ событий');
    expect(review.text()).toContain('Выберите события и сначала оцените объём запроса');
  ...
  describe('characterization: Users chat during Conversation Surface migration', () => {
    it('keeps the current log, translation toggle, pagination and composer landmarks', async () => {
      ...
      expect(wrapper.get('textarea[aria-label="Ответ пользователю"]').element).toBeInstanceOf(
        HTMLTextAreaElement,
      );
    });
