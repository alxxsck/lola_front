...
describe('scenario localization model', () => {
  it('uses every project locale for ALL and only selected locales for SELECTED', () => {
    expect(requiredLocales(catalog, defaultLocalizationPolicy())).toEqual(['en', 'es', 'pt-BR']);
    expect(
  ...
  it('migrates a legacy scalar to the project default without dropping archived variants', () => {
    ...
    expect(localizedValue({ en: 'Hello', 'fr-CA': 'Bonjour' }, 'en')).toEqual({
      en: 'Hello',
      'fr-CA': 'Bonjour',
    });
  });
  ...
  it('previews the requested locale and explains fallback to the project default', () => {
    expect(resolveLocalizedContent({ en: 'Hello', es: 'Hola' }, 'es', 'en')).toEqual({
      text: 'Hola',
      contentLocale: 'es',
      fallbackReason: null,
    });
    expect(resolveLocalizedContent({ en: 'Hello' }, 'pt-BR', 'en')).toEqual({
      text: 'Hello',
      contentLocale: 'en',
