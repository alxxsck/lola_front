...
import { translationRepository, type TranslationUsage } from '../api/translation-repository';
...
const successRate = computed(() => {
  ...
  return totals?.requests ? Math.round((totals.successes / totals.requests) * 100) : 0;
});
const cost = computed(() => {
  const totals = report.value?.totals;
  if (!totals) return '—';
...
async function load(force = false) {
  ...
  const key =
    range.value === 'custom' ? `custom:${customFrom.value}:${customTo.value}` : range.value;
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : 'Не удалось загрузить статистику переводов';
  } finally {
    loading.value = false;
  }
}
  ...
  <section class="translation-usage" aria-labelledby="translation-usage-title">
    <header class="translation-header">
      ...
      <Button
        ...
        text
        rounded
        aria-label="Обновить статистику переводов"
        :loading="loading"
        @click="load(true)"
      />
    </header>
    ...
    <div v-else-if="report && !report.totals.requests" class="translation-empty">
      ...
      <div>
        <strong>За выбранный период переводов не было</strong>
        <p>Автоматические переводы ещё не используются.</p>
      </div>
    </div>
    <template v-else-if="report">
      <div v-if="report.budget?.hardExhausted" class="budget-warning" role="alert">
        <strong>Лимит исчерпан.</strong> Новые автоматические переводы недоступны до обновления
        бюджета.
      </div>
      <div class="translation-summary">
        <article>
          <span>Оплачиваемые символы</span
          ><strong>{{ report.totals.billableCharacters.toLocaleString('ru-RU') }}</strong
          ><small>{{ report.totals.inputCharacters.toLocaleString('ru-RU') }} исходных</small>
        </article>
        <article>
          <span>Успешность</span><strong>{{ successRate }}%</strong
          ><small>{{ report.totals.successes }} успешно · {{ report.totals.errors }} ошибок</small>
        </article>
        <article>
          <span>Кэш</span><strong>{{ report.totals.cacheHits }}</strong
          ><small>экономия {{ report.totals.estimatedSavingsMicros }} μUSD</small>
        </article>
        <article>
          <span>{{
            report.totals.actualCostMicros ? 'Фактическая стоимость' : 'Расчётная стоимость'
          }}</span
          ><strong>{{ cost }}</strong
          ><small>{{ report.totals.billingCurrency }}</small>
        </article>
      </div>
      <div v-if="report.budget" class="budget-meter">
        ...
        <small class="budget-caption"
          >Предупреждение: {{ report.budget.softPercent ?? 0 }}% · жёсткий лимит:
          {{ report.budget.hardPercent ?? 0 }}%</small
        >
      </div>
      <section v-if="report.series.length" class="chart-panel">
        ...
        <div class="translation-chart" aria-label="Переводы по дням">
          <div
            v-for="item in report.series"
            :key="item.day"
            :title="`${item.day}: ${item.billableCharacters}`"
          >
            <span
              :style="{ height: `${Math.max(4, (item.billableCharacters / maximumDay) * 100)}%` }"
            />
      ...
      <div class="translation-details">
        <div v-if="report.targetLocales.length" class="target-breakdown">
          <span v-for="item in report.targetLocales" :key="item.targetLocale"
            ><strong>{{ item.targetLocale }}</strong
            >{{ item.billableCharacters.toLocaleString('ru-RU') }}</span
          >
        </div>
        <div v-if="report.statuses.some((item) => item.errors)" class="status-breakdown">
          <strong>Ошибки по категориям</strong>
          <span v-for="item in report.statuses.filter((status) => status.errors)" :key="item.status"
            >{{ statusLabel(item.status) }} · {{ item.errors }}</span
          >
        </div>
        <div class="translation-meta">
          <small v-if="report.totals.latencyP50Ms !== null" class="latency-note">
            Время ответа p50 {{ report.totals.latencyP50Ms }} мс · p95
            {{ report.totals.latencyP95Ms ?? '—' }} мс
          </small>
          <small v-if="loadedAt" class="loaded-at"
            >Обновлено
            {{
              loadedAt.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })
            }}</small
          >
        </div>
      </div>
...
.usage-ranges button.active {
  ...
  background: var(--status-accent-soft);
  color: var(--status-accent-text);
  box-shadow: inset 0 0 0 1px color-mix(in srgb, var(--status-accent) 12%, transparent);
}
.custom-range {
  display: flex;
  align-items: end;
...
.budget-track {
  ...
  box-shadow: inset 0 1px 2px color-mix(in srgb, var(--text-primary) 9%, transparent);
}
.budget-track-fill {
  display: block;
  height: 100%;
  border-radius: inherit;
  background: linear-gradient(90deg, var(--status-accent), var(--action-primary));
  transition: width 0.25s ease;
}
.budget-track-fill.exhausted {
