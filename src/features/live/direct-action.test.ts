...
describe('direct admin action contract', () => {
  it('uses backend target keys for page and modal commands', () => {
    ...
    expect(buildDirectActions({ ...input, action: 'OPEN_MODAL', target: 'deposit' })).toEqual([
      { type: 'OPEN_MODAL', config: { modalId: 'deposit' } },
    ]);
  });

  it('uses lowercase CTA action and its matching target key', () => {
    expect(
      buildDirectActions({ ...input, type: 'BUTTON', label: ' Open ', action: 'OPEN_PAGE' }),
    ).toEqual([
      { type: 'SHOW_CTA', config: { label: 'Open', action: 'open_page', pageId: 'home' } },
    ]);
    expect(
      buildDirectActions({
        ...input,
      ...
      { type: 'SHOW_CTA', config: { label: 'Open', action: 'open_modal', modalId: 'deposit' } },
    ]);
    expect(
      buildDirectActions({ ...input, type: 'BUTTON', label: 'Open', action: 'HIGHLIGHT_ELEMENT' }),
    ).toBeUndefined();
  });

  it('uses a public SDK animation code', () => {
    expect(buildDirectActions({ ...input, type: 'ANIMATION' })).toEqual([
      { type: 'PLAY_ANIMATION', config: { animation: 'deposit' } },
    ]);
  });
  ...
  it('retains a key for an uncertain retry and rotates it after the payload changes', () => {
    ...
    expect(resolveIdempotencyAttempt('payload-b', first, () => 'key-b')).toEqual({
