...
const form = reactive({
  ...
  action: 'OPEN_PAGE',
  target: '',
  animation: ASSISTANT_ANIMATIONS[0],
  voice: 'eve',
  integrationCode: 'welcome_bonus',
  amount: 10,
  currency: 'EUR',
});

const availableSessions = computed(() =>
  props.sessions?.length ? props.sessions : props.session ? [props.session] : [],
);
const selectedSession = computed(
  () => availableSessions.value.find((item) => item.id === selectedSessionId.value) ?? null,
);
const sessionOptions = computed(() =>
  availableSessions.value.map((item) => ({
...
const typeCatalog = computed<ActionPickerItem[]>(() =>
...
const commandActions = [
  {
    ...
    value: 'OPEN_PAGE',
    description: 'Переходит на опубликованную страницу продукта.',
  },
  {
    label: 'Открыть модалку',
    value: 'OPEN_MODAL',
    description: 'Открывает опубликованное модальное окно.',
  },
  {
    label: 'Подсветить элемент',
    value: 'HIGHLIGHT_ELEMENT',
    description: 'Привлекает внимание к элементу или кнопке.',
  },
];
const buttonActions = computed(() =>
  form.type === 'BUTTON' ? commandActions.slice(0, 2) : commandActions,
);
const commandActionCatalog = computed<ActionPickerItem[]>(() =>
  buttonActions.value.map((action) => ({
...
const valid = computed(() => {
  ...
  if (form.type === 'BUTTON')
    return Boolean(
      ['OPEN_PAGE', 'OPEN_MODAL'].includes(form.action) &&
      form.label.trim() &&
      form.target.trim() &&
      selectedSession.value,
    );
  if (form.type === 'COMMAND')
    return Boolean(form.action && form.target.trim() && selectedSession.value);
  if (form.type === 'BONUS')
    return Boolean(
      selectedSession.value &&
      form.integrationCode.trim() &&
      form.amount > 0 &&
      form.currency.trim(),
    );
...
watch(
  ...
  async (value) => {
    ...
    try {
      ...
      error.value =
        cause instanceof Error ? cause.message : 'Не удалось загрузить опубликованные цели';
      elements.value = [];
    } finally {
...
watch(
  () => form.type,
  (type) => {
    if (type === 'BUTTON' && !['OPEN_PAGE', 'OPEN_MODAL'].includes(form.action))
      form.action = 'OPEN_PAGE';
  },
);
...
async function send() {
  ...
  try {
    if (repository.mode === 'mock' && (form.type === 'VOICE' || form.type === 'BONUS')) {
      ...
      toast.add({
        severity: 'success',
        summary: 'Demo-команда отправлена',
        detail: 'Действие записано в локальный журнал.',
        life: 3500,
      });
      emit('sent');
    ...
    const request = {
      text:
        form.text.trim() ||
        (form.type === 'ANIMATION'
          ? 'Анимация от администратора'
          : form.label.trim() || 'Команда администратора'),
    ...
    const result = await repository.sendAdminMessage(props.projectId, internalUserId.value, {
      ...request,
    ...
    toast.add({
      ...
      detail: result.duplicate
        ? 'Backend вернул сохранённый результат без повторной доставки.'
        : `${recipient.value} получит сообщение от Retenive.`,
      life: 4000,
    });
    emit('sent');
    emit('update:visible', false);
...
<template>
  ...
  >
    <div class="stack">
        ...
        <div>
          ...
          ><small>{{
            availableSessions.length
              ? `${availableSessions.length} активных сессий`
              : 'Пользователь сейчас offline'
          }}</small>
        </div>
      </div>
      <ActionPicker
      ...
      <div v-if="availableSessions.length" class="field">
        <label>Активная сессия</label
        ><Select
          ...
          option-value="value"
        /><small
          >Для текста backend может выбрать сессию автоматически. Кнопка или frontend-команда будет
          отправлена строго в выбранную сессию.</small
        >
      </div>
      <template v-if="form.type === 'TEXT' || form.type === 'VOICE'">
        <div v-if="form.type === 'VOICE'" class="field">
          <label>Голос</label
          ><Select v-model="form.voice" :options="['ara', 'eve', 'leo', 'rex', 'sal']" />
        </div>
        <div class="field">
          <label>{{ form.type === 'VOICE' ? 'Текст для озвучивания' : 'Сообщение' }}</label
          ><Textarea
            v-model="form.text"
            rows="5"
            maxlength="10000"
            placeholder="Что Retenive должна сказать пользователю?"
          />
        </div>
      </template>
      <template v-else-if="form.type === 'BUTTON' || form.type === 'COMMAND'">
        <div class="grid grid-2">
          <div v-if="form.type === 'BUTTON'" class="field">
            <label>Текст кнопки</label
            ><InputText v-model="form.label" placeholder="Открыть бонусы" />
          </div>
          <ActionPicker
            v-model="form.action"
            class="send-action-type-picker"
        ...
        <div class="field">
          ...
          />
          <small v-if="!targetsLoading && !compatibleTargets.length"
            >Нет активных целей совместимого типа.</small
          >
        </div>
      </template>
      <div v-else-if="form.type === 'ANIMATION'" class="field">
      ...
      <template v-else-if="form.type === 'BONUS'">
        <div class="field">
          <label>Интеграция</label><InputText v-model="form.integrationCode" class="mono" />
        </div>
        <div class="grid grid-2">
          <div class="field">
            <label>Сумма</label><InputNumber v-model="form.amount" :min="1" />
          </div>
          <div class="field">
            <label>Валюта</label><InputText v-model="form.currency" maxlength="3" />
          </div>
        </div>
        <Message severity="secondary" size="small"
          >Demo-only: действие записывается локально и не вызывается в production API.</Message
        >
      </template>
      <Message
        v-if="repository.mode === 'api' && !availableSessions.length"
        severity="info"
        size="small"
        >Backend принимает сообщения только для online-пользователей. Кнопки и frontend-действия
        доступны при активной interaction session.</Message
      >
    ...
    <template #footer
      ><Button
        ...
        severity="secondary"
        text
        @click="emit('update:visible', false)" /><Button
        label="Отправить"
        icon="pi pi-send"
        :disabled="!valid"
        :loading="loading"
...
.offline-dot {
  ...
  box-shadow: 0 0 0 5px var(--surface-subtle);
}
