...
describe("support search route", () => {
  ...
  it("keeps free text local while normalizing it for the server", () => {
    ...
    expect(
      writeSupportSearchRoute(unsafe),
  ...
  it("keeps exact external identities local and makes content sort executable", () => {
    ...
    expect(writeSupportSearchRoute(state)).not.toHaveProperty(
      "externalEndUserId",
    );
    expect(normalizeSupportSearchState(state).filters.externalEndUserIds).toEqual([
      "customer-17",
    ]);
