...
type RouteQuery = Record<
  string,
  string | readonly (string | null)[] | null | undefined
>;
type WrittenRouteQuery = Record<string, string>;

const scopes = new Set<SupportSearchScope>([
  "CASES",
  "CONVERSATIONS",
  "MESSAGES",
  "END_USERS",
...
const allowed = {
  ...
  waitingSides: new Set<SupportCaseSearchQueryDtoWaitingSidesItem>(["END_USER", "SUPPORT", "SYSTEM", "NONE"]),
  assignmentStates: new Set<SupportCaseSearchQueryDtoAssignmentStatesItem>(["ASSIGNED", "UNASSIGNED"]),
  priorities: new Set<SupportCaseSearchQueryDtoPrioritiesItem>(["LOW", "NORMAL", "HIGH", "URGENT", "CRITICAL"]),
  slaStates: new Set<SupportCaseSearchQueryDtoSlaStatesItem>(["ON_TRACK", "AT_RISK", "BREACHED", "NOT_CONFIGURED"]),
  ...
  const result = [
    ...new Set(
      single(value)
        ...
        .filter(
          (item) =>
            item && (!values || (values as ReadonlySet<string>).has(item)),
        ),
    ),
  ].slice(0, limit) as T[];
...
export function readSupportSearchRoute(query: RouteQuery): SupportSearchRouteState {
  ...
  const scope = scopes.has(requestedScope as SupportSearchScope)
    ...
    : "CASES";
  const timeRange = normalizeSearchTimeRange(
    single(query.from),
    single(query.to),
  );
  const filters: SupportCaseSearchFilters =
    scope === "CASES"
      ? {
          ...
          ...(list(query.endUserId)
            ? { endUserIds: list(query.endUserId) }
            : {}),
          ...
          ...(list(query.assignment, allowed.assignmentStates, 2)
            ? {
                assignmentStates: list(
                  query.assignment,
                  allowed.assignmentStates,
                  2,
                ),
              }
            : {}),
          ...
          ...(list(query.assignee)
            ? { assigneeCmsUserIds: list(query.assignee) }
            : {}),
      ...
      : {
          ...
          ...(scope === "CONVERSATIONS" && list(query.conversationId)
            ...
            : {}),
          ...(scope === "MESSAGES" && list(query.caseId)
            ? { caseIds: list(query.caseId) }
            : {}),
  ...
  const field = (sortFields.has(requestedSort)
    ? requestedSort
    : "RELEVANCE") as SupportSearchRouteState["sort"]["field"];
...
export function normalizeSupportSearchState(
  ...
  const normalizedExternalIds = [
    ...new Set(
      (state.filters.externalEndUserIds ?? [])
  ...
  return {
    ...
    filters: {
      ...routed.filters,
      ...(normalizedExternalIds.length
        ? { externalEndUserIds: normalizedExternalIds }
        : {}),
    },
  };
}

export function writeSupportSearchRoute(
  state: SupportSearchRouteState,
): WrittenRouteQuery {
  ...
  if (state.scope === "CASES") {
    ...
    if (filters.assignmentStates?.length)
      query.assignment = filters.assignmentStates.join(",");
    ...
    if (filters.categoryCodes?.length)
      query.category = filters.categoryCodes.join(",");
    ...
    if (filters.assigneeCmsUserIds?.length)
      query.assignee = filters.assigneeCmsUserIds.join(",");
  ...
  if (filters.conversationIds?.length)
    query.conversationId = filters.conversationIds.join(",");
  if (filters.messageIds?.length)
    query.messageId = filters.messageIds.join(",");
  if (filters.endUserIds?.length)
    query.endUserId = filters.endUserIds.join(",");
