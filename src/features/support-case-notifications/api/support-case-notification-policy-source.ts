...
export type NotificationPolicyOperation = ReadSupportCaseNotificationCommandResultDtoOperation;
...
export const apiSupportCaseNotificationPolicySource: SupportCaseNotificationPolicySource = {
  async read(projectId, signal) {
    try {
      return await supportCaseNotificationPolicyReadCurrent(projectId, options(signal));
    } catch (cause) {
  ...
  async readMetrics(projectId, signal) {
    try {
      return await supportCaseNotificationPolicyReadMetrics(projectId, options(signal));
    } catch (cause) {
      throw normalizeApiError(cause);
    }
  },
  ...
  async preview(projectId, input, signal) {
    try {
      return await supportCaseNotificationPolicyPreview(projectId, input, options(signal));
    } catch (cause) {
      throw normalizeApiError(cause);
    }
  },
...
const mockPolicies = new Map<string, SupportCaseNotificationPolicyCurrentResponseDto>();
const mockReceipts = new Map<string, SupportCaseNotificationPolicyReceiptResponseDto>();

function mockCurrent(projectId: string): SupportCaseNotificationPolicyCurrentResponseDto {
...
const mockSupportCaseNotificationPolicySource: SupportCaseNotificationPolicySource = {
  ...
  async preview(_projectId, input) {
    ...
    const recipients = input.recipientRule === 'TEAM_SUBSCRIBERS' ? input.teamIds.length * 4 : 12;
    return {
      issues: [],
      estimatedEligibleRecipients: recipients,
      matchingOccurrencesLast7Days: off ? 0 : 38,
      estimatedImmediateDeliveriesLast7Days: input.mode === 'IMMEDIATE' ? 38 * recipients : 0,
  ...
  async publish(projectId, body, key) {
    return receipt(projectId, key, (current) => {
      const source = current.draft?.id === body.revisionId ? current.draft : current.current;
  ...
  async disable(projectId, _body, key) {
    return receipt(projectId, key, (current) => ({
      ...
      version: current.version + 1,
      effectiveStatus: 'OFF',
      current: current.current ? { ...current.current, status: 'DISABLED' } : null,
    }));
  },
  async restore(projectId, body, key) {
    return receipt(projectId, key, (current) => {
      const source = current.restorableRevisions.find((item) => item.id === body.revisionId);
      return source
        ? {
            ...current,
            version: current.version + 1,
