...
describe('mock End User Cases repository', () => {
  ...
  it('keeps demo mode useful with active cases, summary and escalations', async () => {
    const page = await mockEndUserCasesRepository.list('project-demo', defaultEndUserCaseFilters());
    const summary = await mockEndUserCasesRepository.summary('project-demo');
    const detail = await mockEndUserCasesRepository.detail('project-demo', 'case-demo-deposit');
  ...
  it('persists an escalation on the concrete Case', async () => {
    const before = await mockEndUserCasesRepository.detail('project-demo', 'case-demo-deposit');
    ...
    const after = await mockEndUserCasesRepository.detail('project-demo', 'case-demo-deposit');
  ...
  it('resets the Case escalation timeline between demo sessions', async () => {
    ...
    const mutated = await mockEndUserCasesRepository.detail('project-demo', 'case-demo-deposit');
    ...
    const restoredGame = await mockEndUserCasesRepository.detail('project-demo', 'case-demo-game');
