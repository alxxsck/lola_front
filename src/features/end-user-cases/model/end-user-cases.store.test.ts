...
describe('End User Cases store', () => {
  ...
  it('requests specialist help with the selected Case version and an idempotency key', async () => {
    ...
    expect(
      await store.requestEscalation(' DEPOSIT_HELP ', ' Нужна ручная проверка депозита '),
    ).toBe(true);
  ...
  it('sends exact OCC payloads for release, transfer, close and cancel', async () => {
    ...
    expect(await store.transferEscalation(' cms-2 ', ' Передать эксперту ')).toBe(true);
    ...
    expect(await store.cancelEscalation('WAITING_END_USER', ' Ошибочный запрос ')).toBe(true);
  ...
  it('submits merge with every source version and reconciles the survivor', async () => {
    ...
    expect(await store.merge([{ id: 'case-2', version: 4 }], 'Duplicate goal')).toBe(true);
  ...
  it('appends the next linked-message page without duplicating evidence', async () => {
    ...
    expect(repository.messages).toHaveBeenCalledWith('project-1', 'case-1', 'cursor-2');
    expect(store.selected?.messages.items.map((link) => link.message.id)).toEqual([
      'message-1',
      'message-2',
    ]);
  ...
  it('splits selected evidence and opens the backend-created Case', async () => {
    ...
    repository.detail.mockImplementation(async (_project: string, id: string) => ({
      case: { ...item, id },
      messages: { items: [], nextCursor: null },
      timeline: { events: [], revisions: [] },
      escalations: { items: [] },
    ...
    expect(
      await store.split(['message-1'], ' Refund ', ' separate goal ', ' REFUND ', ['evidence-1']),
    ).toBe('case-2');
  ...
  it('reconciles a summary sequence gap and schedules a follow-up after an in-flight refresh', async () => {
    const store = useEndUserCasesStore();
    await store.activateProject('project-1');
    let releaseList!: (value: { items: Array<typeof item>; nextCursor: null }) => void;
  ...
  it('starts a new Project reconciliation without waiting for the previous Project', async () => {
    ...
    let releaseOld!: (value: { items: Array<typeof item>; nextCursor: null }) => void;
    repository.list.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
          releaseOld = resolve;
