...
const PRESETS = new Set<EndUserCasePreset>(['ACTIVE', 'ATTENTION', 'WAITING', 'RESOLVED', 'ALL']);
...
const PRIORITIES = new Set<EndUserCasePriority>(['LOW', 'NORMAL', 'HIGH', 'URGENT', 'CRITICAL']);
...
const selected = <T extends string>(value: unknown, allowed: ReadonlySet<T>): T[] =>
  values(value).filter(
...
export function endUserCaseFiltersFromRoute(query: Query): EndUserCaseFilters {
  ...
  const sort = SORTS.has(String(query.sort) as EndUserCaseSort)
    ...
    : 'ATTENTION_FIRST';
  const priority = values(query.priority).filter((value): value is EndUserCasePriority =>
  ...
  const resolutionAssessment = selected(query.resolution, RESOLUTION_ASSESSMENTS);
  const resolutionSource = selected(query.resolutionSource, RESOLUTION_SOURCES);
  const channel = values(query.channel).filter((value): value is 'TEXT' | 'VOICE' | 'CMS' =>
    CHANNELS.has(String(value) as 'TEXT' | 'VOICE' | 'CMS'),
  );
  const aiCapabilityOutcome = values(query.capabilityOutcome).filter(
    (value): value is 'RESERVED' | 'ACCEPTED' | 'COMPLETED' | 'REJECTED' | 'FAILED' =>
      CAPABILITY_OUTCOMES.has(
        String(value) as 'RESERVED' | 'ACCEPTED' | 'COMPLETED' | 'REJECTED' | 'FAILED',
      ),
  );
  const assignment = ['ASSIGNED', 'UNASSIGNED'].includes(String(query.assignment))
  ...
  return {
    ...
    ...(one(query.capability) ? { aiCapabilityCode: one(query.capability) } : {}),
    ...(aiCapabilityOutcome.length ? { aiCapabilityOutcome } : {}),
    ...(oneOf(query.adminAttention, ['OPEN', 'NONE'] as const)
      ? {
          adminAttention: oneOf(query.adminAttention, ['OPEN', 'NONE'] as const),
        }
      : {}),
    ...(yesNo(query.cmsParticipation) ? { cmsParticipation: yesNo(query.cmsParticipation) } : {}),
    ...
    ...(iso(query.activityFrom) ? { lastActivityFrom: iso(query.activityFrom) } : {}),
    ...(iso(query.activityTo) ? { lastActivityTo: iso(query.activityTo) } : {}),
  };
}

export function endUserCaseRouteQuery(filters: EndUserCaseFilters): LocationQueryRaw {
  return {
    ...
    ...(filters.resolutionAssessment?.length ? { resolution: filters.resolutionAssessment } : {}),
    ...(filters.resolutionSource?.length ? { resolutionSource: filters.resolutionSource } : {}),
    ...
    ...(filters.assignedCmsUserId ? { assignee: filters.assignedCmsUserId } : {}),
    ...(filters.primaryLanguage ? { language: filters.primaryLanguage } : {}),
    ...(filters.channel?.length ? { channel: filters.channel } : {}),
    ...(filters.aiCapabilityCode ? { capability: filters.aiCapabilityCode } : {}),
    ...(filters.aiCapabilityOutcome?.length
      ? { capabilityOutcome: filters.aiCapabilityOutcome }
      : {}),
    ...(filters.adminAttention ? { adminAttention: filters.adminAttention } : {}),
    ...(filters.cmsParticipation ? { cmsParticipation: filters.cmsParticipation } : {}),
    ...
    ...(filters.lastActivityFrom ? { activityFrom: filters.lastActivityFrom } : {}),
