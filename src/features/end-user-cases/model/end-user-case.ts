...
export type EndUserCasePreset = 'ACTIVE' | 'ATTENTION' | 'WAITING' | 'RESOLVED' | 'ALL';
...
export interface EndUserCaseFilters {
  ...
  resolutionAssessment?: NonNullable<EndUserCasesListParams['resolutionAssessment']>;
  ...
  aiCapabilityOutcome?: NonNullable<EndUserCasesListParams['aiCapabilityOutcome']>;
...
export function endUserCaseStatusesForPreset(
  ...
  if (preset === 'ACTIVE' || preset === 'ATTENTION') return [...ACTIVE_STATUSES];
  if (preset === 'WAITING') return ['WAITING_END_USER', 'WAITING_SYSTEM', 'WAITING_ADMIN'];
...
export function endUserCaseListParams(
  ...
  return {
    ...
    ...(filters.assignedCmsUserId ? { assignedCmsUserId: filters.assignedCmsUserId } : {}),
    ...(filters.primaryLanguage ? { primaryLanguage: filters.primaryLanguage } : {}),
    ...(filters.channel?.length ? { channel: filters.channel } : {}),
    ...(filters.aiCapabilityCode ? { aiCapabilityCode: filters.aiCapabilityCode } : {}),
    ...
    ...(filters.lastActivityFrom ? { lastActivityFrom: filters.lastActivityFrom } : {}),
    ...(filters.lastActivityTo ? { lastActivityTo: filters.lastActivityTo } : {}),
    sort: filters.sort,
    ...(cursor ? { cursor } : {}),
    limit: 30,
  };
...
export function isEndUserCaseRealtimeEvent(value: unknown): value is EndUserCaseRealtimeEvent {
  ...
  if (envelope.type === 'end_user_case.summary')
    return typeof data.lastProjectSequence === 'string' && typeof data.openCount === 'number';
  if (envelope.type !== 'end_user_case.created' && envelope.type !== 'end_user_case.updated')
    return false;
  const item = record(data.case);
  return item !== null && typeof item.id === 'string' && typeof item.version === 'number';
}
