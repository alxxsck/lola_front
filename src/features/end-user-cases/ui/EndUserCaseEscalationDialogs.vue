...
const scopeChanged = computed(
  () => visible.value && !isSameEndUserCaseEscalationScope(scope.value, currentScope.value),
);
...
const canSubmit = computed(() => {
  if (store.mutating || scopeChanged.value || !actionAllowed.value) return false;
...
async function requestEscalationAction(nextAction: EndUserCaseEscalationAction): Promise<void> {
  const nextScope = currentScope.value;
  const active = activeEndUserCaseEscalation(store.selected?.escalations.items ?? []);
  if (
    !nextScope ||
...
function isActionAllowed(value: EndUserCaseEscalationAction): boolean {
  ...
  if (value === 'CLAIM' || value === 'TRANSFER') return props.canAssign === true;
...
async function loadAssignees(): Promise<void> {
  ...
  try {
    ...
    if (generation === assigneeRequestGeneration) assigneesLoading.value = false;
  }
}
      ...
      : action.value === 'CLAIM'
        ...
        : action.value === 'RELEASE'
          ...
          : action.value === 'TRANSFER'
            ? await store.transferEscalation(targetCmsUserId.value, normalizedReason)
            : action.value === 'CLOSE'
              ? await store.closeEscalation(nextCaseStatus.value, normalizedReason)
              : await store.cancelEscalation(
                  nextCaseStatus.value as 'OPEN' | 'WAITING_END_USER' | 'WAITING_SYSTEM',
                  normalizedReason,
                );
  if (succeeded) dismiss();
  else actionError.value = store.detailError ?? 'Не удалось выполнить действие. Повторите попытку.';
}
...
<template>
  ...
  >
    <Message v-if="action === 'REQUEST'" severity="info" :closable="false">
      Запрос появится в очереди обращений и отправит одно уведомление по настроенным каналам
      проекта.
    </Message>
    <Message
      ...
      :closable="false"
    >
      Выбирайте «Решено» только при явном подтверждении пользователя или проверенном результате.
    </Message>
    ...
    <label v-if="action === 'REQUEST'" class="dialog-field" for="case-escalation-reason">
      <span>Причина</span>
      <Select
        v-model="reasonCode"
    ...
    <label v-if="action === 'REQUEST'" class="dialog-field" for="case-escalation-summary">
      <span>Что должен увидеть специалист</span>
      <Textarea id="case-escalation-summary" v-model="summary" rows="4" maxlength="1000" />
    </label>
    <label v-if="action === 'TRANSFER'" class="dialog-field" for="case-escalation-assignee">
    ...
    <label v-if="action !== 'REQUEST'" class="dialog-field" for="case-escalation-action-reason">
      <span>Основание</span>
      <Textarea id="case-escalation-action-reason" v-model="reason" rows="3" maxlength="1000" />
    </label>
