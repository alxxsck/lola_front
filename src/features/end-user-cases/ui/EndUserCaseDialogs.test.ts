...
describe('EndUserCaseDialogs', () => {
  ...
  it('requires an explicit reason and submits a confirmed workflow transition', async () => {
    ...
    expect(store.transition).toHaveBeenCalledWith('RESOLVED', 'Проверено администратором');
  });
  ...
  it('submits classification and unlink corrections with mandatory reasons', async () => {
    ...
    expect(wrapper.findAllComponents({ name: 'Select' })[0]!.props('options')).toEqual([
    ...
    await wrapper
      ...
      .trigger('click');
    expect(store.unlinkMessage).toHaveBeenCalledWith('message-2', 'Не относится к цели');
  });
  ...
  it('keeps merge candidates tenant-user scoped and routes to a successful split', async () => {
    ...
    wrapper.findComponent({ name: 'MultiSelect' }).vm.$emit('update:modelValue', ['case-2']);
    await wrapper.vm.$nextTick();
    ...
    expect(splitSelectors[0]!.props('options')[0].label).toBe('Пользователь: first');
    expect(
      splitSelectors[1]!.props('options').map(({ label }: { label: string }) => label),
    ).toEqual([
    ...
    await wrapper
      ...
      .trigger('click');
    await flushPromises();
    expect(store.split).toHaveBeenCalledWith(['message-2'], 'Новая цель', 'Другая цель', 'REFUND', [
      'evidence-1',
    ]);
