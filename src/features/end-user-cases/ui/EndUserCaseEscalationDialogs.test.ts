...
describe('EndUserCaseEscalationDialogs', () => {
  ...
  it('creates a request from the business reason and operator summary only', async () => {
    ...
    expect(wrapper.get('section').attributes('data-header')).toBe('Позвать специалиста');
    expect(wrapper.find("label[for='case-escalation-reason']").exists()).toBe(true);
  ...
  it('normalizes a long summary and surfaces a bounded mutation error', async () => {
    ...
    expect((wrapper.get('textarea').element as HTMLTextAreaElement).value).toHaveLength(1_000);
    await wrapper
      .findAll('button')
      .find((button) => button.text() === 'Отправить запрос')!
      .trigger('click');
  ...
  it('wires release, close and cancel to their typed commands', async () => {
    ...
    await release
      ...
      .trigger('click');
    expect(store.releaseEscalation).toHaveBeenCalledWith('Вернуть профильной очереди');
    ...
    await close
      ...
      .trigger('click');
    expect(store.closeEscalation).toHaveBeenCalledWith('RESOLVED', 'Результат подтверждён');
    ...
    await cancel
      ...
      .trigger('click');
    expect(store.cancelEscalation).toHaveBeenCalledWith('WAITING_SYSTEM', 'Запрос создан ошибочно');
  });
});
