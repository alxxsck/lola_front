...
describe('EndUserCaseDetail', () => {
  it('organizes the case into overview, actions and tabbed related context', async () => {
    ...
    expect(wrapper.get('[data-test="status-select"]').text()).not.toContain('Нужен администратор');
    await wrapper.get('[data-test="status-select"]').trigger('click');
    expect(wrapper.emitted('requestTransition')?.[0]).toEqual(['IN_PROGRESS']);
    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toContain('Доказательства');
    expect(wrapper.find('#case-messages-panel').exists()).toBe(true);
    await wrapper.get('#case-messages-tab').trigger('keydown', { key: 'ArrowRight' });
    await new Promise((resolve) => requestAnimationFrame(resolve));

    expect(wrapper.get('#case-history-tab').attributes('aria-selected')).toBe('true');
    ...
    expect(wrapper.get('#case-messages-tab').attributes('aria-selected')).toBe('true');
    wrapper.unmount();
  });
  ...
  it('opens each evidence item in its own canonical conversation', () => {
    ...
    expect(targets.every((target) => target.includes('"name":"users"'))).toBe(true);
  });
