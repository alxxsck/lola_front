...
describe('EndUserCaseFilters', () => {
  ...
  it('emits every server-side filter and clears empty selections', async () => {
    ...
    field('Код инструмента Retenive').vm.$emit('update:modelValue', 'check_deposit');
    ...
    field('Активность до').vm.$emit('update:modelValue', '2026-07-31T23:59:59Z');
    await wrapper.vm.$nextTick();

    const values = wrapper.emitted('update:modelValue')!.map((event) => event[0]);
    expect(values).toContainEqual(expect.objectContaining({ status: ['WAITING_ADMIN'] }));
    expect(values).toContainEqual(expect.objectContaining({ priority: ['CRITICAL'] }));
    expect(values).toContainEqual(expect.objectContaining({ impact: ['HIGH'] }));
    expect(values).toContainEqual(expect.objectContaining({ urgency: ['IMMEDIATE'] }));
    expect(values).toContainEqual(
      expect.objectContaining({ resolutionAssessment: ['LIKELY_RESOLVED'] }),
    );
    expect(values).toContainEqual(expect.objectContaining({ resolutionSource: ['AI_INFERENCE'] }));
    expect(values).toContainEqual(expect.objectContaining({ groupCode: 'DEPOSIT' }));
    expect(values).toContainEqual(expect.objectContaining({ assignedCmsUserId: 'admin-id' }));
    expect(values).toContainEqual(expect.objectContaining({ endUserId: 'user-id' }));
    expect(values).toContainEqual(expect.objectContaining({ assignment: 'ASSIGNED' }));
    expect(values).toContainEqual(expect.objectContaining({ primaryLanguage: 'es' }));
    expect(values).toContainEqual(expect.objectContaining({ adminAttention: 'OPEN' }));
    expect(values).toContainEqual(expect.objectContaining({ cmsParticipation: 'YES' }));
    expect(values).toContainEqual(expect.objectContaining({ recontacted: 'YES' }));
    ...
    expect(values).toContainEqual(expect.objectContaining({ channel: ['VOICE'] }));
    expect(values).toContainEqual(expect.objectContaining({ aiCapabilityCode: 'check_deposit' }));
    expect(values).toContainEqual(expect.objectContaining({ aiCapabilityOutcome: ['COMPLETED'] }));
    expect(values).toContainEqual(
    ...
    expect(cleared[0]?.[0]).toEqual(expect.objectContaining({ status: undefined }));
    expect(cleared[1]?.[0]).toEqual(expect.objectContaining({ groupCode: undefined }));
  });
});
