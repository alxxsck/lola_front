...
function add() {
  emit('update:modelValue', [
    ...props.modelValue,
    { path: props.paths[0] ?? 'user.segment', operator: 'eq', value: '' },
  ]);
}
  ...
  <div class="conditions">
    <div v-for="(condition, index) in modelValue" :key="index" class="condition-row">
      <Select
        ...
        @update:model-value="update(index, { path: $event })"
      />
      <Select
        ...
        option-label="label"
        option-value="value"
        @update:model-value="update(index, { operator: $event })"
      />
      <InputText
...
.condition-row {
  display: grid;
  grid-template-columns: minmax(0, 1.25fr) 120px minmax(0, 0.75fr) 36px;
  gap: 7px;
  align-items: center;
}
.condition-row :deep(.p-select),
