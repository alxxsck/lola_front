...
export function scenarioApiErrorMessage(
  cause: unknown,
  fallback = 'Не удалось сохранить сценарий',
): string {
  ...
  const translationMessages: Record<string, string> = {
    TRANSLATION_DISABLED: 'Автоматический перевод временно недоступен. Заполните переводы вручную.',
    TRANSLATION_LOCALE_UNSUPPORTED:
      'Один из языков больше не поддерживается. Обновите каталог и повторите перевод — введённый текст сохранён.',
    TRANSLATION_BUDGET_EXCEEDED:
      'Лимит автоматических переводов проекта исчерпан. Проверьте бюджет в статистике переводов в настройках проекта.',
    TRANSLATION_GLOBAL_BUDGET_EXCEEDED:
      'Общий лимит автоматических переводов исчерпан. Проверьте бюджет в статистике переводов в настройках проекта.',
    TRANSLATION_IDEMPOTENCY_CONFLICT:
      'Запрос перевода изменился. Проверьте исходный текст и запустите перевод ещё раз.',
