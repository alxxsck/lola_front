...
vi.mock('@vue-flow/core', async (importOriginal) => ({
  ...
  useVueFlow: () => ({
    findNode: vi.fn((id: string) => id === 'decision' ? {
...
describe('ScenarioFlowControls', () => {
  ...
  it('offers named viewport commands without depending on edit or layout mode', async () => {
    ...
    expect(wrapper.get('[aria-label="Скрыть мини-карту"]').attributes())
      .toMatchObject({
        'aria-controls': 'scenario-graph-minimap',
        'aria-expanded': 'true',
      })
    expect(wrapper.get('[aria-label="Текущий масштаб 82%. Сбросить до 100%"]')
      .text()).toBe('82%')
    await wrapper.get('[aria-label="Текущий масштаб 82%. Сбросить до 100%"]')
      .trigger('click')
  ...
  it('disables selection commands and omits the minimap toggle for a small graph', () => {
    ...
    expect(wrapper.get('[aria-label="Показать выбранную ветку"]').attributes('disabled'))
      .toBeDefined()
    expect(wrapper.get('[aria-label="Центрировать выбранное действие"]').attributes('disabled'))
      .toBeDefined()
