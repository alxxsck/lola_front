...
describe('ScenarioFlowEdge', () => {
  it('renders timeout as a dashed route with an icon label chip', () => {
    ...
    expect(wrapper.get('.scenario-edge-label-timeout').attributes('data-branch-id'))
      .toBe('timeout')
  ...
  it('renders fallback as a dotted route with a non-color icon cue', () => {
    ...
    expect(wrapper.get('.scenario-edge-label-fallback').attributes('data-branch-kind'))
      .toBe('fallback')
    expect(wrapper.find('.scenario-edge-label-fallback .pi-ellipsis-h').exists())
      .toBe(true)
