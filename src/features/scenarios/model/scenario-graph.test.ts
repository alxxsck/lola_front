...
describe('scenario graph model', () => {
  ...
  it('derives labelled choice and timeout transitions', () => {
    ...
    expect(graphTransitions(actions).map(({ target, label }) => ({ target, label }))).toEqual([
      { target: 'yes', label: 'Да' },
      { target: 'no', label: 'Нет' },
      { target: 'no', label: 'Тайм-аут' },
      { target: 'finish', label: undefined },
      { target: 'finish', label: undefined },
    ]);
  ...
  it('falls back from the requested locale to the default and keeps ids while labels change', () => {
    const actions = normalizeScenarioActions([
      {
        ...
        type: 'ASK_CHOICE',
        config: {
          message: { ru: 'Продолжить?' },
          timeoutMs: 30_000,
          onTimeout: 'done',
          options: [{ id: 'yes', label: { ru: 'Да' }, nextNodeKey: 'done' }],
        },
      },
      { position: 1, nodeKey: 'done', type: 'COMPLETE_SCENARIO', config: {} },
    ]);
    ...
    expect(after.map(({ branchId }) => branchId)).toEqual(before.map(({ branchId }) => branchId));
    ...
    const actions = normalizeScenarioActions([
      {
        ...
        config: {
          branches: [
            {
              conditions: [{ path: 'user.country', operator: 'eq', value: 'ES' }],
              nextNodeKey: 'done',
            },
            {
              conditions: [{ path: 'user.country', operator: 'eq', value: 'PT' }],
              nextNodeKey: 'done',
            },
          ],
  ...
  it('derives both durable WAIT_FOR_GOAL branches and validates their references', () => {
    ...
    expect(validateScenarioGraph(actions).map((issue) => issue.message)).toContain(
      'Переход ведёт в неизвестный узел «removed_node»',
    );
  });
  ...
  it('reports backward, missing and unreachable transitions before save', () => {
    ...
    expect(validateScenarioGraph(actions).map((issue) => issue.message)).toEqual(
      expect.arrayContaining([
        expect.stringContaining('неизвестный узел'),
        expect.stringContaining('только вперёд'),
        expect.stringContaining('недостижим'),
      ]),
    );
  });
  ...
  it('offers earlier nodes when they do not create a cycle and sorts them after the source', () => {
    const actions = normalizeScenarioActions([
      ...
      { position: 1, nodeKey: 'answer', nextNodeKey: null, type: 'SAY', config: {} },
      {
        ...
        nodeKey: 'question',
        type: 'ASK_CHOICE',
        config: { message: 'Choose', options: [], timeoutMs: 30_000 },
      },
    ]);
  ...
  it('renames WAIT_FOR_GOAL and ordinary graph references atomically', () => {
    ...
    expect(graphTransitions(actions).some((transition) => transition.target === 'done')).toBe(
      false,
    );
  });
