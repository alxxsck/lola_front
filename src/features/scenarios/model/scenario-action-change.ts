...
export function planScenarioEntryPointChange(
  ...
  const reachableActions = ordered.filter(
    (action) => action.nodeKey && reachable.has(action.nodeKey),
  );
  const unreachableActions = ordered.filter(
    (action) => !action.nodeKey || !reachable.has(action.nodeKey),
  );
  ...
  return {
    ...
    unreachableNodeKeys: unreachableActions.flatMap((action) =>
      action.nodeKey ? [action.nodeKey] : [],
    ),
...
export function planScenarioActionTypeReplacement(
  ...
  const transitionImpact =
    outgoingCount === 0 ? 'none' : compatibleTransitionContract ? 'preserved' : 'reset-required';
  ...
  return {
    replacement: {
      ...
      config,
      nextNodeKey:
        compatibleTransitionContract && targetContract === 'linear'
