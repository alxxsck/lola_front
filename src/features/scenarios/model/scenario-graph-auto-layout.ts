...
export type ScenarioGraphLabelMeasurer = (
  input: ScenarioGraphLabelMeasureInput,
...
function actionPorts(node: Node) {
  return node.type === 'scenario'
    ? (node.data as ScenarioGraphNodeData).ports
    : []
}
...
export const conservativeScenarioGraphLabelSize: ScenarioGraphLabelMeasurer = ({
  ...
  const iconWidth = ['timeout', 'goal-timeout', 'fallback', 'goal'].includes(kind)
    ...
    : 0
  return {
    width: Math.ceil(Array.from(label).length * metrics.fontSize * 1.05)
      + metrics.paddingX * 2
      + iconWidth
      + 2,
...
function labelSize(
  edge: Edge<ScenarioGraphEdgeData>,
  measureLabel: ScenarioGraphLabelMeasurer,
) {
...
function toElkGraph(
  ...
  const children: ElkNode[] = viewModel.nodes.map((node) => {
    ...
    return {
      ...
      width: size.width,
      height: size.height,
      layoutOptions: node.id === 'trigger'
        ? undefined
        : { 'elk.portConstraints': 'FIXED_ORDER' },
      ports: node.id === 'trigger'
        ? []
        : [
            {
  ...
  const edges: ElkExtendedEdge[] = viewModel.edges.map((edge) => {
    ...
    return {
      ...
      labels: edge.data?.label && size
        ? [{ text: edge.data.label, ...size }]
        : undefined,
    }
  })
...
function laidOutPorts(node: Node, elkNode: ElkNode): ScenarioGraphNodeData | Node['data'] {
  ...
  return {
    ...
    ports: data.ports.map((port) => {
      ...
      return {
        ...port,
        position: Math.max(0, Math.min(100, Math.round(
...
export async function layoutScenarioGraphViewModel(
  ...
  try {
    const graph = await options.engine.layout(toElkGraph(
      viewModel,
      options.measureLabel ?? conservativeScenarioGraphLabelSize,
    ))
    return {
      status: 'laid-out',
