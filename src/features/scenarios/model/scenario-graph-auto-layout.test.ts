...
function expectEdgesFlowDown(
  model: Awaited<ReturnType<typeof layoutScenarioGraphViewModel>>['viewModel'],
) {
...
function segmentCrosses(
  ...
  if (leftVertical === rightVertical) {
    const sameAxis = leftVertical ? leftStart.x === rightStart.x : leftStart.y === rightStart.y;
  ...
  if (
    !between(point.y, vertical[0]!.y, vertical[1]!.y) ||
    !between(point.x, horizontal[0]!.x, horizontal[1]!.x)
  )
    return false;
  return !allowedSharedEndpoint(point);
}
...
function routeCrossings(edges: ScenarioGraphViewModel['edges']) {
  return edges.flatMap((left, index) =>
    edges.slice(index + 1).flatMap((right) => {
      const leftPoints = left.data?.routePoints ?? [];
      const rightPoints = right.data?.routePoints ?? [];
      const crossing = leftPoints
        .slice(1)
        .some((leftPoint, leftIndex) =>
          rightPoints
            .slice(1)
            .some((rightPoint, rightIndex) =>
              segmentCrosses(
...
describe('scenario graph ELK auto-layout adapter', () => {
  it('keeps audited choice lanes in semantic order without a crossing', async () => {
    ...
    expect(
      new Set(
        result.viewModel.edges
          .filter(({ source }) => source === 'question')
          .map(({ data }) => JSON.stringify(data?.routePoints)),
      ).size,
    ).toBe(3);
    expect(
      routeCrossings(result.viewModel.edges.filter(({ source }) => source === 'question')),
    ).toEqual([]);
  ...
  it('is deterministic and lays out 40 nodes within the 750 ms budget', async () => {
    ...
    expect(second.viewModel.nodes.map(({ id, position }) => ({ id, position }))).toEqual(
      first.viewModel.nodes.map(({ id, position }) => ({ id, position })),
    );
    expect(
      second.viewModel.edges.map(({ id, data }) => ({ id, route: data?.routePoints })),
  ...
  it('maps exact worker node, port, section and label output through the adapter seam', async () => {
    ...
    expect(receivedGraph?.edges?.find(({ id }) => id === 'question-choice:yes')?.labels).toEqual([
      { text: 'Да', width: 137, height: 31 },
    ]);
    expect(result.viewModel.nodes.find(({ id }) => id === 'question')?.position).toEqual({
      x: 320,
      y: 220,
    });
    expect(result.viewModel.nodes.find(({ id }) => id === 'question')?.data.ports).toEqual([
      { id: 'choice:yes', label: 'Да', position: 34 },
      { id: 'choice:no', label: 'Нет', position: 57 },
      { id: 'timeout', label: 'Тайм-аут', position: 81 },
    ]);
    expect(
      result.viewModel.edges.find(({ id }) => id === 'question-choice:yes')?.data,
    ).toMatchObject({
      routePoints: [
        { x: 120, y: 300 },
        { x: 120, y: 360 },
