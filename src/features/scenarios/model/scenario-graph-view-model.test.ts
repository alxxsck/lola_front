...
function actionNodeSize(model: ScenarioGraphViewModel, node: Node) {
  return node.id === 'trigger' ? model.layout.trigger : model.layout.node;
}

function overlappingPairs(model: ScenarioGraphViewModel) {
  return model.nodes.flatMap((left, index) =>
    model.nodes.slice(index + 1).flatMap((right) => {
...
describe('scenario graph view model', () => {
  it.each([
    ...
    'keeps $type presentation consistent across canvas and outline',
    ({ type, executor, kind, label, icon }) => {
  ...
  it.each(scenarioGraphFixtures)(
    'builds deterministic, non-overlapping $name coordinates and valid routes',
    ({ actions }) => {
  ...
  it('keeps parallel branch identity and label separate from the route target', () => {
    ...
    expect(
      edges
        .map(({ data }) => data!)
        .map(({ branchId, kind, label }) => ({ branchId, kind, label })),
    ).toEqual([
    ...
    expect(
      edges
        .map(({ data }) => data!)
        .map(({ routeIndex, routeCount }) => ({ routeIndex, routeCount })),
    ).toEqual([
  ...
  it('classifies action, decision, wait and terminal nodes as presentation semantics', () => {
    ...
    expect(
      model.nodes.slice(1).map(({ id, data }) => ({
        id,
        kind: data.kind,
