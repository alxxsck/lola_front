...
function localizedText(value: unknown, options: GraphTransitionDisplayOptions): string {
...
export function normalizeScenarioActions(actions: ScenarioAction[]): ScenarioAction[] {
  ...
  return ordered.map((action, position) => ({
    ...
    nextNodeKey: action.type === 'WAIT_FOR_GOAL' ? null : (action.nextNodeKey ?? null),
    config: structuredClone(action.config),
  }));
}

export function renameScenarioNode(
  actions: ScenarioAction[],
  oldKey: string,
  newKey: string,
): boolean {
  const action = actions.find((item) => item.nodeKey === oldKey);
  if (!action || !newKey || actions.some((item) => item !== action && item.nodeKey === newKey))
    return false;
  action.nodeKey = newKey;
  for (const item of actions) {
    if (item.nextNodeKey === oldKey) item.nextNodeKey = newKey;
    if (item.type === 'ASK_CHOICE') {
      item.config.options = choiceOptions(item).map((option) =>
        option.nextNodeKey === oldKey ? { ...option, nextNodeKey: newKey } : option,
      );
      if (item.config.onTimeout === oldKey) item.config.onTimeout = newKey;
    }
    if (item.type === 'CONDITION') {
      item.config.branches = conditionBranches(item).map((branch) =>
        branch.nextNodeKey === oldKey ? { ...branch, nextNodeKey: newKey } : branch,
      );
    ...
    .map((option) => ({
      ...
      label:
        typeof option.label === 'string'
          ? option.label
          : Object.fromEntries(
              Object.entries(record(option.label)).filter(
...
export function choiceReminders(action: ScenarioAction): ChoiceReminder[] {
  ...
  return reminders
    .map((reminder) => record(reminder))
    .map((reminder) => ({
      afterMs: typeof reminder.afterMs === 'number' ? reminder.afterMs : 10_000,
      actions: Array.isArray(reminder.actions)
        ? reminder.actions
  ...
  return branches
    ...
    .map((branch) => ({
      conditions: Array.isArray(branch.conditions)
        ? (branch.conditions as ScenarioCondition[])
        : [],
  ...
  actions.forEach((action) => {
    ...
    if (action.type === 'WAIT_FOR_GOAL') {
      ...
      if (typeof config.onGoal === 'string' && config.onGoal) {
        transitions.push({
          source,
          target: config.onGoal,
          branchId: 'goal',
          label: GRAPH_SYSTEM_LABELS.goal,
          kind: 'goal',
        });
      }
      if (typeof config.onTimeout === 'string' && config.onTimeout) {
        transitions.push({
          source,
          target: config.onTimeout,
          branchId: 'goal-timeout',
          label: GRAPH_SYSTEM_LABELS.goalTimeout,
          kind: 'goal-timeout',
        });
      }
      return;
    ...
    if (action.type === 'ASK_CHOICE') {
      const options = choiceOptions(action)
        .filter((option) => option.nextNodeKey)
        .map((option) => ({
          source,
          target: option.nextNodeKey,
          branchId: `choice:${option.id}`,
          label: localizedText(option.label, display) || option.id,
        ...
        ...(typeof timeout === 'string' && timeout
          ? [
              ...options,
              {
                ...
                target: timeout,
                branchId: 'timeout',
                label: GRAPH_SYSTEM_LABELS.timeout,
    ...
    if (action.type === 'CONDITION') {
      const branches = conditionBranches(action)
        .filter((branch) => branch.nextNodeKey)
        .map((branch, index) => ({
          source,
          target: branch.nextNodeKey,
          branchId: `condition:${index}`,
          label: `${GRAPH_SYSTEM_LABELS.condition} ${index + 1}`,
        ...
        ...(typeof fallback === 'string' && fallback
          ? [
              ...branches,
              {
                ...
                target: fallback,
                branchId: 'fallback',
                label: GRAPH_SYSTEM_LABELS.fallback,
    ...
    if (action.nextNodeKey)
      transitions.push({
        source,
        target: action.nextNodeKey,
        branchId: 'default',
        kind: 'default',
      });
  });
...
export function availableTargets(actions: ScenarioAction[], action: ScenarioAction) {
  ...
  return actions
    ...
    .map((candidate) => ({
      label: candidate.nodeKey ?? `Шаг ${candidate.position + 1}`,
...
export function sortScenarioActions(actions: ScenarioAction[]): ScenarioAction[] {
  ...
  if (byKey.size !== ordered.length || byKey.has(''))
    return ordered.map((action, position) => ({ ...action, position }));
  ...
  for (const transition of graphTransitions(ordered)) {
    if (!byKey.has(transition.target) || outgoing.get(transition.source)?.has(transition.target))
      continue;
...
export function validateScenarioGraph(actions: ScenarioAction[]): GraphValidationIssue[] {
  ...
  ordered.forEach((action, index) => {
    ...
    if (action.position !== index)
      issues.push({ nodeKey: key, message: 'Позиции узлов должны идти непрерывно от 0' });
    if (!NODE_KEY_PATTERN.test(key))
      issues.push({
        nodeKey: key,
        message: 'Ключ: латиница в нижнем регистре, цифры, _ или -, начиная с буквы',
      });
    if (keys.indexOf(key) !== index)
      issues.push({ nodeKey: key, message: `Ключ «${key}» повторяется` });
    if (action.type === 'ASK_CHOICE') {
      const options = choiceOptions(action);
      if (!options.length) issues.push({ nodeKey: key, message: 'Добавьте вариант ответа' });
      const optionIds = options.map((option) => option.id);
      if (new Set(optionIds).size !== optionIds.length)
        issues.push({ nodeKey: key, message: 'ID вариантов ответа должны быть уникальны' });
      if (options.some((option) => !NODE_KEY_PATTERN.test(option.id)))
        issues.push({
          nodeKey: key,
          message: 'ID варианта должен соответствовать формату ключа узла',
        });
      const config = record(action.config);
      if (!hasLocalizedText(config.message))
        issues.push({ nodeKey: key, message: 'Заполните текст вопроса' });
      if (typeof config.timeoutMs !== 'number' || config.timeoutMs < 1_000)
        issues.push({ nodeKey: key, message: 'Timeout должен быть не меньше 1000 мс' });
      if (typeof config.onTimeout !== 'string' || !config.onTimeout)
        issues.push({ nodeKey: key, message: 'Выберите переход по timeout' });
    }
    if (action.type === 'CONDITION') {
      const branches = conditionBranches(action);
      if (!branches.length)
        issues.push({ nodeKey: key, message: 'Добавьте минимум одну ветку условия' });
      if (branches.some((branch) => !branch.conditions.length))
        issues.push({ nodeKey: key, message: 'Каждая ветка должна содержать условие' });
      if (!record(action.config).fallbackNodeKey)
        issues.push({ nodeKey: key, message: 'Выберите запасной переход' });
    }
    if (action.type === 'WAIT_FOR_GOAL') {
      const config = record(action.config);
      if (typeof config.onGoal !== 'string' || !config.onGoal)
        issues.push({ nodeKey: key, message: 'Выберите ветку «Цель достигнута»' });
      if (typeof config.onTimeout !== 'string' || !config.onTimeout)
        issues.push({ nodeKey: key, message: 'Выберите ветку «Срок истёк»' });
    }
  });
  ...
  for (const transition of transitions) {
    if (!positions.has(transition.target)) {
      issues.push({
        nodeKey: transition.source,
        message: `Переход ведёт в неизвестный узел «${transition.target}»`,
      });
    } else if (positions.get(transition.target)! <= positions.get(transition.source)!) {
      issues.push({
        nodeKey: transition.source,
        message: `Переход в «${transition.target}» должен вести только вперёд`,
      });
    }
  }
  ...
  for (const key of keys)
    if (!reachable.has(key))
      issues.push({ nodeKey: key, message: `Узел «${key}» недостижим из первого шага` });
  return issues;
}
