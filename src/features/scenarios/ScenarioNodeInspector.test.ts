...
describe('ScenarioNodeInspector', () => {
  it('replaces an action type and applies existing or newly created next actions through catalogs', async () => {
    ...
    wrapper.getComponent(ActionPicker).vm.$emit('update:modelValue', 'CLOSE_CHAT');
    ...
    expect(wrapper.emitted('createTarget')).toEqual([['CLOSE_CHAT', 'next', undefined]]);
  });
  ...
  it('offers enabled actions for both WAIT_FOR_GOAL branches when no target nodes exist', async () => {
    ...
    expect(branchPickers[0]!.props('options').map((option) => option.actionType)).toEqual([
      'WAIT_FOR_GOAL',
      'SAY',
      'CLOSE_CHAT',
    ]);
    ...
    branchPickers[0]!.vm.$emit(
      'select',
      branchPickers[0]!.props('options').find((option) => option.actionType === 'SAY'),
    );
    branchPickers[1]!.vm.$emit(
      'select',
      branchPickers[1]!.props('options').find((option) => option.actionType === 'CLOSE_CHAT'),
    );
    await wrapper.vm.$nextTick();
    ...
    await wrapper.get('input[aria-label="Порог цели"]').setValue('2');
