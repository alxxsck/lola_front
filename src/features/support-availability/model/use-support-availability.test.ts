...
describe('support availability controller', () => {
  it('keeps a manually declared AVAILABLE state after workspace teardown and two minutes away', async () => {
    ...
    const firstWorkspace = createSupportAvailabilityController(context, source({ read, setOwn }));
    ...
    expect(reopenedWorkspace.availability.value?.effectiveState).toBe('AVAILABLE');
  ...
  it('does not commit an availability snapshot for another operator', async () => {
    ...
    expect(controller.error.value).toBe('Статус доступности вернул данные другого сотрудника');
  });
