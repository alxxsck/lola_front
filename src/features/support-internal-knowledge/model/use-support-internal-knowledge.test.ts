...
function setup() {
  ...
  const controller = createSupportInternalKnowledgeController(
    { scope: () => scope, allowed: () => allowed, canInsert: () => canInsert, onInsert, onForbidden },
    source,
  );
  return {
    controller,
    ...
    onInsert,
    onForbidden,
    setScope: (next: SupportKnowledgeScope | null) => { scope = next; },
    deny: () => { allowed = false; },
    denyInsert: () => { canInsert = false; },
  };
}
...
describe("Support Internal Knowledge controller", () => {
  ...
  it("creates an editable citation and pins the final text before send", async () => {
    ...
    await expect(controller.prepareForSend("Ответ оператора\n\nПроверьте статус операции.")).resolves.toBe("citation-1");
  ...
  it("does not restore protected text after an in-flight insert is discarded", async () => {
    ...
    const pending = controller.insert(
      item,
      "QUOTE",
      "Проверьте статус операции.",
    );
  ...
  it("fails closed when citation edit changes immutable identity", async () => {
    ...
    await expect(
      controller.prepareForSend("Отредактированный ответ"),
  ...
  it("purges the protected projection on concealed access loss", async () => {
    ...
    controller.query.value = "депозит";
    await controller.search();
    vi.mocked(source.open).mockRejectedValueOnce(new ApiError(404, "NOT_FOUND_OR_FORBIDDEN", "hidden"));
  ...
  it("fences a late response from the previous Case", async () => {
    ...
    vi.mocked(source.search).mockReturnValueOnce(new Promise((done) => { resolve = done; }));
    controller.query.value = "депозит";
