...
function selectValue(value: string, field: ScenarioAuthoringField | undefined): unknown {
  if (field?.valueType === 'number' || field?.valueType === 'integer') return value === '' ? '' : Number(value)
...
function listValues(event: Event, field: ScenarioAuthoringField | undefined) {
  return (event.target as HTMLInputElement).value.split(',').map((value) => value.trim()).filter(Boolean).map((value) => selectValue(value, field))
}
...
function compareInputValue(event: Event) {
  ...
  if (value === '') return ''
  if (selectedMeasureField.value?.semanticType?.startsWith('money')) return value
...
function aggregateCompareValueValid() {
  ...
  if (semanticType?.startsWith('money')) {
    return typeof value === 'string' && /^-?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?$/.test(value)
      && (semanticType !== 'money_minor' || !value.includes('.'))
  }
    ...
    <form class="leaf-form" @submit.prevent="apply">
      ...
      <template v-if="buffer.kind === 'eventField'">
        ...
        <section class="form-section"><span class="section-number">2</span><div class="section-fields">
          ...
          <div v-if="selectedField && buffer.operator && needsEventValue" class="field"><label>С каким значением?</label><select v-if="selectedField.allowedValues?.length && buffer.operator === 'in'" multiple :value="Array.isArray(buffer.value) ? buffer.value.map(String) : []" :aria-label="`Значение поля ${selectedField.label}`" :aria-invalid="issueMatches('value')" :aria-describedby="issueDescription('value')" @change="buffer.value = selectValues($event, selectedField)"><option v-for="value in selectedField.allowedValues" :key="String(value)" :value="String(value)">{{ value }}</option></select><select v-else-if="selectedField.allowedValues?.length" :value="buffer.value" :aria-label="`Значение поля ${selectedField.label}`" :aria-invalid="issueMatches('value')" :aria-describedby="issueDescription('value')" @change="buffer.value = selectValue(($event.target as HTMLSelectElement).value, selectedField)"><option value="">Выберите значение</option><option v-for="value in selectedField.allowedValues" :key="String(value)" :value="String(value)">{{ value }}</option></select><input v-else-if="buffer.operator === 'in'" :value="listText(buffer.value)" type="text" placeholder="Значения через запятую" :aria-label="`Значение поля ${selectedField.label}`" :aria-invalid="issueMatches('value')" :aria-describedby="issueDescription('value')" @input="buffer.value = listValues($event, selectedField)" /><input v-else :value="buffer.value" :type="selectedField.valueType === 'number' || selectedField.valueType === 'integer' ? 'number' : 'text'" :aria-label="`Значение поля ${selectedField.label}`" :aria-invalid="issueMatches('value')" :aria-describedby="issueDescription('value')" @input="buffer.value = inputValue($event, selectedField)" /><small v-if="buffer.operator === 'in'">Можно выбрать несколько значений.</small><small v-if="selectedField.unit">Единица: {{ selectedField.unit }}. Значение не конвертируется.</small><small v-if="selectedField.sensitive">Поле помечено как чувствительное. Preview может скрыть фактическое значение.</small></div>
        </div></section>
      </template>
...
<style scoped>
.drawer-title small{color:var(--muted);font-size:.64rem;text-transform:uppercase;letter-spacing:.1em}.drawer-title h2{margin-top:3px;font-size:1.05rem}.leaf-form{display:flex;flex-direction:column;gap:14px;min-height:100%}.leaf-form>section{display:grid;grid-template-columns:28px minmax(0,1fr);gap:11px;padding:14px;border:1px solid var(--line);border-radius:14px;background:#fafaf8}.section-number{display:grid;place-items:center;width:28px;height:28px;border-radius:9px;background:#ece8ff;color:#6550c8;font-size:.65rem;font-weight:800}.leaf-form h3,.leaf-form h4{margin:0}.leaf-form h3{font-size:.82rem}.leaf-form h4{font-size:.72rem}.leaf-form p{margin:4px 0 0;color:var(--muted);font-size:.67rem}.leaf-form code{display:block;margin-top:5px;color:#777c72;font-size:.63rem}.section-fields{display:flex;flex-direction:column;gap:12px;min-width:0}.field{display:flex;flex-direction:column;gap:6px;min-width:0}.field label{font-size:.7rem;font-weight:700;color:#595e55}.field small{color:var(--muted);font-size:.63rem}.leaf-form select,.leaf-form input{width:100%;min-width:0;border:1px solid #d8dad3;border-radius:10px;background:#fff;padding:9px 10px;color:var(--ink);font:inherit;font-size:.72rem}.filters{padding:11px;border:1px solid #e2e4dd;border-radius:12px;background:#fff}.filters>header{display:flex;align-items:flex-start;justify-content:space-between;gap:8px}.filters>header button{border:0;border-radius:8px;background:#f0ecff;color:#6650ca;padding:7px;font-size:.65rem;font-weight:700}.filter-row{display:grid;grid-template-columns:1fr .85fr 1fr 30px;gap:6px;margin-top:9px}.icon-button{display:grid;place-items:center;border:0;border-radius:8px;background:transparent;color:#777c72;cursor:pointer}.icon-button.danger{color:#bd5647}.no-value{align-self:center;color:var(--muted);font-size:.64rem}.period-row,.compare-row{display:grid;grid-template-columns:1fr 1fr;gap:9px}.help{padding:9px;border-radius:9px;background:#f0f1ec;color:#62675e!important}.drawer-actions{position:sticky;bottom:0;display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:auto;padding:12px 0 2px;background:#fff}.drawer-actions button{min-height:42px;border-radius:11px;font-weight:700;cursor:pointer}.drawer-actions .secondary{border:1px solid var(--line);background:#fff;color:#555a51}.drawer-actions .primary{border:1px solid #765fe0;background:#765fe0;color:#fff}.drawer-actions .primary:disabled{opacity:.45;cursor:not-allowed}
