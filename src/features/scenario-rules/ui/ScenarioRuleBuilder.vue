...
const editedNode = computed(() =>
  editorSession.value?.nodeId
    ? findNode(props.modelValue.root, editorSession.value.nodeId)
    : undefined,
);
const recipeLimitReached = computed(
  () =>
    summary.value.nodes >= RULE_LIMITS.maxNodes ||
    summary.value.leaves >= RULE_LIMITS.maxLeaves ||
    summary.value.aggregateLeaves >= RULE_LIMITS.maxAggregateLeaves ||
    ((props.modelValue.root.kind === 'all' || props.modelValue.root.kind === 'any') &&
      props.modelValue.root.children.length >= RULE_LIMITS.maxGroupChildren),
);
const recipes = computed(() => createRuleQuickStartRecipes(props.context));

function collectGroups(
  node: RuleDraftNode,
  result: Array<{ nodeId: string; label: string; childCount: number }> = [],
) {
  if (node.kind === 'all' || node.kind === 'any') {
    result.push({
      nodeId: node.nodeId,
      label: `${node.kind === 'all' ? 'Должны выполняться все условия' : 'Достаточно одного условия'} · ${summary.value.byNodeId[node.nodeId] ?? ''}`,
...
function findParent(node: RuleDraftNode, nodeId: string): RuleDraftNode | undefined {
  if (
    ...
    node.children.some((child) => child.nodeId === nodeId)
  )
    return node;
...
function runCommand(command: RuleCommand) {
  const previousParent =
    command.type === 'remove' ? findParent(props.modelValue.root, command.nodeId) : undefined;
  ...
  void nextTick(() => {
    const focusId =
      command.type === 'remove'
        ? (previousParent?.nodeId ?? result.focusNodeId)
        : result.focusNodeId;
...
function focusNode(nodeId?: string) {
  ...
  document
    .querySelector<HTMLElement>(
      `[data-rule-node="${nodeId}"] > section, [data-rule-node="${nodeId}"] > article`,
    )
    ?.focus();
}
...
function issueControlSelector(fieldPath?: string) {
  ...
  const label =
    filter[2] === 'fieldKey' ? 'Поле' : filter[2] === 'operator' ? 'Оператор' : 'Значение';
...
function focusIssue(target: { nodeId?: string; fieldPath?: string; message?: string }) {
  ...
  const opener = document.querySelector<HTMLElement>(
    `[data-rule-node="${target.nodeId}"] > section, [data-rule-node="${target.nodeId}"] > article`,
  );
  ...
  activeIssue.value = target.message
    ? { fieldPath: target.fieldPath, message: target.message }
    : null;
  editorSession.value = { kind: kind as PartialRuleLeaf['kind'], nodeId: node.nodeId, opener };
  void nextTick(() =>
    document.querySelector<HTMLElement>(issueControlSelector(target.fieldPath))?.focus(),
  );
}
...
<template>
  <section class="rule-builder" aria-labelledby="rule-builder-title">
    <Message v-if="commandError" severity="error" :closable="false" role="alert">{{
      commandError
    }}</Message>
    <p class="sr-live" aria-live="polite">
      {{ commandError || (serialization.ok ? 'Условия заполнены' : 'Черновик условий изменён') }}
    </p>
      ...
      <div class="recipe-actions">
        ...
        >
          ...
          <span
            ...
            ><small>{{ recipe.description }}</small></span
          >
    ...
    <details class="glossary">
      <summary>Как работают группы условий?</summary>
      <dl>
        <div>
          <dt>Должны выполняться все условия</dt>
          <dd>Пользователь подходит, только если выполнено каждое условие группы.</dd>
        </div>
        <div>
          <dt>Достаточно одного условия</dt>
          <dd>Пользователь подходит, если выполнено хотя бы одно условие группы.</dd>
        </div>
        <div>
          <dt>Исключение</dt>
          <dd>Пользователь подходит, если выбранное условие не выполнено.</dd>
        </div>
      </dl>
    </details>
    ...
    <RuleSourcePicker
      v-if="sourceSession"
      :group-label="sourceSession.groupLabel"
      :aggregate-limit-reached="summary.aggregateLeaves >= RULE_LIMITS.maxAggregateLeaves"
      @select="chooseSource"
      @close="closeSources"
    />
    ...
    <aside class="builder-summary" aria-label="Итог условий">
      <div>
        <span>Результат</span><strong>{{ summary.text }}</strong>
      </div>
      <dl>
        <div>
          <dt>Условия</dt>
        ...
        <div>
          <dt>История</dt>
          <dd>{{ summary.aggregateLeaves }}</dd>
        </div>
        <div>
          <dt>Макс. период</dt>
          <dd>
            {{ summary.maxWindowMs ? `${Math.ceil(summary.maxWindowMs / 86_400_000)} дн.` : '—' }}
          </dd>
        </div>
      </dl>
      <details>
        <summary>Технические детали и лимиты</summary>
        <p>
          Версия каталога <code>{{ context.contract.revision }}</code
          >. Неизменяемые коды создаются автоматически.
        </p>
        <p>
          {{ summary.nodes }}/{{ RULE_LIMITS.maxNodes }} узлов · {{ summary.leaves }}/{{
            RULE_LIMITS.maxLeaves
          }}
          условий · {{ summary.aggregateLeaves }}/{{ RULE_LIMITS.maxAggregateLeaves }} условий
          истории и активности · {{ Math.ceil(summary.totalWindowMs / 86_400_000) }}/{{
            RULE_LIMITS.maxTotalWindowMs / 86_400_000
          }}
          дней суммарной истории.
        </p>
      </details>
    </aside>
...
<style scoped>
.rule-builder {
  container: rule-builder / inline-size;
  display: flex;
...
.recipe-panel[open] .recipe-chevron {
...
.recipe-actions button > i:first-child {
  display: grid;
  place-items: center;
  width: 26px;
  height: 26px;
...
.recipe-actions button > span {
  min-width: 0;
}
.recipe-actions button strong,
.recipe-actions button small {
  display: block;
}
.recipe-actions button strong {
  font-size: 0.65rem;
...
.builder-summary details summary {
  cursor: pointer;
}
...
@container rule-builder (max-width:1024px) {
  .rule-builder {
    padding: 18px;
  }
  .recipe-actions {
