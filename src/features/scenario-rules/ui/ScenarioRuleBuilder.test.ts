...
const contract: ScenarioAuthoringContract = {
  ...
  events: [
      ...
      fields: [
        {
          ...
          required: true,
          valueType: 'string',
          control: { type: 'select', options: ['promotions', 'profile'] },
          allowedValues: ['promotions', 'profile'],
          capabilities: {
            eventField: { operators: ['eq', 'neq', 'in'] },
            aggregateFilter: { operators: ['eq', 'neq', 'in'] },
    ...
    },
    {
      ...
      fields: [
        ...
        },
        {
          ...
          control: { type: 'select', options: ['EUR', 'USD'] },
          allowedValues: ['EUR', 'USD'],
          capabilities: {
            eventField: { operators: ['eq', 'in'] },
            aggregateFilter: { operators: ['eq', 'in'] },
            aggregateMeasure: { measures: [] },
          },
        },
      ],
    ...
    {
      code: 'registration.completed',
      definitionId: 'event-registration',
      definitionKeyId: 'key-registration',
...
async function addSource(wrapper: ReturnType<typeof mountBuilder>, source: string) {
  await wrapper
    .get('button[aria-label="Добавить условие в группу Должны выполняться все условия"]')
    .trigger('click');
...
async function selectHistoryEvent(wrapper: ReturnType<typeof mountBuilder>, code: string) {
  ...
  await flushPromises();
  const option = picker
    .findAll('[data-testid="event-picker-option"]')
    .find((candidate) => candidate.text().includes(code));
...
describe('ScenarioRuleBuilder', () => {
  ...
  it('names group logic as a clear outcome', () => {
    ...
    expect(wrapper.get('button[aria-label="Должны выполняться все условия"]').text()).toBe(
      'Должны выполняться все условия',
    );
    expect(wrapper.get('button[aria-label="Достаточно одного условия"]').text()).toBe(
  ...
  it('keeps aggregate choices catalog-compatible and never offers sinceTrigger', async () => {
    ...
    if (
      serializedMoney.ok &&
      serializedMoney.value.root.kind === 'all' &&
      serializedMoney.value.root.children[0]?.kind === 'eventAggregate'
    ) {
      expect(serializedMoney.value.root.children[0].compare.value).toBe('500');
      expect(serializedMoney.value.root.children[0].filters).toEqual([
        { fieldKey: 'deposit.currency', operator: 'eq', value: 'EUR' },
      ]);
    }

    await wrapper.get('button[aria-label^="Изменить условие:"]').trigger('click');
    await flushPromises();
    expect(wrapper.get('[data-testid="event-picker-trigger"]').text()).toContain(
      'deposit.succeeded',
    );
  ...
  it('discards a local leaf buffer and restores focus to its opener', async () => {
    const wrapper = mountBuilder();
    const opener = wrapper.get(
      'button[aria-label="Добавить условие в группу Должны выполняться все условия"]',
    );
    ...
    expect(wrapper.getComponent(ScenarioRuleBuilder).emitted('editing-dirty')?.at(-1)).toEqual([
      true,
    ]);
    ...
    expect(wrapper.find('input[aria-label="Количество активных дней подряд"]').exists()).toBe(
      false,
    );
    expect(wrapper.getComponent(ScenarioRuleBuilder).emitted('editing-dirty')?.at(-1)).toEqual([
      false,
  ...
  it('supports group logic, negation and keyboard-safe removal with focus recovery', async () => {
    const wrapper = mountBuilder();
    await wrapper
      .get('button[aria-label="Добавить группу в Должны выполняться все условия"]')
      .trigger('click');
    await wrapper.get('button[aria-label="Достаточно одного условия"]').trigger('click');
    expect(
      wrapper.get('button[aria-label="Достаточно одного условия"]').attributes('aria-pressed'),
    ).toBe('true');

    const nestedAdd = wrapper.findAll(
      'button[aria-label="Добавить условие в группу Достаточно одного условия"]',
    )[0]!;
  ...
  it('keeps the local buffer visible when a catalog refresh removes its dependency', async () => {
    ...
    const refreshedContract: ScenarioAuthoringContract = {
      ...
      events: contract.events.map((event) =>
        event.code === 'page.opened' ? { ...event, fields: [] } : event,
      ),
    };
    (wrapper.vm as unknown as { context: RuleDomainContext }).context = {
      ...context,
    ...
    expect(
      (wrapper.get('select[aria-label="Поле события запуска"]').element as HTMLSelectElement).value,
    ).toBe('page.name');
    expect(wrapper.get('button[aria-label="Применить условие"]').attributes()).toHaveProperty(
      'disabled',
    );
  });
  ...
  it('uses the exact trigger revision for fields and operators when event codes are duplicated', async () => {
    ...
    const duplicateContract: ScenarioAuthoringContract = {
      ...contract,
      events: [
        { ...contract.events[0]!, definitionId: 'event-page-old', fields: [] },
        ...contract.events,
      ],
    };
    (wrapper.vm as unknown as { context: RuleDomainContext }).context = {
    ...
    expect(wrapper.get('button[aria-label="Применить условие"]').attributes()).not.toHaveProperty(
      'disabled',
    );
  });
  ...
  it('opens the exact leaf control requested by a backend issue', async () => {
    ...
    wrapper
      ...
      .vm.focusIssue({ nodeId, fieldPath: 'compare.value', message: 'Укажите целое число дней.' });
    await flushPromises();
  ...
  it('does not create an empty group whose maximum depth prevents adding a child', async () => {
    ...
    const deepestConditionButton = wrapper
      .findAll('button[aria-label^="Добавить условие в группу"]')
      .at(0)!;
    expect(deepestGroupButton.attributes()).toHaveProperty('disabled');
    expect(deepestGroupButton.attributes('title')).toContain(
      'Новая группа не сможет содержать условия',
    );
  ...
  it('reopens an existing leaf from its edit button', async () => {
    ...
    expect(
      wrapper.get('input[aria-label="Количество активных дней подряд"]').element,
  ...
  it('authors catalog in-operators as typed arrays for fields and aggregate filters', async () => {
    ...
    await fieldWrapper
      .get('select[aria-label="Значение поля Страница"]')
      .setValue(['promotions', 'profile']);
    await fieldWrapper.get('button[aria-label="Применить условие"]').trigger('click');
    await flushPromises();
    expect(
      serializeRuleDraft((fieldWrapper.vm as unknown as { draft: RuleDraft }).draft, context).ok,
    ).toBe(true);
    ...
    await flushPromises();
    expect(
      serializeRuleDraft((filterWrapper.vm as unknown as { draft: RuleDraft }).draft, context).ok,
    ).toBe(true);
  });
  ...
  it('does not turn an empty aggregate threshold into zero or accept a fractional streak', async () => {
    ...
    expect(
      aggregateWrapper.get('button[aria-label="Применить условие"]').attributes(),
    ...
    expect(streakWrapper.get('button[aria-label="Применить условие"]').attributes()).toHaveProperty(
      'disabled',
    );
  });
});
