...
function cloneNode(node: RuleDraftNode): RuleDraftNode {
  ...
  if (node.kind === 'eventAggregate') {
    return {
      ...
      filters: node.filters.map((filter) => ({
        ...filter,
        ...(filter.value !== undefined ? { value: cloneValue(filter.value) } : {}),
      })),
      window: { ...node.window },
  ...
  if (node.kind === 'eventField')
    return { ...node, ...(node.value !== undefined ? { value: cloneValue(node.value) } : {}) };
    ...
    if (node.kind === 'all' || node.kind === 'any') {
      for (const [index, child] of node.children.entries()) {
        if (child.nodeId === nodeId)
          return { node: child, parent: { kind: 'group', parent: node, index } };
        const nested = visit(child);
        if (nested) return nested;
      }
    }
    if (node.kind === 'not') {
      if (node.child.nodeId === nodeId)
        return { node: node.child, parent: { kind: 'not', parent: node } };
  ...
  if (input.kind === 'eventAggregate') {
    return {
      ...
      filters: input.filters.map((filter) => ({
        ...cloneValue(filter),
        filterId: filter.filterId ?? uid('rule_filter'),
      })),
    };
  }
...
function createNode(input: NewRuleNode, context: RuleDomainContext): RuleDraftNode {
  ...
  if (input.kind === 'all' || input.kind === 'any')
    return { nodeId: uid('rule_node'), kind: input.kind, children: [] };
  if (input.kind === 'incomplete')
    return { nodeId: uid('rule_node'), kind: 'incomplete', leaf: cloneValue(input.leaf) };
...
function replaceAt(draft: RuleDraft, location: NodeLocation, replacement: RuleDraftNode) {
  ...
  else if (location.parent.kind === 'group')
    location.parent.parent.children[location.parent.index] = replacement;
...
function missingNode(draft: RuleDraft, nodeId: string): RuleCommandResult {
  return commandFailure(draft, {
    code: 'node-not-found',
    nodeId,
    message: 'Условие больше не найдено. Обновите редактор и повторите действие.',
  });
}
...
export function validateRuleLimits(root: RuleDraftNode): RuleCommandError | undefined {
  ...
  const visit = (node: RuleDraftNode, depth: number) => {
    ...
    if (depth > RULE_LIMITS.maxDepth) {
      firstError = {
        code: 'depth-limit',
        nodeId: node.nodeId,
        limit: RULE_LIMITS.maxDepth,
        message: `Допустима вложенность не более ${RULE_LIMITS.maxDepth} уровней. Переместите условие выше.`,
      };
      return;
    }
    if (node.kind === 'all' || node.kind === 'any') {
      if (node.children.length > RULE_LIMITS.maxGroupChildren) {
        firstError = {
          code: 'group-children-limit',
          nodeId: node.nodeId,
          limit: RULE_LIMITS.maxGroupChildren,
          message: `В одной группе допустимо не более ${RULE_LIMITS.maxGroupChildren} условий. Создайте дополнительную группу.`,
        };
        return;
      }
    ...
    if (intendedKind === 'eventAggregate' || intendedKind === 'activityDayStreak')
      aggregateLeaves += 1;
    if (node.kind === 'eventAggregate') {
      if (node.filters.length > RULE_LIMITS.maxFilters) {
        firstError = {
          code: 'filter-limit',
          nodeId: node.nodeId,
          limit: RULE_LIMITS.maxFilters,
          message: `Для одного условия истории допустимо не более ${RULE_LIMITS.maxFilters} фильтров.`,
        };
        return;
      }
      if (node.window.kind === 'last') {
        if (node.window.durationMs > RULE_LIMITS.maxWindowMs) {
          firstError = {
            code: 'window-limit',
            nodeId: node.nodeId,
            limit: RULE_LIMITS.maxWindowMs,
            message: 'Период одного условия истории не может превышать 90 дней.',
          };
          return;
        }
    ...
    if (node.kind === 'activityDayStreak' && node.compare.value > RULE_LIMITS.maxStreakDays) {
      firstError = {
        code: 'streak-limit',
        nodeId: node.nodeId,
        limit: RULE_LIMITS.maxStreakDays,
        message: `Серия активных дней не может превышать ${RULE_LIMITS.maxStreakDays}.`,
      };
    }
  };
  visit(root, 0);
  ...
  if (nodes > RULE_LIMITS.maxNodes)
    return {
      code: 'node-limit',
      limit: RULE_LIMITS.maxNodes,
      message: `В правиле допустимо не более ${RULE_LIMITS.maxNodes} узлов.`,
    };
  if (leaves > RULE_LIMITS.maxLeaves)
    return {
      code: 'leaf-limit',
      limit: RULE_LIMITS.maxLeaves,
      message: `В правиле допустимо не более ${RULE_LIMITS.maxLeaves} условий.`,
    };
  if (aggregateLeaves > RULE_LIMITS.maxAggregateLeaves)
    return {
      code: 'aggregate-limit',
      limit: RULE_LIMITS.maxAggregateLeaves,
      message: `В правиле допустимо не более ${RULE_LIMITS.maxAggregateLeaves} условий истории и активных дней.`,
    };
  if (totalWindowMs > RULE_LIMITS.maxTotalWindowMs)
    return {
      code: 'window-budget-limit',
      limit: RULE_LIMITS.maxTotalWindowMs,
      message: 'Суммарный период условий истории не может превышать 365 дней.',
    };
  return undefined;
}
...
export function applyRuleCommand(
  ...
  if (command.type === 'add') {
    ...
    if (parent.node.kind !== 'all' && parent.node.kind !== 'any') {
      return commandFailure(draft, {
        ...
        message: 'Добавлять условия можно только внутрь группы.',
      });
    }
    ...
    if (!Number.isInteger(index) || index < 0 || index > parent.node.children.length) {
      return commandFailure(draft, {
        ...
        nodeId: command.parentNodeId,
        message: 'Не удалось определить место нового условия. Повторите действие.',
      });
    }
    ...
    return limitError
      ? commandFailure(draft, limitError)
      : { ok: true, draft: next, focusNodeId: node.nodeId };
  }
  ...
  if (command.type === 'remove') {
    ...
    if (location.parent.kind === 'not')
      return commandFailure(draft, {
        code: 'not-child-required',
        nodeId: command.nodeId,
        message: 'Сначала уберите отрицание, затем удалите условие.',
      });
  ...
  if (command.type === 'move') {
    if (location.parent.kind === 'root')
      return commandFailure(draft, {
        code: 'root-required',
        nodeId: command.nodeId,
        message: 'Корневую группу нельзя перемещать.',
      });
    if (location.parent.kind === 'not')
      return commandFailure(draft, {
        code: 'not-child-move',
        nodeId: command.nodeId,
        message: 'Перемещайте условие вместе с его отрицанием.',
      });
    if (containsNode(location.node, command.toParentNodeId)) {
      return commandFailure(draft, {
        code: 'move-cycle',
        nodeId: command.nodeId,
        message: 'Нельзя переместить группу в её собственную вложенную группу.',
      });
    }
    ...
    if (target.node.kind !== 'all' && target.node.kind !== 'any') {
      return commandFailure(draft, {
        code: 'parent-not-group',
        nodeId: command.toParentNodeId,
        message: 'Переместить условие можно только внутрь группы.',
      });
    }
    if (
      !Number.isInteger(command.toIndex) ||
      command.toIndex < 0 ||
      command.toIndex > target.node.children.length
    ) {
      return commandFailure(draft, {
        code: 'invalid-position',
        nodeId: command.toParentNodeId,
        message: 'Не удалось определить новое место условия.',
      });
    }
    ...
    return limitError
      ? commandFailure(draft, limitError)
      : { ok: true, draft: next, focusNodeId: moving.nodeId };
  }

  if (command.type === 'wrap') {
    const wrapper: RuleDraftNode =
      command.wrapper === 'not'
    ...
    return limitError
      ? commandFailure(draft, limitError)
      : { ok: true, draft: next, focusNodeId: wrapper.nodeId };
  }
  ...
  if (command.type === 'unwrap') {
    ...
    if (location.parent.kind === 'root')
      return commandFailure(draft, {
        code: 'root-required',
        nodeId: command.nodeId,
        message: 'Корневую группу нельзя убрать.',
      });
    if (location.node.kind === 'all' || location.node.kind === 'any') {
      if (location.parent.kind !== 'group')
        return commandFailure(draft, {
          code: 'unwrap-parent',
          nodeId: command.nodeId,
          message: 'Эту группу нельзя развернуть в текущем месте.',
        });
      ...
      return {
        ok: true,
        draft: next,
        focusNodeId: location.node.children[0]?.nodeId ?? location.parent.parent.nodeId,
      };
    }
    return commandFailure(draft, {
      code: 'not-container',
      nodeId: command.nodeId,
      message: 'Убирать оболочку можно только у группы или отрицания.',
    });
  }
  ...
  if (command.type === 'changeGroup') {
    if (location.node.kind !== 'all' && location.node.kind !== 'any') {
      return commandFailure(draft, {
        code: 'not-group',
        nodeId: command.nodeId,
        message: 'Способ объединения условий можно выбрать только для группы.',
      });
    }
  ...
  if (
    ...
    return commandFailure(draft, {
      code: 'not-leaf',
      nodeId: command.nodeId,
      message: 'Группу нельзя заменить настройками отдельного условия.',
    });
  }
  const replacement =
    command.leaf.kind === 'empty'
      ? { nodeId: command.nodeId, kind: 'empty' as const }
      : command.leaf.kind === 'incomplete'
        ? {
            nodeId: command.nodeId,
            kind: 'incomplete' as const,
            leaf: cloneValue(command.leaf.leaf),
          }
  ...
  return limitError
    ? commandFailure(draft, limitError)
    : { ok: true, draft: next, focusNodeId: replacement.nodeId };
}
