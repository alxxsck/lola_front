...
describe('Rule natural-language summary', () => {
          ...
          children: [
            ...
            },
            {
              ...
              child: {
                ...
                fieldKey: 'deposit.amount',
                filters: [],
                window: { kind: 'last', durationMs: 30 * day },
                compare: { operator: 'gte', value: 50_000 },
              },
            },
  ...
  it('makes empty, incomplete and unsupported states explicit', () => {
    expect(summarizeRule(createRuleDraft(), context)).toMatchObject({
      status: 'empty',
      text: 'Условия ещё не добавлены',
    });

    const incomplete = deserializeRule(
      { version: 1, root: { kind: 'all', children: [{ kind: 'futureRule', value: 1 }] } },
      context,
    ).draft;
