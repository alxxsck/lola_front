      ...
      aggregateMeasures: [
        {
          ...
          field: 'none',
          resultType: 'boolean',
          compareValueType: 'boolean',
          compareOperators: ['eq', 'neq'],
        },
        {
          measure: 'count',
          field: 'none',
...
describe('Rule domain serialization', () => {
      ...
      root: {
        ...
        children: [
          ...
          },
          {
            ...
            child: {
              kind: 'any',
              children: [{ kind: 'activityDayStreak', compare: { operator: 'gte', value: 2 } }],
            },
          },
        ],
      },
  ...
  it('preserves an unknown subtree opaquely and blocks serialization', () => {
    ...
    const parsed = deserializeRule(
      { version: 1, root: { kind: 'all', children: [source] } },
      context,
    );
    ...
    expect(opaque.kind === 'opaque' ? opaque.source : undefined).toEqual({
      kind: 'futureScore',
      model: 'risk-v2',
      config: { threshold: 0.73 },
    });

    const result = serializeRuleDraft(parsed.draft, context);
    expect(result).toMatchObject({
      ok: false,
      issues: [{ nodeId: opaque.nodeId, code: 'unsupported-node' }],
    });
  });
  ...
  it('resolves fields from the exact trigger Event revision when codes are duplicated', () => {
    const exactContext: RuleDomainContext = {
      ...
      contract: {
        ...contract,
        events: [
          { ...contract.events[0]!, definitionId: 'page-revision-old', fields: [] },
          ...contract.events,
        ],
      },
    };
