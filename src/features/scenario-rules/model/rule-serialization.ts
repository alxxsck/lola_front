...
function parseNode(source: unknown, issues: DraftIssue[]): RuleDraftNode {
  if (!isRecord(source) || typeof source.kind !== 'string') {
    ...
    issues.push({
      ...
      nodeId: node.nodeId,
      message: 'Узел правила имеет неподдерживаемый формат. Удалите или замените его.',
    });
    return node;
  }
  ...
  if (
    ...
    return {
      ...
      ...(Object.hasOwn(source, 'value')
        ? { value: clone(source.value) as EventFieldRuleNodeDto['value'] }
        : {}),
    };
  }
  if (
    source.kind === 'activityDayStreak' &&
    isRecord(source.compare) &&
    typeof source.compare.operator === 'string' &&
    typeof source.compare.value === 'number'
  ) {
    return {
      nodeId: uid('rule_node'),
      kind: 'activityDayStreak',
      compare: {
        operator: source.compare.operator as ActivityDayStreakRuleNodeDto['compare']['operator'],
        value: source.compare.value,
      },
    };
  }
  if (
    ...
    isRecord(source.compare) &&
    typeof source.compare.operator === 'string' &&
    (typeof source.compare.value === 'string' ||
      typeof source.compare.value === 'number' ||
      typeof source.compare.value === 'boolean')
  ) {
        ...
        filters: filters.map((filter) => {
          ...
          return {
            ...
            ...(Object.hasOwn(value, 'value')
              ? { value: clone(value.value) as AggregateFilterDto['value'] }
              : {}),
          };
        }),
  ...
  issues.push({
    code: 'unsupported-node',
    nodeId: node.nodeId,
    message: `Условие «${node.kind === 'opaque' ? (node.reportedKind ?? 'неизвестное') : 'неизвестное'}» пока не поддерживается. Замените его, чтобы опубликовать правило.`,
  });
  return node;
}
...
export function deserializeRule(input: unknown, context: RuleDomainContext): RuleDeserializeResult {
      ...
      issues: [
        {
          ...
          nodeId: root.nodeId,
          message: 'Версия или структура правила не поддерживается. Создайте правило заново.',
        },
      ],
    };
  }
  ...
  return {
    ...
    issues: [
      ...issues,
      ...semanticIssues.filter((issue) => !seen.has(`${issue.code}:${issue.nodeId ?? ''}`)),
    ],
  };
}
...
function serializeNode(
  node: RuleDraftNode,
  path: string,
  index: Record<string, RulePathEntry>,
  issues: DraftIssue[],
): ScenarioRuleDtoRoot | undefined {
  index[path] = { nodeId: node.nodeId, nodePath: path };
  if (node.kind === 'empty') {
    issues.push({
      code: 'empty-node',
      nodeId: node.nodeId,
      message: 'Выберите условие или удалите пустую строку.',
    });
    return undefined;
  }
  if (node.kind === 'incomplete') {
    issues.push({
      ...
      nodeId: node.nodeId,
      message: 'Заполните все обязательные поля условия.',
    });
    return undefined;
  }
  if (node.kind === 'opaque') {
    issues.push({
      code: 'unsupported-node',
      nodeId: node.nodeId,
      message: 'Неподдерживаемое условие нельзя опубликовать. Удалите или замените его.',
    });
    return undefined;
  }
  if (node.kind === 'all' || node.kind === 'any') {
    const children = node.children.map((child, childIndex) =>
      serializeNode(child, `${path}.children.${childIndex}`, index, issues),
    );
...
export function serializeRuleDraft(
  draft: import('./rule-types').RuleDraft,
