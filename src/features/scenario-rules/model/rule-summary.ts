...
function displayFieldValue(value: RuleLiteral | undefined, field?: ScenarioAuthoringField): string {
  ...
  if (Array.isArray(value))
    return value
      .map((item) => formatMoneyDisplay(item, field.display?.scale ?? 1, field.display?.precision))
      .join(', ');
...
function aggregateSummary(
  node: Extract<RuleDraftNode, { kind: 'eventAggregate' }>,
  context: RuleDomainContext,
): string {
...
function summarizeNode(
  ...
  if (node.kind === 'empty') text = 'условие ещё не выбрано';
    ...
    text = `неподдерживаемое условие${node.reportedKind ? ` «${node.reportedKind}»` : ''}`;
    ...
    text = children.length
      ...
      : 'Условия ещё не добавлены';
  } else if (node.kind === 'not')
    text = `Исключить, если: ${summarizeNode(node.child, context, byNodeId)}`;
    ...
    text = `активен ${streakOperatorLabels[node.compare.operator] ?? node.compare.operator} ${node.compare.value} дней подряд`;
...
export function summarizeRule(draft: RuleDraft, context: RuleDomainContext): RuleSummary {
  ...
  const visit = (node: RuleDraftNode) => {
    ...
    if (node.kind === 'incomplete') {
      hasIncomplete = true;
      if (node.leaf.kind === 'eventAggregate' || node.leaf.kind === 'activityDayStreak')
        aggregateLeaves += 1;
      return;
    }
    ...
    if (node.kind === 'eventAggregate') {
      const windowMs =
        node.window.kind === 'last' ? node.window.durationMs : RULE_LIMITS.maxWindowMs;
