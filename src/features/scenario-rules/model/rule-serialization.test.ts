  ...
  events: [
    {
      ...
      fields: [
        {
          capabilities: {
            eventField: { operators: ['eq', 'exists', 'not_exists'] },
            aggregateFilter: { operators: ['eq', 'exists'] },
            aggregateMeasure: { measures: [] },
          },
          control: { type: 'select', options: ['promotions', 'cashier'] },
          fieldKey: 'page.code',
          label: 'Страница',
    ...
    },
    {
      ...
      fields: [
        {
          capabilities: {
            eventField: { operators: ['eq', 'gt', 'gte'] },
...
const context: RuleDomainContext = {
  triggerEventDefinitionId: 'page-1',
...
describe('Rule DTO contract', () => {
  it('serializes a catalog-compatible aggregate using stable keys and no UI metadata', () => {
    ...
    expect(aggregate.kind === 'eventAggregate' ? aggregate.filters[0]?.filterId : '').toMatch(
      /^rule_filter_/,
    );
    expect(result).toMatchObject({ ok: true, value: dto });
    expect(JSON.stringify(result.ok ? result.value : null)).not.toMatch(
      /nodeId|filterId|label|path/,
    );
  });
  ...
  it('keeps known stale selections editable and explains why they cannot be serialized', () => {
    const parsed = deserializeRule(
      {
        ...
        root: {
          kind: 'eventField',
          eventCode: 'other.event',
          fieldKey: 'removed.field',
          operator: 'in',
          value: ['x'],
        },
      },
    ...
    expect(parsed.issues.map((issue) => issue.code)).toEqual(
      expect.arrayContaining(['trigger-event-mismatch', 'field-unavailable']),
    );
  ...
  it('rejects semantically incomplete existence values, money filters and initial since-trigger windows', () => {
    ...
    expect(result.ok ? '' : result.issues.map((issue) => issue.message).join(' ')).toContain(
      'Уберите значение',
    );
  });
  ...
  it('maps a backend issue through the path index captured for the request', () => {
    const parsed = deserializeRule(
      { version: 1, root: { kind: 'all', children: [validAggregate] } },
      context,
    );
    const aggregate = firstChild(parsed.draft.root);
    ...
    const mapped = mapBackendRuleIssues(
      [
        {
          code: 'AGGREGATE_FILTER_INVALID',
          message: 'Filter value is invalid',
          path: 'rule.root.children.0.filters.0.value',
        },
    ...
    const parsed = deserializeRule(
      {
        ...
        root: {
          ...
          measure: 'count',
          filters: [],
          window: { kind: 'last', durationMs: day },
          compare: { operator: 'gte', value: '1'.repeat(10_001) },
        },
      },
      context,
    );
