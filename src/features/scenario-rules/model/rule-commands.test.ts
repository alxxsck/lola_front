...
describe('Rule domain commands', () => {
  it('applies add, replace, move, wrap, unwrap, group change and remove immutably', () => {
    ...
    const addedLeaf = applyRuleCommand(
      initial,
      { type: 'add', parentNodeId: rootId, node: { kind: 'empty' } },
      context,
    );
    ...
    expect(group(draft).children[0]).toMatchObject({
      nodeId: leafId,
      kind: 'eventField',
      eventCode: 'page.opened',
    });

    const addedGroup = applyRuleCommand(
      draft,
      { type: 'add', parentNodeId: rootId, node: { kind: 'all' } },
      context,
    );
    ...
    draft = value(
      applyRuleCommand(
        draft,
        { type: 'move', nodeId: leafId, toParentNodeId: nestedId, toIndex: 0 },
        context,
      ),
    );
    expect(group(draft, nestedId).children[0]?.nodeId).toBe(leafId);

    const wrapped = applyRuleCommand(
      draft,
      { type: 'wrap', nodeId: leafId, wrapper: 'not' },
      context,
    );
    const wrapperId = wrapped.ok ? wrapped.focusNodeId : '';
    draft = value(wrapped);
    expect(group(draft, nestedId).children[0]).toMatchObject({
      nodeId: wrapperId,
      kind: 'not',
      child: { nodeId: leafId },
    });
    ...
    expect(group(draft, nestedId).children[0]?.nodeId).toBe(leafId);
    draft = value(
      applyRuleCommand(draft, { type: 'changeGroup', nodeId: nestedId, kind: 'any' }, context),
    );
  ...
  it('rejects moving a group into its descendant without changing the draft', () => {
    const draft = deserializeRule(
      {
        ...
        root: {
          kind: 'all',
          children: [{ kind: 'any', children: [{ kind: 'all', children: [] }] }],
        },
      },
      context,
    ).draft;
    ...
    expect(result).toMatchObject({
      ok: false,
      draft,
      error: { code: 'move-cycle', nodeId: outer.nodeId },
    });
  ...
  it('moves a condition down within the same group using the final target index', () => {
    ...
    const result = applyRuleCommand(
      draft,
      {
        type: 'move',
        nodeId: first!.nodeId,
        toParentNodeId: draft.root.nodeId,
        toIndex: 1,
      },
      context,
    );
    ...
    expect(result.ok ? group(result.draft).children.map((node) => node.nodeId) : []).toEqual([
      second!.nodeId,
      first!.nodeId,
    ]);
  });
  ...
  it('rejects wrapping a depth-four leaf with an actionable depth error', () => {
    ...
    const result = applyRuleCommand(
      draft,
      { type: 'wrap', nodeId: leaf.nodeId, wrapper: 'not' },
      context,
    );

    expect(result).toMatchObject({
      ok: false,
      error: { code: 'depth-limit', nodeId: leaf.nodeId, limit: 4 },
    });
  ...
  it('allows ten aggregate leaves and rejects the eleventh', () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      draft = value(
        applyRuleCommand(draft, { type: 'add', parentNodeId: rootId, node: aggregate }, context),
      );
    }

    const result = applyRuleCommand(
      draft,
      { type: 'add', parentNodeId: rootId, node: aggregate },
      context,
    );
  ...
  it('accepts reactive UI buffers and returns a plain immutable draft', () => {
    ...
    const result = applyRuleCommand(
      draft,
      { type: 'add', parentNodeId: draft.root.nodeId, node: leaf },
      context,
    );
  ...
  it('can remove an inversion wrapper from the root condition', () => {
    ...
    const wrapped = applyRuleCommand(
      draft,
      { type: 'wrap', nodeId: originalRootId, wrapper: 'not' },
      context,
    );
    ...
    const result = applyRuleCommand(
      wrapped.draft,
      { type: 'unwrap', nodeId: wrapped.focusNodeId },
      context,
    );

    expect(result).toMatchObject({
      ok: true,
      focusNodeId: originalRootId,
      draft: { root: { nodeId: originalRootId, kind: 'eventField' } },
    });
  });
});
