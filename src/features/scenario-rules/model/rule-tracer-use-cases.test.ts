...
const contract: ScenarioAuthoringContract = {
  projectId: 'project-1',
  revision: 'catalog-tracer',
  version: 1,
  events: [
    {
      code: 'page.opened',
      definitionId: 'page-revision',
...
const context: RuleDomainContext = {
  triggerEventDefinitionId: 'page-revision',
...
describe('FE-V2-06 behavioral tracer DTOs', () => {
      ...
      root: {
        ...
        children: [
          ...
          },
          {
            ...
            fieldKey: 'deposit.amount',
            filters: [{ fieldKey: 'deposit.currency', operator: 'eq' as const, value: 'EUR' }],
            window: {
              kind: 'last' as const,
              durationMs: 30 * day,
              boundary: 'beforeTrigger' as const,
            },
            compare: { operator: 'gte' as const, value: '500.25' },
          },
          {
            kind: 'any' as const,
            children: [
              {
                ...
                compare: { operator: 'lt' as const, value: '3' },
              },
              {
                kind: 'not' as const,
                child: {
                  kind: 'activityDayStreak' as const,
                  compare: { operator: 'lt' as const, value: 2 },
                },
              },
            ],
          },
  ...
  it('accepts exact server boundaries and keeps sinceTrigger recheck-only', () => {
    const boundary = deserializeRule(
      {
        version: 1,
        root: {
          kind: 'all',
          children: [
            {
              ...
              measure: 'count',
              filters: [],
              window: { kind: 'last', durationMs: 90 * day, boundary: 'beforeTrigger' },
              compare: { operator: 'gte', value: '0' },
            },
    ...
    const recheckOnly = deserializeRule(
      {
        version: 1,
        root: {
          kind: 'eventAggregate',
          ...
          measure: 'count',
          filters: [],
          window: { kind: 'sinceTrigger', boundary: 'beforeTrigger' },
          compare: { operator: 'gte', value: '1' },
        },
      },
      context,
    );
    expect(serializeRuleDraft(recheckOnly.draft, context)).toMatchObject({
      ok: false,
      issues: [{ code: 'since-trigger-unavailable' }],
    });
  });
});
