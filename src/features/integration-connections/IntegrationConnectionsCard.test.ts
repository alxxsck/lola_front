...
describe('IntegrationConnectionsCard', () => {
  ...
  it('ignores inbound-only connections without an outbound credential', async () => {
    ...
    expect(wrapper.find('[data-connection-id="inbound-only"]').exists()).toBe(false);
    expect(wrapper.find('[data-connection-id="outbound-ready"]').exists()).toBe(true);
  });
  ...
  it('renders multiple project-scoped connections read-only without secret controls', async () => {
    ...
    expect(wrapper.find('input[name="amplitudeProjectApiKey"]').exists()).toBe(false);
    expect(wrapper.find('input[name="amplitudeRotationKey"]').exists()).toBe(false);
  ...
  it('keeps verified connections compact and opens creation explicitly', async () => {
    ...
    expect(wrapper.get('details.connection-settings').attributes('open')).toBeUndefined();
    expect(wrapper.find('form[data-form="create-amplitude"]').exists()).toBe(false);

    await wrapper.get('.provider-create-toggle').trigger('click');

    expect(wrapper.get('form[data-form="create-amplitude"]').isVisible()).toBe(true);
  ...
  it('creates once, clears the write-only key immediately and polls the durable test with GET', async () => {
    ...
    api.requestTest.mockResolvedValue(testResult({ status: 'PENDING', attemptCount: 0 }));
    ...
    expect(api.requestTest).toHaveBeenCalledOnce();
    expect(api.getTest).toHaveBeenCalledWith('project-1', 'connection-1', 'test-1');
  ...
  it('retries an ambiguous credential rotation with the same payload and idempotency key', async () => {
    ...
    expect(
      (wrapper.get('input[name="amplitudeRotationKey"]').element as HTMLInputElement).value,
    ).toBe('');
    expect(wrapper.text()).toContain('Сервер не подтвердил результат');
    expect(wrapper.html()).not.toContain(secret);
    await wrapper.get('.provider-create-toggle').trigger('click');
    expect(
      wrapper.get('input[name="amplitudeProjectApiKey"]').attributes('disabled'),
    ).toBeDefined();
    expect(wrapper.find('[data-action="discard-rotate-amplitude"]').exists()).toBe(true);
    const firstCall = api.rotateAmplitude.mock.calls[0];

    await wrapper.get('input[name="amplitudeRotationKey"]').setValue(secret);
    await wrapper.get('[data-action="retry-rotate-amplitude"]').trigger('click');
    await flushPromises();
    ...
    expect(wrapper.text()).toContain('Новый Project API Key сохранён и проверен');
  });
  ...
  it('keeps the credential receipt after an idempotency conflict', async () => {
    ...
    api.createAmplitude
      ...
      .mockRejectedValueOnce(
        new ApiError(409, 'conflict', undefined, undefined, 'IDEMPOTENCY_KEY_CONFLICT'),
      )
    ...
    await wrapper.get('input[name="amplitudeProjectApiKey"]').setValue(originalSecret);
    await wrapper.get('form[data-form="create-amplitude"]').trigger('submit');
    await flushPromises();
    const firstCall = api.createAmplitude.mock.calls[0];

    await wrapper.get('input[name="amplitudeProjectApiKey"]').setValue('e'.repeat(32));
    await wrapper.get('[data-action="retry-create-amplitude"]').trigger('click');
    await flushPromises();

    expect(wrapper.find('[data-action="retry-create-amplitude"]').exists()).toBe(true);
    expect(
      window.sessionStorage.getItem('retenive:amplitude-unresolved-secret:project-1'),
    ).not.toBeNull();
  ...
  it('isolates Customer.io connections, credentials, and retry receipts from Amplitude', async () => {
    ...
    api.createCustomerIo.mockRejectedValue(new ApiError(0, 'network outcome unknown'));
    ...
    expect(wrapper.find('[data-integration="customer-io"]').exists()).toBe(true);
    await wrapper.get('.provider-create-toggle').trigger('click');
    ...
    expect(
      window.sessionStorage.getItem('retenive:customer-io-unresolved-secret:project-1'),
    ).not.toBeNull();
    expect(
      window.sessionStorage.getItem('retenive:amplitude-unresolved-secret:project-1'),
    ).not.toBeNull();
  ...
  it('resumes a persisted durable test with GET after polling failed', async () => {
    ...
    expect(api.requestTest).toHaveBeenCalledOnce();
    expect(window.sessionStorage.getItem('retenive:amplitude-pending-tests:project-1')).toContain(
      'test-1',
    );
    first.unmount();
    ...
    expect(api.requestTest).toHaveBeenCalledOnce();
    expect(api.getTest).toHaveBeenLastCalledWith('project-1', 'connection-1', 'test-1');
    expect(window.sessionStorage.getItem('retenive:amplitude-pending-tests:project-1')).toBeNull();
  ...
  it('replays an ambiguous test POST with the same durable idempotency key after remount', async () => {
    const current = connection();
    api.list.mockResolvedValue({ items: [current] });
    api.requestTest.mockRejectedValueOnce(new ApiError(0, 'request outcome unknown'));
    const first = mountCard();
    await flushPromises();
  ...
  it('never reuses a pending test from an older credential revision', async () => {
    ...
    api.requestTest.mockResolvedValue(testResult({ id: 'new-test', credentialRevision: 2 }));
    const wrapper = mountCard();
    await flushPromises();
  ...
  it('keeps a nonsensitive credential guard after remount until reconciliation', async () => {
    ...
    await first.get('input[name="amplitudeProjectApiKey"]').setValue('c'.repeat(32));
    ...
    expect(resumed.find('[data-action="retry-create-amplitude"]').exists()).toBe(true);
    expect(
      resumed.get('input[name="amplitudeProjectApiKey"]').attributes('disabled'),
    ).toBeUndefined();
    expect(api.createAmplitude).toHaveBeenCalledOnce();

    await resumed.get('input[name="amplitudeProjectApiKey"]').setValue('c'.repeat(32));
    await resumed.get('[data-action="retry-create-amplitude"]').trigger('click');
    await flushPromises();
  ...
  it('renders archived connections as read-only terminal state', async () => {
    ...
    expect(wrapper.find('form[data-form="update-amplitude"]').exists()).toBe(false);
    expect(wrapper.find('form[data-form="rotate-amplitude"]').exists()).toBe(false);
  });

  it('ignores a stale list response after the selected project changes', async () => {
    let resolveFirst!: (value: { items: ReturnType<typeof connection>[] }) => void;
    api.list
  ...
  it('reloads after an OCC conflict and activates only the current tested revision', async () => {
    ...
    api.activate.mockResolvedValue(connection({ ...tested, lifecycle: 'ACTIVE', version: 6 }));
    const wrapper = mountCard();
    await flushPromises();

    await wrapper.get('form[data-form="update-amplitude"] input').setValue('Renamed');
