...
describe('integrationConnectionsApi', () => {
  ...
  it('passes caller-owned idempotency keys to every mutation', async () => {
    ...
    await integrationConnectionsApi.createAmplitude('project-1', create, 'create-key');
    ...
    await integrationConnectionsApi.requestTest('project-1', 'connection-1', version, 'test-key');
    await integrationConnectionsApi.activate('project-1', 'connection-1', version, 'activate-key');
    await integrationConnectionsApi.disable('project-1', 'connection-1', version, 'disable-key');
  ...
  it('uses GET for list and durable test polling without command options', async () => {
    ...
    await integrationConnectionsApi.getTest('project-1', 'connection-1', 'test-1');

    expect(generated.list).toHaveBeenCalledWith('project-1');
    expect(generated.getTest).toHaveBeenCalledWith('project-1', 'connection-1', 'test-1');
  });
});
