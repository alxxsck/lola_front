...
export function restoreScenarioAuthoringSource(
  ...
  const actions = Array.isArray(rawActions)
    ? normalizeScenarioActions(
        rawActions.flatMap((value): ScenarioAction[] => {
          ...
          return [
            {
              ...
              ...(typeof action.nextNodeKey === 'string' || action.nextNodeKey === null
                ? { nextNodeKey: action.nextNodeKey }
                : {}),
            },
          ];
        }),
  ...
  return {
    rule:
      ruleContext && source.rule
        ? deserializeRule(source.rule, ruleContext).draft
        : createRuleDraft(),
    audience:
      audienceContext && source.audience
        ? deserializeAudience(source.audience, audienceContext).draft
        : createAudienceDraft(),
    delivery: source.deliveryPolicy
      ? deserializeDeliveryPolicy(source.deliveryPolicy)
      : createDeliveryPolicyDraft(),
    actions,
    localization:
      source.localization && typeof source.localization === 'object'
