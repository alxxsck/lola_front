...
describe('support-case-escalation-policy', () => {
  ...
  it('rejects unstable codes, empty phrases and invalid server thresholds', () => {
    const policy = createDefaultEscalationPolicy();
    policy.explicitHumanRequestRules = [{ code: 'human request', locales: [], phrases: [] }];
  ...
  it('creates fenced simulator identifiers and honest routing copy', () => {
    ...
    expect(routingAdmissionPresentation('OUT_OF_HOURS').copy).toContain('в рабочее время');
  ...
  it('validates and normalizes every simulator safety-state boundary', () => {
    ...
    expect(simulationStepSafetyIssue(failed)).toContain('только для обнаруженного риска');
    ...
    expect(simulationStepSafetyIssue(policySwitch)).toContain('безопасного состояния');
  ...
  it('rejects simulator events without their exact required reference', () => {
    expect(simulationStepShapeIssue(createSimulationStep('EXPLICIT_HUMAN_REQUEST', 0))).toContain(
      'правило',
    );
    expect(simulationStepShapeIssue(createSimulationStep('SCENARIO', 1))).toContain('сценарий');
    expect(simulationStepShapeIssue(createSimulationStep('TRUSTED_OUTCOME', 2))).toContain(
      'результат',
    );
    expect(simulationStepShapeIssue(createSimulationStep('POLICY_SWITCH', 3))).toContain(
      'версию правил',
    );

    const removedRule = createSimulationStep('EXPLICIT_HUMAN_REQUEST', 4);
    removedRule.ruleCode = 'REMOVED_RULE';
    expect(simulationStepReferenceIssue(removedRule, createDefaultEscalationPolicy())).toContain(
    ...
    expect(
      simulationStepReferenceIssue(forbiddenReference, createDefaultEscalationPolicy()),
