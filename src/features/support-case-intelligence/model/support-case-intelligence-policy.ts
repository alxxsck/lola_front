...
export function presentCaseIntelligenceRuntime(
  ...
  const activeDetectionRevisionId = snapshot?.release?.detectionPolicyRevisionId ?? null;
...
export function prepareDetectionPolicyForAuthoring(
  ...
  if (
    ...
    policy.runtimeLimits.maxRulesEvaluated = defaults.runtimeLimits.maxRulesEvaluated;
  }
  return policy;
}
...
export function normalizeStableCode(value: string): string {
  ...
  return /^[A-Z]/.test(normalized) ? normalized.slice(0, 64) : `C_${normalized}`.slice(0, 64);
}
...
function scalarIssue(value: unknown): string {
  ...
  if (typeof value === 'number') return Number.isFinite(value) ? '' : 'Введите конечное число.';
  if (typeof value === 'boolean') return '';
  if (!Array.isArray(value) || value.length === 0 || value.length > 50)
    return 'Добавьте от 1 до 50 значений.';
  if (value.some((item) => typeof item !== 'string' || !item.trim() || item.trim().length > 500))
...
function addDuplicateIssues(issues: PolicyIssue[], values: string[], path: string, label: string) {
...
export function validateDetectionPolicy(policy: CaseIntelligenceDetectionPolicyDto): PolicyIssue[] {
  ...
  required(policy.scope.length <= 2000, 'scope', 'Назначение должно быть короче 2000 знаков.');
  ...
  required(policy.channels.length > 0, 'channels', 'Выберите хотя бы один канал.');
  ...
  required(
    policy.fallbackLocale.length <= 35,
    'fallbackLocale',
  ...
  required(
    policy.attachWindowMs >= 60_000 && policy.attachWindowMs <= 365 * 86_400_000,
    'attachWindowMs',
    'Период привязки — от минуты до года.',
  );
  required(
    policy.reopenWindowMs >= 60_000 && policy.reopenWindowMs <= 365 * 86_400_000,
    'reopenWindowMs',
    'Период повторного открытия — от минуты до года.',
  );
  required(
    policy.candidateLimit >= 1 && policy.candidateLimit <= 20,
    'candidateLimit',
    'Укажите от 1 до 20 кандидатов.',
  );
  required(
    policy.debounceMs >= 0 && policy.debounceMs <= 60_000,
    'debounceMs',
    'Пауза должна быть от 0 до 60 000 мс.',
  );
  required(
    policy.routerContext.maxSignals >= 1 && policy.routerContext.maxSignals <= 8,
    'routerContext.maxSignals',
    'Укажите от 1 до 8 сигналов.',
  );
  required(
    policy.routerContext.maxContextMessages >= 0 && policy.routerContext.maxContextMessages <= 50,
    'routerContext.maxContextMessages',
    'Укажите от 0 до 50 сообщений.',
  );
  required(
    policy.routerContext.maxCandidateCases >= 0 && policy.routerContext.maxCandidateCases <= 20,
    'routerContext.maxCandidateCases',
    'Укажите от 0 до 20 обращений.',
  );
  required(
    policy.runtimeLimits.maxRulesEvaluated >= 1 && policy.runtimeLimits.maxRulesEvaluated <= 20,
    'runtimeLimits.maxRulesEvaluated',
    'Лимит точных правил на одну проверку — от 1 до 20.',
  );
  required(
    policy.runtimeLimits.maxSemanticStatements >= 0 &&
      policy.runtimeLimits.maxSemanticStatements <= 50,
    'runtimeLimits.maxSemanticStatements',
    'Укажите от 0 до 50 смысловых признаков.',
  );
  required(
    policy.runtimeLimits.maxEvaluationMs >= 1 && policy.runtimeLimits.maxEvaluationMs <= 5000,
    'runtimeLimits.maxEvaluationMs',
    'Укажите от 1 до 5000 мс.',
  );
  for (const kind of ['include', 'exclude'] as const) {
    required(
      policy.audience[kind].length <= 100,
      `audience.${kind}`,
      'Можно задать не больше 100 условий.',
    );
    policy.audience[kind].forEach((predicate, index) => {
      required(
        isStableCode(predicate.attributeCode),
        `audience.${kind}.${index}.attributeCode`,
        'Код поля: латинские заглавные буквы, цифры и подчёркивание.',
      );
  ...
  policy.topics.forEach((topic, index) => {
    ...
    required(
      ...
      `topics.${index}.description`,
      'Описание должно быть короче 1000 знаков.',
    );
    required(
      topic.positiveExamples.length <= 20,
      `topics.${index}.positiveExamples`,
      'Можно добавить не больше 20 примеров.',
    );
    required(
      topic.negativeExamples.length <= 20,
      `topics.${index}.negativeExamples`,
      'Можно добавить не больше 20 исключений.',
    );
    required(
      topic.positiveExamples.every((item) => Boolean(item.trim()) && item.trim().length <= 500),
      `topics.${index}.positiveExamples`,
      'Каждый пример должен быть непустым и короче 500 знаков.',
    );
    required(
      topic.negativeExamples.every((item) => Boolean(item.trim()) && item.trim().length <= 500),
      `topics.${index}.negativeExamples`,
      'Каждое исключение должно быть непустым и короче 500 знаков.',
    );
    required(
  ...
  policy.rules.forEach((rule, index) => {
    ...
    required(
      rule.priority >= 0 && rule.priority <= 10_000,
      `rules.${index}.priority`,
      'Порядок должен быть от 0 до 10 000.',
    );
    required(
      !rule.locale || rule.locale.length <= 35,
      `rules.${index}.locale`,
      'Код языка должен быть короче 35 знаков.',
    );
    if (rule.kind === 'EXACT' || rule.kind === 'PHRASE') {
      required(Boolean(rule.phrase?.trim()), `rules.${index}.phrase`, 'Введите текст правила.');
      required(
        (rule.phrase?.length ?? 0) <= 500,
        `rules.${index}.phrase`,
        'Текст правила должен быть короче 500 знаков.',
      );
    }
    if (rule.kind === 'SEMANTIC_STATEMENT') {
      ...
      required(
        (rule.statement?.length ?? 0) <= 1000,
        `rules.${index}.statement`,
        'Описание смысла должно быть короче 1000 знаков.',
      );
    }
    if (rule.kind === 'ATTRIBUTE') {
      ...
      required(
        isStableCode(rule.attributeCode ?? ''),
        `rules.${index}.attributeCode`,
        'Код поля: латинские заглавные буквы, цифры и подчёркивание.',
      );
      required(Boolean(rule.operator), `rules.${index}.operator`, 'Выберите условие сравнения.');
...
export function validateBudgetPolicy(budget: CaseIntelligenceBudgetPolicyDto): PolicyIssue[] {
  const issues: PolicyIssue[] = [];
  ...
  if (
    ...
    BigInt(budget.dailyCostMicroUsdSoftCap) > BigInt(budget.dailyCostMicroUsdHardCap)
  ) {
    issues.push({
...
function serverIssueMessage(code: string): string {
  const labels: Record<string, string> = {
    CASE_INTELLIGENCE_DUPLICATE_TOPIC_CODE: 'Код категории уже используется.',
    CASE_INTELLIGENCE_DUPLICATE_RULE_CODE: 'Код правила уже используется.',
    CASE_INTELLIGENCE_DUPLICATE_RULE: 'Другое правило уже описывает это совпадение.',
    ...
    CASE_INTELLIGENCE_FALLBACK_LOCALE_NOT_DECLARED: 'Основной язык должен входить в список языков.',
    ...
    CASE_INTELLIGENCE_RULE_FIELD_FORBIDDEN: 'Это поле не используется выбранным типом правила.',
    CASE_INTELLIGENCE_RULE_LOCALE_NOT_DECLARED: 'Язык правила должен входить в список языков.',
    CASE_INTELLIGENCE_FIELD_INVALID: 'Проверьте значение поля.',
    CASE_INTELLIGENCE_DUPLICATE_TOPIC_EXAMPLE: 'Такой пример уже добавлен в эту категорию.',
    ...
    CASE_INTELLIGENCE_POLICY_INVALID: 'Правила не прошли обязательную серверную проверку.',
...
export function mergePolicyIssues(...groups: readonly (readonly PolicyIssue[])[]): PolicyIssue[] {
...
export function calibrationBlockedReasonLabel(value: string | null): string {
  ...
  if (value === 'CONFIDENCE_INTERVAL_TOO_WIDE') return 'Слишком широкий интервал доверия';
...
export function caseIntelligenceReasonLabel(code: string): string {
  const labels: Record<string, string> = {
    CASE_INTELLIGENCE_DETERMINISTIC_RULE_MATCH: 'совпало опубликованное детерминированное правило',
    CASE_INTELLIGENCE_NO_DETERMINISTIC_MATCH: 'детерминированное правило не найдено',
  ...
  return (
    labels[code] ?? 'сервер вернул неизвестную причину; автоматическое решение не подтверждено'
  );
}
