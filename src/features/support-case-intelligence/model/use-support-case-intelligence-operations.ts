...
function writeRetained(scope: CaseIntelligenceOperationsAuthority, value: CaseIntelligencePendingCommand | null) {
  const key = scopeKey(scope);
...
function canRetainAttempt(scope: CaseIntelligenceOperationsAuthority, attempt: CaseIntelligencePendingCommand) {
...
function isTerminalAccess(status: number, code?: string) {
  return status === 401 || status === 403 || status === 404 || status === 428 || code === "MFA_REQUIRED" || code === "MFA_ENROLLMENT_REQUIRED" || code === "REAUTHENTICATION_REQUIRED";
}
...
export function useSupportCaseIntelligenceOperations(context: SupportCaseIntelligenceOperationsContext) {
  ...
  const canRead = computed(() => has(authority.value, "project.case_intelligence.read") && !accessDenied.value);
  const canManageRelease = computed(() => has(authority.value, "project.case_intelligence.release.manage"));
  const canReadCost = computed(() => has(authority.value, "project.case_intelligence.cost.read"));
  const canReadDecisions = computed(() => has(authority.value, "project.case_intelligence.decisions.read"));
  const canReviewLabels = computed(() => has(authority.value, "project.case_intelligence.labels.review"));
  const canActivate = computed(() => canManageRelease.value && current.value?.allowedActions.includes("ACTIVATE") === true);
  const canRollback = computed(() => canManageRelease.value && current.value?.allowedActions.includes("ROLLBACK") === true);
  const canCorrect = computed(() => canReviewLabels.value && current.value?.allowedActions.includes("CORRECT_DECISION") === true);

  function isCurrent(scope: CaseIntelligenceOperationsAuthority, token: number) {
    const next = context.authority();
    return generation.value === token && next?.actorId === scope.actorId && next.projectId === scope.projectId;
  }
  ...
  async function handleError(cause: unknown, scope: CaseIntelligenceOperationsAuthority, forget = false) {
    const problem = normalizeApiError(cause);
    if (forget || isTerminalAccess(problem.status, problem.code)) {
      writeRetained(scope, null);
      pending.value = null;
    }
    if (problem.status === 401 || problem.status === 428 || problem.code?.includes("MFA") || problem.code === "REAUTHENTICATION_REQUIRED") {
      clearProtected();
    ...
    error.value = problem.status === 409
  ...
  async function load(section: CaseIntelligenceOperationsSection, options: { from?: string; to?: string } = {}) {
    ...
    try {
      ...
      pending.value = retainedAttempt && canRetainAttempt(scope, retainedAttempt) ? retainedAttempt : null;
      if (section === "evaluation" && canManageRelease.value) {
        ...
        const page = await source.listDecisions(scope.projectId, undefined, undefined, readAbort.signal);
        if (!isCurrent(scope, token)) return;
      ...
      if (!isCurrent(scope, token) || (cause instanceof DOMException && cause.name === "AbortError")) return;
  ...
  async function selectDataset(id: string) {
    ...
    const token = generation.value;
    const value = await source.readDataset(scope.projectId, id).catch(async (cause) => { await handleError(cause, scope); return null; });
  ...
  async function selectEvaluation(id: string) {
    ...
    if (!scope || !canManageRelease.value) return;
    const token = generation.value;
    const value = await source.readEvaluation(scope.projectId, id).catch(async (cause) => { await handleError(cause, scope); return null; });
  ...
  async function loadMore(kind: "datasets" | "evaluations" | "decisions") {
    const scope = context.authority();
    const cursor = kind === "datasets" ? datasetCursor.value : kind === "evaluations" ? evaluationCursor.value : decisionCursor.value;
  ...
  async function explainCase(caseId: string) {
    ...
    if (!scope || !canReadDecisions.value) return [];
    const token = generation.value;
    const page = await source.explainCase(scope.projectId, caseId).catch(async (cause) => { await handleError(cause, scope); return null; });
  ...
  async function execute(attempt: CaseIntelligencePendingCommand) {
    ...
    try {
      if (attempt.operation === "RUN_EVALUATION") {
        const value = await source.runEvaluation(scope.projectId, attempt.body, mutationAbort.signal);
        evaluation.value = {
          ...value,
      ...
      else if (attempt.operation === "ACTIVATE_RELEASE") current.value = { ...(current.value!), release: await source.activateRelease(scope.projectId, attempt.body, mutationAbort.signal) };
      else if (attempt.operation === "ROLLBACK_RELEASE") current.value = { ...(current.value!), release: await source.rollbackRelease(scope.projectId, attempt.body, mutationAbort.signal) };
      else await source.correctDecision(scope.projectId, attempt.body, mutationAbort.signal);
      if (!isCurrent(scope, token)) return false;
      writeRetained(scope, null);
      pending.value = null;
      feedback.value = attempt.operation === "RUN_EVALUATION" ? "Проверка завершена. Результаты получены с сервера." : attempt.operation === "CORRECT_DECISION" ? "Исправление сохранено в истории решения." : "Новая рабочая версия подтверждена сервером.";
      return true;
    } catch (cause) {
      const problem = normalizeApiError(cause);
      ...
      if (problem.status === 409) {
        ...
        if (isCurrent(scope, token)) error.value = "Данные изменились на сервере. Перечитайте актуальную версию и подтвердите действие ещё раз.";
      } else if (!isUnknown(problem.status)) await handleError(problem, scope, true);
      else error.value = "Результат команды пока неизвестен. Новые действия заблокированы; проверьте эту же команду.";
      return false;
    } finally {
  ...
  function runEvaluation(datasetId: string) {
    ...
    const body: RunCaseIntelligenceEvaluationCommand = {
      ...
      detectionPolicyRevisionId: snapshot.detection.publishedRevisionId ?? snapshot.release.detectionPolicyRevisionId,
      escalationPolicyRevisionId: snapshot.escalation.publishedRevisionId ?? snapshot.release.escalationPolicyRevisionId,
      safetyPolicyRevisionId: snapshot.minimumSafetyRevisionId ?? snapshot.release.safetyPolicyRevisionId,
      modelProfileRevisionId: snapshot.detection.modelProfileRevisionId ?? snapshot.release.modelProfileRevisionId,
      calibratorRevisionId: snapshot.release.calibratorRevisionId,
      routingOverlayRevisionId: snapshot.escalation.routingOverlayRevisionId ?? snapshot.release.routingOverlayRevisionId,
      compilerRevisionId: snapshot.detection.compilerRevisionId ?? snapshot.release.compilerRevisionId,
  ...
  function activateSelected() {
    ...
    if (!snapshot?.runtime || !value || value.pending || value.status !== "PASSED" || !Object.values(value.gates).every(Boolean) || !canActivate.value || pending.value) return;
    const body: ActivateCaseIntelligenceReleaseCommand = {
      expectedVersion: snapshot.runtime.version,
      detectionPolicyRevisionId: value.detectionPolicyRevisionId || snapshot.detection.publishedRevisionId!,
      escalationPolicyRevisionId: value.escalationPolicyRevisionId || snapshot.escalation.publishedRevisionId!,
      safetyPolicyRevisionId: value.safetyPolicyRevisionId || snapshot.minimumSafetyRevisionId!,
      modelProfileRevisionId: value.modelProfileRevisionId || snapshot.detection.modelProfileRevisionId!,
      ...
      routingOverlayRevisionId: value.routingOverlayRevisionId || snapshot.escalation.routingOverlayRevisionId!,
  ...
  function rollback(target: CaseIntelligenceRelease, reason: string) {
    const snapshot = current.value;
    if (!snapshot?.runtime || !canRollback.value || pending.value || reason.trim().length < 3) return;
    const body: RollbackCaseIntelligenceReleaseCommand = { releaseRevisionId: target.id, expectedVersion: snapshot.runtime.version, reason: reason.trim(), idempotencyKey: context.createIdempotencyKey?.() ?? crypto.randomUUID() };
  ...
  function correct(body: Omit<CorrectCaseIntelligenceDecisionCommand, "idempotencyKey">) {
    ...
    return execute({ operation: "CORRECT_DECISION", body: { ...body, idempotencyKey: context.createIdempotencyKey?.() ?? crypto.randomUUID() } });
  }
  ...
  async function reconcilePending() {
    ...
    try {
      ...
      const expectedKind = attempt.operation === "RUN_EVALUATION"
        ? "EVALUATION_REPORT"
        : attempt.operation === "CORRECT_DECISION" ? "CORRECTION" : "RELEASE";
      const expectedOperation = attempt.operation === "RUN_EVALUATION"
        ? "RUN_CASE_INTELLIGENCE_EVALUATION"
        : attempt.operation;
      if (outcome.pending) {
        error.value = "Сервер всё ещё выполняет эту команду. Новые изменения остаются заблокированными.";
        return;
      }
      if (outcome.operation !== expectedOperation || outcome.resultKind !== expectedKind) {
        error.value = "Ответ сервера не совпал с сохранённой командой. Изменения заблокированы до ручной сверки.";
        return;
      }
      writeRetained(scope, null);
      pending.value = null;
      ...
      const problem = normalizeApiError(cause);
      if (problem.status === 404) error.value = "Сервер ещё не подтвердил исход. Новая команда остаётся заблокированной.";
      else await handleError(problem, scope);
    }
  }

  return {
    current, datasets, dataset, evaluations, evaluation, observability, decisions, decision, releases,
    loading, loadingMore, mutating, accessDenied, error, feedback, pending,
    datasetCursor, evaluationCursor, decisionCursor,
    canRead, canManageRelease, canReadCost, canReadDecisions, canReviewLabels, canActivate, canRollback, canCorrect,
    load, loadMore, reset, selectDataset, selectEvaluation, selectDecision, explainCase,
    runEvaluation, activateSelected, rollback, correct, reconcilePending,
  };
}
