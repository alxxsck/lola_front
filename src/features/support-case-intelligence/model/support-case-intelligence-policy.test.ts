...
describe('Case Intelligence policy form', () => {
  ...
  it('maps every published dry-run reason to safe Russian copy', () => {
    ...
    expect(caseIntelligenceReasonLabel('NEW_SERVER_REASON')).toContain('неизвестную причину');
  });
  ...
  it('enforces server collection and runtime bounds before save', () => {
    ...
    policy.audience.include = [{ attributeCode: 'segment', operator: 'EQ', value: '' }];
    policy.rules = [
      {
        ...createRule(1),
        kind: 'ATTRIBUTE',
  ...
  it('describes only the detection revision pinned by the active bundle as working', () => {
    type Snapshot = NonNullable<Parameters<typeof presentCaseIntelligenceRuntime>[0]>;
    const withoutBundle = {
      allowedActions: [],
      detection: {
        draft: null,
    ...
    expect(presentCaseIntelligenceRuntime(withoutBundle).label).toContain('ещё не собрана');
    ...
    expect(presentCaseIntelligenceRuntime(withBundle).label).toBe('Правила применяются');
    expect(
      presentCaseIntelligenceRuntime({
        ...withBundle,
