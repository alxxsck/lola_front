...
function source(overrides: Partial<SupportCaseEscalationSource> = {}): SupportCaseEscalationSource {
  return {
...
describe('useSupportCaseEscalation', () => {
  ...
  it('loads the editable policy and immutable Project Safety projection', async () => {
    ...
    expect(controller.policy.value.explicitHumanRequestRules[0]?.code).toBe('HUMAN_REQUEST_RU');
  ...
  it('never sends an incomplete simulator event to the server', async () => {
    ...
    await controller.load();
    controller.simulationSteps.value = [createSimulationStep('POLICY_SWITCH', 0)];
  ...
  it('retains the exact command after an unknown outcome and reconciles it by lookup', async () => {
    const adapter = source({
      saveDraft: vi.fn().mockRejectedValue(new ApiError(503, 'Временно недоступно')),
      read: vi.fn().mockResolvedValue(snapshot()),
      lookupCommand: vi.fn().mockResolvedValue({ resultKind: 'ESCALATION_POLICY' }),
    });
    const controller = useSupportCaseEscalation({
      authority: () => ({
        actorId: 'lead-1',
  ...
  it('preserves the local draft and requires a fresh confirmation after version conflict', async () => {
    ...
    const adapter = source({
      saveDraft: vi.fn().mockRejectedValue(new ApiError(409, 'Версия изменилась')),
    ...
    expect(await controller.save()).toBe(false);
    expect(controller.policy.value.routingPolicyRevisionId).toBe('local-routing-draft');
    expect(controller.pendingAttempt.value).toBeNull();
  ...
  it('restores an old Project attempt when the actor returns', async () => {
    const adapter = source({
      saveDraft: vi.fn().mockRejectedValue(new ApiError(504, 'Исход неизвестен')),
    });
    let projectId = 'project-1';
    const controller = useSupportCaseEscalation({
      authority: () => ({ actorId: 'lead-1', projectId, permissions }),
  ...
  it('uses the same intent for exact replay when command lookup has no receipt', async () => {
    ...
    const adapter = source({
      saveDraft,
      lookupCommand: vi.fn().mockRejectedValue(new ApiError(404, 'Команда не найдена')),
    });
    const controller = useSupportCaseEscalation({
      authority: () => ({
        actorId: 'lead-1',
