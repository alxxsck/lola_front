...
describe('useSupportCaseIntelligence', () => {
  it('loads the authoritative draft and saves with the current version and stable key', async () => {
    ...
    const source: SupportCaseIntelligenceSource = {
      ...mockSupportCaseIntelligenceSource,
      saveDetectionDraft: vi.fn(mockSupportCaseIntelligenceSource.saveDetectionDraft),
    };
  ...
  it('retains an unknown command and replays its exact body and key', async () => {
    ...
    const source: SupportCaseIntelligenceSource = {
      ...
      saveDetectionDraft: vi
        ...
        .mockImplementation(mockSupportCaseIntelligenceSource.saveDetectionDraft),
    };
    const controller = useSupportCaseIntelligence({
  ...
  it('discards a late previous-project read', async () => {
    const first = deferred<Awaited<ReturnType<SupportCaseIntelligenceSource['read']>>>();
    let projectId = 'project-a';
    const source: SupportCaseIntelligenceSource = {
      ...mockSupportCaseIntelligenceSource,
      read: vi.fn(async (id) =>
        id === 'project-a' ? first.promise : mockSupportCaseIntelligenceSource.read(id),
      ),
    };
    const controller = useSupportCaseIntelligence({
    ...
    projectId = 'project-b';
    await controller.load();
    const currentProject = controller.snapshot.value?.detection?.published?.projectId;
    first.resolve(await mockSupportCaseIntelligenceSource.read('project-a'));
    await oldLoad;

    expect(controller.snapshot.value?.detection?.published?.projectId).toBe(currentProject);
  ...
  it('forgets a definite authentication failure before the same scope returns', async () => {
    ...
    const source: SupportCaseIntelligenceSource = {
      ...mockSupportCaseIntelligenceSource,
      saveDetectionDraft: vi.fn().mockRejectedValue(new ApiError(428, 'fresh auth')),
    };
  ...
  it('does not publish a late preview from a previous project', async () => {
    const previewResult = deferred<Awaited<ReturnType<SupportCaseIntelligenceSource['dryRun']>>>();
    ...
    const source: SupportCaseIntelligenceSource = {
      ...
      dryRun: vi.fn(async (id, definition, messages, signal) =>
        id === 'preview-project-a'
          ...
          : mockSupportCaseIntelligenceSource.dryRun(id, definition, messages, signal),
      ),
    };
    const controller = useSupportCaseIntelligence({
  ...
  it('purges retained recovery when the permission scope is revoked', async () => {
    ...
    const source: SupportCaseIntelligenceSource = {
      ...mockSupportCaseIntelligenceSource,
      saveDetectionDraft: vi.fn().mockRejectedValue(new ApiError(503, 'unknown outcome')),
    };
    const authority = () => ({
  ...
  it('uses the server-selected model for a new policy and blocks an unknown profile', async () => {
    ...
    await controller.load();
    expect(controller.detection.value.modelProfileRevisionId).toBe('profile-approved');
  ...
  it('accepts the server-assigned model even when it is not repeated in the selectable catalog', async () => {
    const projectId = crypto.randomUUID();
    const snapshot = await mockSupportCaseIntelligenceSource.read(projectId);
    const assignedRevisionId = snapshot.detection!.published!.definition.modelProfileRevisionId;
    const source: SupportCaseIntelligenceSource = {
      ...mockSupportCaseIntelligenceSource,
      read: vi.fn().mockResolvedValue(snapshot),
      readModelProfiles: vi.fn().mockResolvedValue({
  ...
  it('discards a late calibration response from a previous project', async () => {
    const result =
      deferred<Awaited<ReturnType<SupportCaseIntelligenceSource['readCalibration']>>>();
    ...
    const source: SupportCaseIntelligenceSource = {
      ...
      readCalibration: vi.fn(async (id, definition, signal) =>
        id === 'calibration-project-a'
          ? result.promise
          : mockSupportCaseIntelligenceSource.readCalibration(id, definition, signal),
      ),
    };
    const controller = useSupportCaseIntelligence({
      authority: () => ({
