...
export function createAmbiguousRequestRule(
  index: number,
): EscalationAmbiguousRule {
  return {
...
function validatePhraseRules(
  ...
  if (rules.length > 100)
    issues.push({ path, message: "Можно добавить не больше 100 правил." });
...
export function validateEscalationPolicy(
  ...
  validatePhraseRules(
    issues,
    "ambiguousHumanTermRules",
    policy.ambiguousHumanTermRules,
  );
  validatePhraseRules(
    issues,
    "doNotEscalateRules",
    policy.doNotEscalateRules ?? [],
  );
    ...
    scenario.dataToCollect.forEach((code, fieldIndex) => {
      if (!stableCode.test(code))
        issues.push({
          ...
          message:
            "Код данных: заглавные латинские буквы, цифры и подчёркивание.",
        });
    });
  });
  ...
  if (
    !policy.routingPolicyRevisionId.trim() ||
    policy.routingPolicyRevisionId.length > 128
  )
    issues.push({
  ...
  trustedOutcomes.forEach((outcome) => {
    const match = policy.trustedOutcomeLimits.find(
...
export function simulationStepShapeIssue(
  step: EscalationSimulationStep,
): string {
  if (
...
export function simulationStepReferenceIssue(
  ...
  if (
    step.kind === "EXPLICIT_HUMAN_REQUEST" &&
    !policy.explicitHumanRequestRules.some(
...
export function normalizeSimulationStepSafety(
  ...
  if (normalized.kind === "POLICY_SWITCH") {
    ...
    normalized.safetyState !== "SUSPECTED" &&
    normalized.safetyState !== "URGENT"
  ) {
    normalized.safetyRiskClass = null;
  }
  return normalized;
