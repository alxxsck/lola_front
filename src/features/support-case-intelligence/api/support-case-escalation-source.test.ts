...
describe("apiSupportCaseEscalationSource", () => {
  ...
  it("uses generated read, locked Safety and non-dispatching dry-run operations", async () => {
    ...
    expect(caseIntelligenceProjectSafetyPolicy).toHaveBeenCalledWith(
      "project-1",
      undefined,
    );
  ...
  it("sends exact version, idempotency key and disables automatic auth replay", async () => {
    vi.mocked(caseIntelligenceSaveEscalationDraft).mockResolvedValue(
      {} as never,
    );
    vi.mocked(caseIntelligenceDiscardEscalationDraft).mockResolvedValue(
      {} as never,
    );
    vi.mocked(caseIntelligencePublishEscalation).mockResolvedValue({} as never);
    const definition = createDefaultEscalationPolicy();

    await apiSupportCaseEscalationSource.saveDraft(
      "project-1",
      definition,
      4,
      "save-key",
  ...
  it("mirrors attempt and outcome conflict semantics in the mock simulator", async () => {
    ...
    const attemptConflict = await mockSupportCaseEscalationSource.dryRun(
      "project-1",
      definition,
      [first, changedOutcome],
    );
    const outcomeConflict = await mockSupportCaseEscalationSource.dryRun(
      "project-1",
      definition,
      [first, reusedOutcome],
    );
  ...
  it("mirrors pending, failed and class-specific Safety consequences", async () => {
    ...
    const [pendingResult, failedResult, responsibleGamingResult] =
      await Promise.all(
        [pending, failed, responsibleGaming].map((step) =>
          mockSupportCaseEscalationSource.dryRun("project-1", definition, [
            step,
          ]),
        ),
      );
    const result = {
      steps: [
        pendingResult.steps[0],
        failedResult.steps[0],
        responsibleGamingResult.steps[0],
      ],
    };
  ...
  it("enforces offer deadlines before accept, decline and timeout", async () => {
    ...
    const afterCooldown = await mockSupportCaseEscalationSource.dryRun(
      "project-1",
      definition,
      [...cooldownSteps, nextOffer],
    );
