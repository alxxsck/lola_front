...
const route = (id: string, provider: 'AMPLITUDE' | 'CUSTOMER_IO', sourcePath: string[]) => ({
  id,
  projectId: 'project-1',
...
describe('IntegrationCanonicalIdentityPolicyCard', () => {
  ...
  it('renders the active immutable revision and all participant extractors', async () => {
    ...
    expect(wrapper.text()).toContain('пробелы удаляются, сравнение в нижнем регистре');
    expect(wrapper.find('button[data-action="preview-canonical-policy"]').exists()).toBe(false);
  });
  ...
  it('previews two providers, warns about conflict semantics and publishes the exact preview', async () => {
    ...
    await wrapper.get('input[name="canonicalKeyName"]').setValue('transaction_id');
    for (const checkbox of wrapper.findAll('input[name="canonicalParticipant"]')) {
      await checkbox.setValue(true);
    }
    expect(wrapper.findAll('input[name="canonicalParticipant"]')).toHaveLength(2);
    expect(
      wrapper.get('button[data-action="preview-canonical-policy"]').attributes('disabled'),
    ).toBeUndefined();
    ...
    await wrapper.get('button[data-action="publish-canonical-policy"]').trigger('click');
    await flushPromises();
  ...
  it('shows a bounded load error and retries without exposing backend details', async () => {
    api.listDefinitions.mockRejectedValueOnce(new Error('secret upstream trace'));
    const wrapper = mountCard();
    await flushPromises();
  ...
  it('translates OCC conflicts without rendering backend error details', async () => {
    ...
    expect(wrapper.get('[role="alert"]').text()).toContain('Правило уже изменилось');
