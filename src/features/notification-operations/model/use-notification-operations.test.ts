...
function api(): NotificationOperationsApi {
  return {
    ...
    deliveries: vi.fn().mockResolvedValue({ items: [delivery()], nextCursor: null }),
    integrations: vi.fn().mockResolvedValue({ items: [integration()], nextCursor: null }),
...
describe('notification operations controller', () => {
  it('keeps only the latest same-authority response and loading owner for every read resource', async () => {
    ...
    const oldDeliveries = deferred<Awaited<ReturnType<NotificationOperationsApi['deliveries']>>>();
    const newDeliveries = deferred<Awaited<ReturnType<NotificationOperationsApi['deliveries']>>>();
    const oldIntegrations =
      deferred<Awaited<ReturnType<NotificationOperationsApi['integrations']>>>();
    ...
    expect(controller.health.value?.observedAt).toBe('2026-07-23T11:00:00.000Z');
    expect(controller.deliveries.value.map(({ id }) => id)).toEqual(['new-delivery']);
    expect(controller.integrations.value.map(({ integrationId }) => integrationId)).toEqual([
      'new-integration',
    ]);
    expect(controller.nextDeliveryCursor.value).toBe('new-delivery-cursor');
    expect(controller.nextIntegrationCursor.value).toBe('new-integration-cursor');
  ...
  it('does not append an older cursor page over a newer replacement page', async () => {
    ...
    const controller = createNotificationOperationsController({ api: client });
    activate(controller);
    await Promise.all([controller.loadDeliveries(), controller.loadIntegrations()]);

    const oldAppends = [controller.loadMoreDeliveries(), controller.loadMoreIntegrations()];
    const replacements = [controller.loadDeliveries(), controller.loadIntegrations()];
    ...
    expect(controller.deliveries.value.map(({ id }) => id)).toEqual(['replacement-delivery']);
    expect(controller.integrations.value.map(({ integrationId }) => integrationId)).toEqual([
      'replacement-integration',
    ]);
    expect(controller.nextDeliveryCursor.value).toBe('replacement-delivery-cursor');
    expect(controller.nextIntegrationCursor.value).toBe('replacement-integration-cursor');
  });
  ...
  it('loads safe health and independent cursor pages under one authority', async () => {
    ...
    expect(controller.deliveries.value.map(({ id }) => id)).toEqual(['delivery-1', 'delivery-2']);
    expect(controller.integrations.value.map(({ integrationId }) => integrationId)).toEqual([
      'integration-1',
      'integration-2',
    ]);
  ...
  it('clears protected state and rejects late responses after authority changes', async () => {
    const client = api();
    const late = deferred<Awaited<ReturnType<NotificationOperationsApi['deliveries']>>>();
  ...
  it('resolves replay by the current server row and rejects fabricated or unknown caller data', async () => {
    ...
    expect(vi.mocked(client.replay).mock.calls[0]?.[1].idempotencyKey).toBe('stable-replay-key');
    expect(vi.mocked(client.replay).mock.calls[1]?.[1].idempotencyKey).toBe('stable-replay-key');
    expect(controller.deliveries.value).toEqual([]);
  });
