...
describe('QuarantineIntegrationDialog', () => {
  it('requires a closed reason and exact server-issued identity', async () => {
    ...
    await wrapper.get('select[aria-label="Причина quarantine"]').setValue('CREDENTIAL_COMPROMISED');
    await wrapper.get('input[aria-label="Подтверждение masked identity"]').setValue('Slack wrong');
    expect(confirm.attributes('disabled')).toBeDefined();
    ...
    expect(wrapper.emitted('confirm')).toEqual([['CREDENTIAL_COMPROMISED', target.maskedIdentity]]);
