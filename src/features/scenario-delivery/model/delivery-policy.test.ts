...
describe('Delivery Policy domain', () => {
  it.each([
    ...
    expect(serializeDeliveryPolicy({ ...createDeliveryPolicyDraft(), kind })).toEqual({
      ok: true,
      value: expected,
    });
  });
  ...
  it('restores a published policy without trusting an unknown response shape', () => {
    ...
    expect(deserializeDeliveryPolicy({ kind: 'FUTURE_POLICY', secret: true })).toEqual({
      kind: 'IMMEDIATE',
    });
  });
  ...
  it.each([
    ...
    expect(
      deliveryPolicySummary({ kind: 'WAIT_UNTIL_ONLINE', expiryMs, recheckEligibility: false }),
    ).toContain(label);
  });
});
