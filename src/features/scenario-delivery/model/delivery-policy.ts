...
export function deserializeDeliveryPolicy(value: unknown): DeliveryPolicyDraft {
  ...
  if (
    ...
    return {
      kind: source.kind,
      expiryMs: source.expiryMs,
      recheckEligibility: source.recheckEligibility,
    };
  }
  if (
    source.kind === 'IMMEDIATE' ||
    source.kind === 'SKIP_IF_OFFLINE' ||
    source.kind === 'FAIL_IF_OFFLINE'
  ) {
...
export function serializeDeliveryPolicy(draft: DeliveryPolicyDraft): DeliveryPolicyResult {
  ...
  if (
    !Number.isInteger(draft.expiryMs) ||
    draft.expiryMs < 1_000 ||
    draft.expiryMs > 7 * 86_400_000
  ) {
    return {
      ok: false,
      issues: [
        {
          code: 'expiry-invalid',
          field: 'expiryMs',
          message: 'Срок ожидания появления в сети должен быть от 1 секунды до 7 дней.',
        },
      ],
    };
  }
  return {
    ...
    value: {
      ...
      recheckEligibility: draft.recheckEligibility,
    },
  };
}
...
export function deliveryPolicySummary(draft: DeliveryPolicyDraft): string {
  ...
  if (draft.kind === 'FAIL_IF_OFFLINE')
    return 'Завершить действие ошибкой, если пользователь не в сети';
