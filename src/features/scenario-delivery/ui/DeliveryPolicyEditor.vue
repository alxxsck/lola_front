...
const cards: Array<{ kind: DeliveryPolicyKind; title: string; description: string }> = [
  {
    kind: 'IMMEDIATE',
    title: 'Выполнить сразу',
    description:
      'Действие запускается сразу; отсутствие пользователя в сети само по себе его не отменяет.',
  },
  {
    kind: 'SKIP_IF_OFFLINE',
    title: 'Пропустить, если пользователь не в сети',
    description: 'Сценарий продолжится без этого действия.',
  },
  {
    kind: 'FAIL_IF_OFFLINE',
    title: 'Завершить ошибкой, если пользователь не в сети',
    description: 'Run явно зафиксирует неуспешную доставку.',
  },
  {
    kind: 'WAIT_UNTIL_ONLINE',
    title: 'Подождать появления в сети',
    description: 'Ожидание всегда ограничено отдельным сроком.',
  },
];
const units = [
...
function current(): DeliveryPolicyDraft {
  ...
  return {
    kind: kind.value,
    expiryMs: Number(duration.value) * multiplier,
    recheckEligibility: recheckEligibility.value,
  };
}
...
<template>
  <section class="delivery-editor" aria-labelledby="delivery-title">
    <div class="delivery-head">
      ...
      <h2 id="delivery-title">Что делать, если пользователь не в сети</h2>
      <p>Доставка настраивается отдельно от условий и срока цели.</p>
    </div>
    ...
    <div v-if="kind === 'WAIT_UNTIL_ONLINE'" class="online-controls">
      <div>
        <strong>Срок ожидания появления в сети</strong>
        <p>По истечении этого срока действие больше не ждёт. Ожидание не продлевает срок цели.</p>
      </div>
      <div class="duration-row">
        ...
        /><select
          ...
          aria-label="Единица срока ожидания появления в сети"
          @change="commit"
        >
      ...
      <label class="recheck"
        ><input
          v-model="recheckEligibility"
          type="checkbox"
          aria-label="Повторно проверить условия перед доставкой"
          @change="commit"
        /><span
          ><strong>Повторно проверить условия перед доставкой</strong
          ><small
            >Backend повторно применит pinned V2 rule; новый draft сценария на активный Run не
            влияет.</small
          ></span
        ></label
      >
    ...
    <div class="delivery-summary">
      <i class="pi pi-info-circle" /><span>{{ deliveryPolicySummary(current()) }}</span>
    </div>
  </section>
</template>
...
.policy-card.selected {
  ...
  box-shadow: 0 0 0 1px var(--status-accent-text);
}
.policy-card input {
  margin-top: 3px;
}
