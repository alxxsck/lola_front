...
const preference = (conversationLocale = 'de'): ConversationTranslationResponseDto => ({
...
const readyDraft = (sourceText = 'Здравствуйте'): ReplyTranslationDraftResponseDto => ({
...
function api(overrides: Partial<ConversationTranslationApi> = {}): ConversationTranslationApi {
  return {
...
describe('conversation translation controller', () => {
  ...
  it('игнорирует preference от ранее выбранного диалога', async () => {
    ...
    const client = api({
      getConversation: vi.fn().mockReturnValueOnce(first).mockResolvedValueOnce(preference('fr')),
    });
  ...
  it('не переносит поздний edit preview в другой диалог', async () => {
    ...
    expect(
      sessionStorage.getItem('retenive:reply-translation-draft:project-1:user-1:conversation-2'),
    ).toBeNull();
  });
  ...
  it.each([
    ...
    ['budget exhausted', { available: true, reason: null }, true, 'Лимит переводов исчерпан'],
  ])('не создаёт preview при %s', async (_label, availability, hardExhausted, expectedError) => {
  ...
  it('мерджит только актуальный CMS translation realtime', async () => {
    ...
    expect(controller.messageTranslations.value.get('message-1')?.translatedText).toBe(
      'Добрый день',
    );
    expect(
  ...
  it('после reload возобновляет polling незавершённого draft', async () => {
    ...
    try {
      ...
      const client = api({
        getReplyDraft: vi.fn().mockResolvedValueOnce(pending).mockResolvedValueOnce(readyDraft()),
      });
      const controller = createConversationTranslationController(
        {
          projectId: () => 'project-1',
  ...
  it('сбрасывает busy-флаги предыдущего диалога при навигации', async () => {
    ...
    const update = new Promise<ConversationTranslationResponseDto>((resolve) => {
  ...
  it('не сохраняет source plaintext в CMS-scoped session recovery envelope', async () => {
    const storageKey = 'retenive:reply-translation-draft:project-1:user-1:conversation-1';
    const controller = createConversationTranslationController(
      {
        projectId: () => 'project-1',
        endUserId: () => 'user-1',
      ...
      api({
        createReplyDraft: vi.fn().mockResolvedValue(readyDraft('Секретный исходный текст')),
      }),
    );
  ...
  it('после reload восстанавливает composer и draft без pre-seeded source', async () => {
    ...
    const getReplyDraft = vi.fn().mockResolvedValue(readyDraft('Секретный исходный текст'));
  ...
  it('удаляет legacy plaintext до завершения первого network request', async () => {
    ...
    let resolveConversation!: (value: ConversationTranslationResponseDto) => void;
    const pendingConversation = new Promise<ConversationTranslationResponseDto>((resolve) => {
  ...
  it('очищает recovery envelope при явном discard', async () => {
    ...
    expect(controller.draft.value).toBeNull();
    expect(
      sessionStorage.getItem('retenive:reply-translation-draft:project-1:user-1:conversation-1'),
    ).toBeNull();
  });
  ...
  it('продолжает polling входящего перевода после transient GET error', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(350);
      expect(controller.messageTranslations.value.get('message-1')?.state).toBe('PENDING');
      await vi.advanceTimersByTimeAsync(700);
      await request;

      expect(controller.messageTranslations.value.get('message-1')?.state).toBe('COMPLETED');
