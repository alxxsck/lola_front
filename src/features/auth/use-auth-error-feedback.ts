...
function boundedRetryAfterSeconds(error: ApiError): number {
  const fromDetails =
    error.details && typeof error.details === 'object' && 'retryAfterSeconds' in error.details
...
function safeMessage(cause: unknown, fallback: string): string {
  ...
  if (cause.code === 'PASSWORD_POLICY_REJECTED') {
    ...
    return reason
      ? PASSWORD_POLICY_MESSAGES[reason]
      : 'Этот пароль нельзя использовать. Выберите другой.';
  }
