...
describe('MFA enrollment retry boundary', () => {
  ...
  it('does not consume another recovery code after browser cancellation', async () => {
    ...
    vi.mocked(mfaApi.completeRecovery).mockResolvedValue(enrollmentOptions as never);
    ...
    await expect(authApi.completeMfaRecovery(challenge, 'lrc_last')).rejects.toMatchObject({
      name: 'NotAllowedError',
    });
    await expect(authApi.completeMfaRecovery(challenge, 'lrc_last')).resolves.toMatchObject({
      kind: 'MFA_ENROLLED',
    });
  ...
  it('reuses exchanged initial enrollment options after browser cancellation', async () => {
    const challenge: Extract<MfaChallenge, { kind: 'MFA_ENROLLMENT_REQUIRED' }> = {
  ...
  it('serializes concurrent MFA session completions', async () => {
    const firstChallenge: Extract<MfaChallenge, { kind: 'MFA_ENROLLMENT_REQUIRED' }> = {
      kind: 'MFA_ENROLLMENT_REQUIRED',
    ...
    await vi.waitFor(() => expect(mfaApi.completeEnrollment).toHaveBeenCalledTimes(1));
  ...
  it('publishes an MFA token before a queued session replacement can clear it', async () => {
    ...
    await vi.waitFor(() => expect(mfaApi.completeAuthentication).toHaveBeenCalledOnce());
  ...
  it('does not call the MFA backend after a queued operation is cancelled', async () => {
    ...
    const completion = authApi.completeMfaPasskey(challenge, undefined, () => current);
    await Promise.resolve();
    current = false;
    releaseReplacement();
