...
describe('support attachments controller', () => {
  ...
  it('rejects unsupported and oversized files locally without upload', async () => {
    ...
    await controller.select();
    await controller.addFiles([file('malware.exe', 'application/octet-stream'), file()]);

    expect(api.startUpload).not.toHaveBeenCalled();
    expect(controller.items.value.map((item) => item.state)).toEqual(['REJECTED', 'REJECTED']);
  });
  ...
  it('revokes a start intent resolved after the operator removed its queued row', async () => {
    const api = source();
    let resolveStart!: (
      value: Awaited<ReturnType<SupportAttachmentsSource['startUpload']>>,
    ) => void;
    vi.mocked(api.startUpload).mockReturnValue(
      new Promise((resolve) => {
  ...
  it.each(['consumeDraft', 'purge'] as const)(
    ...
    async (finalizeDraftB) => {
      ...
      vi.mocked(api.uploadBinary).mockReturnValue(
        new Promise((resolve) => {
      ...
      const secondScope: SupportAttachmentScope = {
        ...firstScope,
  ...
  it('revokes a READY result that resolves after the controller was disposed', async () => {
    const api = source();
    let resolveComplete!: (
      value: Awaited<ReturnType<SupportAttachmentsSource['completeUpload']>>,
    ) => void;
