...
export function createSupportAttachmentsController(
  ...
  const busy = computed(() =>
    items.value.some((item) => item.state === "QUEUED" || item.state === "UPLOADING" || item.state === "SCANNING"),
  );
  ...
  function schedulePoll(requestGeneration: number): void {
    clearTimer();
    if (!items.value.some((item) => item.state === "SCANNING" || item.state === "UPLOADING")) return;
  ...
  async function poll(requestGeneration = generation): Promise<void> {
    ...
    const pending = items.value.filter((item) => !terminal.has(item.state) && item.state !== "QUEUED");
    await Promise.all(pending.map(async (item) => {
      try {
        const next = await source.status(scope, item.id, abort?.signal);
        if (requestGeneration === generation && scopeKey(scope) === selectedScopeKey) replace(next);
      } catch (cause) {
        if (
  ...
  function rejected(file: File, reason: string): SupportAttachmentItem {
    return {
      localId: uuid(), id: "", filename: file.name, contentType: file.type || "application/octet-stream",
      sizeBytes: file.size, state: "REJECTED", canAttach: false, failureCode: reason, canRetry: false,
    };
  }

  async function upload(file: File, scope: SupportAttachmentScope, requestGeneration: number): Promise<void> {
    const localId = uuid();
    ...
    items.value = [...items.value, {
      localId, id: "", filename: file.name, contentType: file.type, sizeBytes: file.size,
      state: "QUEUED", canAttach: false, failureCode: null, canRetry: false,
    }];
    try {
      const checksumSha256 = await (context.sha256 ?? sha256)(file);
      if (requestGeneration !== generation || cancelledUploads.has(localId)) return;
      const intent = await source.startUpload(scope, {
        draftKey: capturedDraftKey, filename: file.name, contentType: file.type,
      ...
      if (requestGeneration === generation && scopeKey(scope) === selectedScopeKey)
        items.value = items.value.map((item) => item.localId === localId
          ? { ...item, id: intent.id, state: "UPLOADING" }
          : item);
      ...
      const currentScope =
        requestGeneration === generation && scopeKey(scope) === selectedScopeKey;
      ...
      items.value = items.value.map((item) => item.localId === localId
        ? { ...item, state: "FAILED", canAttach: false, failureCode: "UPLOAD_FAILED", canRetry: true }
        : item);
    } finally {
  ...
  async function addFiles(files: readonly File[]): Promise<void> {
    ...
    for (const file of files) {
      ...
      else if (!capability.contentTypes.includes(file.type)) rejectedItems.push(rejected(file, "UNSUPPORTED_TYPE"));
      else if (!file.size || file.size > capability.maxFileBytes) rejectedItems.push(rejected(file, "FILE_TOO_LARGE"));
      else if (projectedBytes + file.size > capability.maxTotalBytes) rejectedItems.push(rejected(file, "TOTAL_TOO_LARGE"));
      else { accepted.push(file); projectedBytes += file.size; }
    }
  ...
  async function remove(localId: string): Promise<void> {
    ...
    if (item.id && item.state !== "REVOKED") {
      try { await source.revoke(scope, item.id); }
      catch (cause) {
        if (cause instanceof ApiError && (cause.status === 403 || cause.status === 404)) {
          purge();
          await context.onForbidden?.(cause.status as 403 | 404);
  ...
  function consumeDraft(): void {
    const scope = context.scope();
    if (scope && draftKey.value)
      abortUploads(uploadScopeKey(scope, draftKey.value));
  ...
  return { items, draftKey, readyIds, busy, canSend, totalBytes, loading, error, select, addFiles, remove, retry, download, consumeDraft, purge, dispose };
}
