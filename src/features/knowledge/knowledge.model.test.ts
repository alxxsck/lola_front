...
describe('knowledge model', () => {
  it('accepts a project-scoped document page', () => {
    expect(
      parseKnowledgeDocumentPage({ items: [document], nextCursor: null }, 'project-1'),
  ...
  it('validates file extension and size before upload', () => {
    expect(
      validateKnowledgeFile(new File(['hello'], 'guide.md', { type: 'text/markdown' })),
    ).toBeNull();
    expect(validateKnowledgeFile(new File(['a,b'], 'users.csv', { type: 'text/csv' }))).toContain(
      'не поддерживается',
    );
  ...
  it('accepts only a matching successful delete response', () => {
    expect(parseKnowledgeDelete({ deleted: true, id: 'document-1' }, 'document-1')).toBe(true);
    expect(parseKnowledgeDelete({ deleted: true, id: 'document-2' }, 'document-1')).toBe(false);
    expect(parseKnowledgeDelete({ deleted: false, id: 'document-1' }, 'document-1')).toBe(false);
  });
});
