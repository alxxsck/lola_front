...
function parseDocument(value: unknown, projectId: string): KnowledgeDocument | undefined {
  if (!isRecord(value) || value.projectId !== projectId) return undefined;
  if (!boundedString(value.id, 200) || !boundedString(value.title, 200)) return undefined;
  if (!boundedString(value.filename, 255) || !boundedString(value.mimeType, 200)) return undefined;
  if (value.sourceType !== 'FILE' && value.sourceType !== 'TEXT') return undefined;
  if (value.status !== 'INDEXING' && value.status !== 'READY' && value.status !== 'FAILED')
    return undefined;
  if (!Number.isSafeInteger(value.sizeBytes) || Number(value.sizeBytes) < 0) return undefined;
  if (!nullableString(value.locale, 20) || !nullableString(value.category, 80)) return undefined;
  if (!nullableString(value.errorCode, 200) || !nullableString(value.error, 4_000))
    return undefined;
  if (!boundedString(value.createdAt, 64) || !boundedString(value.updatedAt, 64)) return undefined;
...
export function parseKnowledgeDocumentPage(
  ...
  if (value.nextCursor !== null && !boundedString(value.nextCursor, 200)) return undefined;
...
export function parseKnowledgeDelete(value: unknown, documentId: string): boolean {
...
export function parseKnowledgeDocumentDetail(
  ...
  if (value.contentText !== null && typeof value.contentText !== 'string') return undefined;
  if (typeof value.contentText === 'string' && value.contentText.length > MAX_KNOWLEDGE_TEXT_LENGTH)
    return undefined;
...
export function validateKnowledgeFile(file: File): string | null {
  ...
  if (file.size > MAX_KNOWLEDGE_FILE_BYTES) return 'Файл превышает ограничение 25 МБ.';
