...
describe('ActivitySettingsSection', () => {
  ...
  it('starts collapsed and expands from the shared project header', async () => {
    ...
    expect(wrapper.get('#activity-settings-content').attributes('style')).toContain(
      'display: none',
    );
    ...
    expect(wrapper.get('#activity-settings-content').attributes('style')).not.toContain(
      'display: none',
    );
  });
});
