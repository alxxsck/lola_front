...
async function load() {
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : 'Не удалось загрузить настройки активности';
  } finally {
    loading.value = false;
  }
}
...
async function save() {
  ...
  try {
    fill(
      await repository.updateActivitySettings(props.projectId, {
    ...
    error.value =
      cause instanceof Error ? cause.message : 'Не удалось сохранить настройки активности';
  } finally {
    saving.value = false;
  }
}
...
<template>
  <section class="settings-section card activity-settings" :class="{ collapsed: !expanded }">
    <ProjectSettingsSectionHeader
      v-model:expanded="expanded"
      title="Активность и визиты"
      description="Определите, когда начинается новый день и завершается визит пользователя."
      icon="pi pi-clock"
      tone="lime"
      content-id="activity-settings-content"
    />
    <div id="activity-settings-content" v-show="expanded">
      <Message v-if="error" severity="error" :closable="false">{{ error }}</Message>
      <p v-if="loading" class="activity-loading">
        <i class="pi pi-spin pi-spinner" /> Загружаем настройки активности…
      </p>
      <form v-else-if="settings" class="activity-form" @submit.prevent="save">
        <div class="activity-fields">
          ...
          <label class="activity-field" for="visit-inactivity">
            ...
            <InputNumber
              input-id="visit-inactivity"
              data-testid="visit-inactivity"
              v-model="form.visitInactivitySeconds"
              :min="settings.limits.visitInactivitySeconds.min"
              :max="settings.limits.visitInactivitySeconds.max"
              :use-grouping="false"
              :disabled="!editable"
            />
            <small
              >После этой паузы следующее подключение станет новым визитом. Допустимо
              {{ settings.limits.visitInactivitySeconds.min }}–{{
                settings.limits.visitInactivitySeconds.max
              }}.</small
            >
          </label>
          <label class="activity-field" for="reconnect-grace">
            ...
            <InputNumber
              input-id="reconnect-grace"
              data-testid="reconnect-grace"
              v-model="form.reconnectGraceSeconds"
              :min="settings.limits.reconnectGraceSeconds.min"
              :max="settings.limits.reconnectGraceSeconds.max"
              :use-grouping="false"
              :disabled="!editable"
            />
            <small
              >Сохраняет сессию при коротком разрыве соединения. Допустимо
              {{ settings.limits.reconnectGraceSeconds.min }}–{{
                settings.limits.reconnectGraceSeconds.max
              }}.</small
            >
          </label>
        </div>
        <footer v-if="editable" class="activity-actions">
          <p>
            <i class="pi pi-info-circle" /> Эти параметры сохраняются отдельно от основных настроек
            проекта.
          </p>
...
.activity-actions p i {
  margin-top: 2px;
}
.activity-actions :deep(.p-button) {
  flex: 0 0 auto;
}
