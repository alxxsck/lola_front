...
describe("support conversation collaboration controller", () => {
  ...
  it("purges expired viewers and typers and ignores an old typing stop", async () => {
    ...
    const controller = createSupportConversationCollaborationController(
      source,
      { actorId: () => "operator-1" },
    );
    ...
    expect(controller.typers.value.map((item) => item.displayName)).toEqual([
      "Анна",
    ]);
  ...
  it("does not let a stale REST response overwrite a newer event", async () => {
    ...
    expect(controller.viewers.value.map((item) => item.displayName)).toEqual([
      "Анна",
    ]);
