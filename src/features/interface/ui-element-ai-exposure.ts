...
export function requiresUiElementAiAuditReason(
  ...
  return (
    draft.aiEnabled &&
    (!current?.aiEnabled || aiExposureChanged(current, draft) || aiEffectChanged(current, draft))
  );
}
...
export function validateUiElementAiExposure(
  ...
  if (draft.aiEnabled && !aiTargetBound(draft))
    issues.push('Сначала заполните адрес страницы, имя окна или признак элемента и включите его.');
  ...
  if (aliases.length > 20 || aliases.some((alias) => alias.length > 100)) {
    issues.push('Можно указать не более 20 дополнительных названий длиной до 100 символов.');
  }
