...
export type SupportReplyOutcomeState =
  'IDLE' | 'SENDING' | 'CHECKING_OUTCOME' | 'RETRYABLE' | 'BLOCKED';
...
function restoreAttempt(scope: string): PendingReplyAttempt | undefined {
  try {
    ...
    if (
      ...
      !('state' in value) ||
      !['CHECKING_OUTCOME', 'RETRYABLE', 'BLOCKED'].includes(String(value.state))
    ) {
      forgetAttempt(scope);
      return undefined;
    }
    const attempt = value as PendingReplyAttempt;
    if (draftKey(attempt.projectId, attempt.actorId, attempt.conversationId) !== scope) {
      forgetAttempt(scope);
      return undefined;
    }
    return attempt;
...
export function createSupportReplyController(
  ...
  function syncSelection(): void {
    ...
    if (!activeDraftKey && nextDraftKey && draft.value) {
      const restored = pendingAttempts.get(nextDraftKey) ?? restoreAttempt(nextDraftKey);
  ...
  async function checkAttemptOutcome(scope: string, attempt: PendingReplyAttempt): Promise<void> {
    if (!currentScopeMatches(attempt)) return;
    ...
    try {
      ...
      if (status === 404) {
        ...
        outcomeState.value = 'CHECKING_OUTCOME';
        error.value =
          'Результат пока неизвестен. Повторите проверку — сообщение не отправляется заново.';
      }
      persistAttempt(scope, attempt);
    }
  }
  ...
  async function checkOutcome(): Promise<void> {
    ...
    const attempt = scope ? (pendingAttempts.get(scope) ?? restoreAttempt(scope)) : undefined;
  ...
  async function send(delivery: SupportReplyDelivery = {}): Promise<boolean> {
    ...
    const sendWithoutTranslationReason = delivery.sendWithoutTranslationReason?.trim();
    ...
    const normalizedDelivery: SupportReplyDelivery = {
      ...
      ...(delivery.supportKnowledgeCitationDraftId?.trim()
        ? {
            supportKnowledgeCitationDraftId: delivery.supportKnowledgeCitationDraftId.trim(),
          }
        : {}),
    ...
    const existingIdentity = existing
      ? attemptIdentity(scope, existing.text, existing, existing.endUserCaseId)
      : undefined;
    const replaysOriginalPayload = Boolean(
      outcomeState.value === 'RETRYABLE' &&
      existing &&
      attemptIdentity(scope, text, normalizedDelivery, existing.endUserCaseId) === existingIdentity,
    );
    const endUserCaseId = replaysOriginalPayload ? existing?.endUserCaseId : selection.case?.id;
    const identity = attemptIdentity(scope, text, normalizedDelivery, endUserCaseId);
    ...
    try {
      const result = await source.sendAdminMessage(projectId, selection.endUser.id, {
        ...
        ...(attempt.endUserCaseId ? { endUserCaseId: attempt.endUserCaseId } : {}),
        ...(attempt.replyTranslationDraftId
          ? { replyTranslationDraftId: attempt.replyTranslationDraftId }
          : {}),
        ...(attempt.macroReplyDraftId ? { macroReplyDraftId: attempt.macroReplyDraftId } : {}),
        ...(attempt.supportKnowledgeCitationDraftId
          ? {
              supportKnowledgeCitationDraftId: attempt.supportKnowledgeCitationDraftId,
            }
          : {}),
      ...
      if (attempt.macroReplyDraftId && isMacroDraftFailure(caught)) {
        ...
        error.value =
          'Шаблон изменился или больше недоступен. Текст сохранён — выберите актуальный шаблон.';
      } else if (attempt.supportKnowledgeCitationDraftId && isKnowledgeCitationFailure(caught)) {
        pendingAttempts.delete(scope);
        forgetAttempt(scope);
        outcomeState.value = 'IDLE';
