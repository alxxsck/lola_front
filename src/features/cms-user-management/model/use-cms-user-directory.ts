...
import { cmsUserManagementApi, type CmsUserLifecycleAction } from '../api/cms-user-management.api';
...
export function useCmsUserDirectory(api: CmsUserManagementClient = cmsUserManagementApi) {
...
export function cmsUserSafeError(cause: unknown): string {
  ...
  const messages: Record<string, string> = {
    LAST_PLATFORM_OPERATOR:
      'Нельзя остановить последнего Platform Operator с правом восстановления.',
  ...
  return (
    (cause.code && messages[cause.code]) || 'Не удалось выполнить действие. Попробуйте ещё раз.'
  );
}
