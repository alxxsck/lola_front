...
export type CmsUserLifecycleAction = 'SUSPEND' | 'DEACTIVATE' | 'REACTIVATE' | 'RESET_CREDENTIALS';
...
export const cmsUserManagementApi = {
  ...
  replacePlatformRoles: (cmsUserId: string, version: number, roleIds: string[], reason: string) =>
    platformRoleAssignmentReplace(cmsUserId, {
      version,
      roleIds,
      reason: reason.trim().normalize('NFC'),
    }),
  provision: (body: CmsUserProvisioningDto, idempotencyKey: string) =>
    cmsUserProvisioningProvision(
      {
        ...body,
  ...
  mutate(cmsUserId: string, action: CmsUserLifecycleAction, version: number, reason: string) {
