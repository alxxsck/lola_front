...
describe('End User AI consumption card', () => {
  ...
  it('loads a Project-local report with factual, calculated and operation totals', async () => {
    ...
    expect(mocks.fetch).toHaveBeenCalledWith('project-1', 'user-1', '7d', expect.any(AbortSignal));
  ...
  it('explains immutable historical pricing without exposing platform controls', async () => {
    ...
    expect(pricing.text()).toContain('Если ставка xAI изменилась, сообщите администрации');
  ...
  it('does not render an empty speech row or pricing note without speech usage', async () => {
    mocks.fetch.mockResolvedValueOnce({
      ...report,
      categories: report.categories.filter((category) => category.category !== 'SPEECH'),
    });
    const wrapper = mount(EndUserAiUsageCard, {
      props: {
        projectId: 'project-1',
    ...
    expect(wrapper.find('[data-testid="end-user-tts-pricing"]').exists()).toBe(false);
  });
  ...
  it('ignores a stale report after switching the end user', async () => {
    ...
    second.resolve({ ...report, endUserId: 'user-2' });
    await flushPromises();
    expect(wrapper.text()).toContain('12,5');

    first.resolve({
      ...report,
      totals: { ...report.totals, totalTokens: 999_999 },
    });
    await flushPromises();
    expect(wrapper.text()).not.toContain('1 млн');
  });
});
