...
const models = computed(() => report.value ? aggregateModelUsage(report.value.breakdown) : [])
const currency = computed(() => report.value ? getReportCurrency(report.value) : 'USD')
...
const cachedShare = computed(() => {
  ...
  const share = (totals.value.cachedInputTokens / totals.value.inputTokens) * 100
...
async function load(force = false) {
  ...
  try {
    const nextReport = await fetchAiUsageReport(props.projectId, getAiUsageRange(requestedRange), controller.signal)
    if (generation !== requestGeneration) return
    cache.set(requestedRange, nextReport)
    report.value = nextReport
  } catch (cause) {
    if (controller.signal.aborted || generation !== requestGeneration) return
    error.value = cause instanceof Error ? cause.message : 'Не удалось загрузить расходы на AI'
  } finally {
  ...
  <section class="ai-usage card" aria-labelledby="ai-usage-title">
    <header class="usage-header">
      ...
      <div class="usage-actions">
        ...
        <Button icon="pi pi-refresh" severity="secondary" text rounded aria-label="Обновить статистику" :loading="loading" @click="load(true)" />
      </div>
    </header>

    <Message v-if="error" severity="error" :closable="false">
      <div class="error-row"><span>{{ error }}</span><Button label="Повторить" icon="pi pi-refresh" size="small" text @click="load(true)" /></div>
    </Message>

    <div v-if="loading && !report" class="usage-skeleton" aria-label="Загрузка статистики">
      <Skeleton v-for="item in 4" :key="item" height="7.5rem" border-radius="16px" />
    ...
    <template v-else-if="report && totals">
      <div class="summary-grid">
        ...
        <article class="summary-card">
          ...
          <small>{{ formatTokenCount(totals.inputTokens) }} ввод · {{ formatTokenCount(totals.outputTokens) }} вывод</small>
        </article>
        <article class="summary-card">
