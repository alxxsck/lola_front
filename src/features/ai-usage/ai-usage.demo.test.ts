...
describe('AI usage demo report', () => {
  it('keeps project totals aligned with OpenAI estimates and ElevenLabs billed units', async () => {
    ...
    const elevenLabs = report.breakdown.find((row) => row.provider === 'elevenlabs')!

    expect(report.totals.records).toBe(report.breakdown.reduce((sum, row) => sum + row.records, 0))
    expect(report.totals.estimatedCost).toBeCloseTo(
      report.breakdown.reduce((sum, row) => sum + row.estimatedCost, 0),
      12,
    )
    expect(report.totals.providerBilledUnits).toBe(elevenLabs.providerBilledUnits)
