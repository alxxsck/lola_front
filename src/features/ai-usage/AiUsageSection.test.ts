...
describe('AiUsageSection', () => {
  ...
  it('renders one xAI surface with separate model, Voice and text-to-speech slices', async () => {
    ...
    expect(wrapper.getComponent(AiModelUsageChart).props('rows')).toHaveLength(1);
  ...
  it('shows Event Query consumption inside the Grok panel', async () => {
    ...
    expect(wrapper.get('.event-query-usage').text()).toContain('1,1\u00a0тыс. токенов');
    expect(wrapper.get('.event-query-usage').text()).toContain('74,5\u00a0тыс. токенов');
  ...
  it('never combines mixed currencies under a fabricated USD label', async () => {
    ...
    expect(wrapper.findAll('.metric-switch button')[1]!.attributes()).toHaveProperty('disabled');
  });
  ...
  it('presents TTS from backend characters, cost and current pricing context', async () => {
    ...
    expect(speech.text()).toContain('Если ставка xAI изменилась, сообщите администрации');
