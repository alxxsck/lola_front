export type RoutingSection =
  'overview' | 'identities' | 'workforce' | 'queues' | 'policies' | 'decisions';
...
export interface RoutingOperator {
  ...
  languages: Array<{
    languageTag: string;
    proficiency: 'WORKING' | 'FLUENT' | 'NATIVE';
    preferred: boolean;
  }>;
...
export type QueuePredicate =
  ...
  | {
      kind: 'BOOLEAN';
      field: 'ADMIN_ATTENTION_REQUIRED' | 'DEGRADED' | 'ACTIONABLE';
      value: boolean;
    }
  | {
      kind: 'TIME_RANGE';
      field: 'CREATED_AT' | 'LAST_ACTIVITY_AT' | 'WAITING_SINCE' | 'REOPENED_AT' | 'SLA_DUE_AT';
      from: string | null;
      to: string | null;
    }
  | {
      kind: 'RELATIVE_WINDOW';
      field: 'UNASSIGNED_AGE' | 'WAITING_AGE' | 'SLA_DUE_IN' | 'RESOLVED_AGE';
      days: number;
    };
...
export interface QueueDraft {
  ...
  sort: Array<{
    field:
      'EFFECTIVE_PRIORITY' | 'SLA_DUE_AT' | 'ELIGIBLE_SINCE' | 'LAST_ACTIVITY_AT' | 'CREATED_AT';
    direction: 'ASC' | 'DESC';
  }>;
...
export interface RoutingControlPlaneSource {
  ...
  listDecisions(
    ...
    signal?: AbortSignal,
    cursor?: string,
  ): Promise<{ items: RoutingDecision[]; nextCursor: string | null }>;
  loadMoreCatalog(
    projectId: string,
    ...
    items: Array<RoutingIdentity | RoutingOperator | RoutingQueue | RoutingSlot>;
  ...
  decision(
    ...
    decisionId: string,
    signal?: AbortSignal,
  ): Promise<RoutingDecisionDetail>;
  createTeam(
    projectId: string,
    value: { code: string; name: string },
    context: RoutingCommandContext,
  ): Promise<void>;
  createSkill(
    projectId: string,
    value: { code: string; name: string; kind: 'GENERAL' | 'SAFETY' | 'CHANNEL' },
    context: RoutingCommandContext,
  ): Promise<void>;
  renameIdentity(
    projectId: string,
    kind: 'TEAM' | 'SKILL',
    id: string,
    value: { name: string; expectedVersion: number },
    context: RoutingCommandContext,
  ): Promise<void>;
  archiveIdentity(
    projectId: string,
    kind: 'TEAM' | 'SKILL',
    id: string,
    value: { expectedVersion: number; reason: string },
    context: RoutingCommandContext,
  ): Promise<void>;
  saveWorkforce(
    projectId: string,
  ...
  previewQueue(
    ...
    diagnostics: string[];
    evaluatedAt: string;
    sourceHighWater: Record<string, unknown>;
  }>;
  ...
  revisionDiff(
    ...
    resourceId?: string,
    signal?: AbortSignal,
  ): Promise<RoutingRevisionDiff>;
  restoreRevision(
    projectId: string,
    kind: 'QUEUE' | 'POLICY' | 'WORKFORCE',
    revisionId: string,
    resourceId: string | undefined,
    reasonCode: 'CONFIGURATION_REGRESSION' | 'INCIDENT_RECOVERY' | 'OPERATOR_REQUEST' | 'OTHER',
    context: RoutingCommandContext,
  ): Promise<void>;
}
...
export const emptyQueueDraft = (name = 'Новая очередь'): QueueDraft => ({
  ...
  filter: {
    schemaVersion: 1,
    predicate: { kind: 'AND', children: [{ kind: 'ENUM_IN', field: 'STATUS', values: ['OPEN'] }] },
  },
...
export function routingPolicyLabel(policy: Pick<RoutingPolicy, 'code'> | null | undefined): string {
...
export function routingQueueLabel(queue: Pick<RoutingQueue, 'code' | 'name' | 'kind'>): string {
