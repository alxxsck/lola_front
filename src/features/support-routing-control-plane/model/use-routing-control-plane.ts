...
function message(error: ApiError): string {
  const byCode: Record<string, string> = {
    ...
    IDEMPOTENCY_KEY_REUSED: 'Сервер отклонил ключ команды с другим содержимым.',
    SUPPORT_ROUTING_CODE_INVALID:
      'Идентификатор должен начинаться с латинской буквы и содержать только латинские буквы, цифры и дефис.',
  };
...
export function useRoutingControlPlane(
  ...
  const revisionDiff = shallowRef<Awaited<
    ReturnType<RoutingControlPlaneSource['revisionDiff']>
  > | null>(null);
  ...
  async function command(
    ...
    try {
      const context = commandContext(
        intent,
        payload,
        (payload as { actionEtag?: string })?.actionEtag,
      );
  ...
  async function hydrateResource(kind: 'QUEUE' | 'POLICY', id: string): Promise<void> {
    ...
    try {
      const item =
        kind === 'QUEUE' ? await source.queue(projectId, id) : await source.policy(projectId, id);
  ...
  async function loadMoreCatalog(
    kind: 'teams' | 'skills' | 'operators' | 'queues' | 'slots',
  ): Promise<void> {
    const projectId = authority.projectId.value;
  ...
  const api = {
    createTeam: (code: string, name: string) =>
      command('TEAMS', 'create-team', { code, name }, (projectId, context) =>
        source.createTeam(projectId, { code, name }, context),
      ),
    createSkill: (code: string, name: string, kind: 'GENERAL' | 'SAFETY' | 'CHANNEL') =>
      command('TEAMS', 'create-skill', { code, name, kind }, (projectId, context) =>
        source.createSkill(projectId, { code, name, kind }, context),
      ),
    renameIdentity: (kind: 'TEAM' | 'SKILL', id: string, name: string, expectedVersion: number) =>
      command('TEAMS', `rename:${kind}:${id}`, { name, expectedVersion }, (projectId, context) =>
        source.renameIdentity(projectId, kind, id, { name, expectedVersion }, context),
      ),
    archiveIdentity: (
      kind: 'TEAM' | 'SKILL',
      id: string,
      ...
      command('TEAMS', `archive:${kind}:${id}`, { expectedVersion, reason }, (projectId, context) =>
        source.archiveIdentity(projectId, kind, id, { expectedVersion, reason }, context),
      ),
    saveWorkforce: (configuration: WorkforceConfiguration) => {
      ...
      return command(
        'TEAMS',
        'save-workforce',
        { configuration, actionEtag },
        (projectId, context) => source.saveWorkforce(projectId, configuration, context),
      );
    },
    discardWorkforce: () => {
      const actionEtag = snapshot.value?.workforce.actionEtag;
      return command('TEAMS', 'discard-workforce', { actionEtag }, (projectId, context) =>
        source.discardWorkforce(projectId, context),
      );
    },
    publishWorkforce: () => {
      const actionEtag = snapshot.value?.workforce.actionEtag;
      return command('TEAMS', 'publish-workforce', { actionEtag }, (projectId, context) =>
        source.publishWorkforce(projectId, context),
      );
    },
    createQueue: (code: string, draft: QueueDraft) =>
      command('QUEUES', 'create-queue', { code, draft }, (projectId, context) =>
        source.createQueue(projectId, code, draft, context),
      ),
    saveQueue: (queueId: string, draft: QueueDraft) => {
      ...
      return command(
        ...
        `save-queue:${queueId}`,
        { draft, actionEtag },
        (projectId, context) => source.saveQueue(projectId, queueId, draft, context),
      );
    },
    publishQueue: (queueId: string) => {
      const actionEtag = snapshot.value?.queues.find((item) => item.id === queueId)?.actionEtag;
      return command('QUEUES', `publish-queue:${queueId}`, { actionEtag }, (projectId, context) =>
        source.publishQueue(projectId, queueId, context),
      );
    },
    createPolicy: (code: string, draft: PolicyDraft) =>
      command('ROUTING', 'create-policy', { code, draft }, (projectId, context) =>
        source.createPolicy(projectId, code, draft, context),
      ),
    ...
    savePolicy: (policyId: string, draft: PolicyDraft) => {
      ...
      return command(
        'ROUTING',
        `save-policy:${policyId}`,
        { draft, actionEtag },
        (projectId, context) => source.savePolicy(projectId, policyId, draft, context),
      );
    },
    publishPolicy: (policyId: string) => {
      ...
      return command(
        'ROUTING',
        `publish-policy:${policyId}`,
        { actionEtag },
        (projectId, context) => source.publishPolicy(projectId, policyId, context),
      );
    },
    bind: (queueId: string, policyId: string, routePriority: number) => {
      const actionEtag =
        snapshot.value?.slots.find((item) => item.queueId === queueId)?.actionEtag ??
        snapshot.value?.slotActionEtag;
      return command(
        'ROUTING',
        `bind:${queueId}`,
        { policyId, routePriority, actionEtag },
        (projectId, context) => source.bind(projectId, queueId, policyId, routePriority, context),
      );
    },
    runShadow: async (limit = 50) => {
      ...
      try {
        ...
        for (
          let attempt = 0;
          attempt < 30 && (terminal.state === 'QUEUED' || terminal.state === 'RUNNING');
          attempt += 1
        ) {
    ...
    activate: (
      queueId: string,
      targetMode: 'DISABLED' | 'OFFER' | 'AUTO_ASSIGN',
      reasonCode: string,
    ) => {
      const expected =
        snapshot.value?.readiness.find((item) => item.queueId === queueId)?.activation?.version ??
        0;
      return command(
        'ROUTING',
        `activate:${queueId}`,
        { targetMode, reasonCode, expected },
        (projectId, context) =>
          source.activate(projectId, queueId, targetMode, expected, reasonCode, context),
      );
    },
    loadRevisions: async (kind: 'QUEUE' | 'POLICY' | 'WORKFORCE', resourceId?: string) => {
      ...
      const nextDiff =
        nextRevisions.length > 1
          ? await source.revisionDiff(
              projectId,
              kind,
              nextRevisions[1]!.id,
              nextRevisions[0]!.id,
    ...
    restoreRevision: (
      ...
      command(
        kind === 'QUEUE' ? 'QUEUES' : kind === 'WORKFORCE' ? 'TEAMS' : 'ROUTING',
        `restore:${kind}:${revisionId}`,
    ...
    loadAudit: async (
      resourceType:
        'SUPPORT_QUEUE' | 'SUPPORT_QUEUE_ROUTING' | 'SUPPORT_ROUTING_POLICY' | 'SUPPORT_WORKFORCE',
      resourceId: string,
    ) => {
      const projectId = authority.projectId.value;
      if (!projectId) return;
  ...
  watch(
    [
      authority.projectId,
      authority.actorId,
      authority.permissionRevision,
      authority.canRead,
      authority.canReadRouting,
      authority.canReadTeams,
      authority.canReadAvailability,
      authority.canReadQueues,
    ],
    () => void load(),
    { immediate: true },
  );
  ...
  return {
    ...
    error,
    announcement,
    hasBlockingReadiness: computed(
      () => snapshot.value?.readiness.some((item) => item.status !== 'READY') ?? false,
    ),
    reload: () => load(false),
