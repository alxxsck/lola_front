...
function asConfiguration(value: unknown): WorkforceConfiguration {
  ...
  return {
    ...
    operators: Array.isArray(record.operators)
      ? structuredClone(record.operators)
      : [],
  };
}
...
function mapWorkforce(value: Awaited<ReturnType<typeof supportWorkforceGetWorkforce>>): WorkforceState {
...
function mapPolicy(value: Awaited<ReturnType<typeof supportRoutingDetail>>, actionEtag: string): RoutingPolicy {
  return {
    id: value.id,
    ...
    detailLoaded: true,
    draft: value.draft
      ? { generation: value.draft.generation, version: value.draft.version, configuration: policyDraft(value.draft.configuration as PolicyDraft) }
      : null,
...
function mapQueue(value: Awaited<ReturnType<typeof supportQueueDetail>>): RoutingQueue {
  ...
  return {
    ...
    actionEtag: value.actionEtag,
    detailLoaded: true,
    draft: value.draft && draftConfiguration
      ? { generation: value.draft.generation, version: value.draft.version, configuration: draftConfiguration }
      : null,
    published: value.publishedRevision
      ? { id: value.publishedRevision.id, revisionNumber: value.publishedRevision.revisionNumber, publishedAt: value.publishedRevision.publishedAt }
      : null,
  };
}

function excludedCount(value: object): number {
  return Object.values(value).reduce<number>((total, current) => total + (typeof current === "number" ? current : 0), 0);
}

function mapDecision(value: Awaited<ReturnType<typeof supportRoutingRuntimeDecisionList>>["items"][number]): RoutingDecision {
  return {
    id: value.id,
    caseId: value.caseId,
    mode: value.mode,
...
function mapReadiness(value: Awaited<ReturnType<typeof supportRoutingRuntimeReadiness>>["items"][number]): RoutingReadiness {
  return {
    queueId: value.queueId,
...
const apiSource: RoutingControlPlaneSource = {
  async load(projectId, access, signal) {
    const [teamPage, skillPage, workforceDto, queuePage, policyPage, slotPage, readinessPage, activationPage, operatorPage] = await Promise.all([
      access.teams ? supportWorkforceListTeams(projectId, { limit: 100 }, readOptions(signal)) : Promise.resolve(null),
      access.teams ? supportWorkforceListSkills(projectId, { limit: 100 }, readOptions(signal)) : Promise.resolve(null),
      access.teams ? supportWorkforceGetWorkforce(projectId, readOptions(signal)) : Promise.resolve(null),
      access.queues ? supportQueueList(projectId, { limit: 100 }, readOptions(signal)) : Promise.resolve(null),
      access.routing ? supportRoutingList(projectId, { limit: 200 }, readOptions(signal)) : Promise.resolve(null),
      access.routing ? supportRoutingRuntimeQueueSlots(projectId, { limit: 100 }, readOptions(signal)) : Promise.resolve(null),
      access.routing ? supportRoutingRuntimeReadiness(projectId, undefined, readOptions(signal)) : Promise.resolve(null),
      access.routing ? supportRoutingRuntimeActivation(projectId, readOptions(signal)) : Promise.resolve(null),
      access.teams ? supportPresentationsCatalogOperators(projectId, { limit: 100 }, readOptions(signal)) : Promise.resolve(null),
    ]);
    ...
    const policies = (policyPage?.items ?? []).map((policy) => ({ ...mapPolicy(policy, ""), detailLoaded: false }));
    const availabilityPage = access.availability && operatorPage?.items.length
    ...
    const configuration = (access.teamsManage ? workforceDto?.draft?.configuration : undefined) ?? workforceDto?.publishedRevision?.configuration;
    const operators = (operatorPage?.items ?? []).map((item) => {
      const configured = configuration?.operators.find((operator) => operator.cmsUserId === item.cmsUserId);
      const teamIds = configuration?.teams.filter((team) => team.members.includes(item.cmsUserId)).map((team) => team.teamId) ?? [];
      return {
        id: item.cmsUserId,
        name: item.displayName,
        state: item.membershipState === "ACTIVE" ? "ACTIVE" as const : item.membershipState === "INACTIVE" ? "INACTIVE" as const : "UNKNOWN" as const,
        maxCapacityUnits: configured?.maxCapacityUnits ?? 1,
        teamIds,
        skills: configured?.skills.map((skill) => ({ ...skill, preferred: skill.preferred ?? false })) ?? [],
        languages: configured?.languages.map((language) => ({ ...language, preferred: language.preferred ?? false })) ?? [],
        availability: availabilityByOperator.get(item.cmsUserId)?.status ?? "UNKNOWN",
      };
    });
    const slots: RoutingSlot[] = (slotPage?.items ?? []).map((slot) => ({ ...slot, actionEtag: slotPage!.actionEtag }));
    return {
      ...
      workforce: workforceDto
        ...
        : { actionEtag: "", rootVersion: 0, currentRevisionNumber: 0, draft: null, published: null },
      queues,
      policies,
      slots,
  ...
  async loadMoreCatalog(projectId, kind, cursor, signal, canManageTeams = false, canReadAvailability = false) {
    if (kind === "queues") {
    ...
    if (kind === "slots") {
      const response = await supportRoutingRuntimeQueueSlots(projectId, { limit: 100, cursor }, readOptions(signal));
      return {
        items: response.items.map((slot): RoutingSlot => ({ ...slot, actionEtag: response.actionEtag })),
        nextCursor: response.nextCursor,
        actionEtag: response.actionEtag,
      };
    }
    if (kind === "teams" || kind === "skills") {
      const response = kind === "teams"
        ? await supportWorkforceListTeams(projectId, { limit: 100, cursor }, readOptions(signal))
        : await supportWorkforceListSkills(projectId, { limit: 100, cursor }, readOptions(signal));
    ...
    const availabilityPage = canReadAvailability && response.items.length
      ? await supportRoutingRuntimeResolveOperatorAvailability(
          projectId,
    ...
    const configuration = (canManageTeams ? workforce.draft?.configuration : undefined) ?? workforce.publishedRevision?.configuration;
    const items: RoutingOperator[] = response.items.map((item) => {
      const configured = configuration?.operators.find((operator) => operator.cmsUserId === item.cmsUserId);
      return {
        id: item.cmsUserId,
        name: item.displayName,
        state: item.membershipState === "ACTIVE" ? "ACTIVE" : item.membershipState === "INACTIVE" ? "INACTIVE" : "UNKNOWN",
        maxCapacityUnits: configured?.maxCapacityUnits ?? 1,
        teamIds: configuration?.teams.filter((team) => team.members.includes(item.cmsUserId)).map((team) => team.teamId) ?? [],
        skills: configured?.skills.map((skill) => ({ ...skill, preferred: skill.preferred ?? false })) ?? [],
        languages: configured?.languages.map((language) => ({ ...language, preferred: language.preferred ?? false })) ?? [],
        availability: availabilityByOperator.get(item.cmsUserId)?.status ?? "UNKNOWN",
      };
  ...
  async policy(projectId, policyId, signal) {
    ...
    const detail = await supportRoutingDetail(projectId, policyId, {
      ...readOptions(signal),
      onResponse: ({ headers }) => { etag = String(headers.etag ?? headers.ETag ?? ""); },
    });
    return mapPolicy(detail, etag);
  },
  async listDecisions(projectId, signal, cursor) {
    const response = await supportRoutingRuntimeDecisionList(projectId, { limit: 100, ...(cursor ? { cursor } : {}) }, readOptions(signal));
    return { items: response.items.map(mapDecision), nextCursor: typeof response.nextCursor === "string" ? response.nextCursor : null };
  },
  async decision(projectId, decisionId, signal) {
    const value = await supportRoutingRuntimeDecisionDetail(projectId, decisionId, readOptions(signal));
    return {
      ...mapDecision(value),
  ...
  async createTeam(projectId, value, context) { await supportWorkforceCreateTeam(projectId, value, options(context)); },
  async createSkill(projectId, value, context) { await supportWorkforceCreateSkill(projectId, value, options(context)); },
  ...
  async archiveIdentity(projectId, kind, id, value, context) {
    ...
    if (kind === "TEAM") await supportWorkforceArchiveTeam(projectId, id, command, options(context));
    else await supportWorkforceArchiveSkill(projectId, id, command, options(context));
  },
  async saveWorkforce(projectId, configuration, context) { await supportWorkforceReplaceDraft(projectId, configuration as unknown as ReplaceSupportWorkforceDraftDto, options(context)); },
  async discardWorkforce(projectId, context) { await supportWorkforceDiscardDraft(projectId, {}, options(context)); },
  async publishWorkforce(projectId, context) { await supportWorkforcePublish(projectId, {}, options(context)); },
  async createQueue(projectId, code, draft, context) { await supportQueueCreate(projectId, { code, draft: draft as unknown as SupportQueueDraftDto }, options(context)); },
  async saveQueue(projectId, queueId, draft, context) { await supportQueueReplaceDraft(projectId, queueId, { draft: draft as unknown as SupportQueueDraftDto }, options(context)); },
  async previewQueue(projectId, queueId, sampleLimit, signal) {
    const value = await supportQueuePreviewDraft(projectId, queueId, { sampleLimit }, readOptions(signal));
    return { count: value.count ?? value.countLowerBound ?? 0, exact: value.countExact, lowerBound: value.countLowerBound, caseIds: value.caseIds, diagnostics: [...value.diagnostics, ...value.reasonSummary], evaluatedAt: value.evaluatedAt, sourceHighWater: { ...value.sourceHighWater } };
  },
  async publishQueue(projectId, queueId, context) { await supportQueuePublish(projectId, queueId, options(context)); },
  async createPolicy(projectId, code, draft, context) { await supportRoutingCreate(projectId, { stableCode: code, draft: draft as SupportRoutingPolicyDraftDto }, options(context)); },
  async savePolicy(projectId, policyId, draft, context) { await supportRoutingReplaceDraft(projectId, policyId, { draft: draft as SupportRoutingPolicyDraftDto }, options(context)); },
  async publishPolicy(projectId, policyId, context) { await supportRoutingPublish(projectId, policyId, options(context)); },
  async bind(projectId, queueId, policyId, routePriority, context) { await supportRoutingRuntimeQueueSlot(projectId, queueId, { policyId, routePriority }, options(context)); },
  async runShadow(projectId, limit, context) { return mapShadow(await supportRoutingRuntimeRequest(projectId, { limit }, options(context))); },
  async shadowRun(projectId, runId, signal) { return mapShadow(await supportRoutingRuntimeShadowRun(projectId, runId, readOptions(signal))); },
  async shadowRunDecisionIds(projectId, runId, signal) {
    const response = await supportRoutingRuntimeShadowRunDecisions(projectId, runId, { limit: 100 }, readOptions(signal));
    return response.items.flatMap((item) => typeof item.decisionId === "string" ? [item.decisionId] : []);
  },
  async activate(projectId, queueId, targetMode, expectedActivationVersion, reasonCode, context) {
    await supportRoutingRuntimeTransitionQueueActivation(projectId, queueId, { targetMode, expectedActivationVersion, reasonCode }, options(context));
  },
  async audit(projectId, resourceType, resourceId, signal) {
    const response = await supportRoutingRuntimeConfigurationAuditTimeline(projectId, { resourceType, resourceId, limit: 100 }, readOptions(signal));
    return response.items.map((event): RoutingAuditEvent => ({
      id: event.id, eventType: event.eventType, occurredAt: event.occurredAt,
      actorName: event.actor.displayName, outcome: event.outcome,
  ...
  async revisions(projectId, kind, resourceId, signal) {
    if (kind === "QUEUE") {
      ...
      const value = await supportQueueRevisions(projectId, resourceId, { limit: 100 }, readOptions(signal));
      return value.items.map((item): RoutingRevision => ({ id: item.id, revisionNumber: item.revisionNumber, publishedAt: item.publishedAt, publisherName: item.publisher.displayName, contentHash: item.contentHash }));
    }
    if (kind === "POLICY") {
      if (!resourceId) return [];
      const value = await supportRoutingRevisionHistory(projectId, resourceId, { limit: 100 }, readOptions(signal));
      return value.items.map((item): RoutingRevision => ({ id: item.id, revisionNumber: item.revisionNumber, publishedAt: item.publishedAt, publisherName: item.publisher.displayName, contentHash: item.contentHash }));
    }
  ...
  async revisionDiff(projectId, kind, fromRevisionId, toRevisionId, resourceId, signal) {
    if (kind === "QUEUE") {
      ...
      const value = await supportQueueRevisionDiff(projectId, resourceId, { fromRevisionId, toRevisionId }, readOptions(signal));
      return { fromRevision: value.from.revisionNumber, toRevision: value.to.revisionNumber, sections: [...value.changedSections], summary: value.changedSections.length ? `Изменены разделы: ${value.changedSections.join(", ")}` : "Семантических изменений нет" };
    }
    if (kind === "POLICY") {
      if (!resourceId) throw new Error("Policy id is required");
      const value = await supportRoutingRevisionDiff(projectId, resourceId, { fromRevisionId, toRevisionId }, readOptions(signal));
      return { fromRevision: value.from.revisionNumber, toRevision: value.to.revisionNumber, sections: [...value.codeSetChanges.map((item) => item.field), ...value.numericChanges.map((item) => item.field)], summary: `${value.codeSetChanges.length} изменений списков, ${value.numericChanges.length} числовых изменений` };
    }
    const value = await supportWorkforceDiff(projectId, fromRevisionId, toRevisionId, readOptions(signal));
    return { fromRevision: value.from.revisionNumber, toRevision: value.to.revisionNumber, sections: ["TEAMS", "OPERATORS", "SKILLS"].filter((_, index) => [value.teams, value.operators, value.skills][index]!.totalChanged > 0), summary: `Команды: ${value.teams.totalChanged}, операторы: ${value.operators.totalChanged}, навыки: ${value.skills.totalChanged}` };
  },
  async restoreRevision(projectId, kind, revisionId, resourceId, reasonCode, context) {
    if (kind === "QUEUE") {
      if (!resourceId) throw new Error("Queue id is required");
      await supportQueueRestoreRevisionAsDraft(projectId, resourceId, revisionId, { reasonCode }, options(context));
    } else if (kind === "POLICY") {
      if (!resourceId) throw new Error("Policy id is required");
      await supportRoutingRestoreRevisionDraft(projectId, resourceId, revisionId, { reasonCode }, options(context));
    } else {
