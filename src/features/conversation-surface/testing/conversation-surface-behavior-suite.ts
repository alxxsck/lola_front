...
interface ConversationSurfaceBehaviorSuiteOptions {
  ...
  alternateComposer: Extract<ConversationSurfaceComposer, { mode: 'PUBLIC_REPLY' }>;
...
export function runConversationSurfaceBehaviorSuite(
  ...
  describe(`shared Conversation Surface behavior: ${options.name}`, () => {
    it('uses the canonical log, translation controls and public composer', async () => {
      ...
      expect(wrapper.findAll('[aria-label="Режим отображения сообщений"]')).toHaveLength(
        options.translationAvailable ? 1 : 0,
      );
      expect(wrapper.find('textarea[aria-label="Ответ пользователю"]').exists()).toBe(true);
    ...
    it('switches between original and translated rendering through the adapter', async () => {
      ...
      expect(
        wrapper.get('[data-action="show-translated-messages"]').attributes('aria-pressed'),
      ).toBe('true');
    });
    ...
    it('requests authoritative reconcile and recovers the conversation-scoped draft', async () => {
      ...
      expect(wrapper.emitted('reconcile-required')?.at(-1)?.[0]).toEqual(
        expect.arrayContaining([{ kind: 'ORDINAL_GAP', afterOrdinal: 1, beforeOrdinal: 3 }]),
      );

      await wrapper.setProps({ composer: options.alternateComposer });
      await wrapper.get('textarea[aria-label="Ответ пользователю"]').setValue('Другой диалог');
      await wrapper.setProps({ composer: options.composer });

      expect(wrapper.get('textarea[aria-label="Ответ пользователю"]').element).toHaveProperty(
        'value',
        'Черновик переживает reconnect',
      );
    });
  });
}
