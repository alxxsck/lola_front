...
function messageStatus(message: ConversationMessage): ConversationSurfaceStatus | undefined {
  if (message.status === 'PENDING') return { label: 'Отправляется…', tone: 'WARNING' };
  if (message.status === 'WRITING')
    return {
      label: message.text ? 'Обновляется…' : 'Пользователь печатает',
      tone: 'WARNING',
    };
  if (message.status === 'FAILED') return { label: 'Не доставлено', tone: 'DANGER' };
  if (message.status === 'CANCELLED')
    return { label: 'Ответ остановлен оператором', tone: 'WARNING' };
  return undefined;
}

function deliveryStatus(message: ConversationMessage): ConversationSurfaceStatus | undefined {
  if (message.author === 'USER' || message.author === 'SYSTEM') return undefined;
  ...
  if (status === 'CANCELLED') return { label: 'Доставка отменена', tone: 'DANGER' };
          ...
          message.author === 'ASSISTANT'
            ...
            : message.author === 'SCENARIO'
              ...
              : message.author === 'SYSTEM'
                ...
                : 'DEFAULT',
        author: {
          displayName: message.authorSnapshot?.displayName ?? labels[message.author],
