...
vi.mock('@/shared/api/generated/retenive-backend', async (importOriginal) => {
  const original = await importOriginal<typeof import('@/shared/api/generated/retenive-backend')>();
  return {
    ...original,
...
describe('Attribute Contract repository', () => {
  ...
  it('exposes publication list and detail as first-class history', async () => {
    ...
    await attributeContractRepository.publication('project-1', 'publication-7');

    expect(api.publications).toHaveBeenCalledWith('project-1', { limit: 25 });
    expect(api.publication).toHaveBeenCalledWith('project-1', 'publication-7');
  });
});
