...
export function isCmsReadVisible(field: AttributeContractDraftFieldDto): boolean {
  return field.policies.cmsRead.mode === 'VISIBLE';
}

export function fieldNeedsPurpose(field: AttributeContractDraftFieldDto): boolean {
  return (
...
export function createContractField(position = 0): AttributeContractDraftFieldDto {
  return {
    key: '',
    label: '',
    description: null,
  ...
  document.fields.forEach((field, index) => {
    ...
    if (field.classification === 'SENSITIVE' && field.policies.clientRead)
      issues.push({
        ...
        message: 'Чувствительное поле нельзя передавать в браузер без отдельной проверки доступа.',
        severity: 'warning',
      });
    ...
    if (field.lifecycle === 'DEPRECATED' && !field.replacementDefinitionId && !field.sunsetAt)
      issues.push({
