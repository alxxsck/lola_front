...
export function profileFieldPreset(kind: ProfileFieldKind | null | undefined): ProfileFieldPreset {
  return (
...
export function applyProfileFieldPreset(
  ...
  if (!field.key.trim() || (previousSuggestion?.key && field.key === previousSuggestion.key))
    field.key = next.key ?? '';
