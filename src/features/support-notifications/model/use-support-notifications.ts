...
export function createSupportNotificationsController(
  ...
  const deviceBusy = computed(() => connecting.value || revokingDeviceId.value !== null);
  ...
  function storeRegistration(actorId: string, registration: StoredBrowserPushRegistration): void {
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      if (stored && !storedDevice && !canResumeAfterLogin) clearRegistration(actorId);
      else if (
        stored &&
  ...
  async function setPreference(
    ...
    if (!projectId || !actorId || !current || !canSet(topic, subscribed) || savingTopic.value)
      return;
    const requestGeneration = generation;
  ...
  async function connectBrowser(): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        await runSupportNotificationBrowserLifecycle(() => browser.unsubscribe());
        if (!isCurrent(projectId, actorId, requestGeneration) || deviceMutation !== mutation)
          return;
      ...
      if (
        ...
        const retirementKey = revokeAttempts.get(currentDevice.id) ?? crypto.randomUUID();
        ...
        devices.value = devices.value.map((item) => (item.id === retired.id ? retired : item));
      }
      const registerInput = {
        ...attempt.material,
