...
export function createSupportLeadAssignmentController(
  ...
  function scopeIsCurrent(projectId: string, selectedCaseId: string): boolean {
    return (
      context.projectId() === projectId && caseId.value === selectedCaseId && context.canOverride()
    );
  }
  ...
  async function load(selectedCaseId = caseId.value): Promise<void> {
    ...
    loading.value = true;
    try {
      const value = await source.readCase(projectId, selectedCaseId, controller.signal);
      if (requestGeneration === generation && scopeIsCurrent(projectId, selectedCaseId))
  ...
  async function loadAudit(selectedCaseId = caseId.value): Promise<void> {
    ...
    auditFacts.value = [];
    auditError.value = '';
    if (!projectId || !selectedCaseId || !context.canReadAudit?.() || !context.canOverride())
      return;
    const controller = new AbortController();
    auditAbort = controller;
    auditLoading.value = true;
    try {
      const facts = await source.readAudit(projectId, selectedCaseId, controller.signal);
  ...
  function captureAction(): CapturedLeadAction | null {
    ...
    if (!projectId || !caseId.value || !value || !current || !context.canOverride()) return null;
    ...
    if (value.kind === 'RELEASE') {
      ...
      const operator = team?.operators.find((item) => item.id === value.operatorId);
      ...
      if (
        !current.actions[actionName] ||
        !team.actions[actionName] ||
        !operator.actions[actionName]
      )
        return null;
      if (value.kind === 'ASSIGN' || value.kind === 'TRANSFER') {
        ...
        if (!bypassAvailability && !bypassCapacity && !required.has('RESERVATION')) return null;
        intent = {
          kind: value.kind,
          snapshot: current,
          teamId: team.id,
  ...
  async function applyKnownReceipt(
    ...
    if (receipt.caseId !== action.caseId || !actionScopeIsCurrent(action)) return;
    ...
    try {
      ...
      error.value = 'Назначение выполнено, но рабочее место не обновилось. Обновите данные.';
    }
  }

  async function reconcileUnknownOutcome(action = pendingAction.value): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        error.value = 'Сервер пока не нашёл результат команды. Проверьте статус ещё раз.';
        return;
      }
      if (cause instanceof ApiError && (cause.status === 403 || cause.status === 404)) {
        reset();
  ...
  async function submit(): Promise<void> {
    ...
    try {
      ...
      if (requestGeneration !== mutationGeneration || !actionScopeIsCurrent(action)) return;
