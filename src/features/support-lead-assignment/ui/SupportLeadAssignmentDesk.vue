...
function targetCanBeUsed(
  operator: NonNullable<typeof snapshot.value>['teams'][number]['operators'][number],
): boolean {
  const ordinary = isTransfer.value ? operator.actions.transfer : operator.actions.assign;
...
const teams = computed(
  () => snapshot.value?.teams.filter((team) => team.operators.some(targetCanBeUsed)) ?? [],
);
...
const requiredOverrides = computed(() => selectedOperator.value?.requiredOverrides ?? []);
...
function auditReasonLabel(value: string): string {
  return auditReasonLabels[value as keyof typeof auditReasonLabels] ?? 'Системная причина';
}
...
function auditTeamLabel(teamId: string | null): string {
  ...
  return snapshot.value?.teams.find((team) => team.id === teamId)?.name ?? 'Команда недоступна';
}
...
function auditOperatorLabel(operatorId: string | null): string {
  ...
  return (
    snapshot.value?.teams
      ...
      .find((operator) => operator.id === operatorId)?.displayName ?? 'Оператор недоступен'
  );
}
...
async function submitTarget(): Promise<void> {
  ...
  if (!props.controller.draft.value && !props.controller.unknownOutcome.value) closeDesk();
}
    ...
    <template v-if="snapshot && !controller.loading.value">
      ...
      <form v-if="mode === 'TARGET'" class="lead-assignment-form" @submit.prevent="submitTarget">
        ...
        >
          ...
          {{ requiredOverrides.map(overrideLabel).join(' и ') }}. Действие попадёт в журнал.
        </Message>

        <label>
          <span>{{ isForce ? 'Причина исключения' : 'Причина назначения' }}</span>
          <Select
            v-model="reasonCode"
        ...
        <label>
          <span>
            {{ isForce ? 'Обоснование исключения · обязательно' : 'Комментарий · необязательно' }}
          </span>
          <Textarea
            v-model="reasonNote"
            :aria-label="isForce ? 'Обоснование исключения' : 'Комментарий к назначению'"
            rows="3"
            maxlength="500"
            auto-resize
            fluid
      ...
      <form v-else class="lead-assignment-form" @submit.prevent="submitRelease">
        <Message severity="warn" :closable="false">
          Обращение вернётся в очередь без владельца. Текущий контекст разговора сохранится.
        </Message>
        <label>
          ...
          <li v-for="fact in controller.auditFacts.value.slice(0, 6)" :key="fact.activityId">
            ...
            <div>
              ...
              <small v-if="fact.eligibilityOverride">
                ...
                {{ fact.eligibilityOverride.bypassAvailability ? 'да' : 'нет' }}, лимит нагрузки
                {{ fact.eligibilityOverride.bypassCapacity ? 'да' : 'нет' }}
              </small>
              <small v-if="fact.reasonCode || fact.commandOutcome">
                Причина:
                {{ fact.reasonCode ? auditReasonLabel(fact.reasonCode) : 'не указана' }}
                ·
                {{
                  fact.commandOutcome
      ...
      <div class="lead-assignment-footer">
        ...
        <div>
          ...
          <Button
            ...
            :label="
              isForce ? 'Назначить с исключением' : isTransfer ? 'Переназначить' : 'Назначить'
            "
