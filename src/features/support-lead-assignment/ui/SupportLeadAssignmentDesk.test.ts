...
describe('SupportLeadAssignmentDesk', () => {
  it('opens the same audited Lead use case and exposes only eligible operator-in-Team targets', async () => {
    ...
    expect(wrapper.get("select[aria-label='Оператор назначения']").text()).toContain(
      'Анна Смирнова',
    );
  ...
  it('makes the force bypass and protected reason explicit before confirmation', async () => {
    ...
    expect(wrapper.get('[data-force-warning]').text()).toContain('лимит нагрузки');
    expect(wrapper.get("textarea[aria-label='Обоснование исключения']")).toBeDefined();
    expect(
      wrapper.get("button[aria-label='Подтвердить назначение лидом']").attributes('disabled'),
    ).toBeDefined();
  });
  ...
  it('shows safe assignment audit facts without protected free-form notes', async () => {
    ...
    expect(wrapper.text()).toContain('Исключение: доступность да, лимит нагрузки нет');
