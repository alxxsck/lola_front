...
export const supportLeadAssignmentCommandSource: Omit<
...
> = {
  ...
  async executeBatch(projectId, items, idempotencyKey, signal) {
    try {
      return validateBatchReceipt(await supportCaseAssignmentBatchExecute(
        projectId,
        { items },
        {
          ...(signal ? { signal } : {}),
  ...
  async lookupBatchOutcome(projectId, idempotencyKey, expectedItems, signal) {
    try {
      return validateBatchReceipt(await supportCaseAssignmentBatchOutcome(projectId, {
        ...(signal ? { signal } : {}),
        headers: { "Idempotency-Key": idempotencyKey },
      }), expectedItems);
    } catch (cause) {
