...
function conversation(id: string, status: 'ACTIVE' | 'ARCHIVED' = 'ACTIVE', isCurrent = false) {
  return {
    id,
    userId: 'user-1',
    title: `Диалог ${id}`,
...
describe('панель ответа администратором', () => {
  ...
  it('при reconciliation заменяет realtime projection authoritative REST-версией', async () => {
    ...
    expect(console.messages.value[0]?.translation?.translatedText).toBe('Добрый день');
  });
  ...
  it('передаёт ready translation draft в атомарную отправку', async () => {
    ...
    console.selectedConversation.value = conversation('conversation-1', 'ACTIVE', true);
    console.onlineSession.value = {
      id: 'session-1',
      userId: 'user-1',
      externalId: 'external-user-1',
  ...
  it('отличает подтверждённый отказ сервера от неизвестного результата', async () => {
    mocks.sendAdminMessage.mockRejectedValue(new ApiError(422, 'invalid', undefined, 'request-1'));
    const console = useAdminConversationConsole({
      projectId: () => 'project-1',
      endUserId: () => 'user-1',
      updateRoute: vi.fn(),
  ...
  it('по умолчанию открывает текущий диалог и первую страницу сообщений', async () => {
    ...
    expect(mocks.getMessages).toHaveBeenCalledWith('project-1', 'user-1', 'current', { limit: 20 });
  });
  ...
  it('добавляет предыдущую страницу в начало, сортирует и убирает дубликаты', async () => {
    ...
    expect(console.messages.value.map((message) => message.id)).toEqual(['old', 'new']);
  ...
  it('обновляет live-сообщение по id без повторного счётчика', () => {
    ...
    expect(console.messages.value).toEqual([{ ...writing, status: 'COMPLETED', text: 'Готово' }]);
  ...
  it('создаёт отдельный диалог только через явную политику create_new', async () => {
    ...
    await expect(console.sendNewConversation(' Новый вопрос ')).resolves.toBe('conversation-new');
  ...
  it('повторяет неизвестный результат обычной отправки с тем же idempotency key', async () => {
    ...
    expect(mocks.sendAdminMessage.mock.calls[0]?.[2].endUserCaseId).toBe('case-1');
  });
  ...
  it('открывает текущий диалог online-сессии, даже если он вне первой страницы', async () => {
    ...
    expect(mocks.getConversation).toHaveBeenCalledWith('project-1', 'user-1', 'current-outside');
  ...
  it('догружает и поднимает все текущие диалоги нескольких online-сессий', async () => {
    ...
    mocks.getConversation.mockImplementation((_projectId, _endUserId, conversationId: string) =>
    ...
    expect(
      console.conversations.value.filter((item) => item.isCurrent).map((item) => item.id),
  ...
  it('очищает старый Current при переносе active session в другой диалог', async () => {
    mocks.getConversations.mockResolvedValue({
      items: [conversation('conversation-a', 'ACTIVE', true), conversation('conversation-b')],
      nextCursor: null,
    });
    ...
    await console.loadConversations('user-1');
    mocks.getConversations.mockResolvedValue({
      items: [conversation('conversation-a'), conversation('conversation-b', 'ACTIVE', true)],
      nextCursor: null,
    });
    mocks.getSessions.mockResolvedValue([
      {
  ...
  it('сохраняет новый текст, набранный пока предыдущий ответ отправлялся', async () => {
    ...
    mocks.getConversation.mockResolvedValue(conversation('conversation-1', 'ACTIVE', true));
    const console = useAdminConversationConsole({
      projectId: () => 'project-1',
      endUserId: () => 'user-1',
    });
