...
function mergeMessages(
  ...
  for (const message of incoming) {
    ...
    merged.set(message.id, existing ? newerMessage(existing, message) : message);
  }
...
export function useAdminConversationConsole(options: AdminConversationConsoleOptions) {
  ...
  function currentContext(projectId: string, endUserId: string, conversationId?: string): boolean {
    return (
  ...
  function hasAnyDraft(): boolean {
    return (
      Boolean(replyText.value.trim()) || [...drafts.values()].some((draft) => Boolean(draft.trim()))
    );
  }
  ...
  async function refreshPresence(): Promise<void> {
    ...
    try {
      ...
      if (conversationPagesLoaded <= 1) nextConversationCursor.value = page.nextCursor;
    } catch {
  ...
  async function loadConversations(
    ...
    try {
      ...
      if (request !== conversationRequestSequence || !currentContext(projectId, endUserId)) return;
      ...
      let conversation = preferredConversationId
        ? (conversations.value.find((item) => item.id === preferredConversationId) ?? null)
        : null;
      conversation ??=
      ...
      if (request === conversationRequestSequence && currentContext(projectId, endUserId)) {
        conversationError.value = 'Не удалось загрузить диалоги';
      }
    } finally {
      if (request === conversationRequestSequence) conversationsLoading.value = false;
    }
  }
  ...
  async function loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      if (request === conversationRequestSequence) conversationsLoadingMore.value = false;
    }
  }

  async function loadMessages(conversation: Conversation, updateRoute = true): Promise<void> {
    const projectId = options.projectId();
    const endUserId = options.endUserId();
    if (!projectId || !endUserId) return;
    ...
    try {
      ...
      if (
        ...
        return;
      const page = await repository.getMessages(projectId, endUserId, conversation.id, {
      ...
      if (request === messageRequestSequence && currentContext(projectId, endUserId)) {
  ...
  async function loadOlderMessages(): Promise<number> {
    ...
    if (!projectId || !endUserId || !conversation || !cursor || messagesLoadingMore.value) return 0;
    const request = messageRequestSequence;
    ...
    try {
      ...
      const older = orderedMessages(page.items.filter((item) => !existing.has(item.id)));
  ...
  function upsertMessage(message: ConversationMessage, announce = true): boolean {
    const conversation = selectedConversation.value;
    if (!conversation || message.conversationId !== conversation.id) return false;
  ...
  async function reconcileSelected(): Promise<void> {
    ...
    const request = messageRequestSequence;
    try {
      const [freshConversation, page, sessions, conversationPage] = await Promise.all([
      ...
      conversations.value = orderConversations([...byId.values()]);
      if (conversationPagesLoaded <= 1) nextConversationCursor.value = conversationPage.nextCursor;
      selectedConversation.value = freshConversation;
      messages.value = reconcileMessages(messages.value, page.items);
      if (!messages.value.length || page.nextCursor) nextMessageCursor.value = page.nextCursor;
    } catch {
      if (
        request === messageRequestSequence &&
        currentContext(projectId, endUserId, conversation.id)
      ) {
        conversationError.value = 'Live-обновление недоступно. История будет сверена повторно.';
      }
    }
  }
  ...
  async function sendReply(delivery?: {
    ...
    const activeGeneration = generation;
    if (!projectId || !endUserId || !conversation || !onlineSession.value || !text) return;
    ...
    try {
      const attempt =
        ...
        replyAttempt.replyTranslationDraftId === delivery?.replyTranslationDraftId &&
        replyAttempt.sendWithoutTranslationReason === delivery?.sendWithoutTranslationReason
          ? replyAttempt
          : {
              ...
              ...(delivery?.sendWithoutTranslationReason
                ? {
                    sendWithoutTranslationReason: delivery.sendWithoutTranslationReason,
                  }
                : {}),
      ...
      const result = await repository.sendAdminMessage(projectId, endUserId, {
        text,
        conversationId: conversation.id,
        ...(options.endUserCaseId?.() ? { endUserCaseId: options.endUserCaseId() } : {}),
      ...
      if (generation !== activeGeneration || !currentContext(projectId, endUserId, conversation.id))
        return;
  ...
  async function sendNewConversation(text: string): Promise<string | null> {
    ...
    const activeGeneration = generation;
    if (!projectId || !endUserId || !session || !normalized || creatingConversation.value)
      return null;
    ...
    try {
      ...
      if (generation !== activeGeneration || !currentContext(projectId, endUserId)) return null;
    ...
    try {
      const created = await repository.getConversation(projectId, endUserId, createdThreadId);
      if (generation !== activeGeneration || !currentContext(projectId, endUserId))
        return createdThreadId;
      ingestRestConversations([created]);
      conversations.value = orderConversations([created, ...conversations.value]);
