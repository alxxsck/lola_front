...
describe("Support External Work source", () => {
  ...
  it("reads connection authority through the generated operation and caller signal", async () => {
    ...
    await apiSupportExternalWorkSource.listConnections(
      "project-1",
      undefined,
      signal,
    );
  ...
  it("keeps quoted command OCC and idempotency headers for retry and evidence", async () => {
    ...
    vi.mocked(supportExternalCommandRefreshEvidence).mockResolvedValue(
      {} as never,
    );
  ...
  it("uses generated Case operations and preserves exact audited command identity", async () => {
    ...
    vi.mocked(supportExternalCommandResolveUnknown).mockResolvedValue(
      {} as never,
    );
    vi.mocked(supportExternalInboxLinkToCase).mockResolvedValue({} as never);

    await apiSupportExternalWorkSource.readCaseCreateOptions(
      "project-1",
      "case-1",
      signal,
    );
    await apiSupportExternalWorkSource.listCaseLinks(
      "project-1",
      "case-1",
      undefined,
      signal,
    );
    await apiSupportExternalWorkSource.readCommand(
      "project-1",
      "case-1",
      "command-1",
      signal,
    ...
    expect(supportExternalCaseCreateOptionsRead).toHaveBeenCalledWith(
      "project-1",
      "case-1",
      {
        signal,
    ...
    expect(supportExternalCommandRead).toHaveBeenCalledWith(
      "project-1",
      "case-1",
      "command-1",
      { signal },
