...
const acceptedReceipts = new Map<string, { commandId: string; status: string }>();
const forgottenAcceptedReceiptKeys = new Set<string>();
const acceptedReceiptStoragePrefix = 'support-external-work:case-receipt:v1:';
...
function receiptStorageKey(scope: Scope): string {
  return `${acceptedReceiptStoragePrefix}${[scope.actorId, scope.projectId, scope.caseId]
...
function retainAcceptedReceipt(scope: Scope, receipt: { commandId: string; status: string }): void {
  const key = scopeKey(scope);
...
export function createSupportCaseExternalWorkController(
  ...
  const newIntentBlocked = computed(() => unknownAttempt.value || hasPendingCommand.value);

  function currentScope(): Scope | null {
    const projectId = context.projectId();
    const actorId = context.actorId();
    const caseId = context.caseId();
    return projectId && actorId && caseId ? { projectId, actorId, caseId } : null;
  }
  ...
  function attemptAllowed(attempt: RetainedAttempt): boolean {
    ...
    if (attempt.kind === 'RETRY')
      return (
        ...
        commands.value.some(
          (command) =>
            command.commandId === attempt.commandId && command.allowedActions.includes('RETRY'),
        )
      );
    ...
    if (attempt.body.intent === 'CREATE' && 'optionId' in attempt.body) {
      ...
      return (
        ...
        createOptions.value.some(
          (option) => option.optionId === optionId && option.allowedActions.includes('CREATE'),
        )
      );
    }
    ...
    if (attempt.body.intent === 'COMMENT')
      return attempt.body.audience === 'PUBLIC'
        ? permissions.commentPublic && link.item.allowedActions.includes('COMMENT_PUBLIC')
        : permissions.commentInternal && link.item.allowedActions.includes('COMMENT_INTERNAL');
    return permissions.read && link.item.allowedActions.includes(attempt.body.intent);
  }
  ...
  async function load(): Promise<void> {
    ...
    try {
      const [linkPage, commandPage, optionPage, inboxPage] = await Promise.all([
        permissions.read
          ? source.listCaseLinks(scope.projectId, scope.caseId, undefined, abort.signal)
          : Promise.resolve({ items: [], nextCursor: null }),
        permissions.read
          ? source.listCaseCommands(scope.projectId, scope.caseId, { limit: 50 }, abort.signal)
          : Promise.resolve({ items: [], nextCursor: null }),
        permissions.create && permissions.read
          ? source
              .readCaseCreateOptions(scope.projectId, scope.caseId, abort.signal)
              .catch((cause: unknown) => {
                if (
      ...
      pendingAttempt.value = retained && attemptAllowed(retained) ? retained : null;
  ...
  async function selectLink(linkId: string): Promise<void> {
    ...
    try {
      ...
      if (!sameScope(scope, requestGeneration) || selectedLinkId.value !== linkId) return;
  ...
  function createBody(): SupportExternalCommandSubmitBody | null {
    ...
    const option = createOptions.value.find((item) => item.optionId === draft.optionId);
    if (!option) {
    ...
    if (!title || !contextParts.length) {
      validationError.value = 'Заполните заголовок и редактируемый безопасный контекст.';
      return null;
    }
    ...
    const body = {
      ...
      ...(requesterEmail
        ...
        : {}),
      ...(Object.keys(draft.fieldValues).length ? { fieldValues: draft.fieldValues } : {}),
    };
    validationError.value = '';
  ...
  async function reconcileCommand(commandId: string): Promise<void> {
    ...
    const requestGeneration = generation;
    try {
      const result = await source.readCommand(scope.projectId, scope.caseId, commandId);
      if (!sameScope(scope, requestGeneration)) return;
      commands.value = [result, ...commands.value.filter((item) => item.commandId !== commandId)];
      feedback.value = {
        commandId,
  ...
  async function runAttempt(attempt: RetainedAttempt): Promise<void> {
    if (mutating.value || !sameScope(attempt.scope) || !attemptAllowed(attempt)) return;
    mutationAbort?.abort();
    const abort = new AbortController();
    mutationAbort = abort;
    const requestGeneration = generation;
    ...
    try {
      ...
      if (cause instanceof ApiError && [401, 403, 404, 428].includes(cause.status))
        retainedAttempts.delete(key);
      if (cause instanceof ApiError && [401, 403, 404, 428].includes(cause.status))
        forgetAcceptedReceipt(attempt.scope);
      if (!sameScope(attempt.scope, requestGeneration)) return;
      if (cause instanceof ApiError && [0, 429, 502, 503, 504].includes(cause.status)) {
        error.value =
    ...
    if (commandToReconcile && sameScope(attempt.scope)) await reconcileCommand(commandToReconcile);
  }

  async function submit(body: SupportExternalCommandSubmitBody, version?: number): Promise<void> {
    const scope = currentScope();
  ...
  async function comment(linkId: string): Promise<void> {
    ...
    const action = draft.audience === 'PUBLIC' ? 'COMMENT_PUBLIC' : 'COMMENT_INTERNAL';
  ...
  async function runLinkCommand(linkId: string, intent: 'REFRESH' | 'UNLINK'): Promise<void> {
    const link = links.value.find((item) => item.linkId === linkId);
  ...
  async function refreshEvidence(commandId: string, remoteItemId: string): Promise<void> {
    const scope = currentScope();
    const command = commands.value.find((item) => item.commandId === commandId);
    if (
      !scope ||
  ...
  async function resolveCommand(
    ...
    if (!(allowedDecisions[command.intent] ?? []).includes(resolution.decision)) {
      validationError.value = 'Это решение недоступно для текущей команды.';
      return;
    }
    if (resolution.decision === 'LINK_EXISTING' && !resolution.remoteItemId?.trim()) {
      validationError.value =
        'Укажите идентификатор внешней задачи из подтверждённых данных сервера.';
      return;
    }
    if (resolution.decision === 'CONFIRM_DELIVERED' && !resolution.providerCorrelation?.trim()) {
      validationError.value = 'Укажите идентификатор подтверждения из данных внешней системы.';
      return;
    }
    validationError.value = '';
    await runAttempt({
  ...
  async function linkExisting(itemId: string, optionId: string): Promise<void> {
    const scope = currentScope();
    const item = inboxItems.value.find((candidate) => candidate.itemId === itemId);
    const option = createOptions.value.find((candidate) => candidate.optionId === optionId);
    if (
      !scope ||
      !item ||
      !option ||
