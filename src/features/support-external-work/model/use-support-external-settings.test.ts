...
describe('Support External Work settings controller', () => {
  ...
  it('suppresses duplicate tests and exact-replays the retained attempt after timeout', async () => {
    ...
    const source = {
      ...
      testConnection: vi.fn(
        async (...args: Parameters<typeof mockSupportExternalWorkSource.testConnection>) => {
          attempts += 1;
          keys.push(args[2]);
    ...
    await Promise.all([controller.testSelectedConnection(), controller.testSelectedConnection()]);
  ...
  it('blocks a new intent when a receipt is known but authoritative reread fails', async () => {
    ...
    const source = {
      ...
      listConnections: vi.fn(
        async (...args: Parameters<typeof mockSupportExternalWorkSource.listConnections>) => {
          reads += 1;
  ...
  it('reloads the target roots on conflict and preserves the operator mapping draft', async () => {
    ...
    const listMappings = vi.fn(
      async (...args: Parameters<typeof mockSupportExternalWorkSource.listMappings>) => {
  ...
  it('couples each mapping and catalog to its owning connection', async () => {
    ...
    await controller.load();
    const helpdesk = controller.connections.value.find((item) => item.provider === 'HELPDESK')!;
  ...
  it('consumes connection cursors and keeps two sites for the same provider selectable', async () => {
    ...
    expect(controller.connections.value.filter((item) => item.provider === 'JSM')).toHaveLength(2);
  ...
  it('restores a conflicted mapping by exact id and never applies its draft to a sibling', async () => {
    const mappingPage = await mockSupportExternalWorkSource.listMappings('project-mapping-sites', {
      limit: 50,
    });
    ...
    const listMappings = vi.fn(async () => {
      reads += 1;
      return {
        items: [mappingA, reads === 1 ? mappingB : { ...mappingB, version: 12 }],
        nextCursor: null,
      };
    });
