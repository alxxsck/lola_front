...
export function compareSuspensionVersions(left: string, right: string): -1 | 0 | 1 {
  if (!/^\d+$/.test(left) || !/^\d+$/.test(right))
    throw new Error('Некорректная версия состояния AI');
...
export function isConversationAISuspended(
  ...
  if (summary.mode !== 'SUSPENDED' || summary.lifecycle !== 'ACTIVE' || !summary.suspendedUntil)
    return false;
...
export function roundedDurationSeconds(durationMs: number): number {
  if (durationMs < MIN_SUSPENSION_SECONDS * 1_000)
    throw new Error('Срок должен быть не меньше одной минуты');
