...
describe('хранилище приостановок AI по диалогам', () => {
  ...
  it('не смешивает состояния двух диалогов и блокирует повтор одной команды', async () => {
    ...
    const duplicate = await store.start(
      'user-1',
      'conversation-1',
      { durationSeconds: 3600, reason: 'OPERATOR_TAKEOVER' },
      'key-2',
  ...
  it('не читает detail для автоматического состояния и объединяет параллельные чтения', async () => {
    ...
    const reads = Array.from({ length: 11 }, () => store.ensureDetail('user-1', 'conversation-2'));
  ...
  it('игнорирует запоздалое событие и принимает более новое', async () => {
    ...
    await store.activateProject('project-1');
    store.ingestConversations([
      { ...conversation('conversation-1'), aiSuspension: { ...automatic, version: '10' } },
    ]);
    ...
    store.applyRealtimeEvent({
      ...
      projectId: 'project-1',
      sequence: '4',
      occurredAt: automatic.serverTime,
    ...
    expect(store.getEntry('conversation-1')?.summary).toMatchObject({
      version: '11',
      suspendedUntil: '2026-07-20T15:00:00.000Z',
    });
  });
