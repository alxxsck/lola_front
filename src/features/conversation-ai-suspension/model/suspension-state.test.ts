...
describe('состояние приостановки AI', () => {
  ...
  it('считает время по часам сервера при расхождении часов клиента', () => {
    ...
    expect(clock.now(Date.parse('2026-07-20T12:02:00.000Z'))).toBe(
      Date.parse('2026-07-20T12:12:00.000Z'),
    );
  });
  ...
  it('снимает активное отображение точно в момент окончания', () => {
    ...
    expect(isConversationAISuspended({ ...summary, suspendedUntil: 'не дата' }, Date.now())).toBe(
      false,
    );
  });
