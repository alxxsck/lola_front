...
watch(
  () => [props.visible, props.projectId, props.endUserId, props.conversationId] as const,
  ([visible]) => {
    requestSequence += 1;
    items.value = [];
...
function actionLabel(type: ConversationAISuspensionHistoryItemResponseDto['type']): string {
  return type === 'STARTED'
    ? 'AI приостановлен'
    : type === 'EXTENDED'
      ? 'Приостановка продлена'
      : 'Ответы AI возобновлены';
}

function reasonLabel(
  reason?: ConversationAISuspensionHistoryItemResponseDto['reason'] | null,
): string {
  return reason
    ? ({
...
<template>
  <Dialog
    ...
    modal
    header="История приостановки AI"
        ...
        <article>
          <header>
            ...
            ><time :datetime="item.acceptedAt">{{
              new Date(item.acceptedAt).toLocaleString('ru-RU')
            }}</time>
          </header>
          <p>{{ item.actor.displayName }} · версия {{ item.version }}</p>
          <p v-if="item.reason">Причина: {{ reasonLabel(item.reason) }}</p>
          <p v-if="item.newSuspendedUntil">
            До {{ new Date(item.newSuspendedUntil).toLocaleString('ru-RU') }}
          </p>
...
.suspension-history li:last-child .history-marker::after {
  display: none;
}
.suspension-history article {
  padding: 0 0 18px;
}
...
.suspension-history time,
.suspension-history p {
