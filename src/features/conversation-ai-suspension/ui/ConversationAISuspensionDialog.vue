...
watch([durationSeconds, () => customDeadline.value?.getTime(), reason, note, resumeReason], () => {
...
const validationMessage = computed(() => {
  ...
  if (props.mode === 'EXTEND') {
    ...
    if (
      !targetDeadline.value ||
      targetDeadline.value.getTime() > estimatedServerNow.value + MAX_SUSPENSION_SECONDS * 1_000
    )
      return 'Итоговый срок не может превышать семь дней.';
  }
  ...
  >
    <form class="suspension-form" @submit.prevent="submit">
      ...
      <template v-if="mode === 'START'">
        <fieldset>
          ...
          <div class="choice-grid">
            <label
              ...
              :key="option.value"
              :class="{ selected: durationSeconds === option.value }"
            >
              <input
        ...
        <label class="field">
          <span
            >Внутренний комментарий <small>{{ note.length }}/500</small></span
          >
          <Textarea
            v-model="note"
            rows="3"
            maxlength="500"
            placeholder="Необязательно, кроме причины «Другая»"
          />
        </label>
        <p v-if="deadlineLabel" class="result-line">
          AI автоматически возобновит ответы {{ deadlineLabel }}.
        </p>
        <p class="explanation">
          Пользователь сможет писать сюда и общаться с AI в других диалогах.
        </p>
      </template>
        ...
        <p>
          ...
          <strong>{{
            current?.suspendedUntil
              ? new Date(current.suspendedUntil).toLocaleString('ru-RU')
              : 'неизвестно'
          }}</strong>
        </p>
        <fieldset>
      ...
      <template v-else>
        ...
        <button
          v-if="!showResumeReason"
          type="button"
          class="comment-toggle"
          @click="showResumeReason = true"
        >
          Добавить комментарий
        </button>
        <label v-else class="field">
          <span
            >Комментарий <small>{{ resumeReason.length }}/200</small></span
          >
          <Textarea v-model="resumeReason" rows="2" maxlength="200" />
        </label>
      </template>

      <Message v-if="validationMessage" severity="error" :closable="false">{{
        validationMessage
      }}</Message>
      <Message v-if="error" severity="warn" :closable="false">
        {{ error.message
        }}<small v-if="error.requestId"> Номер обращения: {{ error.requestId }}.</small>
      </Message>
      ...
      <div class="dialog-actions">
        ...
        <Button
          ...
          :label="
            mode === 'START'
              ? `Приостановить${deadlineLabel ? ` до ${targetDeadline?.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}` : ''}`
              : mode === 'EXTEND'
                ? 'Продлить'
                : 'Возобновить AI'
          "
