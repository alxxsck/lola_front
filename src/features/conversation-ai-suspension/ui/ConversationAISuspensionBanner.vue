...
const active = computed(
  () =>
    ...
    isConversationAISuspended(props.entry.summary, estimatedServerNow.value),
);
...
const reasonLabel = computed(() =>
  detail.value?.reason ? reasonLabels[detail.value.reason] : 'не указана',
);
</script>
  ...
  >
    ...
    <div class="suspension-copy">
      ...
      <p v-if="canManage && detail" class="suspension-meta">
        Причина: {{ reasonLabel
        }}<template v-if="detail.startedBy"> · включил {{ detail.startedBy.displayName }}</template>
      </p>
...
.suspension-actions :deep(.p-button) {
  min-height: 32px;
  padding-block: 0.35rem;
  font-size: 0.65rem;
}
.suspension-banner.compact {
  flex: 1 1 auto;
  min-width: 0;
  margin: 0;
  padding: 0;
...
.suspension-banner.compact .suspension-copy {
  flex: 0 1 auto;
}
...
.suspension-banner.compact .suspension-readonly {
  display: none;
}
.suspension-banner.compact .suspension-actions {
  margin-left: auto;
  flex-wrap: nowrap;
}
.suspension-banner.compact .suspension-icon {
  font-size: 0.82rem;
}
.sr-only {
  position: absolute;
