...
describe('история синхронизации профиля', () => {
  ...
  it('по запросу открывает переход версии и состав переданных и удалённых полей', async () => {
    ...
    await wrapper.get('button[aria-label="Открыть историю профиля"]').trigger('click');
    await flushPromises();
    ...
    expect(wrapper.text()).toContain('Передано — поля, пришедшие в этой попытке');
  });
  ...
  it('добавляет более раннюю страницу по opaque cursor и не дублирует попытки', async () => {
    ...
    await wrapper.get('button[aria-label="Загрузить более ранние"]').trigger('click');
    await flushPromises();
