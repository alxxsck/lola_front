...
describe('ProductTelegramCard', () => {
  ...
  it('renders GET null as an independent empty product-bot setup', async () => {
    ...
    expect(wrapper.find('form[data-form="create-product-telegram"]').exists()).toBe(true);
    expect(wrapper.get('[data-integration="telegram-users"]').classes()).toContain(
      'integration-card',
    );
    expect(
      wrapper.findAll('form[data-form="create-product-telegram"] .integration-field'),
    ).toHaveLength(1);
    expect(
      wrapper.get('input[name="productTelegramToken"]').element.closest('label'),
    ).not.toBeNull();
  });
  ...
  it('creates with a durable idempotency key and never echoes the write-only token', async () => {
    ...
    expect(wrapper.text()).not.toContain('123456789:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA');
    expect(
      (wrapper.get('input[name="productTelegramToken"]').element as HTMLInputElement).value,
    ).toBe('');
  });
  ...
  it('clears an initial load error after create and webhook setup succeed', async () => {
    ...
    mocks.get.mockRejectedValueOnce(new Error('initial load failed')).mockResolvedValueOnce({
      ...pendingSetup,
      status: 'ACTIVE',
      webhookSetupStatus: 'SUCCEEDED',
      healthStatus: 'HEALTHY',
    ...
    const wrapper = mountCard();
    await flushPromises();
    expect(wrapper.text()).toContain('Не удалось загрузить пользовательский Telegram.');
    ...
    expect(wrapper.text()).not.toContain('Не удалось загрузить пользовательский Telegram.');
  });
  ...
  it('stops background webhook polling immediately when manage permission is lost', async () => {
    ...
    expect(wrapper.find('form[data-form="rotate-product-telegram"]').exists()).toBe(false);
  });
  ...
  it('uses OCC for rotate and disable and requires explicit disable confirmation', async () => {
    ...
    mocks.disable.mockResolvedValue(installation({ version: 5, status: 'DISABLED' }));
    const wrapper = mountCard();
    await flushPromises();
    ...
    await wrapper.get('form[data-form="rotate-product-telegram"]').trigger('submit');
    await flushPromises();
  ...
  it('changes broadcast delivery with its independent OCC version and idempotency key', async () => {
    ...
    const disabledOption = wrapper.get('button[data-action="product-telegram-broadcasts-disable"]');
    const enabledOption = wrapper.get('button[data-action="product-telegram-broadcasts-enable"]');
  ...
  it('requires confirmation before disabling broadcasts and never sends after rejection', async () => {
    mocks.get.mockResolvedValue(installation({ broadcastsEnabled: true, broadcastsVersion: 9 }));
    vi.mocked(confirm).mockReturnValue(false);
    const wrapper = mountCard();
    await flushPromises();

    await wrapper.get('button[data-action="product-telegram-broadcasts-disable"]').trigger('click');
  ...
  it('offers fresh login after 428 without replaying the broadcast command', async () => {
    ...
    await wrapper.get('button[data-action="product-telegram-broadcasts-enable"]').trigger('click');
    await flushPromises();
  ...
  it('reuses the command key after an ambiguous failure while the server state is unchanged', async () => {
    ...
    mocks.setBroadcastsEnabled
      ...
      .mockResolvedValueOnce(installation({ broadcastsEnabled: true, broadcastsVersion: 8 }));
    const wrapper = mountCard();
    await flushPromises();
  ...
  it('rejects a broadcast response after project switch and resets command state', async () => {
    ...
    void wrapper.get('button[data-action="product-telegram-broadcasts-enable"]').trigger('click');
    await flushPromises();
  ...
  it('rejects a broadcast response after manage permission is lost', async () => {
    ...
    await wrapper.setProps({ canManage: false });
    await flushPromises();
    resolveBroadcasts(installation({ broadcastsEnabled: true, broadcastsVersion: 8 }));
    await flushPromises();

    expect(wrapper.text()).toContain('только для просмотра');
    expect(wrapper.text()).not.toContain('Telegram-рассылки включены.');
    expect(wrapper.find('button[data-action="product-telegram-broadcasts-enable"]').exists()).toBe(
      false,
    );
  });
  ...
  it('polls a durable test with one idempotency key and displays safe failure copy', async () => {
    ...
    await wrapper.get('button[data-action="product-telegram-test"]').trigger('click');
    await vi.advanceTimersByTimeAsync(500);
    await flushPromises();
  ...
  it('refreshes exactly once after an optimistic concurrency conflict', async () => {
    ...
    expect(wrapper.text()).toContain('Настройки изменились в другой вкладке. Данные обновлены.');
    expect(mocks.get).toHaveBeenCalledTimes(2);
  });
  ...
  it('fences stale project responses and clears secrets on project switch', async () => {
    ...
    const wrapper = mountCard();
    await flushPromises();
    await wrapper.get('input[name="productTelegramToken"]').setValue('secret-for-project-one');
  ...
  it("does not let a stale action finally clear the current project's pending state", async () => {
    ...
    await flushPromises();
    expect(
      wrapper.get('button[data-action="product-telegram-test"]').attributes('disabled'),
    ).toBeDefined();
  ...
  it('becomes read-only immediately on manage loss and resets on read loss', async () => {
    ...
    const wrapper = mountCard();
    await flushPromises();
    expect(wrapper.find('form[data-form="rotate-product-telegram"]').exists()).toBe(true);
