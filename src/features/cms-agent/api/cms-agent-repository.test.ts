...
describe('cmsAgentRepository', () => {
  ...
  it('submits and executes through the generated Project-scoped client', async () => {
    ...
    await expect(cmsAgentRepository.execute('project-1', 'request-1')).resolves.toMatchObject({
  ...
  it('rejects the deprecated analysis alias without a canonical domain result', async () => {
    ...
    await expect(cmsAgentRepository.execute('project-1', 'request-legacy')).resolves.toEqual({
      kind: 'PROTOCOL_ERROR',
    });
  });
});
