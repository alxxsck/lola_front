...
export function summarizeEventFieldCapability(
  ...
  if (!catalogLoaded)
    return {
      label: 'Возможности сценариев появятся после обновления',
      availableForScenarios: false,
    };
  if (!field)
    return { label: 'Эта версия поля пока недоступна в сценариях', availableForScenarios: false };
  ...
  const historyLabel =
    historyFilter && historyAggregate
