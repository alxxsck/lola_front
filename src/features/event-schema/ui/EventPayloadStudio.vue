...
const generatedSample = computed(() =>
  JSON.stringify(buildEventSchemaExample(props.modelValue), null, 2),
);
  ...
  <section class="payload-studio">
    <template v-if="showPayload">
      <template v-if="!advancedVisible">
        ...
        <div v-if="modelValue.fields.length" class="schema-fields">
          <article
            v-for="field in modelValue.fields"
            :key="field.id"
            class="schema-field"
            :class="{ unsupported: !field.visuallyEditable }"
            role="group"
            :aria-label="`Поле ${fieldLabel(field)}`"
          >
            <template v-if="field.visuallyEditable">
              <div class="field-main">
                <label :for="`field-title-${field.id}`"
                  >Название<input
                ...
                <label :for="`field-wire-${field.id}`"
                  >Имя в данных<input
                    :id="`field-wire-${field.id}`"
                    data-test="field-wire-key"
                    :value="field.wireKey"
              ...
              <div class="field-meta">
                <span
                  :class="{
                    available: capability(field).availableForScenarios,
                  }"
              ...
              >
                ...
                <label
                  >Смысл данных<select
                    ...
                    :aria-label="`Смысл данных поля ${fieldLabel(field)}`"
                ...
                >
                <label
                  >Способ хранения<select
                    :value="field.unit ?? ''"
                    :aria-label="`Способ хранения поля ${fieldLabel(field)}`"
                ...
                <template v-if="field.semanticType?.includes('money')">
                  <label
                    >Масштаб отображения<input
                      data-test="money-display-scale"
                      type="number"
                      min="0"
                      step="any"
                      :value="field.displayScale"
                      :aria-label="`Масштаб отображения поля ${fieldLabel(field)}`"
                      @input="
                        updateField(field.id, {
                          displayScale: optionalNumber($event),
                        })
                      "
                    /><small
                  ...
                  >
                  <label
                    >Знаков после запятой<input
                      data-test="money-display-precision"
                      type="number"
                      min="0"
                      max="12"
                      step="1"
                      :value="field.displayPrecision"
                      :aria-label="`Точность отображения поля ${fieldLabel(field)}`"
                      @input="
                        updateField(field.id, {
                          displayPrecision: optionalNumber($event),
                        })
                      "
                ...
                <label v-if="canEditEnum(field)" class="wide"
                  >Допустимые варианты<textarea
                    ...
                    :aria-label="`Допустимые варианты поля ${fieldLabel(field)}`"
                    rows="3"
                ...
                <label class="checkbox-label"
                  ><input
                    ...
                    :aria-label="`Скрывать значение поля ${fieldLabel(field)}`"
                    @change="
                      updateField(field.id, {
                    ...
                    "
                  />
                  Скрывать значение в проверках</label
                >
        ...
        <div class="schema-options">
          ...
          >
            <input
              ...
              aria-label="Разрешить дополнительные поля"
              @change="setAdditionalProperties"
            />
      ...
      <section class="advanced-schema">
        <button
          type="button"
          class="link-button"
          data-test="advanced-schema"
          :aria-expanded="advancedVisible"
          aria-controls="advanced-schema-editor"
        ...
        >
          ...
          >
            ...
            ><button
              type="button"
              class="link-button"
              data-test="discard-advanced-schema"
              @click="discardAdvancedDraft"
            >
              Отменить изменения JSON
            </button>
          </div>
          ...
          <div class="advanced-actions">
            <button
              type="button"
              class="secondary-button"
              data-test="review-advanced-schema"
              @click="reviewAdvancedSchema"
            >
              Проверить изменения
            </button>
            <button
              v-if="advancedCandidate"
              type="button"
              class="primary-inline-button"
              data-test="apply-advanced-schema"
              @click="applyAdvancedSchema"
            >
              Применить проверенную схему
            </button>
          </div>
        </div>
      </section>
    ...
    <section v-if="showSample" class="sample-validator">
      ...
      <details class="sample-details">
        ...
        <textarea
          v-model="sampleText"
          aria-label="Пример данных для проверки"
          rows="6"
          spellcheck="false"
          placeholder='{"amountMinor": 125, "currency": "EUR"}'
        />
        <button
          type="button"
          class="secondary-button"
          data-test="validate-sample"
      ...
      <table v-else-if="sampleIssues?.length" class="sample-issues">
        ...
        <tbody>
          ...
          >
            ...
            <td data-label="Ожидалось">{{ issue.expected }}</td>
            <td data-label="Получено">
              <code>{{ issue.actual }}</code>
            </td>
            <td data-label="Как исправить">{{ issue.explanation }}</td>
          </tr>
        </tbody>
      </table>
    </section>
...
.opaque-field strong,
.opaque-field span,
.sample-validator header strong,
.sample-validator header span {
  display: block;
}
.studio-header span,
.opaque-field span,
...
.sample-details textarea {
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 0.74rem;
  line-height: 1.5;
  resize: vertical;
}
.sample-details .secondary-button {
  margin-top: 9px;
}
.sample-success {
  margin: 0;
