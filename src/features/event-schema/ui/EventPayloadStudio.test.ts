...
function mountStudio(
  ...
  return mount(
    defineComponent({
      ...
      template:
        '<EventPayloadStudio v-model="draft" :baseline-schema="options.baseline" :catalog-event="options.event" />',
    }),
...
describe('EventPayloadStudio', () => {
  it('preserves spaces while the user types a field title', async () => {
    ...
    expect(wrapper.getComponent(EventPayloadStudio).props('modelValue').fields[0]?.title).toBe(
      'Название ',
    );
  ...
  it('applies advanced JSON without losing unknown keywords and shows catalog capabilities', async () => {
    ...
    await wrapper.get('[aria-label="JSON Schema"]').setValue(
      JSON.stringify({
        type: 'object',
    ...
    expect(
      serializeEventSchema(wrapper.getComponent(EventPayloadStudio).props('modelValue')),
    ).toEqual({
      type: 'object',
      unevaluatedProperties: false,
      properties: {
  ...
  it('validates a pasted sample payload against the current schema', async () => {
    ...
    await wrapper
      .get('[aria-label="Пример данных для проверки"]')
      .setValue('{"amountMinor":0,"extra":true}');
  ...
  it('keeps an invalid advanced schema open and leaves the visual draft unchanged', async () => {
    ...
    await wrapper
      .get('[aria-label="JSON Schema"]')
      .setValue('{"type":"object","properties":"bad"}');
    await wrapper.get('[data-test="review-advanced-schema"]').trigger('click');
