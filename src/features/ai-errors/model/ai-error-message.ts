const AI_ERROR_MESSAGES: Readonly<Record<string, string>> = Object.freeze({
  ...
  PROJECT_ANALYSIS_PROVIDER_TIMEOUT:
    "AI-провайдер не успел завершить анализ. Повторите запрос.",
  PROJECT_ANALYSIS_PROVIDER_DISCONNECTED:
    "Связь с AI-провайдером прервалась. Повторите запрос.",
...
const AI_LIMITATION_MESSAGES: Readonly<Record<string, string>> = Object.freeze({
  CLARIFICATION_REQUIRED:
    "Для продолжения анализа нужно уточнить, какие данные использовать.",
  EVENT_RETENTION_LIMIT_REACHED:
    "Запрошенный период начинается раньше доступной истории событий, поэтому итог неполный.",
  NONCONFORMING_VALUES_EXCLUDED:
    "Некорректные значения отдельных событий исключены из расчёта.",
  RESULT_BYTE_LIMIT_REACHED:
    "Результат сокращён до разрешённого размера.",
  RESULT_GROUP_LIMIT_REACHED:
    "Показана только часть групп из-за установленного лимита.",
