...
runConversationSurfaceBehaviorSuite({
  ...
  messagesWithGap: messages.map((message) =>
    message.id === 'support-admin-message' ? { ...message, ordinal: 3 } : message,
  ),
});
...
describe('SupportConversationPane', () => {
  ...
  it('forwards typed Surface capabilities without owning message state', async () => {
    ...
    expect(wrapper.emitted('change-translation-mode')?.[0]).toEqual(['TRANSLATED']);
    ...
    expect(wrapper.emitted('retry-delivery')?.[0]).toEqual(['support-admin-message']);
