...
describe("EventPicker", () => {
  ...
  it("passes a debounced global query to the catalog loader", async () => {
    ...
    await flushPromises();
    await wrapper
      .get('[data-testid="event-picker-search"]')
      .setValue("успешное завершение");
  ...
  it("reloads the catalog when the ingestion filter changes", async () => {
    ...
    await flushPromises();
    await wrapper
      .get('[data-testid="event-picker-filter-backend"]')
      .trigger("click");
    await flushPromises();
  ...
  it("can stage and apply clearing an optional single selection", async () => {
    ...
    const clear = wrapper
      .findAll("button")
      .find((button) => button.text() === "Очистить");
  ...
  it("closes and invalidates loaded options when its scope changes", async () => {
    ...
    expect(wrapper.find('[data-testid="event-picker-option"]').exists()).toBe(
      false,
    );
  });
});
