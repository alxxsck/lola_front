        ...
        scenarios: [
          {
            ...
            issues: [{ conditionPath: 'event.payload.amount' }, { position: 0 }],
          },
        ],
      },
      undefined,
      'EVENT_SCHEMA_BREAKS_SCENARIOS',
    );
  ...
  it('explains both deletion dependency types', () => {
    const error = new ApiError(
      ...
      'Conflict',
      {
        eventLogCount: 3,
        scenarios: [
          { id: 'scenario-1', code: 'deposit_success', name: 'Успешный депозит', status: 'ACTIVE' },
        ],
      },
      undefined,
      'EVENT_DEFINITION_IN_USE',
    );
    ...
    expect(eventDefinitionError(error, 'Не удалось удалить').message).toContain(
