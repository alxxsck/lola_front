...
export function eventDefinitionError(cause: unknown, fallback: string): EventDefinitionError {
  ...
  const eventLogCount =
    typeof details?.eventLogCount === 'number' ? details.eventLogCount : undefined;

  if (error?.code === 'EVENT_SCHEMA_BREAKS_SCENARIOS') {
    return {
      message:
        'Новую схему нельзя сохранить: из-за неё перестанут работать связанные сценарии. Сначала исправьте указанные места.',
      scenarios,
    };
  }
...
function deleteConflictMessage(scenarioCount: number, eventLogCount?: number): string {
  ...
  if (eventLogCount)
    return 'По событию уже есть история, поэтому удалить его нельзя. Выключите событие, чтобы сохранить данные.';
...
function readIssues(value: unknown): string[] {
  ...
  return value.flatMap((item) => {
    ...
    if (!issue) return [];
    if (typeof issue.conditionPath === 'string')
      return [`Условие использует поле «${issue.conditionPath}».`];
    if (typeof issue.position === 'number')
      return [`Шаг ${issue.position + 1}: шаблон не соответствует новой схеме события.`];
    return [];
  });
}
