...
describe('EventSchemaAuthoring', () => {
  ...
  it('runs current schema through save, impact and safe publication', async () => {
    ...
    expect(mocks.getSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1');
    ...
    await flushPromises();
    expect(mocks.analyzeSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedDraftVersion: 1,
    });
    expect(wrapper.get('[data-test="compatibility-summary"]').text()).toContain(
      'Безопасное изменение',
    );
    expect(wrapper.get('[data-test="impact-summary"]').text()).toContain('Зависимых consumers: 2');
    ...
    await flushPromises();
    expect(mocks.publishSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedDraftVersion: 1,
    ...
    expect(wrapper.get('[data-test="publish-result"]').text()).toContain('Продолжено consumers');
  ...
  it('reports an indeterminate publish without fabricating server evidence', async () => {
    ...
    mocks.publishSchemaDraft.mockRejectedValue(new ApiError(0, 'Соединение прервано'));
    const wrapper = mountEditor();
    await flushPromises();
    await button(wrapper, 'Проверить влияние').trigger('click');
    await flushPromises();
  ...
  it('counts unextended active waits in post-publish warnings', async () => {
    ...
    await button(wrapper, 'Проверить влияние').trigger('click');
    await flushPromises();
    await wrapper.get('#event-schema-publish-reason').setValue('Проверка предупреждений ожиданий');
    await button(wrapper, 'Опубликовать версию 5').trigger('click');
    await flushPromises();

    expect(wrapper.get('[data-test="publish-result"]').text()).toContain('Предупреждений1');
  });
  ...
  it('freezes payload controls while a state-replacing save is pending', async () => {
    ...
    expect(wrapper.findAll('[data-test="field-title"]')[0]!.element.matches(':disabled')).toBe(
      true,
    );
  ...
  it('resumes a server draft and preserves local JSON after a stale save', async () => {
    ...
    await wrapper.findAll('[data-test="field-title"]')[0]!.setValue('Локальная сумма');
    await button(wrapper, 'Сохранить черновик').trigger('click');
    await flushPromises();

    expect(wrapper.findAll('[data-test="field-title"]')[0]!.element).toHaveProperty(
      'value',
      'Локальная сумма',
    );
    expect(wrapper.get('[role="alert"]').text()).toContain('изменён другим администратором');
    expect(wrapper.get('.conflict-panel').text()).toContain('На сервере уже черновик v1');
  ...
  it('discards only the exact resumed draft and resets to the published schema', async () => {
    ...
    await wrapper.get('#event-schema-discard-reason').setValue('Изменение больше не требуется');
    await button(wrapper, 'Удалить черновик').trigger('click');
    await flushPromises();

    expect(mocks.discardSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedDraftVersion: 1,
      reason: 'Изменение больше не требуется',
    });
    expect(
      wrapper
  ...
  it('requires both explicit confirmations for a producer-breaking publish', async () => {
    ...
    await button(wrapper, 'Проверить влияние').trigger('click');
    await flushPromises();
    await wrapper.get('#event-schema-publish-reason').setValue('Обязательная валюта согласована');
    const publish = button(wrapper, 'Опубликовать версию 5');
    expect(publish.attributes('disabled')).toBeDefined();

    const confirmations = wrapper.findAll('.publish-form input[type="checkbox"]');
  ...
  it('never enables publish while scenarios or active waits block the revision', async () => {
    ...
    expect(button(wrapper, 'Опубликовать версию 5').attributes('disabled')).toBeDefined();
        ...
        externalConsumers: [
          {
            ...
            matchingMode: 'EXACT',
            fieldDependencies: [{ fieldKey: 'deposit.amount', path: ['amount'] }],
  ...
  it('renders archived definitions without any schema mutation controls', async () => {
    ...
    expect(
      wrapper
        .findAll('button')
        .some((item) => ['Сохранить черновик', 'Проверить влияние'].includes(item.text())),
    ).toBe(false);
  });
  ...
  it('keeps draft editing available but disables publish without publish permission', async () => {
    ...
    await button(wrapper as ReturnType<typeof mountEditor>, 'Проверить влияние').trigger('click');
    await flushPromises();
    await wrapper.get('#event-schema-publish-reason').setValue('Готово к публикации');

    expect(wrapper.text()).toContain('нет отдельного права публиковать');
    expect(
      button(wrapper as ReturnType<typeof mountEditor>, 'Опубликовать версию 5').attributes(
        'disabled',
      ),
    ).toBeDefined();
  });
  ...
  it('creates a disabled successor Event Definition for a semantic break', async () => {
    ...
    expect(mocks.createSchemaSuccessor).toHaveBeenCalledWith('project-1', 'event-key-1', {
      code: 'deposit.completed',
  ...
  it('never creates a semantic successor from schema edited after impact analysis', async () => {
    ...
    expect(button(wrapper, 'Создать новое событие').attributes('disabled')).toBeDefined();
    await button(wrapper, 'Создать новое событие').trigger('click');
  ...
  it('rejects semantic successor when the source draft changed remotely', async () => {
    ...
    mocks.createSchemaSuccessor.mockRejectedValue(
      new ApiError(409, 'Черновик изменился', undefined, 'request-1', 'EVENT_SCHEMA_DRAFT_STALE'),
    );
    const wrapper = mountEditor();
    await flushPromises();
    await button(wrapper, 'Проверить влияние').trigger('click');
    ...
    expect(wrapper.get('[role="alert"]').text()).toContain('Черновик изменился');
