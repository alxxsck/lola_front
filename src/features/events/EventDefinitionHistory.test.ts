...
describe('EventDefinitionHistory', () => {
  ...
  it('opens an exact immutable revision linked from Event Logs', async () => {
    ...
    expect(mocks.detail).toHaveBeenCalledWith('project-1', 'event-key-1', 'revision-1');
    expect(wrapper.text()).toContain('Версия v2');
  });

  it('closes and ignores stale revision responses after identity changes', async () => {
    let resolveFirst!: (value: { items: Array<typeof revision>; nextCursor: null }) => void;
    ...
    expect(wrapper.find('[data-testid="event-revision-detail"]').exists()).toBe(false);
  });
});
