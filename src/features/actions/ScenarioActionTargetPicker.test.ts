...
describe('ScenarioActionTargetPicker', () => {
  it('shows existing and new targets together and applies a single outlined choice', async () => {
    ...
    await wrapper.get('[data-testid="action-target-picker-trigger"]').trigger('click');
    await flushPromises();

    expect(
      wrapper.findAll('[data-testid="action-target-picker-option"]').map((option) => option.text()),
    ).toEqual([
    ...
    await wrapper.get('[data-testid="action-target-picker-apply"]').trigger('click');
  ...
  it('filters existing scenario steps separately from actions that create a new step', async () => {
    ...
    await wrapper.get('[data-testid="action-target-picker-trigger"]').trigger('click');
    await flushPromises();
    await wrapper.get('[data-testid="action-target-picker-filter-existing"]').trigger('click');
    await flushPromises();

    const options = wrapper.findAll('[data-testid="action-target-picker-option"]');
