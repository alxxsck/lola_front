...
const catalog = [
  ...
  action('OPEN_PAGE', 'Открыть страницу', 'Показывает нужный раздел интерфейса', 'FRONTEND'),
...
describe('ActionPicker', () => {
  it('searches actions by name, type and description and commits only after confirmation', async () => {
    ...
    await flushPromises();
    await wrapper.get('[data-testid="action-picker-search"]').setValue('интерфейса');
  ...
  it('filters logic, waits and product actions without preloading a flat select', async () => {
    const wrapper = mount(ActionPicker, {
      props: {
        modelValue: '',
        catalog: [...catalog, action('CONDITION', 'Условие', 'Разделяет сценарий на ветки')],
    ...
    await wrapper.get('[data-testid="action-picker-trigger"]').trigger('click');
    await flushPromises();
    await wrapper.get('[data-testid="action-picker-filter-logic"]').trigger('click');
    await flushPromises();

    expect(
      wrapper.findAll('[data-testid="action-picker-option"]').map((option) => option.text()),
