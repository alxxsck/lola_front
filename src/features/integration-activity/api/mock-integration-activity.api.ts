...
function detail(item: IntegrationActivityItem): IntegrationActivityDetail {
  return {
    ...item,
    sourceResourceKind:
      item.activityType === 'CONNECTION' ? 'TELEGRAM_LINK_CHALLENGE' : 'TELEGRAM_PERSONAL_MESSAGE',
...
export const mockIntegrationActivityRepository: IntegrationActivityRepository = {
  async list(_projectId, filters = {}) {
    const filtered = items.filter(
      (item) =>
        (!filters.provider?.length || filters.provider.includes(item.provider)) &&
        (!filters.activityType?.length || filters.activityType.includes(item.activityType)) &&
        (!filters.status?.length || filters.status.includes(item.status)) &&
        (!filters.externalUserId || item.endUser.externalId === filters.externalUserId),
    );
    return {
