...
export type ProjectActionErrorKind =
  'validation' | 'permission' | 'conflict' | 'network' | 'not-found' | 'unknown';
...
const messages: Readonly<Record<string, string>> = {
  ...
  INTEGRATION_ACTION_PROJECT_MISMATCH: 'Подключённое действие относится к другому проекту.',
  ACTION_SURFACE_UNSUPPORTED: 'Это действие нельзя использовать выбранным способом.',
  AI_ACTION_DESCRIPTION_INVALID: 'Подсказка для Retenive должна содержать от 20 до 2000 символов.',
  AI_ACTION_DESCRIPTION_UNSAFE: 'Уберите из подсказки адреса, ключи доступа и программный код.',
  PROJECT_ACTION_CONFIGURATION_INVALID: 'Проверьте обязательные поля в настройках действия.',
  AI_ACTION_AUDIT_REASON_REQUIRED: 'Объясните, зачем Retenive нужен доступ к этому действию.',
  AI_ACTION_AUDIT_REASON_UNSAFE: 'Уберите из причины адреса, ключи доступа и программный код.',
...
export function toProjectActionError(cause: unknown): ProjectActionError {
  ...
  const code =
    error.code ??
    (error.status === 403 ? 'PROJECT_ACTION_OWNER_REQUIRED' : 'PROJECT_ACTION_REQUEST_FAILED');
