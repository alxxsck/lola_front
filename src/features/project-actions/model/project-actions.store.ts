...
export const useProjectActionsStore = defineStore('project-actions', () => {
      ...
      .catch((cause: unknown) => {
        if (requestGeneration === generation) {
          errorsByProject.value = {
            ...errorsByProject.value,
            [projectId]: toProjectActionError(cause),
          };
        }
        throw cause;
      })
    ...
    try {
      ...
      if (requestGeneration === generation) {
        previewErrorsByAction.value = {
          ...previewErrorsByAction.value,
          [actionId]: toProjectActionError(cause),
        };
      }
      throw cause;
    } finally {
  ...
  async function configure(
    ...
    return mutate(projectId, actionId, () =>
      projectActionsRepository.configure(projectId, actionId, input),
    );
  }
