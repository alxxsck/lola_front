...
export function toConfigureProjectActionInput(draft: ProjectActionDraft): ConfigureProjectActionInput {
...
export function validateProjectActionDraft(
  ...
  if (!canConfigureProjectActions(_role) && projectActionDraftChanged(action, draft)) {
    return [{
      field: 'form',
      code: 'PROJECT_ACTION_OWNER_REQUIRED',
      message: 'Изменять и архивировать действия может только владелец проекта.',
    }]
  }
  ...
  if (draft.aiEnabled && !surfaces.has('AI')) {
    issues.push({
      field: 'aiEnabled',
      code: 'ACTION_SURFACE_UNSUPPORTED',
      message: 'Этот тип действия нельзя разрешить AI.',
    })
  }
  if (draft.aiEnabled && surfaces.has('AI')) {
    ...
    if (descriptionLength < 20 || descriptionLength > 2000) {
      issues.push({
        ...
        message: 'Описание для AI должно содержать от 20 до 2000 символов.',
      })
    }
  }
  if (requiresAiAuditReason(action, draft) && surfaces.has('AI')) {
    ...
    if (auditReasonLength < 10 || auditReasonLength > 500) {
      issues.push({
        ...
        message: 'Укажите причину включения AI длиной от 10 до 500 символов.',
      })
    }
  }
...
export function requiresAiAuditReason(action: ProjectAction, draft: ProjectActionDraft): boolean {
  return !action.aiEnabled && draft.aiEnabled
    || action.aiEnabled && draft.aiEnabled && configurationBroadensAuthority(action.configuration, draft.configuration)
}
...
function stableValue(value: unknown): string {
  ...
  if (isRecord(value)) {
    return `{${Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, nested]) => `${key}:${stableValue(nested)}`).join(',')}}`
  }
  return JSON.stringify(value) ?? String(value)
}

function projectActionDraftChanged(action: ProjectAction, draft: ProjectActionDraft): boolean {
  return action.scenarioEnabled !== draft.scenarioEnabled
