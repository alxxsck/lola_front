...
function variantCountLabel(count: number): string {
  if (count % 10 === 1 && count % 100 !== 11) return `${count} доступный вариант`;
  if ([2, 3, 4].includes(count % 10) && ![12, 13, 14].includes(count % 100))
...
function issueMessage(code: string): string {
  return (
    {
      AI_ACTION_DISABLED: 'Разрешите Retenive использовать это действие и сохраните изменения.',
  ...
  <section class="ai-preview" aria-labelledby="ai-preview-heading">
    <div class="preview-heading">
      <div>
        ...
        <p>
          Здесь показано, когда действие доступно Retenive и какие данные она может передать при
          выполнении.
        </p>
      </div>
      <span class="server-owned"
        ><i class="pi pi-shield" /> Данные проекта, пользователя и диалога добавляются
        автоматически</span
      >
    </div>
    ...
    <template v-else-if="preview?.tool">
      <div class="tool-card">
        ...
        <div class="tool-metrics">
          <span>{{ fieldCountLabel(schemaPropertyCount(preview.tool.parameters)) }}</span
    ...
    <div v-else class="preview-empty">
      ...
      <div>
        ...
        <span>
          Проверьте, что доступ для Retenive включён и сохранён. Если действие открывает страницу,
          окно или элемент, сначала опубликуйте его в разделе «Интерфейс».
        </span>
