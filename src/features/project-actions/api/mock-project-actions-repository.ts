...
const curatedDefinitions: DemoProductActionDefinition[] = [
  {
    ...
    description: 'Показывает или восстанавливает виджет Retenive.',
    surfaces: ['SCENARIO', 'AI'],
  ...
  },
  {
    ...
    description: 'Запускает поддерживаемую клиентом анимацию Retenive.',
    surfaces: ['SCENARIO', 'AI'],
    inputSchema: closedSchema({ animation: { type: 'string', enum: [...ASSISTANT_ANIMATIONS] } }, [
      'animation',
    ]),
    uiSchema: {
      fields: [
  ...
  {
    code: 'HIGHLIGHT_ELEMENT',
    name: 'Подсветить элемент',
    description: 'Визуально подсвечивает зарегистрированный элемент продукта.',
    surfaces: ['SCENARIO', 'AI'],
    inputSchema: closedSchema({ target: { type: 'string' } }, ['target']),
    uiSchema: {
      fields: [
  ...
  },
  {
    ...
    description: 'Переходит на зарегистрированную страницу продукта.',
    surfaces: ['SCENARIO', 'AI'],
    inputSchema: closedSchema({ page_code: { type: 'string' } }, ['page_code']),
    uiSchema: {
      fields: [{ key: 'page_code', label: 'Страница', control: 'target', targetKinds: ['PAGE'] }],
    },
    ai: false,
  },
  {
    ...
    description: 'Открывает зарегистрированное модальное окно продукта.',
    surfaces: ['SCENARIO', 'AI'],
    inputSchema: closedSchema({ modal_code: { type: 'string' } }, ['modal_code']),
    uiSchema: {
      fields: [
        { key: 'modal_code', label: 'Модальное окно', control: 'target', targetKinds: ['MODAL'] },
      ],
    },
    ai: false,
  },
  {
    code: 'SAY',
    name: 'Сказать текст',
    description: 'Добавляет реплику ассистента в сценарий.',
    surfaces: ['SCENARIO'],
    inputSchema: closedSchema({ text: { type: 'string', minLength: 1, maxLength: 10000 } }, [
      'text',
    ]),
...
const scenarioCatalogItemByCode = new Map(
  demoScenarioActionCatalog.map((item) => [item.type, item]),
);
...
export const mockProjectActionsRepository: ProjectActionsRepository = {
  ...
  async configure(_projectId, actionId, input) {
    ...
    const saved: ProjectAction = {
      ...
      ...input,
      aiUsageDescription:
        input.aiUsageDescription === undefined
          ? current.aiUsageDescription
          : input.aiUsageDescription,
  ...
  async preview(_projectId, actionId) {
    ...
    if (!action.aiEnabled)
      return {
        tool: null,
        issues: [{ code: 'AI_ACTION_DISABLED', message: 'AI surface is disabled' }],
      };
    ...
    if (!parameters)
      return {
        tool: null,
        issues: [
          { code: 'AI_ACTION_TARGETS_UNAVAILABLE', message: 'No safe AI targets are enabled' },
        ],
      };
    return {
      tool: {
...
function readActions(): ProjectAction[] {
  ...
  try {
    return JSON.parse(raw) as ProjectAction[];
  } catch {
    return structuredClone(initialActions());
  }
}
...
function assertDemoInput(action: ProjectAction, input: ConfigureProjectActionInput) {
  if (input.aiEnabled && !action.actionTypeRevision.supportedSurfaces.includes('AI'))
    throw new Error('AI surface не поддерживается');
  if (input.scenarioEnabled && !action.actionTypeRevision.supportedSurfaces.includes('SCENARIO'))
    throw new Error('Scenario surface не поддерживается');
  if (input.aiEnabled && (input.aiUsageDescription?.trim().length ?? 0) < 20)
    throw new Error('Описание для AI слишком короткое');
  if (!action.aiEnabled && input.aiEnabled && (input.auditReason?.trim().length ?? 0) < 10)
    throw new Error('Нужна причина включения AI');
}
...
function demoParameters(action: ProjectAction): Record<string, unknown> | null {
  ...
  const targetField = Object.keys(properties).find(
    (key) => key.endsWith('_code') || key === 'target',
  );
