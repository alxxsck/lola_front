...
function preparedValidation() {
  ...
  const audienceSummary =
    props.audienceDraft && props.audienceContext
  ...
  return serialized.ok && (!serializedAudience || serializedAudience.ok)
    ? {
        ...
        request: {
          ...
          audiencePathIndex: serializedAudience?.pathIndex ?? {},
        },
      }
    : { valid: false as const, issues: localIssues };
}

const validationMachine = createLatestRuleRequestStateMachine<
  ValidationRequest,
...
const unsubscribeValidation = validationMachine.subscribe((state) => {
...
const validationAudienceIssues = computed(() => {
  ...
  return validationState.value.response.audience
    ? mapAudienceIssues(
        ...
        validationAudiencePathIndex.value,
      )
    : [];
});
...
const previewMachine = createLatestRuleRequestStateMachine<
  PreviewRequest,
...
const unsubscribePreview = previewMachine.subscribe((state) => {
...
watch(
  () =>
    [
      props.draftRevision,
      props.audienceDraftRevision,
      props.context.triggerEventDefinitionId,
      props.context.triggerEventCode,
      props.context.contract.revision,
      props.audienceContext?.catalog.revision,
    ] as const,
  () => previewMachine.cancel(),
...
const audienceExplanationRows = computed(() =>
  ...
  previewState.value.response.audience?.explanation
    ? flattenAudienceExplanation(
        props.audienceDraft.root,
        previewState.value.response.audience.explanation,
      )
    : [],
);

function flattenAudienceExplanation(
  node: AudienceDraftNode,
  explanation: unknown,
  depth = 0,
  key = "audience-root",
  ...
  return [
    ...
    ...children.flatMap((child, index) =>
      flattenAudienceEvidence(child, depth + 1, `${key}.segment.${index}`),
    ),
  ];
}

function flattenAudienceEvidence(
  explanation: unknown,
  depth: number,
  key: string,
): ExplanationRow[] {
  ...
  return [
    row,
    ...children.flatMap((child, index) =>
      flattenAudienceEvidence(child, depth + 1, `${key}.${index}`),
    ),
  ];
}
...
<template>
  ...
  >
    ...
    >
      ...
      >
      <span v-else-if="validationState.status === 'semantic-invalid'"
        >Сервер нашёл ошибки в условии.</span
      >
      <span v-else-if="validationState.status === 'network-error'"
        >Не удалось связаться с сервером.</span
      >
    ...
    <ul
      v-if="validationAudienceIssues.length"
      class="issue-list audience-issues"
      aria-label="Ошибки аудитории"
    >
      <li
        v-for="issue in validationAudienceIssues"
        :key="`${issue.code}:${issue.nodeId}:${issue.fieldPath}`"
      >
    ...
    <section class="anchor-section" aria-labelledby="anchor-title">
      ...
      <p>
        ...
        <code>{{ context.triggerEventCode }}</code> этого проекта.
      </p>
        ...
        <form @submit.prevent="applyLogFilters">
          <label
            >ID пользователя в вашем продукте<input
              v-model="logFilters.externalUserId"
              aria-label="ID пользователя для проверки"
          ...
          <label
            >Получено с<input
              v-model="logFilters.receivedFrom"
              type="datetime-local"
              aria-label="Получено с для проверки"
          /></label>
          <label
            >Получено по<input
              v-model="logFilters.receivedTo"
              type="datetime-local"
              aria-label="Получено по для проверки"
          /></label>
          <label
            >Событие с<input
              v-model="logFilters.occurredFrom"
              type="datetime-local"
              aria-label="Событие с для проверки"
          /></label>
          <label
            >Событие по<input
              v-model="logFilters.occurredTo"
              type="datetime-local"
              aria-label="Событие по для проверки"
          /></label>
          <label
            >На странице<select
      ...
      <div v-else-if="logsError" role="alert">
        <span>{{ logsError }}</span>
        <button type="button" @click="retryLogs">Повторить</button>
      </div>
      ...
      <ul v-else class="event-list">
        <li v-for="log in logs" :key="log.id">
          <label>
            <input
              v-model="selectedEventLogId"
              type="radio"
              name="preview-event-log"
              :value="log.id"
            />
            <span
      ...
      <button
        type="button"
        class="primary-button preview-button"
        aria-label="Проверить правило на событии"
        :disabled="!selectedEventLogId || previewState.status === 'pending'"
        @click="previewNow"
      >
      ...
      <div class="preview-status" aria-live="polite">
        ...
        <p v-else-if="previewState.status === 'semantic-invalid'">
          Предварительная проверка недоступна: исправьте ошибки правила.
        </p>
        <p v-else-if="previewState.status === 'local-invalid'">
          Сначала заполните обязательные поля правила.
        </p>
        <p v-else-if="previewState.status === 'network-error'" role="alert">
          Не удалось выполнить preview.
          <button type="button" @click="previewNow">Повторить preview</button>
        </p>
        <p v-else-if="previewState.status === 'contract-error'" role="alert">
        ...
        >
          ...
          <dl>
            <template v-if="row.actual"
              ><dt>Фактически</dt>
              <dd>{{ row.actual }}</dd></template
            >
            <template v-if="row.expected"
              ><dt>Ожидалось</dt>
              <dd>{{ row.expected }}</dd></template
            >
            <template v-if="row.matchedCount"
              ><dt>Найдено событий</dt>
              <dd>{{ row.matchedCount }}</dd></template
            >
            <template v-if="row.window"
              ><dt>Окно</dt>
              <dd>{{ row.window }}</dd></template
            >
          </dl>
        </li>
      </ol>
      <section
        v-if="audienceExplanationRows.length"
      ...
      >
        ...
        <ol class="explanation-tree" aria-label="Объяснение аудитории">
          <li
            v-for="row in audienceExplanationRows"
            :key="row.key"
            :style="{ '--depth': row.depth }"
          >
            <div>
    ...
    >
      <article>
        ...
        <template v-if="summaryResponse.cost"
          ><strong>{{ summaryResponse.cost.class }}</strong>
          <p>
            {{ summaryResponse.cost.leaves }} условий ·
            {{ summaryResponse.cost.aggregateLeaves }} вычислений ·
            {{ summaryResponse.cost.historyWindowDays }} дней истории
          </p></template
        >
        <p v-else>Сервер не рассчитал сложность.</p>
      </article>
      <article>
        <h3>Зависимости</h3>
        <ul v-if="summaryResponse.dependencies.length">
          <li
            v-for="dependency in summaryResponse.dependencies"
            :key="dependency.eventDefinitionRevisionId"
          >
            {{ dependency.eventCode }} · schema v{{ dependency.schemaVersion }}
          </li>
        </ul>
        <p v-else>Нет внешних зависимостей.</p>
      </article>
      <article>
        ...
        <p v-else>Нет предупреждений.</p>
      </article>
      <article v-if="summaryResponse.audience">
        <h3>Аудитория</h3>
...
<style scoped>
.validation-preview {
  container: validation-preview / inline-size;
  display: grid;
  gap: 20px;
  min-width: 0;
}
...
.validation-status.local-invalid,
.validation-status.semantic-invalid,
.validation-status.contract-error {
  background: var(--status-danger-soft);
  color: var(--status-danger-text);
}
.validation-status.network-error {
...
.audience-explanation h4,
.audience-explanation p {
  margin: 0;
}
.audience-explanation p {
  color: var(--text-small-muted);
...
@container validation-preview (max-width:900px) {
  .log-filters form {
...
@container validation-preview (max-width:700px) {
  .section-header {
    align-items: stretch;
