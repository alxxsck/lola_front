...
watch(
  () => props.expectedCurrentRevisionId,
  (value) => {
    expectedHead.value = value;
  },
);
...
const audienceSummary = computed(() =>
  props.audienceDraft && effectiveAudienceContext.value
...
const localIssues = computed(() => [
  ...(!ruleResult.value || ruleResult.value.ok
    ? []
    : ruleResult.value.issues.map((issue) => issue.message)),
...
const canPublish = computed(
  () =>
    Boolean(props.projectId && props.scenarioId) &&
...
watch(
  ...
  () => {
    if (recoveredContract.value?.revision === props.context.contract.revision)
      recoveredContract.value = null;
    machine.reset();
    void reviewNow();
  },
...
<template>
  <section class="publish-panel" aria-labelledby="publish-title">
    ...
    <div
      v-else-if="reviewState.status === 'semantic-invalid'"
      class="review-state error"
      role="alert"
    >
      <strong>Сервер нашёл ошибки</strong
      ><button
        v-for="issue in reviewState.response.issues"
      ...
      >
        ...
        ><code>{{ issue.code }} · {{ issue.path }}</code></button
      ><button
        v-for="issue in reviewState.response.audience?.issues ?? []"
        :key="`audience:${issue.code}:${issue.path}`"
        type="button"
        class="validation-issue"
        @click="focusAudienceIssue(issue)"
      >
        <span>Аудитория: {{ issue.message }}</span
        ><code>{{ issue.code }} · {{ issue.path }}</code></button
      ><span
        v-for="warning in reviewState.response.audience?.warnings ?? []"
        :key="`audience-warning:${warning.code}`"
        >Предупреждение аудитории: {{ warning.code }}</span
      >
    </div>
    <div
      v-else-if="reviewState.status === 'ready'"
      class="server-review"
      aria-label="Eligibility: стоимость, зависимости и предупреждения до публикации"
    >
      <div>
        <strong>Стоимость условий</strong
        ><span>{{
      ...
      <div>
        <strong>Зависимости условий</strong
        ><span>{{
      ...
      <div v-if="reviewState.response.audience">
        <strong>Стоимость аудитории</strong
        ><span>{{
    ...
    >
      ...
      <dl>
        ...
        <dd>
          {{ state.response.dependencies.eventDefinitionRevisionIds.length }}
        </dd>
...
.document-dependencies strong,
.document-dependencies span {
  font-size: 0.66rem;
}
.document-dependencies span {
  color: var(--text-small-muted);
