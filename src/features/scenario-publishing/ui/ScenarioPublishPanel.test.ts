...
vi.mock('@/shared/api/repository/scenario-authoring', async (importOriginal) => ({
  ...
  scenarioAuthoringRepository: {
    publishScenario: mocks.publishScenario,
    validateScenarioDraft: mocks.validateScenarioDraft,
  },
}));
...
const context: RuleDomainContext = {
  triggerEventDefinitionId: 'event-1',
...
const draft: RuleDraft = {
  version: 1,
  root: { nodeId: 'root', kind: 'activityDayStreak', compare: { operator: 'gte', value: 2 } },
};
const audienceDraft: AudienceDraft = {
  version: 1,
  root: { nodeId: 'audience-root', kind: 'locale', operator: 'eq', value: 'ru-RU' },
};
const audienceContext: AudienceDomainContext = {
  segments: [],
  catalog: {
    version: 1,
    revision: 'audience-catalog-9',
    locales: [{ code: 'ru-RU', language: 'ru', label: 'Русский' }],
    localeSource: { operators: ['eq'], control: 'SELECT', authoringAvailability: 'AVAILABLE' },
    languageSource: { operators: ['eq'], control: 'SELECT', authoringAvailability: 'AVAILABLE' },
    country: {
...
const response = {
  revision: {
    ...
    triggerEventDefinitionRevisionId: 'event-1',
  },
  dependencies: {
    actionTypes: ['SAY'],
    conditionPaths: [],
    eventDefinitionRevisionIds: ['event-1'],
  },
...
describe('ScenarioPublishPanel', () => {
      ...
      dependencies: [
        {
          ...
          definitionKeyId: 'event-key',
          eventDefinitionRevisionId: 'event-1',
          schemaVersion: 1,
        },
      ],
  ...
  it('publishes the reviewed session draft atomically and explains the immutable revision', async () => {
    const wrapper = mount(ScenarioPublishPanel, {
      props: {
        projectId: 'project-1',
        scenarioId: 'scenario-1',
        draft,
    ...
    expect(mocks.publishScenario).toHaveBeenCalledWith('project-1', 'scenario-1', {
      ...
      rule: {
        version: 1,
        root: { kind: 'activityDayStreak', compare: { operator: 'gte', value: 2 } },
      },
    });
    expect(wrapper.text()).toContain('Версия №1 опубликована');
    expect(wrapper.text()).toContain('Новые запуски');
    expect(wrapper.emitted('published')?.at(-1)).toEqual([
      'revision-1',
      {
  ...
  it('serializes reactive action configs before the backend draft review', async () => {
    ...
    expect(mocks.validateScenarioDraft).toHaveBeenCalledWith(
      'project-1',
      'scenario-1',
    ...
    expect(
      wrapper.get('button[aria-label="Опубликовать версию сценария"]').attributes(),
  ...
  it('reviews and publishes Audience as a separate pinned contract', async () => {
    mocks.validateScenarioDraft.mockResolvedValue({
      ...
      audience: {
        ...
        issues: [],
        dependencies: {
          attributeRevisionIds: ['attribute-revision-1'],
          segmentRevisionIds: ['segment-revision-2'],
        },
        cost: { leaves: 1, segmentLeaves: 1 },
        warnings: [],
      },
    });
    mocks.publishScenario.mockResolvedValue({
      ...
      dependencies: {
        ...
        userAttributeRevisionIds: ['attribute-revision-1'],
        segmentRevisionIds: ['segment-revision-2'],
      },
      audienceCost: { leaves: 1, segmentLeaves: 1 },
      audiencePolicy: audienceContext.catalog.snapshotPolicy,
    });
    const wrapper = mount(ScenarioPublishPanel, {
      props: {
        projectId: 'project-1',
    ...
    await wrapper.get('button[aria-label="Опубликовать версию сценария"]').trigger('click');
    await flushPromises();
    expect(mocks.publishScenario).toHaveBeenCalledWith(
      'project-1',
      'scenario-1',
      expect.objectContaining({
        audience: { version: 1, root: { kind: 'locale', operator: 'eq', value: 'ru-RU' } },
        ...
        issues: [
          {
            ...
            path: 'root.value',
            message: 'Выберите допустимый locale',
          },
        ],
  ...
  it('refreshes a stale catalog without losing the draft or publishing automatically', async () => {
    const stale = new (await import('@/shared/api/http/api-error')).ApiError(
      409,
      'Catalog stale',
      { currentCatalogRevision: 'catalog-43' },
      undefined,
  ...
  it('does not overwrite an unknown concurrent head automatically', async () => {
    const conflict = new (await import('@/shared/api/http/api-error')).ApiError(
      409,
      'Head changed',
  ...
  it('ignores an outdated successful review after the draft becomes locally invalid', async () => {
    ...
    mocks.validateScenarioDraft.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    await wrapper.setProps({
      draft: {
        version: 1,
        root: { nodeId: 'broken-root', kind: 'incomplete', leaf: { kind: 'eventField' } },
      },
    });
    await flushPromises();
    resolveReview({
  ...
  it('emits the exact server issue so the editor can focus the failing stage', async () => {
    const issue = {
      ...
      path: 'deliveryPolicy.expiryMs',
      message: 'Срок доставки недопустим',
    };
    mocks.validateScenarioDraft.mockResolvedValueOnce({
      valid: false,
      issues: [issue],
      dependencies: [],
      cost: null,
