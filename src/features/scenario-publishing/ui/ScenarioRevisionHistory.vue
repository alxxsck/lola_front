...
import {
  ...
  type ScenarioRevisionDetailResponseDto,
  type ScenarioRevisionListItemResponseDto,
...
async function load(reset = true) {
  ...
  error.value = '';
  try {
    const page = await scenarioAuthoringRepository.getScenarioRevisions(
      props.projectId,
      props.scenarioId,
      {
        limit: 25,
...
async function openRevision(revision: ScenarioRevisionListItemResponseDto) {
  ...
  error.value = '';
  try {
    selected.value = await scenarioAuthoringRepository.getScenarioRevision(
      props.projectId,
      props.scenarioId,
      revision.id,
    );
...
async function rollback(revision: ScenarioRevisionListItemResponseDto) {
  ...
  if (
    !window.confirm(
      `Создать новую версию на основе №${revision.revisionNumber}? История останется неизменной.`,
    )
  )
    return;
  ...
  try {
    ...
    const document = await scenarioAuthoringRepository.getScenarioDocument(
      props.projectId,
      props.scenarioId,
    );
...
watch(
  () => [props.projectId, props.scenarioId],
  () => void load(),
);
onMounted(() => void load());
</script>
...
<template>
  <section class="revision-history" aria-labelledby="revision-history-title">
    <header>
      <div>
        <span>Версии сценария</span>
        <h2 id="revision-history-title">История публикаций</h2>
        <p>Возврат к прошлой публикации создаёт новую версию, а история остаётся без изменений.</p>
      </div>
      <Button
        icon="pi pi-refresh"
        text
        rounded
        aria-label="Обновить историю версий"
        :loading="loading"
        @click="load()"
      />
    </header>
    <Message v-if="error" severity="error" :closable="false">{{ error }}</Message>
    <div v-if="loading" class="revision-list">
      <Skeleton v-for="item in 3" :key="item" height="72px" />
    </div>
    <div v-else class="revision-list">
      ...
      >
        <button
          type="button"
          class="revision-main"
          :aria-label="`Открыть версию ${revision.revisionNumber}`"
        ...
        >
          <span
            ...
            ><small
              ...
              {{ revision.publishedByAdminId ?? 'система' }}</small
            ></span
          >
          <code :title="`Хеш содержимого: ${revision.contentHash}`">{{ revision.contentHash }}</code
          ><em v-if="revision.current">текущая</em><i class="pi pi-chevron-right" />
        </button>
        <Button
          label="Откатить"
          icon="pi pi-history"
      ...
      <Button
        ...
        severity="secondary"
        text
        aria-label="Загрузить более ранние версии"
        :loading="loadingMore"
        @click="load(false)"
      />
    </div>
    <aside v-if="selected || detailLoading" class="revision-detail">
      ...
      <template v-else-if="selected"
        ><div>
          <strong>Версия №{{ selected.revisionNumber }}</strong
          ><button type="button" aria-label="Закрыть сведения о версии" @click="selected = null">
            ×
          </button>
        </div>
        <dl>
          ...
          <dd>
            <code>{{ selected.catalogRevision }}</code>
          </dd>
          <dt>Автор</dt>
          <dd>{{ selected.publishedByAdminId ?? 'система' }}</dd>
          <dt>Исходные настройки</dt>
          <dd>
            {{
              selected.editable
                ? 'доступны для восстановления'
                : 'доступен только снимок выполнения'
            }}
          </dd>
        </dl>
        <ScenarioLocalePreview
          v-if="localizationCatalog"
          :actions="revisionActions"
          :catalog="localizationCatalog"
          :policy="selected.source.localization"
      /></template>
    </aside>
  </section>
</template>
...
.revision-list article.current {
  border-color: var(--status-success);
  background: var(--status-success-soft);
}
.revision-main {
  display: grid;
  grid-template-columns: minmax(150px, 1fr) minmax(100px, 0.7fr) auto auto;
  align-items: center;
  gap: 9px;
  min-width: 0;
  border: 0;
...
@media (max-width: 620px) {
  ...
  .revision-list article :deep(.p-button) {
    width: 100%;
  }
}
</style>
