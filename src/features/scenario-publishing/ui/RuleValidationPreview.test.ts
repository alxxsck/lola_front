...
const eventLog = {
  ...
  occurredAt: '2026-07-18T09:59:59.000Z', receivedAt: '2026-07-18T10:00:00.000Z', payload: { page: 'promotions' }, context: {},
}
...
const aggregateContext: RuleDomainContext = { ...context, contract: aggregateContract }
const aggregateDraft: RuleDraft = {
  version: 1,
  root: {
    nodeId: 'node-deposits', kind: 'eventAggregate', eventCode: 'deposit.succeeded', measure: 'count', filters: [],
    window: { kind: 'last', durationMs: 86_400_000, boundary: 'beforeTrigger' },
    compare: { operator: 'gte', value: 2 },
  },
}

function mountPreview(props: Partial<InstanceType<typeof RuleValidationPreview>['$props']> = {}) {
  return mount(RuleValidationPreview, {
    props: { projectId: 'project-1', draft, context, draftRevision: 1, ...props },
  })
}
...
describe('RuleValidationPreview', () => {
  beforeEach(() => {
    vi.useFakeTimers()
    vi.clearAllMocks()
    mocks.validateRule.mockResolvedValue({ valid: true, issues: [], dependencies: [], cost: null, warnings: [] })
  ...
  it('debounces automatic validation of a serialized draft', async () => {
    ...
    expect(wrapper.get('[role="status"]').text()).toContain('Подготовка проверки')
    await vi.advanceTimersByTimeAsync(400)
    await flushPromises()

    expect(mocks.validateRule).toHaveBeenCalledWith(
      'project-1',
      { version: 1, root: { kind: 'eventField', eventCode: 'page.opened', fieldKey: 'page.name', operator: 'eq', value: 'promotions' } },
  ...
  it('keeps the latest validation when an aborted request resolves late', async () => {
    ...
    await vi.advanceTimersByTimeAsync(400)
    second.resolve({ valid: true, issues: [], dependencies: [], cost: null, warnings: [] })
    await flushPromises()
    expect(wrapper.text()).toContain('Правило прошло проверку')

    first.resolve({ valid: false, issues: [{ code: 'STALE', path: 'root.value', message: 'Старая ошибка' }], dependencies: [], cost: null, warnings: [] })
    await flushPromises()
  ...
  it('maps backend paths to a focus-node action', async () => {
    mocks.validateRule.mockResolvedValue({
      valid: false,
      issues: [{ code: 'VALUE_NOT_ALLOWED', path: 'root.value', message: 'Выберите другое значение' }],
      dependencies: [],
      cost: null,
      warnings: [],
    })
    ...
    await wrapper.get('ul[aria-label="Ошибки правила"] button').trigger('click')

    expect(wrapper.emitted('focus-node')).toEqual([[{ nodeId: 'node-page', fieldPath: 'value', message: 'Выберите другое значение' }]])
  })
  ...
  it('previews only against a selected project Event Log id', async () => {
    ...
    mocks.previewRule.mockResolvedValue({
      ...
      matched: true,
      explanation: {
        kind: 'eventField', matched: true,
        actual: { visibility: 'VISIBLE', value: 'promotions' },
        expected: { visibility: 'REDACTED' },
      },
      issues: [],
      dependencies: [],
      cost: { class: 'LOW', leaves: 1, aggregateLeaves: 0, historyWindowDays: 0 },
      warnings: [],
    })
    const wrapper = mountPreview()
    await flushPromises()
  ...
  it('resets the anchor and pagination when the trigger Event changes', async () => {
    ...
    await wrapper.setProps({ context: { ...context, triggerEventDefinitionId: 'event-purchase', triggerEventCode: 'purchase.completed' } })
    await flushPromises()
  ...
  it('retries the exact failed cursor page and can return through cursor history', async () => {
    ...
    await wrapper.get('nav[aria-label="Страницы событий"] button:last-child').trigger('click')
    await flushPromises()
    ...
    expect(mocks.getEventLogPage.mock.calls.slice(0, 3)).toEqual([
      ...
      ['project-1', { eventCode: ['page.opened'], limit: 25, cursor: 'cursor-2' }],
      ['project-1', { eventCode: ['page.opened'], limit: 25, cursor: 'cursor-2' }],
    ])
    expect(wrapper.text()).toContain('Страница 2')

    await wrapper.get('nav[aria-label="Страницы событий"] button:first-child').trigger('click')
    await flushPromises()
    expect(mocks.getEventLogPage).toHaveBeenLastCalledWith('project-1', { eventCode: ['page.opened'], limit: 25 })
  })
  ...
  it('hides Event Logs from another revision even when the event code is the same', async () => {
    mocks.getEventLogPage.mockResolvedValueOnce({
      items: [{ ...eventLog, id: 'log-old', eventDefinitionId: 'event-page-old' }, eventLog],
      nextCursor: null,
    })
  ...
  it('zips aggregate explanation details with the draft and shows review metadata', async () => {
    ...
    mocks.previewRule.mockResolvedValue({
      ...
      matched: true,
      explanation: {
        kind: 'eventAggregate', matched: true,
        actual: { visibility: 'VISIBLE', value: '3' },
        expected: { visibility: 'VISIBLE', value: 2 },
        matchedCount: '3',
        window: { from: '2026-07-17T10:00:00.000Z', to: '2026-07-18T10:00:00.000Z' },
      },
      issues: [],
      dependencies: [{ eventCode: 'deposit.succeeded', definitionKeyId: 'key-deposit', eventDefinitionRevisionId: 'event-deposit', schemaVersion: 2 }],
      cost: { class: 'MEDIUM', leaves: 1, aggregateLeaves: 1, historyWindowDays: 1 },
    ...
    expect(wrapper.get('section[aria-label="Стоимость и зависимости"]').text()).toContain('MEDIUM')
  ...
  it('shows a contract state instead of pairing a mismatched explanation tree', async () => {
    ...
    mocks.previewRule.mockResolvedValue({
      ...
      issues: [], dependencies: [], cost: null, warnings: [],
    })
    const wrapper = mountPreview()
    await flushPromises()
    await wrapper.get('input[type="radio"][value="log-1"]').setValue()
  ...
  it('clears an obsolete preview when the catalog revision changes', async () => {
    ...
    await wrapper.setProps({ context: { ...context, contract: { ...contract, revision: 'catalog-2' } } })
    await flushPromises()
