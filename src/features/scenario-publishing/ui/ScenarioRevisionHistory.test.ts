...
const revision = (id: string, revisionNumber: number, current = false) => ({
  ...
  publishedAt: `2026-07-1${revisionNumber}T10:00:00.000Z`,
  publishedByAdminId: `admin-${revisionNumber}`,
  current,
  editable: true,
});
...
describe('ScenarioRevisionHistory', () => {
  beforeEach(() => {
    ...
    mocks.getScenarioRevisions
      .mockResolvedValueOnce({
        items: [revision('revision-3', 3, true), revision('revision-2', 2)],
  ...
  it('pages immutable revisions, loads detail and creates a successor rollback from the observed head', async () => {
    const wrapper = mount(ScenarioRevisionHistory, {
      props: {
        projectId: 'project-1',
        scenarioId: 'scenario-1',
    ...
    await flushPromises();
    expect(mocks.getScenarioRevisions).toHaveBeenNthCalledWith(2, 'project-1', 'scenario-1', {
      limit: 25,
      cursor: 'revision-2',
    });
    ...
    await flushPromises();
    expect(mocks.rollbackScenario).toHaveBeenCalledWith(
      'project-1',
      'scenario-1',
      'revision-2',
      'revision-3',
