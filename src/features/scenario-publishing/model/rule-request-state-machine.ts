export type PreparedRuleRequest<Request, Issue> =
  { valid: true; request: Request } | { valid: false; issues: readonly Issue[] };
...
export function createLatestRuleRequestStateMachine<
  ...
  Issue = unknown,
>(
  options: LatestRuleRequestOptions<Request, Response>,
): LatestRuleRequestStateMachine<Request, Response, Issue> {
  ...
  async function execute(request: Request, requestSequence: number) {
    ...
    try {
      ...
      transition(
        options.isResponse(response)
          ? response.valid
            ? { status: 'valid', response }
            : { status: 'semantic-invalid', response }
          : {
              status: 'contract-error',
              error: new Error('Scenario rule response does not match the expected contract'),
            },
      );
    } catch (error) {
      if (requestSequence !== sequence || requestController.signal.aborted) return;
