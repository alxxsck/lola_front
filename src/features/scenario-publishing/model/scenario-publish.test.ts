...
describe('Scenario publish state machine', () => {
  ...
  it('preserves the exact draft request and exposes the current head after a 409', async () => {
    const conflict = new ApiError(
      409,
      'Revision conflict',
      { currentRevisionId: 'revision-2', expectedCurrentRevisionId: 'revision-1' },
      undefined,
    ...
    expect(machine.getState()).toEqual({
      ...
      kind: 'head',
      error: conflict,
      draft: { ...request, expectedCurrentRevisionId: 'revision-1' },
  ...
  it('distinguishes a stale catalog and keeps the exact request for recovery', async () => {
    const conflict = new ApiError(
      409,
      'Catalog stale',
      { expectedCatalogRevision: 'catalog-1', currentCatalogRevision: 'catalog-2' },
      undefined,
    ...
    expect(machine.getState()).toEqual({
      ...
      error: conflict,
      draft: request,
      currentCatalogRevision: 'catalog-2',
    });
  });
  ...
  it('keeps a durable draft conflict distinct from a published head conflict', async () => {
    const conflict = new ApiError(
      409,
      'Draft stale',
      { expectedDraftVersion: 1, currentDraftVersion: 2 },
      undefined,
    ...
    expect(machine.getState()).toEqual({
      ...
      kind: 'draft',
      error: conflict,
      draft: { ...request, expectedDraftVersion: 1 },
    });
  });
});
