...
function conflictHead(error: ApiError): string | undefined {
  if (
    ...
    !('currentRevisionId' in error.details)
  )
    return undefined;
  return typeof error.details.currentRevisionId === 'string'
    ? error.details.currentRevisionId
    : undefined;
}
...
function conflictCatalog(error: ApiError): string | undefined {
  if (
    !error.details ||
    typeof error.details !== 'object' ||
    !('currentCatalogRevision' in error.details)
  )
    return undefined;
  return typeof error.details.currentCatalogRevision === 'string'
    ? error.details.currentCatalogRevision
    : undefined;
}
          ...
          if (error instanceof ApiError && error.status === 409) {
            if (error.code === 'CATALOG_REVISION_STALE') {
              transition({
                ...
                error,
                draft,
                ...(conflictCatalog(error)
                  ? { currentCatalogRevision: conflictCatalog(error) }
                  : {}),
              });
              ...
              transition({
                ...
                error,
                draft,
                ...(conflictHead(error) ? { currentRevisionId: conflictHead(error) } : {}),
              });
            }
          } else {
