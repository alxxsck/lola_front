...
export type TelegramPersonalLinkStatus = TelegramAdminLinkEffectiveStatus | 'UNKNOWN';
...
export function validateTelegramPersonalDraft(draft: TelegramPersonalDraft): string | null {
  ...
  if (text.length > MAX_TELEGRAM_CAPTION_LENGTH) return 'Подпись не должна превышать 1024 символа.';
  return null;
}

export function terminalTelegramPersonalStatus(status: TelegramPersonalMessageStatus): boolean {
  return ['SENT', 'FAILED_PERMANENT', 'OUTCOME_UNKNOWN', 'CANCELLED'].includes(status);
}

export function telegramPersonalStatusLabel(message: TelegramPersonalMessageResponseDto): string {
...
export type TelegramPersonalSafeErrorCode = keyof typeof TELEGRAM_PERSONAL_SAFE_ERROR_POLICY;

export function telegramPersonalSafeErrorPolicy(
  code: string | null | undefined,
): TelegramPersonalSafeErrorPolicy {
  if (code && Object.prototype.hasOwnProperty.call(TELEGRAM_PERSONAL_SAFE_ERROR_POLICY, code))
    return TELEGRAM_PERSONAL_SAFE_ERROR_POLICY[code as TelegramPersonalSafeErrorCode];
  return {
    staleLinkState: false,
    terminalLabel: GENERIC_TERMINAL_ERROR,
  };
...
export function telegramPersonalFailureLabel(code: string | null | undefined): string {
...
export function telegramPersonalCreateErrorLabel(
  ...
  return telegramPersonalSafeErrorPolicy(code).createLabel ?? GENERIC_CREATE_ERROR;
}
