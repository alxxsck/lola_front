...
describe('Telegram personal message controller', () => {
  ...
  it('retains the exact key and body only for an explicit transport retry', async () => {
    ...
    expect(await controller.send({ text: 'Подпись', file: attachment })).toBe(false);
  ...
  it('keeps one poll flight and never lets an older history row downgrade it', async () => {
    ...
    const api = {
      ...
      create: vi
        .fn()
        .mockResolvedValue(message('QUEUED', { updatedAt: '2026-07-24T09:00:00.000Z' })),
  ...
  it('uses bounded fallback polling for an invalid nextAttemptAt', async () => {
    const api = {
      list: vi.fn().mockResolvedValue({ items: [], nextCursor: null }),
      create: vi.fn().mockResolvedValue(message('RETRY_WAIT', { nextAttemptAt: 'not-a-date' })),
  ...
  it('stops the only poll and exposes a safe error when detail GET is forbidden', async () => {
    const api = {
      list: vi.fn(),
      create: vi.fn().mockResolvedValue(message('QUEUED')),
      get: vi.fn().mockRejectedValue(new ApiError(403, 'raw authorization details')),
    };
    const controller = createTelegramPersonalMessagesController({ api });
    controller.setContext({
      visible: true,
    ...
    expect(controller.error.value).toBe('Недостаточно прав для просмотра отправки.');
  ...
  it('signals that the link summary is stale when create rejects the link or channel', async () => {
    ...
    const api = {
      ...
      create: vi
        .fn()
        .mockRejectedValue(
          new ApiError(409, 'raw link details', undefined, undefined, 'TELEGRAM_LINK_NOT_ACTIVE'),
        ),
      get: vi.fn(),
    };
    const controller = createTelegramPersonalMessagesController({
    ...
    expect(controller.error.value).toBe('Пользователь больше не подключён к Telegram.');
  ...
  it('tracks two accepted messages while serializing their deferred status GETs', async () => {
    let resolveFirst!: (value: TelegramPersonalMessageDetailResponseDto) => void;
    let resolveSecond!: (value: TelegramPersonalMessageDetailResponseDto) => void;
    let concurrentGets = 0;
    let maximumConcurrentGets = 0;
    ...
    const api = {
      list: vi.fn().mockResolvedValue({ items: [], nextCursor: null }),
      create: vi.fn().mockResolvedValueOnce(first).mockResolvedValueOnce(second),
    ...
    await vi.advanceTimersByTimeAsync(500);
    expect(api.get).toHaveBeenCalledWith('project-1', 'end-user-1', 'message-a', expect.anything());
    ...
    expect(controller.activeMessage.value?.id).toBe('message-b');
    expect(controller.activeMessage.value?.status).toBe('SENT');
    expect(controller.history.value.find(({ id }) => id === 'message-a')?.status).toBe('SENT');
    expect(api.get).toHaveBeenCalledWith('project-1', 'end-user-1', 'message-b', expect.anything());
    expect(controller.polling.value).toBe(false);
  });

  it('recovers every non-terminal history item through one shared GET flight', async () => {
    const resolvers = new Map<string, (value: TelegramPersonalMessageDetailResponseDto) => void>();
    ...
    await flushPromises();
    expect(controller.history.value.map(({ status }) => status)).toEqual(['SENT', 'SENT']);
    expect(controller.polling.value).toBe(false);
  });
  ...
  it('uses attempt count and lifecycle precedence for equal updatedAt snapshots', async () => {
    ...
    await controller.loadHistory();
    await controller.loadHistory();
    expect(controller.history.value.find(({ id }) => id === 'retry')).toMatchObject({
      status: 'SENDING',
      attemptCount: 2,
    });
    expect(controller.history.value.find(({ id }) => id === 'terminal')?.status).toBe('SENT');
  ...
  it.each([
    ...
    async (code, expectedCopy) => {
      ...
      const api = {
        ...
        create: vi
          .fn()
          .mockRejectedValue(new ApiError(409, 'raw provider details', undefined, undefined, code)),
        get: vi.fn(),
      };
      const controller = createTelegramPersonalMessagesController({
        api,
