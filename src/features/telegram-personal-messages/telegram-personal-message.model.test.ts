...
describe('Telegram personal message policy', () => {
  ...
  it('accepts bounded text without an attachment', () => {
    ...
    expect(validateTelegramPersonalDraft({ text: '   ', file: null })).toContain('сообщение');
  });
  ...
  it('uses the shorter caption limit when media is attached', () => {
    ...
    expect(validateTelegramPersonalDraft({ text: '   ', file: attachment })).toContain('подпись');
  });
  ...
  it.each([
    ...
    ['TELEGRAM_CHANNEL_VERSION_CONFLICT', 'Настройки Telegram проекта изменились'],
    ['TELEGRAM_CHANNEL_WEBHOOK_GENERATION_STALE', 'Настройки Telegram проекта изменились'],
  ...
  it('uses one typed safe policy for lifecycle refresh and unknown errors', () => {
    ...
    for (const code of lifecycleCodes) {
      expect(TELEGRAM_PERSONAL_SAFE_ERROR_POLICY[code].staleLinkState).toBe(true);
      expect(TELEGRAM_PERSONAL_SAFE_ERROR_POLICY[code].createLabel).toBeTruthy();
      expect(TELEGRAM_PERSONAL_SAFE_ERROR_POLICY[code].terminalLabel).toBeTruthy();
    }
