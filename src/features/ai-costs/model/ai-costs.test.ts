...
describe("AI cost row sorting", () => {
  it("keeps unpriced rows last in both cost directions", () => {
    ...
    expect(
      sortAiCostRows([unpriced, priced], "effectiveCostUsd", "asc").at(-1)
        ?.endUserId,
    ).toBe("unpriced");
    expect(
      sortAiCostRows([unpriced, priced], "effectiveCostUsd", "desc").at(-1)
        ?.endUserId,
    ).toBe("unpriced");
  });
  it("sorts exact costs beyond Number precision without coercion", () => {
    ...
    expect(sortAiCostRows(rows, "effectiveCostUsd", "desc")[0]?.endUserId).toBe(
      "high",
    );
  });
});
