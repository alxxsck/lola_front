...
export type AiCostTab =
  "overview" | "users" | "employees" | "limits" | "journal";
export type AiCostPeriod = "today" | "7d" | "30d" | "custom";
export type AiCostSortKey =
  "identity" | "records" | "unpricedRecords" | "effectiveCostUsd";
...
export function parseAiCostRouteState(
  ...
  const customRange =
    requestedPeriod === "custom"
      ? customDateRange(customFrom, customTo, timezone)
      : undefined;
  ...
  const page =
    Number.isSafeInteger(rawPage) && rawPage >= 1 && rawPage <= 401
      ? rawPage
      : 1;
  return {
    tab,
...
export function aiCostRouteQuery(
  state: AiCostRouteState,
  ...
  return {
    ...
    ...(state.allowanceCursor
      ? { allowanceCursor: state.allowanceCursor }
      : {}),
  };
}
  ...
  return [...rows].sort((left, right) => {
    if (key === "effectiveCostUsd")
      if (left.effectiveCostUsd === null || right.effectiveCostUsd === null) {
        ...
        return (
          compareDecimalStrings(left.effectiveCostUsd, right.effectiveCostUsd) *
          multiplier
        );
    if (key === "identity")
      return identity(left).localeCompare(identity(right), "ru") * multiplier;
...
export function projectTimezone(settings: unknown): string {
  ...
  const activity =
    engine.activity &&
    typeof engine.activity === "object" &&
    !Array.isArray(engine.activity)
      ? (engine.activity as Record<string, unknown>)
      : {};
  const candidate =
    typeof activity.timezone === "string" ? activity.timezone : "UTC";
  try {
...
function enumValue<T extends string>(
  ...
  const candidate = single(value);
  return candidate && allowed.has(candidate as T)
    ? (candidate as T)
    : undefined;
}
