...
import { formatDecimalMoney, type DecimalString } from '@/shared/lib/decimal-money';
...
async function load(): Promise<void> {
  ...
  try {
    ...
    if (requestGeneration === generation)
      error.value = cause instanceof Error ? cause.message : 'Не удалось загрузить журнал';
  } finally {
...
async function select(): Promise<void> {
  ...
  error.value = '';
  try {
    const user = await endUserProfileRepository.resolveIdentity(props.projectId, value);
    if (!user) {
    ...
    error.value = cause instanceof Error ? cause.message : 'Не удалось найти пользователя';
  } finally {
    selectingUser.value = false;
  }
}
function openCorrection(item: AiAllowanceJournalEntry): void {
  if (!props.canReconcile || !balance.value || loadedContextKey.value !== contextKey.value) return;
...
async function submitCorrection(): Promise<void> {
  ...
  if (!target || !props.canReconcile) return correctionFail('Операция больше недоступна.');
  if (!exact || /^-?0(?:\.0+)?$/.test(exact))
    return correctionFail('Укажите ненулевое число, не более 12 знаков после запятой.');
  ...
  if (positive && (!expiresAt || new Date(expiresAt) <= new Date()))
    return correctionFail('Для положительной корректировки укажите будущий срок действия.');
  if (!/^(?:0|[1-9]\d{0,19})$/.test(correctionAccountVersion.value))
    return correctionFail('Версия баланса устарела или некорректна.');
  if (correctionReason.value.trim().length < 3 || correctionReason.value.trim().length > 500)
    return correctionFail('Причина должна содержать от 3 до 500 символов.');
  if (!correctionIdempotencyKey.value.trim() || correctionIdempotencyKey.value.length > 128)
...
function commandKey(): string {
  return globalThis.crypto?.randomUUID?.() ?? `allowance-correction-${Date.now()}`;
}
    ...
    <template v-else>
      <header>
        <div>
          ...
          <p>
            Здесь видно, когда лимит начислялся, резервировался и списывался. Суммы показаны с
            точностью, которую использует система.
          </p>
        </div>
      </header>
      <form v-if="!embedded" class="user-selector" @submit.prevent="select">
        ...
        <label v-else for="allowance-user-id"
          >ID пользователя в вашем продукте<input
            ...
            maxlength="160"
        /></label>
        <Button label="Открыть историю" type="submit" :loading="selectingUser" />
      </form>
      <Message v-if="canReconcile" severity="warn" :closable="false"
        >Корректировка — служебная операция для исключительных случаев. Она всегда привязывается к
        существующей записи и проверяет, что баланс не изменился с момента открытия.</Message
      >
      <Message v-if="notice" severity="success" :closable="false">{{ notice }}</Message>
      <Message v-if="error" severity="error" :closable="false"
        >{{ error }} <Button v-if="endUserId" label="Повторить" text size="small" @click="load"
      /></Message>
      <Message v-if="balanceError" severity="warn" :closable="false" role="alert"
        >{{ balanceError }} Корректировки отключены до успешного повторного чтения.
      ...
      <div v-else-if="!endUserId" class="journal-empty">
        ...
        <p>Найдите пользователя по внутреннему ID или откройте историю из таблицы пользователей.</p>
      </div>
      <footer v-if="visiblePage" class="journal-footer">
        ...
        ><Button
          ...
          icon-pos="right"
          outlined
          :disabled="!visiblePage.pageInfo.hasMore || !visiblePage.pageInfo.nextCursor"
  ...
  >
    <form class="correction-form" @submit.prevent="submitCorrection">
      <Message severity="warn" :closable="false">
        Новая запись исправит выбранное изменение лимита. Это не денежный перевод пользователю.
      </Message>
      <p v-if="correctionTarget">Выбрана запись от {{ date(correctionTarget.occurredAt) }}.</p>
      ...
      <label v-if="!correctionDelta.trim().startsWith('-')" for="correction-expiry">
        Срок действия добавленного лимита
        <input id="correction-expiry" v-model="correctionExpiresAt" type="datetime-local" />
      </label>
      ...
      <label for="correction-reason">
        ...
        <textarea id="correction-reason" v-model="correctionReason" rows="4" maxlength="500" />
      </label>
