...
function mountDashboard(options: ComponentMountingOptions<typeof AiCostsDashboard> = {}) {
...
describe('AiCostsDashboard', () => {
  ...
  it('loads a URL-selected users page and provides the existing profile drilldown', async () => {
    ...
    expect(wrapper.get('a[href="/users/user-1"]').text()).toContain('z-server-first');
  ...
  it('keeps the global tabs above the period filter', async () => {
    ...
    expect(
      tabs.compareDocumentPosition(periodFilter) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it('uses localized date controls without browser-dependent native placeholders', async () => {
    ...
    expect(periodFilter.findAll('input[placeholder="Выбрать даты"]')).toHaveLength(1);
    expect(periodFilter.get('.custom-period').attributes('aria-label')).toBe('Произвольный период');
  ...
  it('writes a selected calendar range to the URL as local calendar dates', async () => {
    ...
    await wrapper.getComponent(DatePicker).setValue([new Date(2026, 6, 3), new Date(2026, 6, 9)]);
  ...
  it('still leaves the protected workspace when remote logout fails', async () => {
    ...
    await wrapper.get('[data-testid="request-allowance-step-up"]').trigger('click');
    await flushPromises();
  ...
  it('returns a journal correction to the same journal and user context', async () => {
    ...
    mocks.route.fullPath = '/ai-costs?period=7d&tab=journal&allowanceUser=end-user-7';
    mocks.auth.project.effectivePermissionCodes = [
      'project.ai_costs.read',
      'project.ai_allowance.read',
    ...
    await wrapper.get('[data-testid="request-journal-step-up"]').trigger('click');
    await flushPromises();
  ...
  it('returns a user allowance mutation to the same user dialog context', async () => {
    ...
    mocks.route.fullPath = '/ai-costs?period=7d&tab=users&allowanceUser=id-user-dialog';
    mocks.auth.project.effectivePermissionCodes = [
      'project.ai_costs.read',
      'project.profiles.read',
      'project.ai_allowance.read',
  ...
  it('closes an open allowance balance when its read permission is revoked', async () => {
    ...
    const wrapper = mountDashboard({
      global: {
        stubs: {
          AiAllowanceUserDialog: {
            props: ['endUserId'],
            template: '<div data-testid="allowance-user-dialog">{{ endUserId }}</div>',
          },
        },
      },
    });
    ...
    await wrapper
      ...
      .trigger('click');
    expect(wrapper.get('[data-testid="allowance-user-dialog"]').text()).toBe('id-allowance-user');
    ...
    expect(wrapper.find('[data-testid="allowance-user-dialog"]').exists()).toBe(false);
  });
