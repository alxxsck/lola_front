...
describe('aiAllowanceRepository', () => {
  ...
  it('accepts an unconfigured project policy at version zero', async () => {
    vi.mocked(axiosInstance.get).mockResolvedValue({
      data: {
        ...
        localization: {
          defaultLocale: 'ru',
          supportedLocales: ['ru', 'en', 'it', 'de', 'es', 'pt', 'pl', 'bn', 'sw', 'so', 'ar'],
    ...
    await expect(aiAllowanceRepository.projectPolicy('project-1')).resolves.toEqual(
  ...
  it('validates the project policy graph and preserves exact money', async () => {
    ...
    expect(result.plans[0]?.revisions[0]?.recurringAmountUsd).toBe('5.000000000001');
    ...
    expect(axiosInstance.get).toHaveBeenCalledWith('/api/v1/admin/projects/project-1/ai-allowance');
  });
  ...
  it.each([
    ...
    async (lowThresholdMode, lowThresholdValue) => {
      ...
      await expect(aiAllowanceRepository.projectPolicy('project-1')).rejects.toThrow(
        'некорректные данные',
      );
    },
  );
  ...
  it('loads exact balance and cursor journal entries with signed deltas', async () => {
    ...
    const balance = await aiAllowanceRepository.endUserBalance('project-1', 'user-1');
  ...
  it('accepts assignment plan summaries and the pinned period revision with category rules', async () => {
    ...
    const result = await aiAllowanceRepository.endUserBalance('project-1', 'user-1');
  ...
  it('sends exact mutation payloads with a stable Idempotency-Key', async () => {
    ...
    await aiAllowanceRepository.createGrant('project-1', 'user-1', payload, 'grant-key-1');
  ...
  it('uses the published PUT paths for default-plan revisions and user assignments', async () => {
    ...
    await aiAllowanceRepository.putDefaultPlan('project-1', defaultPlan, 'default-key');
