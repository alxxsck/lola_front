...
export interface AiCostsRepository {
  ...
  users(projectId: string, query: AiCostPageQuery): Promise<AiCostPage<AiCostUserRow>>;
  cmsUsers(projectId: string, query: AiCostPageQuery): Promise<AiCostPage<AiCostCmsUserRow>>;
}

const apiRepository: AiCostsRepository = {
  async overview(projectId, range) {
    const response = await axiosInstance.get<unknown>(endpoint(projectId, 'overview'), {
      params: range,
    });
    return validOverview(response.data);
  },
  async users(projectId, query) {
    const response = await axiosInstance.get<unknown>(endpoint(projectId, 'users'), {
      params: query,
    });
    return validPage(response.data, parseUser);
  },
  async cmsUsers(projectId, query) {
    const response = await axiosInstance.get<unknown>(endpoint(projectId, 'cms-users'), {
      params: query,
    });
...
export const aiCostsRepository: AiCostsRepository = isMockMode ? mockRepository : apiRepository;
...
function validOverview(value: unknown): AiCostOverview {
  ...
  const providerReportedRecords = integer(completeness?.providerReportedRecords);
  ...
  if (
    ...
    providerReportedRecords + estimatedRecords + unpricedRecords !== totalRecords ||
  ...
  if (categories.some((item) => !item) || daily.some((item) => !item)) invalid();
  return {
    range,
...
function validPage<T extends AiCostRankedRow>(
  ...
  const nextOffset = pagination?.nextOffset === null ? null : integer(pagination?.nextOffset);
  if (
    !source ||
    !range ||
    !projection ||
...
function parseCategory(value: unknown): AiCostOverview['categories'][number] | undefined {
  const source = record(value);
  const costs = parseMoney(source);
  const records = integer(source?.records);
  return source && costs && bounded(source.category, 1, 80) && records !== undefined
    ? { category: source.category, records, ...costs }
    : undefined;
}

function parseDaily(value: unknown): AiCostOverview['daily'][number] | undefined {
  const source = record(value);
  const costs = parseMoney(source);
  const records = integer(source?.records);
...
function parseCmsUser(value: unknown): AiCostCmsUserRow | undefined {
  const source = record(value);
  const base = parseRanked(source);
  return source && base && bounded(source.cmsUserId, 1, 160) && bounded(source.email, 3, 320)
    ? { cmsUserId: source.cmsUserId, email: source.email, ...base }
    : undefined;
}

function parseRanked(source: Record<string, unknown> | undefined): AiCostRankedRow | undefined {
  const costs = parseMoney(source);
  const records = integer(source?.records);
...
function parseMoney(value: unknown): AiCostMoney | undefined {
  ...
  const effectiveCostUsd =
    source.effectiveCostUsd === null ? null : parseDecimalString(source.effectiveCostUsd);
  ...
  return providerReportedCostUsd !== undefined &&
    ...
    ((parts.length === 0 && effectiveCostUsd === null) ||
      (parts.length > 0 &&
        ...
        compareDecimalStrings(addDecimalStrings(parts), effectiveCostUsd) === 0))
    ? {
...
function parseProjection(value: unknown): AiCostProjection | undefined {
  ...
  const status = s?.status === 'FRESH' || s?.status === 'STALE' ? s.status : undefined;
...
function parseRange(value: unknown): AiCostRange | undefined {
  const source = record(value);
  if (!source || !iso(source.from) || !iso(source.to) || source.from >= source.to) return undefined;
