...
describe('notification destinations API', () => {
  beforeEach(() => {
    ...
    vi.mocked(notificationDestinationCreateTelegram).mockResolvedValue({} as never);
    vi.mocked(notificationDestinationCreateTelegramBindingChallenge).mockResolvedValue({} as never);
    vi.mocked(notificationDestinationUpdateTelegram).mockResolvedValue({} as never);
    vi.mocked(notificationDestinationTestTelegram).mockResolvedValue({} as never);
  });
  ...
  it('uses generated project-scoped contracts and fresh idempotency keys for side effects', async () => {
    ...
    expect(notificationDestinationUpdate).toHaveBeenCalledWith('project-1', 'destination-1', {
      expectedVersion: 3,
      desiredStatus: 'ACTIVE',
    });
  ...
  it('keeps operational Telegram on its dedicated generated routes', async () => {
    ...
    expect(notificationDestinationCreateTelegramBindingChallenge).toHaveBeenCalledWith(
      'project-1',
      'destination-telegram',
      {
        expectedVersion: 2,
