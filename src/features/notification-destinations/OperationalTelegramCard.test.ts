...
describe('OperationalTelegramCard', () => {
  ...
  it('uses the shared card and field contract for the empty setup', async () => {
    ...
    expect(wrapper.get('[data-integration="telegram-team"]').classes()).toContain(
      'integration-card',
    );
    expect(wrapper.text()).toContain('Telegram для команды');
    expect(wrapper.findAll('form[data-form="create-telegram"] .integration-field')).toHaveLength(2);
    expect(
      wrapper.get('input[name="telegramDisplayName"]').element.closest('label'),
    ).not.toBeNull();
    expect(wrapper.get('input[name="telegramBotToken"]').element.closest('label')).not.toBeNull();
  });
  ...
  it('validates a write-only token and shows a one-time chat binding command', async () => {
    ...
    mocks.list.mockResolvedValueOnce({ items: [] }).mockResolvedValueOnce({ items: [created] });
    const wrapper = mount(OperationalTelegramCard, {
      props: { projectId: 'project-1', canRead: true, canManage: true },
    });
    await flushPromises();
    ...
    await wrapper.get('input[name="telegramDisplayName"]').setValue('Operations');
    ...
    await wrapper.get('button[data-action="telegram-new-challenge"]').trigger('click');
    await flushPromises();
  ...
  it('waits for durable webhook setup before offering a binding challenge', async () => {
    ...
    expect(wrapper.find('button[data-action="telegram-new-challenge"]').exists()).toBe(false);
    await wrapper.get('button[data-action="telegram-refresh-binding"]').trigger('click');
    await flushPromises();

    expect(wrapper.find('button[data-action="telegram-new-challenge"]').exists()).toBe(true);
    expect(mocks.createTelegramBindingChallenge).not.toHaveBeenCalled();
  });
  ...
  it('clears an old challenge and waits for webhook setup after token rotation', async () => {
    ...
    await wrapper.get('input[name="telegramBotToken"]').setValue(replacementToken);
    await wrapper.get('form[data-form="rotate-telegram"]').trigger('submit');
    await flushPromises();

    expect(mocks.updateOperationalTelegram).toHaveBeenCalledWith('project-1', current.id, {
  ...
  it('tests a bound chat with one durable key and activates only the tested route', async () => {
    ...
    mocks.list.mockResolvedValueOnce({ items: [bound] }).mockResolvedValueOnce({ items: [tested] });
    ...
    await wrapper.get('button[data-action="telegram-activate"]').trigger('click');
    await flushPromises();
    expect(mocks.updateOperationalTelegram).toHaveBeenCalledWith('project-1', bound.id, {
      expectedVersion: 3,
  ...
  it('keeps token controls hidden for a read-only operator', async () => {
    ...
    expect(wrapper.find('button[data-action="telegram-disable"]').exists()).toBe(false);
  });
  ...
  it('shows safe operational identity, failure and update metadata', async () => {
    ...
    expect(wrapper.get('[data-field="telegram-bot-id"]').text()).toContain('10001');
    expect(wrapper.get('[data-field="telegram-last-failure"]').text()).toContain(
      'Чат недоступен или бот заблокирован',
    );
    expect(wrapper.get('[data-field="telegram-updated-by"]').text()).toContain(
      'Администратор · operator-42',
    );
    expect(wrapper.get('[data-field="telegram-updated-at"]').text()).not.toContain('—');
  ...
  it('drops a late response after the selected Project changes', async () => {
    let resolveList!: (value: { items: ReturnType<typeof telegramDestination>[] }) => void;
  ...
  it('drops a late binding challenge after the selected Project changes', async () => {
    ...
    mocks.list.mockResolvedValueOnce({ items: [current] }).mockResolvedValueOnce({ items: [] });
  ...
  it('drops a late destination update after the selected Project changes', async () => {
    ...
    mocks.list.mockResolvedValueOnce({ items: [active] }).mockResolvedValueOnce({ items: [] });
    ...
    await wrapper.get('button[data-action="telegram-disable"]').trigger('click');
    await wrapper.setProps({ projectId: 'project-2' });
    resolveUpdate(
      telegramDestination({
        status: 'DISABLED',
    ...
    expect(wrapper.text()).not.toContain('Служебные Telegram-уведомления отключены.');
  });
});
