...
export const emailIdentityApi = {
  async consume(action: EmailIdentityActionKind, token: string): Promise<EmailActionConsumeResult> {
    return (
      await publicEmailActionHttp.post<EmailActionConsumeResult>(consumePath[action], { token })
    ).data;
  },

  async requestVerification(): Promise<EmailVerificationRequested> {
    return (
      await axiosInstance.post<EmailVerificationRequested>('/api/v1/auth/me/email-verification')
    ).data;
  },

  async requestEmailChange(input: EmailChangeRequest): Promise<EmailChangeRequested> {
    return (await axiosInstance.post<EmailChangeRequested>('/api/v1/auth/me/email-change', input))
      .data;
  },
