...
describe('email identity API', () => {
  ...
  it.each([
    [
      'initial-access',
      '/api/v1/auth/email-invitations/consume',
      {
        kind: 'PASSWORD_SETUP_REQUIRED',
        setupToken: 'lps_setup',
    ...
    'consumes %s only through an explicit public POST body',
    async (action, path, response) => {
  ...
  it('requests verification and a password-proved two-phase email change', async () => {
    ...
    await expect(
      emailIdentityApi.requestEmailChange({
