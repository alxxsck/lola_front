...
function mapPreview(broadcastId: string, value: TelegramBroadcastPreviewResponseDto) {
  return {
    broadcastId,
    version: value.version,
...
export function mapTelegramBroadcastSafeFailureCategory(
  ...
  return PUBLIC_OUTCOME_CATEGORY[errorCode as keyof typeof PUBLIC_OUTCOME_CATEGORY] ?? null;
}

function mapOutcome(value: TelegramBroadcastOutcomeDto): TelegramBroadcastDelivery {
  return {
    id: value.id,
    status: value.status,
    safeFailureCategory: mapTelegramBroadcastSafeFailureCategory(value.errorCode),
    createdAt: value.createdAt,
...
export const telegramBroadcastsApi: TelegramBroadcastsApi = {
  ...
  async get(projectId, broadcastId, options) {
    return mapTelegramBroadcastDetail(await telegramBroadcastGet(projectId, broadcastId, options));
  },
  ...
  async listDeliveries(projectId, broadcastId, query, options) {
    const response = await telegramBroadcastOutcomes(projectId, broadcastId, query, options);
    return {
