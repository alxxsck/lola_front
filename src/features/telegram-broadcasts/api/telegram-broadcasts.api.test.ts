...
describe('telegram broadcasts generated adapter', () => {
  ...
  it('maps paginated outcomes without recipient identity or raw errors', async () => {
    ...
    expect(JSON.stringify(page.items)).not.toMatch(/endUser|telegramUser|chatId|raw/i);
  });
  ...
  it('maps every public backend outcome code to a stable safe category', () => {
    ...
    expect(
      ['PROVIDER_BLOCKED', 'TELEGRAM_USER_BLOCKED'].map(mapTelegramBroadcastSafeFailureCategory),
    ).toEqual(['RECIPIENT_UNAVAILABLE', 'RECIPIENT_UNAVAILABLE']);
    expect(
      ['TELEGRAM_BOT_TOKEN_INVALID', 'TELEGRAM_BROADCAST_INSTALLATION_STALE'].map(
        mapTelegramBroadcastSafeFailureCategory,
      ),
    ).toEqual(['INSTALLATION_UNAVAILABLE', 'INSTALLATION_UNAVAILABLE']);
    expect(
      ['TELEGRAM_PRODUCT_LOCAL_RATE_LIMIT', 'TELEGRAM_RETRY_AFTER_UNSUPPORTED'].map(
        mapTelegramBroadcastSafeFailureCategory,
      ),
    ).toEqual(['RATE_LIMIT_EXHAUSTED', 'RATE_LIMIT_EXHAUSTED']);
    expect(mapTelegramBroadcastSafeFailureCategory('TELEGRAM_BROADCAST_LINK_STALE')).toBe(
      'LINK_NOT_ACTIVE',
    );
    expect(mapTelegramBroadcastSafeFailureCategory('TELEGRAM_BROADCAST_INTERNAL_FAILURE')).toBe(
      'INTERNAL_FAILURE',
    );
    expect(
    ...
    expect(mapTelegramBroadcastSafeFailureCategory('RAW_PROVIDER_SECRET')).toBe(null);
  });
});
