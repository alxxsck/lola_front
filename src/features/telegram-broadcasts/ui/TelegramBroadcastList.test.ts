...
import type { TelegramBroadcast, TelegramBroadcastPermissions } from '../model/telegram-broadcast';
...
describe('TelegramBroadcastList', () => {
  it('renders textual lifecycle, consent-safe audience and opens an item', async () => {
    ...
    await wrapper.get('button[aria-label="Открыть рассылку Июльское обновление"]').trigger('click');
  ...
  it('gates create by draft capability and announces loading', () => {
    ...
    expect(readonly.get('[role="status"]').attributes('aria-live')).toBe('polite');
  ...
  it('uses snapshot-implying status and recipientCount for an approved summary', () => {
    ...
    expect(wrapper.text()).not.toContain('Снимок аудитории ещё не зафиксирован');
  });
