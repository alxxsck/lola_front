...
describe('TelegramBroadcastPreview', () => {
  it('collects an explicit external ID and bounded label accessibly', async () => {
    ...
    expect(externalId.attributes('aria-describedby')).toBe('broadcast-test-external-id-hint');
    expect(label.attributes('maxlength')).toBe('80');
    expect(label.attributes('aria-describedby')).toBe('broadcast-test-label-hint');
