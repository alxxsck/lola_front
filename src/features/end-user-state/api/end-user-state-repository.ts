...
export const endUserStateRepository = {
  async get(projectId: string, endUserId: string): Promise<EndUserOperationalState> {
    return validateState((await axiosInstance.get(root(projectId, endUserId))).data);
  },
  async history(
    projectId: string,
    endUserId: string,
  ...
  async put(
    ...
    return (
      await axiosInstance.put(`${root(projectId, endUserId)}/${encodeURIComponent(key)}`, input, {
...
function validateState(value: unknown): EndUserOperationalState {
  ...
  if (!s || !text(s.projectId) || !text(s.endUserId) || !Array.isArray(s.items)) invalid();
  const items = s.items.map((item) => {
    const x = object(item);
    const definition = parseDefinition(x?.definition);
    const current = x?.current === null ? null : parseCurrent(x?.current);
    return definition && (current || x?.current === null) ? { definition, current } : null;
  });
...
function localizedText(value: unknown): value is Record<string, string> {
  ...
  return Boolean(
    ...
    Object.values(source).every((item) => typeof item === 'string' && item.length <= 2000),
  );
}
...
function invalid(): never {
  throw new Error('Сервер вернул некорректное внутреннее состояние пользователя');
}
