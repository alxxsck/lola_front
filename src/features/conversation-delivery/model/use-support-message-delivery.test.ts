...
describe('support message delivery controller', () => {
  it('retries only the exact server-authorized failed generation', async () => {
    ...
    expect(retryFailedDelivery).toHaveBeenCalledWith('project-1', 'end-user-1', 'message-1', {
      expectedGeneration: 3,
      expectedVersion: 9,
      idempotencyKey: expect.any(String),
    });
    expect(reconcile).toHaveBeenCalledTimes(1);
    expect(applyDeliveryReceipt).toHaveBeenCalledWith('message-1', retriedDelivery());
  ...
  it('applies the authoritative current receipt from a typed stale conflict', async () => {
    ...
    expect(applyDeliveryReceipt).toHaveBeenCalledWith(message.id, retriedDelivery());
    expect(reconcile).toHaveBeenCalledTimes(1);
    expect(controller.deliveryActions.value.get(message.id)?.error).toBe(undefined);
  });

  it('invalidates an in-flight retry before reset permits another operation', async () => {
    let rejectFirst!: (cause: unknown) => void;
    let resolveSecond!: (delivery: NonNullable<ConversationMessage['delivery']>) => void;
