...
export interface ProjectAIAnalysesRepository {
  ...
  detail(projectId: string, analysisId: string): Promise<ProjectAIAnalysisDetailResponseDto>;
  cancel(command: CancelProjectAIAnalysisCommand): Promise<ProjectAIAnalysisReceiptDto>;
}
...
const mockRepository: ProjectAIAnalysesRepository = {
  async list(_projectId, params) {
    const items = mockItems.filter((item) => {
      ...
      if (params?.status && item.latestRun?.status !== params.status) return false;
      if (params?.scopeKind && item.scopeKind !== params.scopeKind) return false;
      if (params?.kind && item.kind !== params.kind) return false;
      if (params?.eventCode && !item.eventCodes.some((code) => code.includes(params.eventCode!)))
        return false;
      return true;
    });
  ...
  async detail(_projectId, analysisId) {
    const item =
      mockItems.find((candidate) => candidate.analysisId === analysisId) ?? mockItems[0]!;
        ...
        ? [
            {
              ...
              receipts: [
                {
                  ...
                  rangeStartedAt: new Date(now.getTime() - 24 * 60 * 60 * 1_000).toISOString(),
                  resultRows: 3,
              ...
              result: {
                title: item.title,
                answer: 'Вчера депозит завершили 12 пользователей. Наибольшая доля пришлась на ES.',
                table: {
                  columns: [
                    { key: 'geo', label: 'GEO' },
                ...
                definitions: [
                  {
                    ...
                    description: 'Событие приходит после успешного завершения депозита.',
                  },
                  {
                    kind: 'EVENT_FIELD',
                    code: 'geo',
  ...
  async cancel({ analysisId }: CancelProjectAIAnalysisCommand) {
    const item = mockItems.find((candidate) => candidate.analysisId === analysisId);
    if (item) {
...
export const projectAIAnalysesRepository = isMockMode ? mockRepository : apiRepository;
