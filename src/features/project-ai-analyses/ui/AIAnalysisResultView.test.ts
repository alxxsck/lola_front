...
describe('AIAnalysisResultView', () => {
  it('shows the clarification question and published candidates', () => {
    ...
    expect(wrapper.text()).toContain('Какое опубликованное событие использовать?');
  ...
  it('renders a validated answer, table and limitations', () => {
    const wrapper = mount(AIAnalysisResultView, {
      props: {
        result: {
          ...
          limitations: [{ code: 'RAW_LIMIT_CODE', message: 'Часть данных исключена' }],
        },
        canReadCost: false,
      },
    });
    ...
    expect(wrapper.find('.result-technical').attributes('open')).toBeUndefined();
