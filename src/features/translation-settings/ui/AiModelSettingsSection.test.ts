...
describe('AI model settings', () => {
  beforeEach(() => {
    ...
    mocks.catalog.mockImplementation((_projectId: string, { workload }: { workload: string }) =>
      Promise.resolve({
        workload,
        stale: false,
  ...
  it('сохраняет assistant и translation одним versioned запросом', async () => {
    ...
    await wrapper.get('[data-testid="save-ai-model-settings"]').trigger('click');
    await flushPromises();
  ...
  it('не предлагает none для модели с обязательным reasoning', async () => {
    ...
    expect(effortOptions.flat()).not.toContainEqual(expect.objectContaining({ value: 'none' }));
