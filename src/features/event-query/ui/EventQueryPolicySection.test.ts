    ...
    global: {
      stubs: {
        ...
        ProjectSettingsSectionHeader: {
          template: '<div><h2>Доступ AI к событиям</h2><slot name="actions" /></div>',
        },
        ToggleSwitch: {
...
describe('EventQueryPolicySection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(eventQueryRepository.getPolicy).mockResolvedValue(structuredClone(state));
  ...
  it('applies the Project master setting behind one action', async () => {
    ...
    expect(eventQueryRepository.applyProject).toHaveBeenCalledWith('project-1', {
      concurrencyToken: 'eq-project-v1.initial',
      masterEnabled: false,
    });
  ...
  it('reloads isolated state when the active Project changes', async () => {
    ...
    expect(eventQueryRepository.getPolicy).toHaveBeenLastCalledWith('project-2');
    expect(
      (wrapper.get('.master-control input[type="checkbox"]').element as HTMLInputElement).checked,
    ).toBe(false);
  });
  ...
  it('keeps the local choice and retries with the replacement token after 409', async () => {
    ...
    expect(eventQueryRepository.applyProject).toHaveBeenLastCalledWith('project-1', {
      concurrencyToken: 'eq-project-v1.current',
      masterEnabled: false,
    });
  });
