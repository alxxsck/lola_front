...
describe('EventQueryPreview', () => {
  ...
  it('builds a valid aggregate query and caps periods by policy', async () => {
    ...
    await flushPromises();
    wrapper.getComponent(EventPicker).vm.$emit('update:modelValue', 'deposit.completed');
    await wrapper.vm.$nextTick();
    await wrapper.get('[data-test="preview-mode"]').setValue('AGGREGATE');

    expect(wrapper.get('[data-test="preview-period"]').text()).not.toContain('30 дней');
    ...
    expect(endUserProfileRepository.resolveIdentity).toHaveBeenCalledWith('project-1', 'player-42');
  ...
  it('does not execute preview when the user identity cannot be resolved', async () => {
    vi.mocked(endUserProfileRepository.resolveIdentity).mockResolvedValueOnce(null);
    const wrapper = mountPreview();
    await flushPromises();
    wrapper.getComponent(EventPicker).vm.$emit('update:modelValue', 'game.started');
    await wrapper.vm.$nextTick();
    await wrapper.get('[data-test="preview-end-user"]').setValue('unknown-player');
  ...
  it('normalizes mode and lookback when the selected event changes', async () => {
    ...
    expect((wrapper.get('[data-test="preview-mode"]').element as HTMLSelectElement).value).toBe(
      'SUMMARY',
    );
    expect(wrapper.get('[data-test="preview-period"]').text()).not.toContain('7 дней');
  });
