...
describe('EventQueryEventAccess', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(eventQueryRepository.getItem).mockResolvedValue(structuredClone(state));
  ...
  it('applies only the current Event with both independent grants', async () => {
    ...
    await wrapper.get('[data-test="event-query-conversation-enabled"]').setValue(true);
  ...
  it('preserves the conversation preference when base access is disabled', async () => {
    ...
    expect(
      wrapper.get('[data-test="event-query-conversation-enabled"]').attributes('disabled'),
    ).toBeDefined();
    await wrapper.get('button[data-test="apply-event-query"]').trigger('click');
    await flushPromises();
  ...
  it('prepares EVENT_COUNT analysis when an Event has no recommended fields', async () => {
    ...
    await wrapper.get('button[data-test="prepare-recommended-safe-fields"]').trigger('click');
    await wrapper.get('button[data-test="apply-event-query"]').trigger('click');
    await flushPromises();
  ...
  it('keeps the Event form and retries with the replacement token after 409', async () => {
    ...
    expect(
      (wrapper.get('[data-test="event-query-conversation-enabled"]').element as HTMLInputElement)
        .checked,
    ).toBe(true);
