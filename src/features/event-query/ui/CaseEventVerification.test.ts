...
function mountComponent(
  extraProps: Partial<
    InstanceType<typeof CaseEventVerification>["$props"]
  > = {},
) {
...
describe("CaseEventVerification", () => {
  ...
  it("estimates a server-scoped query, shows bounded evidence and starts once with a fresh key", async () => {
    ...
    await wrapper
      .get('button[data-test="start-case-verification"]')
      .trigger("click");
    await flushPromises();
  ...
  it("ignores a late verification result after switching to another Case", async () => {
    let resolveStart!: (
      value: Awaited<
        ReturnType<typeof eventQueryRepository.startCaseVerification>
      >,
    ) => void;
