...
export function eventQueryPeriodOptions(input: {
  ...
  const createdAt = input.caseCreatedAt
    ? new Date(input.caseCreatedAt).getTime()
    : Number.NaN;
  ...
  ] satisfies EventQueryPeriodOption[]).filter(
    (option) => option.hours <= input.maxHours,
  );
...
export function eventQueryTimeRange(
  value: EventQueryRange,
  maxHours: number,
  now = new Date(),
) {
