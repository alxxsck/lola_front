...
import {
  eventPolicyConflictState,
  projectPolicyConflictState,
} from "./event-query-conflict";
...
describe("event query concurrency conflict", () => {
  it("extracts the current Project state from a 409 response", () => {
    ...
    expect(
      projectPolicyConflictState(new ApiError(409, "Conflict", { current })),
    ).toEqual(current);
  });
  ...
  it("extracts the current Event state from a 409 response", () => {
    ...
    expect(
      eventPolicyConflictState(new ApiError(409, "Conflict", { current })),
    ).toEqual(current);
  });
});
