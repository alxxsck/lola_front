...
function render(
  props: Partial<InstanceType<typeof SupportConversationContext>['$props']> = {},
  current = selection,
) {
...
describe('support conversation inspector', () => {
  ...
  it('shows assignment context even when the role has no assignment actions', () => {
    ...
    expect(wrapper.get('[data-assignment-state]').text()).toContain('Оператор Алина · Billing');
  ...
  it('places the assignment surface before SLA and routing', () => {
    ...
    const operationsContext = wrapper.get('support-case-operations-context-stub').element;

    expect(
      assignmentDesk.compareDocumentPosition(operationsContext) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it('renders the five permission-aware tabs with Case as the default', () => {
    ...
    expect(wrapper.get('[aria-label="Обращение"]').text()).toContain('Проверить возврат бонусов');
  });
  ...
  it('keeps conversation-only selections on the User tab', () => {
    ...
    expect(wrapper.findAll('[role="tab"]').map((tab) => tab.text())).not.toContain('Обращение');
    expect(wrapper.get('[role="tab"][aria-selected="true"]').text()).toBe('Пользователь');
  });
