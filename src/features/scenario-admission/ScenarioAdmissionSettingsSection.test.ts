...
describe('ScenarioAdmissionSettingsSection', () => {
  ...
  it('keeps the draft and opens acknowledgement on legacy cutover conflict', async () => {
    ...
    expect((wrapper.vm as unknown as { impactedScenarios: number }).impactedScenarios).toBe(4);
    expect((wrapper.vm as unknown as { acknowledgeOpen: boolean }).acknowledgeOpen).toBe(true);
