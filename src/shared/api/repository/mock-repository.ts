...
import type { ConversationAISuspensionMutationResult, ReteniveRepository } from './contracts';
...
function restoreMessageOrdinals(messages: ConversationMessage[]): ConversationMessage[] {
...
const readDemo = (): DemoData => {
  ...
  try {
    ...
    return {
      ...
      conversations: (data.conversations ?? initialData().conversations).map((conversation) => {
        const seeded = demoConversations.find((item) => item.id === conversation.id);
        return {
          ...conversation,
...
export async function retryMockAdminMessageDelivery(
  ...
  if (!message || !delivery) throw new ApiError(404, 'Состояние доставки недоступно');
  if (
  ...
  if (!delivery.retryEligible || !delivery.allowedActions.includes('RETRY_FAILED_DELIVERY'))
    ...
    definitions
      ...
      .map((item) => {
        ...
        return [
          ...
          {
            type:
              item.type === 'NUMBER' ? 'number' : item.type === 'BOOLEAN' ? 'boolean' : 'string',
            ...(item.type === 'DATETIME'
              ? {
                  format: 'date-time',
                  pattern:
...
export const mockRepository: ReteniveRepository = {
  ...
  async getEventDefinitionRevisions(_projectId, definitionKeyId, request) {
    ...
    const revisions = readDemo()
      .events.filter((item) => (item.definitionKeyId ?? item.id) === definitionKeyId)
    ...
    return {
      ...
      nextCursor: offset + limit < revisions.length ? String(offset + limit) : null,
    };
  },
  ...
  async getConversations(_projectId, userId, request) {
    ...
    return {
      items: items.slice(offset, offset + limit),
      nextCursor: items[offset + limit]?.id ? items[offset + limit - 1]!.id : null,
    };
  },
  ...
  async startConversationAISuspension(
    ...
    if (current.mode === 'SUSPENDED' && current.lifecycle === 'ACTIVE')
      throw new ApiError(409, 'AI уже приостановлен', undefined, undefined, 'ALREADY_ACTIVE');
    const now = new Date();
    const state: ConversationAISuspensionDetail = {
      mode: 'SUSPENDED',
      lifecycle: 'ACTIVE',
      version: (BigInt(current.version) + 1n).toString(),
      suspendedUntil: new Date(now.getTime() + command.durationSeconds * 1000).toISOString(),
  ...
  async extendConversationAISuspension(
    ...
    const conversation = findConversation(data, endUserId, conversationId);
    const current = suspensionDetail(data, conversation);
    if (current.mode !== 'SUSPENDED' || current.lifecycle !== 'ACTIVE' || !current.suspendedUntil)
      throw new ApiError(409, 'AI уже отвечает', undefined, undefined, 'NOT_ACTIVE');
  ...
  async getEventLogPage(_projectId, filters) {
    ...
    const logs = data.activity
      ...
      .map((item, index): EventLog => ({
        ...
        eventDefinitionId: data.events.find((event) => event.code === item.title)?.id ?? 'evt_1',
        eventDefinitionKeyId:
          data.events.find((event) => event.code === item.title)?.definitionKeyId ??
        ...
        userId: item.userId,
        userExternalId:
          data.users.find((user) => user.id === item.userId)?.externalId ?? item.userId,
  ...
  async getEventLog(projectId, eventId) {
    const item = (await this.getEventLogPage(projectId, { limit: 100 })).items.find(
  ...
  async getScenarioRuns() {
    ...
    return data.activity
      ...
      .map((item): ScenarioRun => {
        const scenario =
          data.scenarios.find((value) => value.name === item.title) ?? data.scenarios[0]!;
  ...
  async getScenarioRunsPage(projectId, request) {
    ...
    const items = requestedDefinitionKeyId
      ? runs.filter((run) => {
          const scenario = data?.scenarios.find((item) => item.id === run.scenarioId);
  ...
  async getProductApiRequestLogsPage(_projectId, request) {
    ...
    const filtered = items.filter(
      (item) =>
        ...
        (!request?.externalUserId || item.externalUserId === request.externalUserId) &&
        (!request?.outcome || item.outcome === request.outcome),
    );
    const offset = request?.cursor ? Number(request.cursor) : 0;
    const limit = request?.limit ?? 25;
  ...
  async lookupAdminMessageOutcome(_projectId, userId, idempotencyKey) {
    ...
    if (payload.userId !== userId) throw new ApiError(404, 'Результат отправки не найден');
    await pause();
