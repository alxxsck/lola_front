...
export const mockRepository: LolaRepository = {
  ...
  async getScenarioRunsPage(projectId, request) {
    const allItems = await this.getScenarioRuns(projectId);
    const data = readDemo();
    const items = request?.eventDefinitionKeyId
      ? allItems.filter(
          (run) =>
            ...
            (() => {
              ...
              const definition = data.events.find(
                (item) => item.id === scenario?.eventDefinitionId,
              );
              return (
                (definition?.definitionKeyId ?? definition?.id) ===
                request.eventDefinitionKeyId
              );
            })(),
        )
      : allItems;
