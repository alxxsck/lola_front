  ...
  if (source.nullable === true && Array.isArray(source.oneOf)) {
    ...
    const rest = Object.fromEntries(
      Object.entries(source).filter(
        ([key]) => !['nullable', 'oneOf', 'discriminator', 'additionalProperties'].includes(key),
      ),
    );
    return {
      ...(normalizeNullableUnions(rest) as Record<string, unknown>),
      anyOf: [...oneOf.map(normalizeNullableUnions), { type: 'null' }],
    };
  }
  return Object.fromEntries(
    Object.entries(source).map(([key, entry]) => [key, normalizeNullableUnions(entry)]),
  );
}
...
describe('support inbox, Case and workforce contract fixtures', () => {
  it('validates every published fixture against the pinned OpenAPI schema', async () => {
    ...
    const normalizedComponents = normalizeNullableUnions(contract.components);
  ...
  it('keeps unavailable error, partial and recovery scenarios visibly unpublished', () => {
    expect(supportInboxCaseWorkforceContractFixtures.forbiddenInbox).toMatchObject({
      publication: 'NOT_PUBLISHED',
      status: 403,
    });
    expect(supportInboxCaseWorkforceContractFixtures.staleCaseWorkflow).toMatchObject({
      publication: 'NOT_PUBLISHED',
      status: 409,
    });
    expect(supportInboxCaseWorkforceContractFixtures.partialBulkOutcome).toMatchObject({
      publication: 'NOT_PUBLISHED',
    });
    expect(supportInboxCaseWorkforceContractFixtures.unknownOutcome).toMatchObject({
      lookupOperation: 'NOT_PUBLISHED',
    });
  });

  it('keeps an additive enum fixture outside the closed generated union', () => {
    expect(supportInboxCaseWorkforceContractFixtures.unknownQueueFreshness.state).toBe('STALE');
  });
});
