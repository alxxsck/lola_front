...
describe('демонстрационное хранилище', () => {
  ...
  it('updates scenario metadata with the same concurrency contract as the API', async () => {
    ...
    const updatePromise = mockRepository.updateScenarioMetadata('prj_retenive_demo', scenario!.id, {
      status: 'PAUSED',
  ...
  it('повторяет составную отправку целиком без второго сообщения', async () => {
    ...
    const firstPromise = mockRepository.sendAdminMessage('prj_retenive_demo', 'usr_1', request);
    await vi.runAllTimersAsync();
    const first = await firstPromise;
    const replayPromise = mockRepository.sendAdminMessage('prj_retenive_demo', 'usr_1', request);
    await vi.runAllTimersAsync();
    const replay = await replayPromise;
    const messagesPromise = mockRepository.getMessages('prj_retenive_demo', 'usr_1', 'conv_1');
    await vi.runAllTimersAsync();
  ...
  it('возвращает опубликованный conflict code при повторе ключа с другим body', async () => {
    ...
    const first = mockRepository.sendAdminMessage('prj_retenive_demo', 'usr_1', request);
    await vi.runAllTimersAsync();
    await first;
  ...
  it('filters active waits by the requested stable Event Definition key', async () => {
    const matchingPromise = mockRepository.getScenarioRunsPage('prj_retenive_demo', {
      eventDefinitionKeyId: 'evt_1',
      limit: 50,
    });
    await vi.runAllTimersAsync();
    const matching = await matchingPromise;
    const unrelatedPromise = mockRepository.getScenarioRunsPage('prj_retenive_demo', {
    ...
    expect(
      matching.items.every((run) => run.steps.some((step) => step.status.startsWith('WAITING_'))),
    ).toBe(true);
  ...
  it('дополняет server ordinals в сохранённых до миграции demo-сообщениях', async () => {
    ...
    const pending = mockRepository.getMessages('prj_retenive_demo', 'usr_1', 'conv_1');
    await vi.runAllTimersAsync();
