...
describe('scenario authoring repository', () => {
  ...
  it.each([
  ...
  ])('creates a Scenario and its %s draft through one authoring request', async (_case, text) => {
    ...
    vi.mocked(scenarioAuthoringCreateScenario).mockResolvedValue(response as never);

    await expect(scenarioAuthoringRepository.createScenario('project-1', request)).resolves.toBe(
      response,
    );
    expect(scenarioAuthoringCreateScenario).toHaveBeenCalledWith('project-1', request);
  });
  ...
  it('routes scenario collection, metadata and archive through generated target operations', async () => {
    ...
    await expect(scenarioAuthoringRepository.listScenarios('project-1')).resolves.toEqual([
      scenario,
    ]);
  ...
  it('validates a generated rule DTO', async () => {
    ...
    await expect(scenarioAuthoringRepository.validateRule('project-1', rule)).resolves.toBe(
      response,
    );
  ...
  it('validates Audience separately from behavioral Eligibility', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.validateRule('project-1', rule, undefined, audience),
  ...
  it('previews Audience on the same project-owned Event Log anchor', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.previewRule('project-1', rule, scope, undefined, audience),
    ).resolves.toBe(response);
    expect(scenarioAuthoringPreview).toHaveBeenCalledWith('project-1', {
      rule,
      scope,
  ...
  it('uses generated project-scoped Segment lifecycle endpoints', async () => {
    ...
    await expect(scenarioAuthoringRepository.getSegment('project-1', 'segment-1')).resolves.toBe(
      segment,
    );
    await expect(
    ...
    await expect(
      scenarioAuthoringRepository.publishSegmentRevision('project-1', 'segment-1', publishDraft),
    ).resolves.toMatchObject({ segmentId: 'segment-1' });
    await expect(
  ...
  it('evaluates one End User without inventing a Segment population count', async () => {
    ...
    vi.mocked(scenarioAudienceEvaluationEvaluateUser).mockResolvedValue(response);
    ...
    expect(scenarioAudienceEvaluationEvaluateUser).toHaveBeenCalledWith('project-1', {
  ...
  it('publishes through the atomic generated command', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.publishScenario('project-1', 'scenario-1', publishDraft),
  ...
  it('loads and saves the durable authoring document with observed versions', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.getScenarioDocument('project-1', 'scenario-1'),
    ).resolves.toBe(document);
    await expect(
      scenarioAuthoringRepository.saveScenarioDraft('project-1', 'scenario-1', request),
    ).resolves.toBe(draft);
  ...
  it('validates the full source graph and previews a typed Goal on an Event anchor', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.validateScenarioDraft('project-1', 'scenario-1', source),
    ).resolves.toBe(validation);
    await expect(scenarioAuthoringRepository.previewGoal('project-1', goalRequest)).resolves.toBe(
      goalPreview,
    );
  ...
  it('loads immutable revision history with cursor pagination and detail', async () => {
    ...
    await expect(
      scenarioAuthoringRepository.getScenarioRevisions('project-1', 'scenario-1', {
        limit: 25,
        cursor: 'cursor-1',
      }),
    ).resolves.toBe(page);
    ...
    expect(scenarioAuthoringScenarioRevisions).toHaveBeenCalledWith('project-1', 'scenario-1', {
      limit: 25,
      cursor: 'cursor-1',
    });
  ...
  it('loads the generated explanation for a pinned run', async () => {
    ...
    await expect(scenarioAuthoringRepository.explainRun('project-1', 'run-1')).resolves.toBe(
      explainResponse,
    );
