...
export type ScenarioAuthoringField = Omit<
  ConditionCatalogFieldResponseDto,
  'aggregations' | 'operators' | 'capabilities' | 'display'
> & {
...
export type ScenarioAuthoringEvent = Omit<
  ConditionCatalogEventResponseDto,
  'fields' | 'capabilities'
> & {
...
export type ScenarioAuthoringContract = Omit<
  ConditionCatalogResponseDto,
  'events' | 'localization' | 'translation'
> & {
...
const fieldAggregateMeasures = new Set<string>(
  Object.values(EventAggregateRuleNodeDtoMeasure).filter(
    (measure) =>
      measure !== EventAggregateRuleNodeDtoMeasure.exists &&
      measure !== EventAggregateRuleNodeDtoMeasure.count,
  ),
);
...
function adaptField(field: ConditionCatalogFieldResponseDto): ScenarioAuthoringField {
  ...
  if (!Array.isArray(aggregations) || !Array.isArray(operators))
    throw new Error(`Invalid catalog field ${field.fieldKey}`);

  return {
    ...definition,
    capabilities: {
      eventField: {
        operators: capabilities.currentEvent.operators.filter(
          (operator): operator is EventFieldOperator => eventFieldOperators.has(operator),
        ),
      },
      aggregateFilter: {
        operators: capabilities.aggregateFilter.operators.filter(
          (operator): operator is AggregateFilterOperator => aggregateFilterOperators.has(operator),
        ),
      },
      aggregateMeasure: {
        measures: capabilities.aggregateMeasure.measures.filter(
          (measure): measure is FieldAggregateMeasure => fieldAggregateMeasures.has(measure),
        ),
...
function adaptedMeasures(
  event: ConditionCatalogEventResponseDto,
): ScenarioAggregateMeasureCapability[] {
  const eventMeasures: ScenarioAggregateMeasureCapability[] = event.capabilities.eventMeasures.map(
    (capability) => ({
      ...
      compareOperators:
        capability.resultType === 'boolean'
          ? [RuleCompareDtoOperator.eq, RuleCompareDtoOperator.neq]
          : [...allCompareOperators],
    }),
  );
  const fieldMeasures = new Set(
    event.fields.flatMap((field) => field.capabilities.aggregateMeasure.measures),
  );
  return [
    ...eventMeasures,
    ...[...fieldMeasures]
      .filter((measure): measure is FieldAggregateMeasure => fieldAggregateMeasures.has(measure))
      .map((measure) => ({
        measure,
...
export function adaptScenarioAuthoringContract(
