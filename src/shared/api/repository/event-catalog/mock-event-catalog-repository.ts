...
const lifecycleByKey = new Map<string, { lifecycle: 'ACTIVE' | 'ARCHIVED'; version: number }>();
...
function emptyImpact(
  ...
  const existingFieldsUnchanged = Object.entries(publishedProperties).every(
    ([key, value]) => JSON.stringify(draftProperties[key]) === JSON.stringify(value),
  );
...
async function findEvent(projectId: string, definitionKeyId: string) {
  const event = events.find(
    (item) => item.projectId === projectId && (item.definitionKeyId ?? item.id) === definitionKeyId,
  );
...
export const mockEventCatalogRepository: EventCatalogRepository = {
  ...
  async saveSchemaDraft(projectId, definitionKeyId, command) {
    ...
    const changed =
      JSON.stringify(current?.payloadSchema) !== JSON.stringify(command.payloadSchema);
  ...
  async analyzeSchemaDraft(projectId, definitionKeyId, command) {
    ...
    const definition = await this.getDefinition(projectId, definitionKeyId);
    return emptyImpact(definitionKeyId, draft, definition.currentSchema.payloadSchema);
  },
  async publishSchemaDraft(projectId, definitionKeyId, command) {
    ...
    const analysis = emptyImpact(definitionKeyId, draft, definition.currentSchema.payloadSchema);
    if (
  ...
  async updateMetadata(projectId, definitionKeyId, command): Promise<EventMetadataUpdateResult> {
    const event = await findEvent(projectId, definitionKeyId);
    const current = toMockDefinition(event);
    if (command.expectedUpdatedAt !== current.metadata.concurrencyToken) {
      throw new Error('Event Definition metadata уже изменена. Обновите workspace.');
    }
  ...
  async updatePolicy(projectId, definitionKeyId, command) {
    ...
    if (command.expectedVersion !== current.policy.version) {
      throw new Error('Event Ingestion Policy уже изменена. Обновите workspace.');
    }
    const changed =
  ...
  async archive(projectId, definitionKeyId, command) {
    ...
    if (
      ...
      throw new Error('Event Definition lifecycle уже изменён. Обновите workspace.');
    }
  ...
  async hardDelete(projectId, definitionKeyId, command) {
    ...
    if (
      ...
      throw new Error('Event Definition delete intent недействителен. Обновите workspace.');
    }
