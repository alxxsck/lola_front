...
import { applyEventMetadataUpdate, toEventCatalogDefinition } from './event-catalog-contract';
...
describe('toEventCatalogDefinition', () => {
  ...
  it('fails closed when a stable definition has no current revision', () => {
    expect(() => toEventCatalogDefinition({ ...dto, currentRevision: null })).toThrow(
      'has no current schema revision',
    );
  });
});
