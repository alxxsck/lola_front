...
describe('apiEventCatalogRepository', () => {
  ...
  it('uses the canonical lifecycle-filtered stable list', async () => {
    ...
    const result = await apiEventCatalogRepository.listDefinitions('project-1', 'ARCHIVED');
  ...
  it('updates policy with OCC and performs mandatory stable-identity reload', async () => {
    ...
    await apiEventCatalogRepository.updatePolicy('project-1', 'event-key-1', command);
    expect(eventCatalogUpdatePolicy).toHaveBeenCalledWith('project-1', 'event-key-1', command);
  ...
  it('creates a semantic successor through the OCC source-draft command', async () => {
    ...
    await apiEventCatalogRepository.createSchemaSuccessor('project-1', 'event-key-1', command);
    ...
    expect(eventCatalogDetail).toHaveBeenCalledWith('project-1', 'successor-key');
  });
  ...
  it('owns the complete server-backed schema draft lifecycle', async () => {
    ...
    vi.mocked(eventCatalogAnalyzeSchemaDraft).mockResolvedValue(impact as never);
    vi.mocked(eventCatalogPublishSchemaDraft).mockResolvedValue(published as never);

    await expect(
      apiEventCatalogRepository.getSchemaDraft('project-1', 'event-key-1'),
    ).resolves.toEqual(draft);
    await apiEventCatalogRepository.saveSchemaDraft('project-1', 'event-key-1', {
      payloadSchema: draft.payloadSchema,
      expectedDraftVersion: 2,
    });
    await apiEventCatalogRepository.analyzeSchemaDraft('project-1', 'event-key-1', {
      expectedDraftVersion: 2,
    });
    await apiEventCatalogRepository.publishSchemaDraft('project-1', 'event-key-1', {
      expectedDraftVersion: 2,
      expectedBaseRevisionId: 'revision-4',
      reason: 'Add optional field',
    });
    await apiEventCatalogRepository.discardSchemaDraft('project-1', 'event-key-1', {
      expectedDraftVersion: 2,
      reason: 'Discard proposal',
    });
    ...
    expect(eventCatalogAnalyzeSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedDraftVersion: 2,
    });
    ...
    expect(eventCatalogDiscardSchemaDraft).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedDraftVersion: 2,
      reason: 'Discard proposal',
    });
  ...
  it('passes lifecycle OCC commands to archive and restore', async () => {
    ...
    expect(eventCatalogRestore).toHaveBeenCalledWith('project-1', 'event-key-1', {
      expectedLifecycleVersion: 3,
    });
  });
  ...
  it('confirms hard delete only when the stable GET returns 404', async () => {
    ...
    expect(eventCatalogHardDelete).toHaveBeenCalledWith('project-1', 'event-key-1', command);
    expect(eventCatalogDetail).toHaveBeenCalledAfter(vi.mocked(eventCatalogHardDelete));
  });
  ...
  it('accepts DELETE 404 only when retrying the same retained timeout intent', async () => {
    ...
    await expect(
      apiEventCatalogRepository.hardDelete('project-1', 'event-key-timeout', command),
    ).rejects.toBeTruthy();
    await expect(
      apiEventCatalogRepository.hardDelete('project-1', 'event-key-timeout', command),
    ).resolves.toBeUndefined();
