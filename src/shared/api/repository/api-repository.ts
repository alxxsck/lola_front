...
function mapAdminMessageResult(response: Awaited<ReturnType<typeof adminMessagingSend>>) {
  return {
...
export const apiRepository: ReteniveRepository = {
  ...
  async updateProject(projectId, patch) {
    return mapProject(
      await platformOperationsUpdateProjectSettings(projectId, toUpdateProjectSettingsDto(patch)),
    );
  },
    ...
    const response = await adminMessagingSend(
      ...
      userId,
      {
        ...
        ...(message.replyTranslationDraftId
          ...
          : {}),
        ...(message.macroReplyDraftId ? { macroReplyDraftId: message.macroReplyDraftId } : {}),
        ...(message.supportKnowledgeCitationDraftId
          ? {
              supportKnowledgeCitationDraftId: message.supportKnowledgeCitationDraftId,
            }
          : {}),
  ...
  async getStats(projectId, effectivePermissionCodes) {
    ...
    const [project, scenarios, users, sessions, eventLogs, failedEventLogs, runs] =
      await Promise.all([
