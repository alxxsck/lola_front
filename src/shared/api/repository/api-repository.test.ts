...
describe('api repository adapter', () => {
  ...
  it('uses generated event catalog list, create, revision and archive endpoints', async () => {
    ...
    const catalogDefinition = {
      id: 'event-key-1', projectId: 'project-1', code: 'deposit', name: 'Deposit', description: null,
      lifecycle: 'ACTIVE' as const, lifecycleVersion: 1, lifecycleUpdatedAt: '2026-07-20T10:00:00.000Z',
      metadataUpdatedAt: '2026-07-20T10:00:00.000Z', origin: 'CUSTOM' as const, readOnly: false,
      policy: { version: 1, updatedAt: '2026-07-20T10:00:00.000Z', enabled: true, clientIngestible: false, countsAsActivity: true },
      currentRevision: { id: 'event-revision-1', number: 1, payloadSchema: { type: 'object' }, publishedAt: '2026-07-20T10:00:00.000Z' },
    }
    const revision = {
      id: 'event-revision-1', projectId: 'project-1', definitionKeyId: 'event-key-1', isCurrent: true,
      code: 'deposit', number: 1, payloadSchema: { type: 'object' }, publishedAt: '2026-07-20T10:00:00.000Z',
      pinnedScenarioRevisionCount: 0, compatibility: 'CURRENT' as const,
    }
    ...
    vi.mocked(eventCatalogArchive).mockResolvedValue({ ...catalogDefinition, lifecycle: 'ARCHIVED' })
    ...
    await apiRepository.deleteEvent('project-1', 'event-key-1', { expectedLifecycleVersion: 1, expectedPolicyVersion: 1, reason: 'Archive obsolete event' })
    ...
    expect(eventCatalogArchive).toHaveBeenCalledWith('project-1', 'event-key-1', { expectedLifecycleVersion: 1, expectedPolicyVersion: 1, reason: 'Archive obsolete event' })
  })
  ...
  it('updates event ingestion policy with caller concurrency evidence', async () => {
    ...
    const currentDetail = {
      id: current.definitionKeyId, projectId: current.projectId, code: current.code, name: current.name,
      description: current.description, lifecycle: 'ACTIVE' as const, lifecycleVersion: 1, lifecycleUpdatedAt: 'now',
      metadataUpdatedAt: current.metadataUpdatedAt, origin: current.origin, readOnly: current.readOnly,
      policy: { version: 7, updatedAt: 'now', enabled: true, clientIngestible: false, countsAsActivity: true },
      currentRevision: { id: current.id, number: 1, payloadSchema: current.payloadSchema, publishedAt: 'now' },
    }
    vi.mocked(eventCatalogDetail)
      .mockResolvedValueOnce(currentDetail)
      .mockResolvedValueOnce({ ...currentDetail, policy: { ...currentDetail.policy, enabled: false, version: 8 } })
  ...
  it('uses the snapshot cursor CMS endpoint for filtered event logs and detail', async () => {
    const eventLog = {
      id: 'log-1', projectId: 'project-1', eventDefinitionId: 'event-1', eventDefinitionKeyId: 'event-key-1', endUserId: 'user-1',
