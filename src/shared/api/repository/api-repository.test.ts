...
vi.mock("@/shared/api/generated/lola-backend", () => ({
  ...
  platformCreateMember: vi.fn(),
  platformCreateScenario: vi.fn(),
  platformCreateUi: vi.fn(),
  platformDeleteMember: vi.fn(),
  platformDeleteScenario: vi.fn(),
  platformDeleteUi: vi.fn(),
  platformGetProject: vi.fn(),
  platformMembers: vi.fn(),
  platformScenarios: vi.fn(),
  platformUiElements: vi.fn(),
  platformActionDefinitions: vi.fn(),
  platformUpdateProject: vi.fn(),
  platformUpdateScenario: vi.fn(),
  platformUpdateUi: vi.fn(),
  platformUsers: vi.fn(),
  ...
  platformCreateUserAttributeDefinition: vi.fn(),
  platformDeleteUserAttributeDefinition: vi.fn(),
  platformUpdateUserAttributeDefinition: vi.fn(),
  platformUserAttributeDefinitions: vi.fn(),
  platformUsersPage: vi.fn(),
  scenarioRunsPage: vi.fn(),
  platformActivitySettings: vi.fn(),
  platformUpdateActivitySettings: vi.fn(),
...
const eventResponse = {
  ...
  readOnly: false,
} as EventDefinitionCatalogResponseDto;
...
describe("api repository adapter", () => {
  ...
  it("routes UI create, update and delete through the generated client", async () => {
    ...
    expect(platformCreateUi).toHaveBeenCalledWith(
      "project-1",
      expect.objectContaining({ code: "deposit", selector: "#deposit" }),
    );
    expect(platformUpdateUi).toHaveBeenCalledWith("project-1", "ui-1", {
      name: "Deposit updated",
    });
    expect(platformDeleteUi).toHaveBeenCalledWith("project-1", "ui-1");
  });

  it("reads only active Event Definitions through the canonical stable catalog", async () => {
  ...
  it("exposes cursor pages and activity settings through generated contracts", async () => {
    vi.mocked(platformUsersPage).mockResolvedValue({
      items: [],
    ...
    expect(platformUsersPage).toHaveBeenCalledWith("project-1", {
      cursor: "user-1",
      limit: 50,
    });
    ...
    expect(platformUpdateActivitySettings).toHaveBeenCalledWith(
      "project-1",
      expect.objectContaining({ visitInactivitySeconds: 1800 }),
    );
  });
  ...
  it("sends only backend-editable project settings", async () => {
    ...
    expect(platformUpdateProject).toHaveBeenCalledWith("project-1", {
      name: "Updated",
    });
  });
  ...
  it("routes scenario CRUD through generated endpoints with backend DTOs", async () => {
    const response = {
      ...
      conversationPolicy: "reuse_active" as const,
      priority: 0,
      conditions: [],
      cooldownSeconds: 0,
      actions: [
    ...
    const value = {
      ...
      conversationPolicy: "reuse_active" as const,
      actions: [
        { position: 0, type: "OPEN_PAGE" as const, config: { pageId: "home" } },
      ],
    };

    await expect(
      apiRepository.saveScenario("project-1", value),
    ).resolves.toEqual(
      expect.objectContaining({ conversationPolicy: "reuse_active" }),
    );
    await apiRepository.saveScenario("project-1", {
      ...value,
      id: "scenario-1",
    });
    ...
    expect(platformCreateScenario).toHaveBeenCalledWith(
      "project-1",
      expect.objectContaining({
        code: "welcome",
        conversationPolicy: "reuse_active",
        actions: [
          { position: 0, type: "OPEN_PAGE", config: { pageId: "home" } },
        ],
      }),
    );
    ...
    expect(platformUpdateScenario).toHaveBeenCalledWith(
      "project-1",
      "scenario-1",
      expect.not.objectContaining({ code: "welcome" }),
    );
    expect(platformUpdateScenario).toHaveBeenCalledWith(
      "project-1",
      "scenario-1",
      { name: "Updated metadata", priority: 5 },
    );
    expect(platformDeleteScenario).toHaveBeenCalledWith(
      "project-1",
      "scenario-1",
    );
  });
  ...
  it("normalizes legacy non-array scenario conditions before saving", async () => {
    const legacyResponse = {
      id: "scenario-1",
      projectId: "project-1",
      code: "open_deposit_modal",
      name: "Open deposit modal",
      eventDefinitionId: "event-1",
      status: "DRAFT" as const,
      conversationPolicy: "create_new" as const,
      priority: 0,
      conditions: {},
      cooldownSeconds: 0,
      actions: [],
    ...
    vi.mocked(platformUpdateScenario).mockResolvedValue({
      ...legacyResponse,
      conditions: [],
    });
  ...
  it("loads and parses project action definitions", async () => {
    vi.mocked(platformActionDefinitions).mockResolvedValue([
      {
        ...
        uiSchema: {
          fields: [{ key: "durationMs", label: "Duration", control: "number" }],
        },
        createdAt: "now",
        updatedAt: "now",
      } as never,
    ...
    await expect(
      ...
      expect.objectContaining({
        type: "WAIT_FOR",
        executor: "SERVER",
        configSchema: expect.objectContaining({ required: ["durationMs"] }),
      }),
    ]);
  ...
  it("publishes a user attribute definition and reloads the current immutable schema", async () => {
    const definition = {
      ...
      required: true,
      clientVisible: false,
      validation: { allowedValues: ["free", "premium"] },
      enabled: true,
      position: 10,
    ...
    vi.mocked(platformCreateUserAttributeDefinition).mockResolvedValue({
      definition,
      currentRevision: revision,
    });
    vi.mocked(platformUserAttributeDefinitions).mockResolvedValue({
      definitions: [definition],
      currentRevision: revision,
    });

    await expect(
      apiRepository.createUserAttributeDefinition("project-1", {
        key: "plan",
        label: "Plan",
        type: "STRING",
        required: true,
      ...
      expect.objectContaining({
        currentRevision: expect.objectContaining({ version: 3 }),
        definition: expect.objectContaining({ key: "plan" }),
      }),
    );
    expect(platformCreateUserAttributeDefinition).toHaveBeenCalledWith(
      "project-1",
      expect.objectContaining({
        validation: { allowedValues: ["free", "premium"] },
      }),
