...
describe('theme bootstrap', () => {
  ...
  it.each([
    ...
    window.eval(bootstrapSource);
    const bootstrapTheme = document.documentElement.classList.contains('retenive-dark')
      ? 'dark'
      : 'light';
    document.documentElement.className = '';
    document.documentElement.style.colorScheme = '';
    ...
    expect({ bootstrapTheme, runtimeTheme }).toEqual({
      bootstrapTheme: expected,
      runtimeTheme: expected,
    });
  });
});
