...
describe('scenario action catalog schema helpers', () => {
    ...
    expect(
      parseScenarioActionCatalogItem({
        ...
        configSchema: {
          ...
          properties: {
            goal: { type: 'object' },
            timeoutMs: { type: 'integer', minimum: 1_000, maximum: 7_776_000_000 },
            onGoal: { type: 'string' },
      ...
      }).uiSchema.fields.map((field) => field.control),
    ).toEqual(['goal-builder', 'duration', 'node', 'node']);
  });
  ...
  it('parses Project Action revision JSON blobs into a strict catalog item', () => {
    ...
    expect(definition.uiSchema.fields[2]).toMatchObject({
      control: 'target',
      targetKinds: ['PAGE'],
    });
    expect(
      actionFieldOptions(definition.uiSchema.fields[1]!, definition.configSchema.properties.action),
  ...
  it('rejects UI fields that do not match the backend config schema', () => {
    expect(() =>
      parseScenarioActionCatalogItem({
        ...
        uiSchema: {
          fields: [
            ...rawDefinition.uiSchema.fields,
            { key: 'unknown', label: 'Unknown', control: 'text' },
          ],
        },
      }),
  ...
  it('uses schema defaults and removes hidden or unknown config values', () => {
    ...
    expect(
      sanitizeActionConfig(definition, {
        label: 'Открыть',
        action: 'none',
        pageId: 'hidden-page',
        timeoutMs: 30000,
  ...
  it('validates required, conditional target and numeric constraints generically', () => {
    const definition = parseScenarioActionCatalogItem(rawDefinition);
    expect(validateActionConfig(definition, { action: 'none', timeoutMs: 30000 })).toBe(
      'Текст: обязательное поле',
    );
    expect(
      validateActionConfig(definition, { label: 'Go', action: 'open_page', timeoutMs: 30000 }),
    ).toBe('Страница: обязательное поле');
    expect(validateActionConfig(definition, { label: 'Go', action: 'none', timeoutMs: 10 })).toBe(
      'Таймаут: должно быть не меньше 1000',
    );
    expect(
      validateActionConfig(definition, { label: 'Go', action: 'none', timeoutMs: 30000 }),
    ).toBe('');
    expect(
      validateScenarioActionConfig(
        { position: 0, type: 'SHOW_CTA', config: {} },
      ...
      validateScenarioActionConfig(
        {
          position: 0,
          type: 'SHOW_CTA',
          config: { label: { en: 'Open', es: 'Abrir' }, action: 'none', timeoutMs: 30000 },
        },
        definition,
        localization,
      ),
  ...
  it('describes and validates the non-blocking voice conversation action', () => {
    const definition = demoScenarioActionCatalog.find(
      (item) => item.type === 'START_VOICE_CONVERSATION',
    );
    ...
    expect(definition?.uiSchema.fields.map((field) => field.key)).toEqual([
      'text',
      'voice',
      'onUnavailable',
    ]);
    expect(validateActionConfig(definition!, {})).toBe(
      'Первая голосовая реплика Retenive: обязательное поле',
    );
    expect(
      validateActionConfig(definition!, {
        text: 'Привет!',
        voice: 'eve',
        onUnavailable: 'continue',
      }),
    ).toBe('');
    expect(validateActionConfig(definition!, { text: 'Привет!', voice: 'marin' })).toBe(
      'Голос: содержит недопустимое значение',
    );
    expect(validateActionConfig(definition!, { text: 'Привет!', onUnavailable: 'stop' })).toBe(
      'Если голос недоступен: содержит недопустимое значение',
    );
  });
  ...
  it('describes both SPEAK_TEXT playback modes and defaults to waiting', () => {
    ...
    expect(validateActionConfig(definition!, { text: 'Привет!', waitForCompletion: 'false' })).toBe(
      'Дождаться окончания воспроизведения: должно иметь тип boolean',
    );
  });
});
