...
export function parseDecimalString(value: unknown): DecimalString | undefined {
  return typeof value === 'string' && value.length <= 64 && DECIMAL_PATTERN.test(value)
    ? value
    : undefined;
}
...
export function addDecimalStrings(values: readonly DecimalString[]): DecimalString {
  ...
  const total = parsed.reduce((sum, value) => sum + coefficient(value, scale), 0n);
  return fromCoefficient(total, scale);
}

export function decimalRatio(value: DecimalString, total: DecimalString): number {
  if (compareDecimalStrings(total, '0') === 0) return 0;
  const numeratorParts = parts(value);
  const denominatorParts = parts(total);
  const scale = Math.max(numeratorParts.fraction.length, denominatorParts.fraction.length);
...
export function formatDecimalMoney(
  ...
  const lessThan = options.lessThan === undefined ? '0.01' : options.lessThan;
  if (
    lessThan &&
...
function coefficient(value: DecimalParts, scale: number): bigint {
  return BigInt(`${value.integer}${value.fraction.padEnd(scale, '0')}`.replace(/^0+(?=\d)/, ''));
}
