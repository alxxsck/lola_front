...
function schemaType(
  ...
  for (const keyword of ['oneOf', 'anyOf', 'allOf'] as const) {
    ...
    const types = [
      ...new Set(schema[keyword].map((variant) => schemaType(variant, root, visitedRefs))),
    ];
  ...
  const direct = Object.entries(schema.properties).flatMap(([key, property]) => {
    ...
    const current: MarkdownField = {
      ...
      requirement: isRequired
        ...
        : 'необязательно',
      description:
        isRecord(property) && typeof property.description === 'string' ? property.description : '',
    };
    const nestedCondition = isRequired ? conditionPath : path;
    const nested =
      isRecord(property) && property.type === 'array' && property.items !== undefined
        ? eventPayloadFields(
            property.items,
            `${path}[]`,
            root,
