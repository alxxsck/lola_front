...
export function parseAllowedValues(
  ...
  if (type === 'NUMBER') {
    ...
    if (numbers.some((value) => !Number.isFinite(value)))
      throw new Error('Допустимые значения должны быть числами — по одному в строке.');
    return numbers;
  }
  if (type === 'BOOLEAN') {
    if (values.some((value) => value !== 'true' && value !== 'false'))
      throw new Error('Для boolean доступны только true и false.');
...
export function buildUserAttributeValidation(
  input: UserAttributeConstraintInput,
): UserAttributeValidation {
  const validation: UserAttributeValidation = {};
  if (input.type === 'STRING') {
    if (input.minLength !== null && (!Number.isInteger(input.minLength) || input.minLength < 0))
      throw new Error('Минимальная длина должна быть целым неотрицательным числом.');
    if (input.maxLength !== null && (!Number.isInteger(input.maxLength) || input.maxLength < 0))
      throw new Error('Максимальная длина должна быть целым неотрицательным числом.');
  ...
  if (input.type === 'NUMBER') {
    if (input.minimum !== null && !Number.isFinite(input.minimum))
      throw new Error('Минимум должен быть числом.');
    if (input.maximum !== null && !Number.isFinite(input.maximum))
      throw new Error('Максимум должен быть числом.');
