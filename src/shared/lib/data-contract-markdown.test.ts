...
| \`payload.payment.currency\` | \`string\` | обязательно, если передан \`payload.payment\` | ISO 4217 |`);
  ...
  it('keeps advanced JSON Schema types and composed parameters understandable', () => {
    ...
    expect(markdown).toContain(
      '| `payload.channel.name` | `string` | обязательно, если передан `payload.channel` |',
    );
    expect(markdown).toContain(
      '| `payload.contact.email` | `string` | обязательно в соответствующем варианте |',
    );
    expect(markdown).toContain(
      '| `payload.contact.phone` | `string` | обязательно в соответствующем варианте |',
    );
