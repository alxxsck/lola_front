...
function parseSchemaProperty(value: unknown, path: string): ActionConfigPropertySchema {
  ...
  const type =
    typeof value.type === 'string' && schemaTypes.has(value.type) ? value.type : undefined;
  if (
    value.enum !== undefined &&
    (!Array.isArray(value.enum) || value.enum.some((item) => !isJsonValue(item)))
  ) {
    throw new Error(`${path}.enum must contain JSON values`);
  }
  if (value.default !== undefined && !isJsonValue(value.default))
    throw new Error(`${path}.default must be a JSON value`);
  ...
  if (value.items !== undefined && !isRecord(value.items))
  ...
  const properties =
    value.properties === undefined
      ? undefined
      : Object.fromEntries(
          Object.entries(value.properties as Record<string, unknown>).map(([key, property]) => [
            key,
            parseSchemaProperty(property, `${path}.properties.${key}`),
          ]),
        );
  ...
  if (
    ...
    (!Array.isArray(required) ||
      required.some((key) => typeof key !== 'string' || !properties || !(key in properties)))
  ) {
  ...
  return {
    ...
    type,
    items:
      value.items === undefined ? undefined : parseSchemaProperty(value.items, `${path}.items`),
    properties,
...
function parseConfigSchema(value: unknown): ActionConfigSchema {
  if (!isRecord(value) || value.type !== 'object' || !isRecord(value.properties)) {
    throw new Error(
      'scenarioActionCatalogItem.configSchema must describe an object with properties',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'scenarioActionCatalogItem.configSchema.required must reference declared properties',
    );
  }
  if (
    ...
    throw new Error(
      'scenarioActionCatalogItem.configSchema.additionalProperties must be a boolean or schema object',
    );
  }
  const additionalProperties = isRecord(value.additionalProperties)
    ? parseSchemaProperty(
        value.additionalProperties,
        'scenarioActionCatalogItem.configSchema.additionalProperties',
      )
    : value.additionalProperties;
  return {
    ...value,
...
function parseOption(value: unknown, path: string): ActionUiOption {
  if (isJsonValue(value) && (!isRecord(value) || !('label' in value) || !('value' in value)))
    return value;
...
function parseUiField(
  ...
  if (typeof control !== 'string' || !controls.has(control as ActionControl))
    throw new Error(`${path}.control is unsupported`);
  const targetKinds = value.targetKinds === undefined ? undefined : value.targetKinds;
  if (
    targetKinds !== undefined &&
    (!Array.isArray(targetKinds) ||
      targetKinds.some((kind) => typeof kind !== 'string' || !entityKinds.has(kind as EntityKind)))
  ) {
    throw new Error(`${path}.targetKinds is invalid`);
  }
  if (control === 'target' && (!targetKinds || !targetKinds.length))
    throw new Error(`${path}.targetKinds is required`);
  if (
    value.visibleWhen !== undefined &&
    (!isRecord(value.visibleWhen) ||
      Object.keys(value.visibleWhen).some((fieldKey) => !(fieldKey in properties)))
  ) {
  ...
  for (const flag of ['allowCustom', 'supportsTemplates'] as const) {
    if (value[flag] !== undefined && typeof value[flag] !== 'boolean')
      throw new Error(`${path}.${flag} must be a boolean`);
  }
  return {
    ...value,
    key,
...
function parseUiSchema(value: unknown, configSchema: ActionConfigSchema) {
  if (!isRecord(value) || !Array.isArray(value.fields))
    throw new Error('scenarioActionCatalogItem.uiSchema.fields must be an array');
  const fields = value.fields.map((field, index) =>
    parseUiField(
      field,
      configSchema.properties,
      `scenarioActionCatalogItem.uiSchema.fields.${index}`,
    ),
  );
  const keys = fields.map((field) => field.key);
  if (new Set(keys).size !== keys.length)
    throw new Error('scenarioActionCatalogItem.uiSchema contains duplicate fields');
  const missing = Object.keys(configSchema.properties).filter((key) => !keys.includes(key));
  if (missing.length)
    throw new Error(`scenarioActionCatalogItem.uiSchema is missing fields: ${missing.join(', ')}`);
  return { ...value, fields };
}
...
export function parseScenarioActionCatalogItem(value: unknown): ScenarioActionCatalogItem {
  ...
  if (executor !== 'SERVER' && executor !== 'FRONTEND')
    throw new Error('scenarioActionCatalogItem.executor is unsupported');
  return {
...
export function createActionConfig(definition: ScenarioActionCatalogItem): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(definition.configSchema.properties)
...
export function requiredActionConfigKeys(
  ...
  return definition.uiSchema.fields
    ...
    .filter(
      (field) => definition.configSchema.required.includes(field.key) || field.control === 'target',
    )
    .map((field) => field.key);
}

export function actionFieldOptions(
  field: ActionUiField,
  property?: ActionConfigPropertySchema,
): ActionUiOption[] {
...
export function sanitizeActionConfig(
  ...
  const entries = Object.entries(config).filter(
    ([key, value]) =>
      value !== undefined &&
      ((key in definition.configSchema.properties && visibleKeys.has(key)) ||
        definition.configSchema.additionalProperties === true ||
        isRecord(definition.configSchema.additionalProperties)),
  );
...
function typeError(schema: ActionConfigPropertySchema, value: unknown): string {
  ...
  if (schema.type === 'integer')
    return typeof value === 'number' && Number.isInteger(value) ? '' : 'должно быть целым числом';
  return typeof value === schema.type ? '' : `должно иметь тип ${schema.type}`;
}

export function actionConfigPropertyError(
  ...
  if (typeof value === 'string') {
    if (schema.minLength !== undefined && value.length < schema.minLength)
      return `должно содержать минимум ${schema.minLength} симв.`;
    if (schema.maxLength !== undefined && value.length > schema.maxLength)
      return `должно содержать максимум ${schema.maxLength} симв.`;
    if (schema.pattern !== undefined && !new RegExp(schema.pattern).test(value))
      return 'не соответствует требуемому формату';
  }
  if (typeof value === 'number') {
    if (schema.minimum !== undefined && value < schema.minimum)
      return `должно быть не меньше ${schema.minimum}`;
    if (schema.maximum !== undefined && value > schema.maximum)
      return `должно быть не больше ${schema.maximum}`;
  }
  if (Array.isArray(value)) {
    if (schema.minItems !== undefined && value.length < schema.minItems)
      return `должно содержать минимум ${schema.minItems} знач.`;
    if (schema.maxItems !== undefined && value.length > schema.maxItems)
      return `должно содержать максимум ${schema.maxItems} знач.`;
    if (schema.items) {
...
export function validateActionConfig(
  ...
  for (const field of definition.uiSchema.fields) {
    ...
    if (requiredKeys.has(field.key) && (value === undefined || value === null || value === ''))
...
export function validateScenarioActionConfig(
  ...
  if (localization?.enabled) {
    const scalarize = (
      ...
      const descriptors = localization.paths.filter(
        (descriptor) => descriptor.actionType === actionType,
      );
        ...
        if (descriptor.path === 'config.options[].label' && Array.isArray(copy.options)) {
          copy.options = copy.options.map((option) => {
            ...
            const label = isRecord(item.label)
              ? item.label[localization.defaultLocale]
              : item.label;
  ...
  const localizedKeys = new Set(
    localization?.paths
      .filter(
        (descriptor) =>
          descriptor.actionType === action.type && /^config\.[^.]+$/.test(descriptor.path),
      )
