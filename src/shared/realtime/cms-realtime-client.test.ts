...
function fakeSocket() {
  ...
  const emitWithAck = vi
    .fn()
    .mockImplementation(
      async (_event: string, request: { conversationId?: string; caseId?: string } = {}) => ({
        ok: true,
...
describe('CmsRealtimeClient', () => {
  ...
  it('refreshes an expired access token before a reconnect handshake', async () => {
    const socket = fakeSocket();
    mocks.io.mockReturnValue(socket);
    mocks.getAccessToken.mockReturnValueOnce(null).mockReturnValueOnce('fresh-access-token');
    const client = new CmsRealtimeClient();
    await client.connect('project-1', {
      subscriptions: {},
      acknowledgement: {
  ...
  it('retries a rejected watch without waiting for a socket reconnect', async () => {
    vi.useFakeTimers();
    const socket = fakeSocket();
    socket.emitWithAck.mockRejectedValueOnce(new Error('timeout')).mockResolvedValueOnce({
      ok: true,
      conversationId: 'conversation-1',
      generation: '1',
      expiresAt: '2026-08-08T10:01:00.000Z',
  ...
  it('renews the exact watch generation and sends only a typing boolean', async () => {
    ...
    expect(socket.emitWithAck).toHaveBeenCalledWith('conversation.watch.renew.v1', {
      conversationId: 'conversation-1',
      generation: '17',
    });
  ...
  it('closes a stale lease when its watch ack arrives after a route switch', async () => {
    ...
    await first;
    await vi.waitFor(() =>
      expect(socket.emitWithAck).toHaveBeenCalledWith('conversation.unwatch.v1', {
        conversationId: 'conversation-1',
        generation: '1',
      }),
    );
  ...
  it('reissues requested typing after reconnect under the new watch generation', async () => {
    ...
    await vi.waitFor(() =>
      expect(
        socket.emitWithAck.mock.calls.filter(([event]) => event === 'conversation.typing.v1'),
      ).toHaveLength(2),
    );
    expect(socket.emitWithAck).toHaveBeenLastCalledWith('conversation.typing.v1', {
      conversationId: 'conversation-1',
      generation: '2',
      isTyping: true,
    });
  ...
  it('owns an exact renewable internal-note Case watch and closes it on route change', async () => {
    ...
    await expect(client.watchSupportInternalNotes('case-1')).resolves.toBe(true);
    await vi.advanceTimersByTimeAsync(40_000);
    client.unwatchSupportInternalNotes('case-1');

    expect(socket.emitWithAck).toHaveBeenCalledWith('support.internal_note.watch.v1', {
      caseId: 'case-1',
    });
    expect(socket.emitWithAck).toHaveBeenCalledWith('support.internal_note.renew.v1', {
      caseId: 'case-1',
    });
    expect(socket.emitWithAck).toHaveBeenCalledWith('support.internal_note.unwatch.v1', {
      caseId: 'case-1',
    });
    client.deactivateProject();
    vi.useRealTimers();
  ...
  it('does not let a late Case A renew replace the active Case B renew timer', async () => {
    ...
    socket.emitWithAck.mockImplementation(
      async (event: string, request: { caseId?: string } = {}) => {
        if (event === 'support.internal_note.renew.v1' && request.caseId === 'case-1')
  ...
  it('purges a terminal internal-note watch without retrying it', async () => {
    ...
    socket.emitWithAck.mockImplementation(
      async (event: string, request: { caseId?: string } = {}) => {
        if (event === 'support.internal_note.renew.v1') return { ok: false, error: 'UNAUTHORIZED' };
        return {
          ok: true,
          caseId: request.caseId,
          expiresAt: '2026-08-08T10:01:00.000Z',
    ...
    expect(
      socket.emitWithAck.mock.calls.filter(([event]) => event === 'support.internal_note.watch.v1'),
    ).toHaveLength(1);
    expect(
      socket.emitWithAck.mock.calls.filter(([event]) => event === 'support.internal_note.renew.v1'),
    ).toHaveLength(1);
    client.deactivateProject();
    vi.useRealTimers();
  });
  ...
  it('degrades a capacity-limited note watch without revoking REST note authority', async () => {
    ...
    await expect(client.watchSupportInternalNotes('case-1')).resolves.toBe(false);
    await vi.advanceTimersByTimeAsync(10_000);
