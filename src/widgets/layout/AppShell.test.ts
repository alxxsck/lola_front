...
function project(id: string, name: string, effectivePermissionCodes: string[] = []) {
  return {
    id,
    name,
...
describe('AppShell', () => {
  ...
  it('exposes one Case Intelligence entry that opens its overview', async () => {
    ...
    expect(caseIntelligence?.attributes('href')).toBe('/support/settings/case-intelligence');
    expect(
      wrapper
        .findAll('.sidebar-scroll nav a')
        .filter((link) =>
          link.attributes('href')?.startsWith('/support/settings/case-intelligence'),
        ),
    ).toHaveLength(1);
  });
  ...
  it('links a Cases-only operator to the exact canonical inbox mode', async () => {
    ...
    setActivePinia(pinia);
    const auth = useAuthStore();
    authenticateWithProjects(auth, [project('project-1', 'Project One', ['project.cases.read'])]);
    const router = createRouter({
      history: createMemoryHistory(),
      routes: [{ path: '/overview', component: { template: '<div />' } }],
    });
  ...
  it('uses the compact application rail on the Support workspace route', async () => {
    ...
    expect(wrapper.get('.shell').classes()).toContain('shell--sidebar-collapsed');
    expect(wrapper.get('.sidebar').attributes('aria-label')).toBe('Основная навигация CMS');

    await wrapper.get('button[aria-label="Развернуть боковое меню"]').trigger('click');
    expect(wrapper.get('.shell').classes()).not.toContain('shell--sidebar-collapsed');
  });
  ...
  it('lets a desktop user collapse and restore the sidebar from every page', async () => {
    ...
    expect(wrapper.get('.shell').classes()).toContain('shell--sidebar-collapsed');
    expect(localStorage.getItem('retenive-cms-sidebar-collapsed-v1')).toBe('true');
    expect(
      wrapper.get('button[aria-label="Развернуть боковое меню"]').attributes('aria-expanded'),
    ).toBeUndefined();
    ...
    await wrapper.get('button[aria-label="Развернуть боковое меню"]').trigger('click');
    expect(wrapper.get('.shell').classes()).not.toContain('shell--sidebar-collapsed');
    expect(localStorage.getItem('retenive-cms-sidebar-collapsed-v1')).toBe('false');
  });
  ...
  it('covers the old workspace while a Project switch is resolving', async () => {
    ...
    await wrapper.get('button[aria-label="Переключить на проект Project Two"]').trigger('click');

    expect(wrapper.get('.project-switch-state').text()).toContain('Переключаем проект');
    expect(wrapper.get('.project-switch-state').text()).toContain('Project Two');
  ...
  it('opens a Project from its row action without switching the current tab', async () => {
    ...
    const openButton = wrapper
      ...
      .find(
        (button) =>
          button.attributes('aria-label') === 'Открыть проект Project Two в новой вкладке',
      );
    expect(openButton).toBeDefined();
    await openButton!.trigger('click');

    expect(open).toHaveBeenCalledWith('', '_blank');
    expect(setItem).toHaveBeenCalledWith('retenive-cms-selected-project-v1', 'project-2');
  ...
  it('keeps navigation and account controls in separate sidebar regions', async () => {
    ...
    expect({
      ...
      profileInFooter: wrapper.find('.sidebar-footer .sidebar-profile').exists(),
  ...
  it('keeps every Support destination together under one navigation group', async () => {
    ...
    expect(supportLinks.every((link) => link.classes().includes('nav-item--nested'))).toBe(true);

    const toggle = supportGroup.get('button[aria-label="Развернуть раздел «Поддержка»"]');
    ...
    expect(disclosure.classes()).not.toContain('nav-group-disclosure--collapsed');
  ...
  it('shows a non-clickable Support heading when only a nested setting is allowed', async () => {
    ...
    expect(wrapper.get('[role="heading"][aria-label="Поддержка"]').text()).toContain('Поддержка');
  ...
  it('groups project access pages under Project and highlights only the current page', async () => {
    ...
    const toggle = projectGroup.get('button[aria-label="Свернуть раздел «Проект»"]');
    await toggle.trigger('click');
    expect(toggle.attributes('aria-expanded')).toBe('false');
  });
  ...
  it('shows the AI cost-control workspace to cost readers or allowance operators', async () => {
    ...
    setActivePinia(pinia);
    const auth = useAuthStore();
    const selected = project('project-1', 'Project One', ['project.ai_costs.read']);
  ...
  it('shows delivery recovery only to the exact Platform reader', async () => {
    ...
    expect(wrapper.findAll('.sidebar-scroll nav a').map((link) => link.text())).toEqual([
  ...
  it('shows Project administrators only with the exact selected-Project read Permission', async () => {
    ...
    expect(navigation.find('[role="heading"][aria-label="Проект"]').exists()).toBe(true);
  ...
  it('shows Telegram broadcasts only to an exact broadcast reader', async () => {
    ...
    auth.project!.effectivePermissionCodes = ['project.telegram.broadcasts.draft'];
    await wrapper.vm.$nextTick();
