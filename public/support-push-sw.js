...
self.addEventListener("push", (event) => {
  ...
  const created = topic === "SUPPORT_CASE_CREATED";
  event.waitUntil(
    self.registration.showNotification(
      assignment
        ? "Вам назначено обращение"
        : created
          ? "Новое обращение поддержки"
          : "Требуется внимание поддержки",
      {
        ...
        data: {
          path: `/support/notifications/open#capability=${encodeURIComponent(capability)}`,
        },
      },
    ),
  );
...
self.addEventListener("notificationclick", (event) => {
  ...
  if (
    typeof path !== "string" ||
    !path.startsWith("/support/notifications/open#capability=")
  )
    return;
    ...
    (async () => {
      try {
        ...
        for (const client of clients)
          client.postMessage({
            type: "RETENIVE_SUPPORT_PUSH_SUBSCRIPTION_CHANGED",
          });
      }
    })(),
  );
