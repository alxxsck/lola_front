...
CMS хранит редактируемый сценарий как ориентированный ациклический граф (DAG) и отправляет его в Retenive Backend без промежуточного frontend-формата. Редактор принимает только V2-граф с явными `nodeKey`: документ без устойчивых ключей узлов считается несовместимым и не открывается для редактирования.
...
- `model/scenario-graph.ts`: строгая нормализация V2-графа, переходы, target options и бизнес-валидация без Vue/PrimeVue.
...
Тесты `scenario-graph.test.ts` проверяют обязательный `nodeKey`, подписанные переходы и ошибки графа. `scenario-contract.test.ts` фиксирует точный payload с `nodeKey`/`nextNodeKey` и сохранение расширяемых action config.
