...
CMS хранит редактируемый сценарий как ориентированный ациклический граф (DAG) и отправляет его в Lola Backend без промежуточного frontend-формата. Редактор принимает только V2-граф с явными `nodeKey`: документ без устойчивых ключей узлов считается несовместимым и не открывается для редактирования.
