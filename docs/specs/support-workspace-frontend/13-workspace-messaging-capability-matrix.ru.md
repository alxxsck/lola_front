...
| Lookup попытки после timeout | операции нет | не опубликовано | не опубликовано | не опубликовано | отдельный lookup отсутствует | `unknownSendOutcome.lookupOperation=NOT_PUBLISHED` | `NOT_PUBLISHED`; backend owner, frontend Task 13 blocked |
...
| Message delivery | `AdminMessageDeliveryResponseDto` внутри history/send: `id`, `channel`, `commandIds`, `interactionSessionId`, `status`, `acceptedAt` | read/reply permission родительской операции | server receipt; HTTP success не равен `DELIVERED` | merge authority — server status; checkpoint только запускает reconcile | command identity сохраняется; отдельного retry/lookup нет | `fullSelectionSuccess` | `support_durable_delivery`; `RELEASE_GATED` |
...
`400/403/404/409/410/422`; transport timeout не имеет response body и приводит
к повтору того же attempt с тем же idempotency key.
...
| `support_durable_delivery` | backend contract готов, rollout gated | backend SDK ACK/load proof + frontend Tasks 13/15 |
| Idempotency lookup | отсутствует | backend messaging owner |
