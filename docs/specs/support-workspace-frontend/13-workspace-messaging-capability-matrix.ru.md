...
`READY` означает, что операция есть в pinned OpenAPI и доступна по IAM без
Project-level переключателя. `NOT_PUBLISHED` нельзя реализовывать локальным DTO,
таймером или socket inference. Доступность всей платформы приходит только из
session capability `capabilities.supportEnabled`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Case/Conversation selection | `GET SupportWorkspace_read`, `mode=SELECTION` | `project.cases.read` **или** `project.conversations.read` | Boolean `capabilities` текущей projection; это не универсальный `allowedActions` | `actionRevisions` — preconditions; `capabilitiesRevision` — evidence; `checkpoint` — recovery token | нет | `minimalSelectionSuccess`, `fullSelectionSuccess`, `forbiddenSelection`, `concealedSelection` | `READY`; IAM + domain state |
| Conversation history в workspace | `GET SupportWorkspace_read`, `messageCursor`, `messageLimit≤100` | как у selection | read authority выбранной projection | opaque cursor не равен checkpoint; canonical order задаёт Message `ordinal` | нет | `historyNextPage` | `READY` |
| Exact-conversation history в Users | `GET AdminConversations_listMessages`, `cursor`, `limit≤200` | `project.conversations.read` | exact End User + Conversation scope | opaque cursor, Message `ordinal` | нет | pinned OpenAPI + mutation tests | `READY`; независимый Users surface |
| Durable public send | `POST AdminMessaging_send` | `project.conversations.reply` | selection `capabilities.reply`; translation override требует отдельной capability | отдельного expected revision/ETag в этой операции нет | обязательный `Idempotency-Key` | `sendConflict`, `unknownSendOutcome` | `READY`; provider/dependency readiness отображается как domain state |
| Idempotent replay | тот же `AdminMessaging_send` с тем же body/key; response `duplicate=true` | как у send | как у send | authoritative stored Message + delivery receipt | повтор использует **тот же** key | `unknownSendOutcome` | `READY` |
...
## 4. Operational readiness и ownership

| Boundary | Текущее состояние | Owner / proof |
| --- | --- | --- |
| Support Platform | одна session capability `capabilities.supportEnabled`; Project-level control отсутствует | ADR-0047 + route/navigation tests |
| Project inbox | доступен по IAM; отдельный флаг не используется | Task 02 + backend inbox contract |
| Durable delivery | backend contract и release proof готовы; временная недоступность выражается provider/dependency state | backend `0f5404f`; SDK ACK + 60k outbox load proof |
