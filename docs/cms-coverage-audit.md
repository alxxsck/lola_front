...
| Scenario editor | Готов trigger, conditions, priority/frequency, ordered steps, linked targets, variables и graph preview | `SPEAK_TEXT` использует xAI и общий Project voice; остальные возможности действий определяются закреплёнными Action Type revisions в Project Actions |
