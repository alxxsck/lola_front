...
Для Grok backend сохраняет переданную xAI стоимость в `billedCost`; CMS использует её в итогах и графиках без пересчёта на frontend. ElevenLabs по-прежнему учитывается отдельно через `providerBilledUnits`, потому что провайдер не сообщает точную USD-сумму отдельной TTS-операции. `estimatedCost` остаётся частью общего контракта, но для Grok не используется. Workspace credits, subscription limits и overage доступны только в отдельном platform-admin billing snapshot и не прибавляются к расходам проекта.
