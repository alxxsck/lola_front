...
Text-to-Speech имеет отдельную форму и отдельную кнопку сохранения. Общий Project PATCH продолжает сохранять `settings` проекта, включая описание, подключение и OpenAI Realtime. Backend игнорирует только входящий `settings.speechSynthesis` и сохраняет его актуальную DB-версию: изменить этот блок можно исключительно через dedicated TTS endpoint. Первая страница каталога загружает 20 голосов; поиск и cursor pagination выполняются через backend, поэтому ElevenLabs API key не попадает в браузер.
