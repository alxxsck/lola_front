...
Text-to-Speech имеет отдельную форму и отдельную кнопку сохранения. Общий Project PATCH продолжает сохранять `settings` проекта, включая описание, подключение и OpenAI Realtime. Backend игнорирует только входящий `settings.speechSynthesis` и сохраняет его актуальную DB-версию: изменить этот блок можно исключительно через dedicated TTS endpoint. Селект загружает первую страницу из 20 голосов, включая server default и назначенные проекту голоса; ElevenLabs API key не попадает в браузер.
