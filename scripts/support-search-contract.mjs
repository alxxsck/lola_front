...
function requireFields(schema, name, fields) {
  for (const field of fields) {
    if (!schema.required?.includes(field)) throw new Error(`${name} must require ${field}`);
  }
}

function requireEnum(schema, name, values) {
  for (const value of values) {
    if (!schema.enum?.includes(value)) throw new Error(`${name} must include ${value}`);
  }
}
...
export function validateSupportSearchContract(contract) {
  ...
  for (const [operationId, responseName] of operations) {
    ...
    if (!target['x-iam-permission']) throw new Error(`${operationId} must retain IAM authority`);
  ...
  requireEnum(
    requiredSchema(contract, 'SupportCaseSearchSortDto').properties.field,
    'SupportCaseSearchSortDto.field',
    [
      'RELEVANCE',
      'ACTIVITY_AT',
      'PRIORITY',
...
export function validateSupportViewsContract(contract) {
  ...
  for (const operationId of [
    'SavedSupportView_create',
    'SavedSupportView_replace',
    'SavedSupportView_publish',
    'SavedSupportView_archive',
    'SavedSupportView_replaceDefaultView',
  ]) {
    const names =
      operation(contract, operationId).parameters?.map((parameter) => parameter.name) ?? [];
    if (!names.includes('Idempotency-Key'))
      throw new Error(`${operationId} requires Idempotency-Key`);
    if (operationId !== 'SavedSupportView_create' && !names.includes('If-Match'))
      throw new Error(`${operationId} requires If-Match`);
  }
  requireEnum(
    requiredSchema(contract, 'SupportViewPresetResponseDto').properties.code,
    'SupportViewPresetResponseDto.code',
    ['MY_ACTIVE', 'MY_TEAM_UNASSIGNED', 'ALL_CASES', 'ALL_CONVERSATIONS'],
  );
  requireEnum(
    requiredSchema(contract, 'SavedSupportViewCountResponseDto').properties.state,
    'SavedSupportViewCountResponseDto.state',
    ['EXACT', 'LOWER_BOUND', 'UNAVAILABLE'],
  );
  requireFields(
    requiredSchema(contract, 'SavedSupportViewResponseDto'),
    'SavedSupportViewResponseDto',
    ['id', 'scope', 'etag', 'permissions', 'count', 'freshness'],
  );
  requireFields(
    requiredSchema(contract, 'SupportDefaultViewResponseDto'),
    'SupportDefaultViewResponseDto',
    ['selection', 'available', 'etag', 'version'],
  );
}
