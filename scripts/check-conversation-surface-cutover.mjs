...
const canonicalSurfacePath = 'src/features/conversation-surface/ui/ConversationSurface.vue';
...
export function findConversationSurfaceCutoverViolations(entries) {
  const violations = [];
  const productionPaths = new Set(entries.map((entry) => entry.path.split(path.sep).join('/')));

  for (const entry of entries) {
    const normalizedPath = entry.path.split(path.sep).join('/');
    const filename = path.posix.basename(normalizedPath);
    const add = (rule, detail) => violations.push({ path: normalizedPath, rule, detail });

    if (legacyComponentNames.has(filename))
      add('legacy-component', `${filename} must be removed after cutover`);

    if (
      normalizedPath === 'src/features/end-user-cases/ui/EndUserCaseDetail.vue' &&
      /(?:class=["'][^"']*\bmessage-row\b|\.message-row\b)/u.test(entry.source)
    )
      add('case-message-renderer', 'Case detail may expose evidence links, not a message feed');

    if (
      legacySelectors.some(
        (selector) => entry.source.includes(selector) || entry.source.includes(selector.slice(1)),
      )
    )
      add('legacy-selector', 'legacy full-chat selector remains in production source');

    if (normalizedPath !== canonicalSurfacePath && /<TranslatedMessageBody\b/u.test(entry.source))
      add('duplicate-message-renderer', 'TranslatedMessageBody is owned by ConversationSurface');
    ...
    if (requiredAdapters.has(normalizedPath) && !/<ConversationSurface\b/u.test(entry.source))
      add('adapter-root', 'Users and Support adapters must mount ConversationSurface');
  }
...
export async function checkConversationSurfaceCutover(rootDir = repositoryRoot) {
  return findConversationSurfaceCutoverViolations(await productionVueEntries(rootDir));
}
