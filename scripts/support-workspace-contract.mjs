...
export function validateSupportWorkspaceMessagingContract(document) {
  ...
  const lookupMessageOutcome = operation(document, 'AdminMessaging_lookupOutcome');
  const readPosition = operation(document, 'AdminConversationCollaboration_get');
  const markReadPosition = operation(document, 'AdminConversationCollaboration_mark');
  ...
  const internalNoteTombstone = operation(document, 'SupportInternalNote_tombstone');
  const internalNoteRealtime = operation(document, 'SupportInternalNote_realtimeContract');
  ...
  requirePermission(internalNoteTombstone, 'project.support.internal_notes.redact');
  requirePermission(internalNoteRealtime, 'project.support.internal_notes.read');
  ...
  const lookupIdempotencyKey = parameter(lookupMessageOutcome, 'Idempotency-Key');
  if (lookupIdempotencyKey.in !== 'header' || lookupIdempotencyKey.required !== true) {
    throw new Error('AdminMessaging_lookupOutcome must require Idempotency-Key header');
  }
  if (
    lookupMessageOutcome.responses?.['200']?.content?.['application/json']?.schema?.$ref !==
      '#/components/schemas/SendAdminMessageResponseDto' ||
  ...
  requireEnumValues(document, 'SupportWorkspaceInternalNoteCapabilitiesResponseDto', 'state', [
    'AVAILABLE',
    'UNAVAILABLE',
  ]);
  for (const mutation of [internalNoteCreate, internalNoteCorrect, internalNoteTombstone]) {
  ...
  requireProperties(document, 'SupportWorkspaceMessagePageResponseDto', ['items', 'anchorOrdinal']);
  requireFields(document, 'SupportWorkspaceMessagePageResponseDto', ['nextCursor', 'newerCursor']);
  ...
  requireProperties(document, 'SupportWorkspaceConversationRowResponseDto', ['readState']);
  requireProperties(document, 'SupportWorkspaceSelectionConversationResponseDto', ['readState']);
  requireProperties(document, 'MarkCmsConversationReadPositionDto', ['lastReadOrdinal']);
  const readPositionResponse =
    readPosition.responses?.['200']?.content?.['application/json']?.schema?.$ref;
  ...
  const markReadPositionResponse =
    markReadPosition.responses?.['200']?.content?.['application/json']?.schema?.$ref;
  if (
    readPositionResponse !== '#/components/schemas/CmsConversationReadPositionResponseDto' ||
    markReadPositionRequest !== '#/components/schemas/MarkCmsConversationReadPositionDto' ||
    markReadPositionResponse !== '#/components/schemas/CmsConversationReadPositionResponseDto'
  ) {
    throw new Error(
  ...
  requireProperties(document, 'AdminMessageDeliveryResponseDto', ['status', 'commandIds']);
