...
test("workspace contract rejects removal of recovery-critical selection fields", async () => {
  ...
  for (const field of requiredFields) {
    ...
    const required =
      contract.components.schemas.SupportWorkspaceSelectionResponseDto.required;
    contract.components.schemas.SupportWorkspaceSelectionResponseDto.required =
      required.filter((value) => value !== field);
...
test("workspace operations retain authority, pagination bounds and send idempotency", async () => {
  ...
  const mutations = [
    (contract) => {
      delete operation(contract, "SupportWorkspace_read")[
        "x-iam-any-permission"
      ];
    },
    (contract) => {
      delete operation(contract, "AdminConversations_listMessages")[
        "x-iam-permission"
      ];
    },
    (contract) => {
...
test("workspace modes are stable while additive delivery states remain forward compatible", async () => {
  ...
  assert.doesNotThrow(() =>
    validateSupportWorkspaceMessagingContract(additiveDelivery),
  );
});
