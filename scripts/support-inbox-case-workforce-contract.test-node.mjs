...
test("inbox and Case commands retain bounded server-owned query and mutation semantics", async () => {
  ...
  assert.doesNotThrow(() =>
    validateSupportInboxCaseWorkforceContract(baseline),
  );
  ...
  const mutations = [
    ...
    (contract) => {
      delete operation(contract, "AdminProjectConversations_list")[
        "x-iam-permission"
      ];
    },
    (contract) => {
    ...
    (contract) => {
      ...
      sort.schema.enum = sort.schema.enum.filter(
        (value) => value !== "ATTENTION_FIRST",
      );
    },
    (contract) => {
    ...
    (contract) => {
      ...
      target.required = target.required.filter(
        (field) => field !== "availableStatuses",
      );
    },
    (contract) => {
      delete contract.components.schemas.SupportWorkspaceCapabilitiesResponseDto
...
test("assignment and offer commands retain authority, OCC and audited reasons", async () => {
  ...
  const mutations = [
    (contract) => {
      delete operation(contract, "SupportCaseAssignment_claim")[
        "x-iam-permission"
      ];
    },
    (contract) => {
    ...
    (contract) => {
      const target =
        contract.components.schemas.TransferSupportCaseAssignmentDto;
      target.required = target.required.filter(
        (field) => field !== "expectedAssignmentVersion",
      );
    },
    (contract) => {
    ...
    (contract) => {
      const target =
        contract.components.schemas.SupportCaseAssignmentMutationResponseDto;
      target.required = target.required.filter(
        (field) => field !== "assignmentVersion",
      );
    },
    (contract) => {
...
test("availability, workforce, queue, routing and SLA projections retain authority and provenance", async () => {
  ...
  const mutations = [
    ...
    (contract) => {
      const target =
        contract.components.schemas.SupportWorkforceSettingsResponseDto;
      target.required = target.required.filter(
        (field) => field !== "actionEtag",
      );
    },
    (contract) => {
      ...
      target.required = target.required.filter(
        (field) => field !== "maxCapacityUnits",
      );
    },
    (contract) => {
      const target =
        contract.components.schemas.SupportQueueFreshnessResponseDto;
      target.required = target.required.filter((field) => field !== "state");
    },
    (contract) => {
      const target =
        contract.components.schemas.SupportRoutingDecisionDetailResponseDto;
      target.required = target.required.filter(
        (field) => field !== "inputManifest",
      );
    },
    (contract) => {
      operation(
    ...
    (contract) => {
      const state =
        contract.components.schemas.SupportQueueFreshnessResponseDto.properties
          .state;
