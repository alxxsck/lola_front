...
function requireAnyPermissions(operationValue, codes) {
  const actual = new Set(operationValue['x-iam-any-permission']?.map((value) => value.code) ?? []);
...
function requireParameterEnum(operationValue, name, values) {
  ...
  for (const value of values) {
    if (!actual.has(value)) {
      throw new Error(`${operationValue.operationId}.${name} must retain ${value}`);
    }
  }
}
...
function inlineErrorSchema(operationValue, status) {
  ...
  return (
    ...
    responseSchema?.allOf?.find((entry) => entry.properties?.error)?.properties?.error
  );
}
...
function requireInlineErrorEnum(operationValue, status, values) {
  ...
  for (const value of values) {
    if (!actual.has(value)) {
      throw new Error(`${operationValue.operationId} ${status}.error.code must retain ${value}`);
    }
  }
}
...
function requireInlineErrorFields(operationValue, status, fields) {
  ...
  if (!errorSchema) {
    throw new Error(`${operationValue.operationId} must retain typed response ${status}`);
  }

  const properties = errorSchema.properties ?? {};
  for (const field of fields) {
    if (!(field in properties)) {
      throw new Error(`${operationValue.operationId} ${status}.error must publish ${field}`);
    }
  }
}
...
export function validateSupportInboxCaseWorkforceContract(document) {
  ...
  requireAnyPermissions(workspaceRead, ['project.cases.read', 'project.conversations.read']);
  ...
  requireProperties(document, 'SupportWorkspaceCasesPageResponseDto', ['mode', 'items']);
  ...
  requireProperties(document, 'SupportWorkspaceSelectionResponseDto', ['sla', 'routing']);
  ...
  requirePropertyEnum(document, 'SupportWorkspaceSlaSignalAvailableResponseDto', 'signalCode', [
    'SLA_BREACHED',
    'SLA_AT_RISK',
    'SLA_PAUSED',
    'SLA_DUE',
  ]);
  requireProperties(document, 'SupportSlaCaseProjectionResponseDto', ['occurrence', 'clocks']);
  ...
  requireSchemaFields(document, 'SupportCaseRoutingExclusionCountsResponseDto', [
    'TEAM_NOT_ELIGIBLE',
    'SKILL_REQUIRED',
    'LANGUAGE_REQUIRED',
  ...
  const conversationList = operation(document, 'AdminProjectConversations_list');
  requirePermission(conversationList, 'project.conversations.read');
  if (parameter(conversationList, 'cursor').schema?.maxLength !== 1024) {
    throw new Error('AdminProjectConversations_list cursor must remain bounded');
  }
  if (parameter(conversationList, 'limit').schema?.maximum !== 100) {
    throw new Error('AdminProjectConversations_list limit must remain bounded at 100');
  }
  for (const filter of ['status', 'endUserId']) {
    parameter(conversationList, filter);
  }
  requireProperties(document, 'AdminProjectConversationsPageResponseDto', ['items']);
  ...
  requireProperties(document, 'EndUserCaseCommandResponseDto', ['id', 'status', 'version']);
  ...
  const assignmentRelease = operation(document, 'SupportCaseAssignment_release');
  const assignmentTransfer = operation(document, 'SupportCaseAssignment_transfer');
  ...
  requireProperties(document, 'ClaimSupportCaseAssignmentDto', ['teamId', 'expectedCaseVersion']);
  ...
  requirePropertyEnum(document, 'SupportCaseAssignmentMutationResponseDto', 'intent', [
  ...
  const assignmentCandidates = operation(document, 'SupportCaseAssignment_candidatesForCase');
  const forceAssign = operation(document, 'SupportCaseAssignment_assignWithOverride');
  const forceTransfer = operation(document, 'SupportCaseAssignment_transferWithOverride');
  const commandOutcome = operation(document, 'SupportCaseAssignment_commandOutcome');
  const batchExecute = operation(document, 'SupportCaseAssignmentBatch_execute');
  const batchOutcome = operation(document, 'SupportCaseAssignmentBatch_outcome');
  requireResponseSchema(assignmentCandidates, '200', 'SupportCaseAssignmentCandidatesResponseDto');
  ...
  requireInlineErrorEnum(commandOutcome, '404', ['ASSIGNMENT_COMMAND_OUTCOME_NOT_FOUND']);
  ...
  requireParameterEnum(leadTargets, 'purpose', ['ALERT_OWNER', 'OPERATOR_DRILL_DOWN']);
  ...
  requirePropertyEnum(document, 'AssignSupportCaseAssignmentDto', 'reasonCode', [
    'SKILL_MATCH',
    'LOAD_BALANCE',
    'LEAD_INTERVENTION',
    'OTHER',
  ]);
  requirePropertyEnum(document, 'TransferSupportCaseAssignmentDto', 'reasonCode', [
    'SKILL_HANDOFF',
    'LOAD_BALANCE',
    'LEAD_INTERVENTION',
    'OTHER',
  ]);
  requirePropertyEnum(document, 'ReleaseSupportCaseAssignmentDto', 'reasonCode', [
    'WORK_RETURNED',
    'SHIFT_END',
    'LEAD_REBALANCE',
    'OTHER',
  ...
  for (const operationValue of [offerList, offerAccept, offerDecline]) {
    requirePermission(operationValue, 'project.support.assignments.self_manage');
  }
  ...
  requirePropertyEnum(document, 'SupportRoutingOfferActionReceiptDto', 'outcome', [
    'ACCEPTED',
    'DECLINED',
  ]);

  const availabilityRead = operation(document, 'SupportOperatorAvailability_read');
  const availabilitySet = operation(document, 'SupportOperatorAvailability_setOwn');
  const availabilityHeartbeat = operation(document, 'SupportOperatorAvailability_heartbeatOwn');
  const availabilityOverride = operation(document, 'SupportOperatorAvailability_overrideOperator');
  requirePermission(availabilityRead, 'project.support.availability.read');
  requirePermission(availabilitySet, 'project.support.availability.self_manage');
  requirePermission(availabilityHeartbeat, 'project.support.availability.self_manage');
  requirePermission(availabilityOverride, 'project.support.availability.override');
  for (const operationValue of [availabilitySet, availabilityOverride]) {
    requireHeader(operationValue, 'If-Match');
    requireHeader(operationValue, 'Idempotency-Key');
  }
  requireHeader(availabilityHeartbeat, 'If-Match');
  requireProperties(document, 'SetSupportOperatorAvailabilityDto', ['state', 'reasonCode']);
  ...
  requirePropertyEnum(document, 'SupportOperatorAvailabilityResponseDto', 'effectiveState', [
    'OFFLINE',
    'AVAILABLE',
    'BUSY',
    'AWAY',
  ...
  requirePropertyEnum(document, 'SupportOperatorAvailabilityResponseDto', 'reasonCode', [
    'LEASE_EXPIRED',
    'BUSINESS_EXPIRY',
  ]);
  requirePropertyEnum(document, 'SupportOperatorAvailabilityResponseDto', 'source', [
    'SELF',
    'LEAD_OVERRIDE',
    'LEASE_EXPIRY',
    'BUSINESS_EXPIRY',
  ...
  requirePropertyEnum(document, 'PersonalSupportNotificationPreferenceResponseDto', 'topic', [
  ...
  for (const operationValue of [workforceRead, workforceTeams, workforceSkills]) {
  ...
  requireProperties(document, 'ReplaceSupportWorkforceDraftDto', ['operators', 'teams']);
  ...
  requireProperties(document, 'SupportQueueCatalogResponseDto', ['view', 'items', 'nextCursor']);
  ...
  const routingActivation = operation(document, 'SupportRoutingRuntime_activation');
  const routingDecision = operation(document, 'SupportRoutingRuntime_decisionDetail');
  ...
  const slaCorrection = operation(document, 'SupportSlaHumanCommand_correctClock');
  requireAnyPermissions(slaSettings, ['project.support.sla.read', 'project.support.sla.manage']);
  ...
  requirePropertyEnum(document, 'SupportSlaCorrectClockMutationResponseDto', 'risk', [
    'ON_TRACK',
    'AT_RISK',
    'BREACHED',
  ]);
  ...
  const searchConversations = operation(document, 'SupportSearch_conversations');
  ...
  const searchContracts = [
    ...
    [searchMessages, 'SupportMessageSearchQueryDto', 'SupportSearchMessagePageResponseDto'],
    [searchUsers, 'SupportEndUserSearchQueryDto', 'SupportSearchEndUserPageResponseDto'],
  ];
  ...
  for (const operationValue of [savedViewReplace, savedViewPublish, savedViewArchive]) {
    requireHeader(operationValue, 'If-Match');
  }
  ...
  requireProperties(document, 'CreateSavedSupportViewDto', ['code', 'scope', 'draft']);
  ...
  requireResponseSchema(savedViewCatalog, '200', 'SavedSupportViewCatalogResponseDto');
  for (const operationValue of [
    savedViewCreate,
    savedViewReplace,
    savedViewPublish,
    ...
    requireResponseSchema(operationValue, '200', 'SavedSupportViewMutationResponseDto');
  }
  requireResponseSchema(savedViewQuery, '200', 'SavedSupportViewQueryResponseDto');
  requireResponseSchema(activePresetQuery, '200', 'SupportViewPresetCaseQueryResponseDto');
}
