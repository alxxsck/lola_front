...
const requiredOperations = new Map([
  ...
  ['EventCatalog_hardDelete', { label: 'event catalog definition hard delete' }],
  [
    'EventCatalog_usage',
    {
  ...
  ['Translation_get', { label: 'translation job status', response: 'TranslationJobResponseDto' }],
  [
    'Translation_cancel',
    {
...
for (const removedOperationId of [
  ...
  if (operations.some((candidate) => candidate.operationId === removedOperationId)) {
    throw new Error(
...
function requireSchemaProperties(schemaName, propertyNames) {
  ...
  if (missing.length)
    throw new Error(`${schemaName} is missing contract properties: ${missing.join(', ')}`);
}

function forbidSchemaProperties(schemaName, propertyNames) {
  const properties = contractSchema(schemaName).properties ?? {};
  const present = propertyNames.filter((propertyName) => Object.hasOwn(properties, propertyName));
  if (present.length)
    throw new Error(`${schemaName} exposes forbidden contract properties: ${present.join(', ')}`);
}
...
for (const [operationId, expectation] of requiredOperations) {
  ...
  const expectedResponses =
    expectation.responses ?? (expectation.response ? [expectation.response] : []);
  ...
  for (const expectedResponse of expectedResponses) {
    if (!containsSchema(responseSchema, expectedResponse)) {
      throw new Error(`OpenAPI operation ${operationId} does not return ${expectedResponse}`);
    }
  }
}
...
function requireOperationParameters(operationId, parameterNames) {
  ...
  const missing = parameterNames.filter(
    (parameterName) =>
      !parameters.has(parameterName) || parameters.get(parameterName)?.required !== true,
  );
...
requireSchemaProperties('AiTextToSpeechPricingContextResponseDto', ['current', 'sourceUrl']);
requireRequiredProperties('AiTextToSpeechPricingContextResponseDto', ['current', 'sourceUrl']);
...
for (const schemaName of ['AiUsageCategoryBreakdownResponseDto', 'AiUsageRecordResponseDto']) {
  const categories = new Set(contractSchema(schemaName).properties?.category?.enum ?? []);
  for (const requiredCategory of ['VOICE', 'SPEECH', 'CASE_INTELLIGENCE']) {
    if (!categories.has(requiredCategory)) {
      throw new Error(`${schemaName}.category no longer exposes ${requiredCategory}`);
    }
  }
}
...
requireSchemaProperties('PasswordSetupRequiredResponseDto', ['kind', 'setupToken', 'expiresAt']);
requireSchemaProperties('PasswordEstablishedResponseDto', ['kind', 'cmsUserId', 'status', 'next']);
...
requireRequiredProperties('CmsUserProvisioningDto', ['email', 'givenName', 'familyName']);
const provisioningAssignments =
  contractSchema('CmsUserProvisioningDto').properties?.projectAssignments;
if (
...
requireSchemaProperties('MfaRequiredResponseDto', ['publicKey', 'recoveryAvailable']);
requireRequiredProperties('MfaRequiredResponseDto', ['publicKey', 'recoveryAvailable']);
...
requireSchemaProperties('UiElementResponseDto', ['aiEnabled', 'aiDescription', 'aiAliases']);
for (const schemaName of ['CreateUiElementDto', 'UpdateUiElementDto']) {
  requireSchemaProperties(schemaName, ['aiEnabled', 'aiDescription', 'aiAliases', 'auditReason']);
}
...
if (
  JSON.stringify(contractSchema('PreviewScenarioScopeDto').properties?.kind?.enum) !==
...
requireSchemaProperties('AttributeConstraintsDto', ['allowedValues', 'defaultLocale']);
...
requireSchemaProperties('ScenarioLocalizationPolicyDto', ['version', 'mode', 'locales']);
requireSchemaProperties('CreateTranslationJobDto', ['sourceLocale', 'targetLocales', 'units']);
...
requireSchemaProperties('CreateEventCatalogDefinitionDto', ['countsAsActivity', 'payloadSchema']);
...
requireSchemaProperties('NotificationQuarantineDto', ['reason', 'confirmation']);
requireRequiredProperties('NotificationQuarantineDto', ['reason', 'confirmation']);
...
requireRequiredProperties('EventQueryRequestDto', ['eventCodes', 'mode', 'timeRange']);
...
for (const schemaName of [
  ...
  const exposedProperties = Object.keys(contractSchema(schemaName).properties ?? {});
...
for (const schemaName of [
  ...
  const exposedProperties = Object.keys(contractSchema(schemaName).properties ?? {});
  const forbiddenProperties = exposedProperties.filter((propertyName) =>
    /recipient|payload|content(?!Available)|webhook|token|secret|providerRef/i.test(propertyName),
  );
  if (forbiddenProperties.length) {
    throw new Error(
