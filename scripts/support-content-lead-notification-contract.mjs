...
function requireAllPermissions(operationValue, codes) {
  const actual = new Set(operationValue['x-iam-all-permissions']?.map((value) => value.code) ?? []);
...
function requireHeader(operationValue, name) {
  ...
  if (value.in !== 'header' || value.required !== true) {
    throw new Error(`${operationValue.operationId} must require ${name} header`);
  }
}
...
function requireBound(operationValue, name, key, expected) {
  ...
  if (actual !== expected) {
    throw new Error(`${operationValue.operationId}.${name}.${key} must remain ${expected}`);
  }
}

function requireRequestSchema(operationValue, schemaName) {
  const requestSchema = operationValue.requestBody?.content?.['application/json']?.schema;
...
function requireResponseSchema(operationValue, status, schemaName) {
  const responseSchema = operationValue.responses?.[status]?.content?.['application/json']?.schema;
  if (responseSchema?.$ref !== `#/components/schemas/${schemaName}`) {
    throw new Error(`${operationValue.operationId} ${status} must return ${schemaName}`);
  }
}
...
function requireInlineErrorEnum(operationValue, status, values) {
  const actual = new Set(
    operationValue.responses?.[status]?.content?.['application/json']?.schema?.properties?.error
      ?.properties?.code?.enum ?? [],
  );
  for (const value of values) {
    if (!actual.has(value)) {
      throw new Error(`${operationValue.operationId} ${status} must retain ${value}`);
    }
  }
}

function requireBrowserNotificationsPublished(document) {
  const preferencesRead = operation(document, 'PersonalSupportNotification_readPreferences');
  const preferencesUpdate = operation(document, 'PersonalSupportNotification_updatePreference');
  const admissionRead = operation(document, 'PersonalSupportNotification_readAdmission');
  const subscriptionsList = operation(document, 'PersonalBrowserPush_listSubscriptions');
  const subscriptionsRegister = operation(document, 'PersonalBrowserPush_registerSubscription');
  const subscriptionsRevoke = operation(document, 'PersonalBrowserPush_revokeSubscription');
  const deepLinkResolve = operation(document, 'PersonalSupportNotification_resolveDeepLink');
  ...
  requireRequestSchema(preferencesUpdate, 'UpdatePersonalSupportNotificationPreferenceDto');
  requireRequestSchema(subscriptionsRegister, 'RegisterBrowserPushSubscriptionDto');
  requireRequestSchema(subscriptionsRevoke, 'RevokeBrowserPushSubscriptionDto');
  requireResponseSchema(preferencesRead, '200', 'PersonalSupportNotificationSettingsResponseDto');
  requireResponseSchema(preferencesUpdate, '200', 'PersonalSupportNotificationSettingsResponseDto');
  requireResponseSchema(admissionRead, '200', 'PersonalSupportNotificationAdmissionResponseDto');
  requireResponseSchema(subscriptionsList, '200', 'BrowserPushSubscriptionListResponseDto');
  requireResponseSchema(subscriptionsRegister, '200', 'BrowserPushSubscriptionResponseDto');
  requireResponseSchema(subscriptionsRevoke, '200', 'BrowserPushSubscriptionResponseDto');
  requireResponseSchema(deepLinkResolve, '200', 'PersonalSupportNotificationDeepLinkTargetDto');
}

function requireNewCaseNotificationPolicyPublished(document) {
  const permission = 'project.support.notification_policy.manage';
  const readCurrent = operation(document, 'SupportCaseNotificationPolicy_readCurrent');
  const listTeams = operation(document, 'SupportCaseNotificationPolicy_listAvailableTeams');
  const readCommandResult = operation(document, 'SupportCaseNotificationPolicy_readCommandResult');
  const preview = operation(document, 'SupportCaseNotificationPolicy_preview');
  const metrics = operation(document, 'SupportCaseNotificationPolicy_readMetrics');
  const saveDraft = operation(document, 'SupportCaseNotificationPolicy_saveDraft');
  ...
  for (const operationValue of [saveDraft, publish, disable, restore]) {
    ...
    requireResponseSchema(operationValue, '200', 'SupportCaseNotificationPolicyReceiptResponseDto');
  }
  requireBound(listTeams, 'limit', 'maximum', 100);
  if (parameter(listTeams, 'cursor').schema?.format !== 'uuid') {
    throw new Error('Notification policy team cursor must remain an opaque UUID');
  }
  requireRequestSchema(readCommandResult, 'ReadSupportCaseNotificationCommandResultDto');
  ...
  requireResponseSchema(readCurrent, '200', 'SupportCaseNotificationPolicyCurrentResponseDto');
  requireResponseSchema(listTeams, '200', 'SupportCaseNotificationAvailableTeamsResponseDto');
  requireResponseSchema(
    readCommandResult,
    '200',
    'SupportCaseNotificationCommandResultResponseDto',
  );
  requireResponseSchema(preview, '200', 'SupportCaseNotificationPolicyPreviewResponseDto');
  requireResponseSchema(metrics, '200', 'SupportCaseNotificationMetricsResponseDto');

  requirePropertyEnum(document, 'SupportCaseNotificationPolicyInputDto', 'mode', [
    'OFF',
    'IMMEDIATE',
    'DIGEST',
  ]);
  requireArrayItemEnum(document, 'SupportCaseNotificationPolicyInputDto', 'occurrences', [
    'CREATED',
    'REOPENED',
  ]);
  requireArrayItemEnum(document, 'SupportCaseNotificationPolicyInputDto', 'channels', [
    'BROWSER_PUSH',
  ]);
  requirePropertyEnum(document, 'SupportCaseNotificationPolicyInputDto', 'recipientRule', [
    'ALL_ELIGIBLE_SUBSCRIBERS',
    'TEAM_SUBSCRIBERS',
  ]);
  requireProperties(document, 'SupportCaseNotificationPolicyPreviewResponseDto', [
    'issues',
  ...
  const examples =
    document.components.schemas.SupportCaseNotificationPolicyPreviewResponseDto.properties.examples;
  if (examples.maxItems !== 5) {
    throw new Error('Notification policy preview must expose at most five examples');
  }
  requireProperties(document, 'SupportCaseNotificationPolicyPreviewExampleDto', [
    'occurrence',
    'conversationClass',
    'topicCode',
    'priority',
  ...
  for (const forbidden of ['caseId', 'title', 'message', 'endUserId']) {
    if (
      document.components.schemas.SupportCaseNotificationPolicyPreviewExampleDto.properties[
        forbidden
      ]
    ) {
      throw new Error(`Notification preview example must not expose ${forbidden}`);
    }
  }
  requireProperties(document, 'SupportCaseNotificationPolicyReceiptResponseDto', [
    'receiptId',
    'replayed',
    'policy',
  ]);
  requirePropertyEnum(document, 'ReadSupportCaseNotificationCommandResultDto', 'operation', [
    'SAVE_DRAFT',
    'PUBLISH',
    'DISABLE',
    'RESTORE',
  ]);
  requireProperties(document, 'SupportCaseNotificationCommandResultResponseDto', [
    'found',
    'operation',
  ]);
  requirePropertyEnum(document, 'UpdatePersonalSupportNotificationPreferenceDto', 'topic', [
    'SUPPORT_CASE_CREATED',
  ]);
  requireProperties(document, 'PersonalSupportNotificationAdmissionCapabilitiesDto', ['newCases']);
}
...
export function validateSupportContentLeadNotificationContract(document) {
  ...
  requireResponseSchema(noteRevisions, '200', 'SupportInternalNoteRevisionPageResponseDto');
  for (const operationValue of [noteCreate, noteCorrection, noteTombstone]) {
    requireResponseSchema(operationValue, '200', 'SupportInternalNoteResponseDto');
  }
  ...
  requireAllPermissions(contentPanel, ['project.support.macros.read', 'project.cases.read']);
  ...
  requireProperties(document, 'SupportContentPanelResponseDto', ['macros', 'knowledge']);
  requirePropertyEnum(document, 'SupportContentPanelMacrosDto', 'state', ['READY', 'UNAVAILABLE']);
  ...
  for (const operationValue of [macroCreate, macroReplace, macroPublish, macroArchive]) {
  ...
  for (const operationValue of [macroRead, macroCreate, macroReplace, macroPublish, macroArchive]) {
  ...
  for (const operationValue of [macroDraftCreate, macroDraftRead, macroDraftEdit]) {
  ...
  for (const operationValue of [macroDraftCreate, macroDraftRead, macroDraftEdit]) {
    requireResponseSchema(operationValue, '200', 'SupportMacroReplyDraftResponseDto');
  }
  ...
  const knowledgeSearch = operation(document, 'SupportInternalKnowledge_search');
  ...
  const knowledgeDownload = operation(document, 'SupportInternalKnowledge_exchangeDownloadGrant');
  for (const operationValue of [
    knowledgeSearch,
    knowledgeOpen,
    ...
    requireAllPermissions(operationValue, ['project.support.knowledge.read', 'project.cases.read']);
  ...
  requireResponseSchema(knowledgeSearch, '200', 'SupportKnowledgeSearchPageResponseDto');
  requireResponseSchema(knowledgeOpen, '200', 'SupportKnowledgeTextDocumentResponseDto');
  requireResponseSchema(knowledgeDownloadGrant, '200', 'SupportKnowledgeDownloadGrantResponseDto');
  requireResponseSchema(knowledgeDownload, '200', 'SupportKnowledgeFileDownloadResponseDto');
  ...
  for (const operationValue of citationOperations) {
    ...
    requireResponseSchema(operationValue, '200', 'SupportKnowledgeCitationDraftResponseDto');
  }
  ...
  if (
    !document.components?.schemas?.SendAdminMessageDto?.properties?.supportKnowledgeCitationDraftId
  )
    throw new Error('SendAdminMessageDto must publish supportKnowledgeCitationDraftId');
  ...
  requirePropertyEnum(document, 'SupportKnowledgeManagedRevisionResponseDto', 'state', [
    'EDITING',
    'QUARANTINED',
    'SCANNING',
    'PUBLISHABLE',
  ...
  const retentionRead = operation(document, 'SupportContentGovernance_retention');
  const retentionReplace = operation(document, 'SupportContentGovernance_replaceRetentionDraft');
  const retentionPublish = operation(document, 'SupportContentGovernance_publishRetention');
  const retentionPreview = operation(document, 'SupportContentGovernance_previewRetention');
  for (const operationValue of [
    retentionRead,
    retentionReplace,
    retentionPublish,
    ...
    requirePermission(operationValue, 'project.support.content_retention.manage');
  }
  for (const operationValue of [retentionRead, retentionReplace, retentionPublish]) {
    requireResponseSchema(operationValue, '200', 'SupportContentRetentionPolicyResponseDto');
  }
  ...
  const holdRelease = operation(document, 'SupportContentGovernance_releaseHold');
  for (const operationValue of [holdList, holdCreate, holdRelease]) {
    requirePermission(operationValue, 'project.support.content_legal_hold.manage');
  }
  requireResponseSchema(holdList, '200', 'SupportContentLegalHoldPageResponseDto');
  for (const operationValue of [holdCreate, holdRelease]) {
    requireResponseSchema(operationValue, '200', 'SupportContentLegalHoldResponseDto');
  }
  ...
  for (const operationValue of [leadSummary, leadCaseRisks, leadCapacityRisks, leadInvestigation]) {
  ...
  for (const operationValue of [leadCaseRisks, leadCapacityRisks, leadActivity]) {
  ...
  requirePropertyEnum(document, 'SupportLeadCapacityRisksDataDto', 'state', ['AVAILABLE']);
  requireProperties(document, 'SupportLeadCapacityRisksDataDto', ['state', 'snapshotAt', 'items']);
  ...
  requireResponseSchema(alertList, '200', 'SupportOperationalAlertListResponseDto');
  requireResponseSchema(alertDetail, '200', 'SupportOperationalAlertDetailResponseDto');
  ...
  requirePropertyEnum(document, 'SupportOperationalAlertMaterializationDto', 'state', [
    'READY',
    'DEGRADED',
  ]);

  const alertAcknowledge = operation(document, 'SupportOperationalAlertCommand_acknowledge');
  const alertChangeOwner = operation(document, 'SupportOperationalAlertCommand_changeOwner');
  const alertResolve = operation(document, 'SupportOperationalAlertCommand_resolve');
  for (const operationValue of [alertAcknowledge, alertChangeOwner, alertResolve]) {
    requirePermission(operationValue, 'project.support.alerts.manage');
    requireHeader(operationValue, 'If-Match');
    requireHeader(operationValue, 'Idempotency-Key');
    requireResponseSchema(operationValue, '200', 'SupportOperationalAlertCommandReceiptDto');
  ...
  requireRequestSchema(alertAcknowledge, 'AcknowledgeSupportOperationalAlertDto');
  requireRequestSchema(alertChangeOwner, 'ChangeSupportOperationalAlertOwnerDto');
  ...
  requirePropertyEnum(document, 'SupportOperationalAlertCommandReceiptDto', 'state', [
    'NEW',
    'ACKNOWLEDGED',
    'RESOLVED',
  ]);
  requirePropertyEnum(document, 'AcknowledgeSupportOperationalAlertDto', 'reasonCode', [
    'INVESTIGATING',
    'OWNERSHIP_ACCEPTED',
    'ESCALATED',
  ]);
  requirePropertyEnum(document, 'ResolveSupportOperationalAlertDto', 'reasonCode', [
    'RISK_CLEARED',
    'MITIGATED',
    'FALSE_POSITIVE',
    'DUPLICATE',
